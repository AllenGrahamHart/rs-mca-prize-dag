# f_termination_mds

- **status:** PROVABLE
- **closure:** proof
- **refs (legacy repo):** ['proof_sketch/s3b_iii_3_fibers_and_noanchor.md#1']

## Statement

For coordinate-prefix/fiber flats the dual is (shortened) MDS: no words below the Singleton range, so the support lattice is trivial and the descent tree has size 1. The moment count applies immediately. Corollary of f_consumer_scoped branch (a).

## Ledger (migrated notes)

provenance: f_consumer_scoped branch (a) — edge omitted to keep the digraph acyclic | PACKET WRITTEN (wave 1, same note): shortened-MDS-is-MDS + Singleton-dual => trivial lattice, tree size 1; exhaustive dual-weight check at n=16 confirms.
