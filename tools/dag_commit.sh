...
set -e
cd "$(dirname "$0")/.."
tools/ramguard tiny -- python3 tools/auto_discharge.py
...
if ! tools/ramguard tiny -- python3 tools/verify_prize_dag.py > /tmp/dag_validate.out 2>&1; then
...
if [ -f tools/verify_assembly_implications.py ]; then
  if ! tools/ramguard tiny -- python3 tools/verify_assembly_implications.py > /tmp/dag_assembly.out 2>&1; then
    echo "BLOCKED: assembly implication drift:"; tail -5 /tmp/dag_assembly.out; exit 1
  fi
fi
tools/ramguard tiny -- python3 tools/build_critical_orbit.py
git add -A
git commit -q -m "$1
