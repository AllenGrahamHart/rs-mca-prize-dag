#!/usr/bin/env python3
"""Independent Bezout audit for reciprocal-xi near square exclusions.
...
def main() -> None:
    ...
    print(
        f"audit_stage=exception_probe left={args.left} "
        f"factor={sp.factor(opposite.as_expr())}",
        flush=True,
    )
    forbidden_c = sp.Poly(
    ...
    if args.swap and args.left == 0:
        ...
        expected_support = forbidden_c * sp.Poly(c - 5, c, domain=sp.QQ)
    else:
        expected_support = (
            forbidden_c * sp.Poly((c + 2) * (5 * c + 7), c, domain=sp.QQ)
        )
    ...
    if args.swap and args.left == 0:
        ...
        special_c = sp.Rational(-2)
        d_gcd = sp.gcd(
            sp.Poly(lead.subs(c, special_c), d, domain=sp.QQ),
            sp.Poly(constant.subs(c, special_c), d, domain=sp.QQ),
        ).monic()
        expected = sp.Poly((d - 2) * (2 * d - 1), d, domain=sp.QQ).monic()
        check(d_gcd == expected, "audit c=-2 fiber")
    if not args.swap:
        exceptional_c = (
            sp.Rational(5) if args.left == 0 else sp.Rational(-7, 5)
        )
        extra_d = (
            sp.Rational(5, 7) if args.left == 0 else sp.Rational(53, 55)
        )
        check(sp.cancel(
    ...
    print(
        "KB_C2_112_NEAR_TAU_XI_SQUARE_AUDIT_PASS "
