#!/usr/bin/env python3
"""Direct exact exclusion of reciprocal-xi near-aligned positive square charts.

The chart has ``a=2``, ``xi=tau(a)=1/2``, ``eta=c``, ``ell=d``, and
``w=1/c``. It uses the fixed-moving internal template. By default it assigns
the residual over c to ``(W-2)^2`` and the residual over d to
``(W-1/d)^2``; ``--swap``
...
def main() -> None:
    ...
    middle_equations = []
    targets = (
        ((c, 1 / d), (d, sp.Integer(2)))
        if args.swap else
        ((c, sp.Integer(2)), (d, 1 / d))
    )
        ...
        for right_index in right_indices:
            ...
            if args.swap and (left_index, right_index) == (1, 0):
                expected_generic = sp.Poly(
                    (d - 2) * (d - 1) * (d + 1), d, domain=sp.QQ
                ).monic()
            actual_generic = common.sqf_part().monic()
            require(
                actual_generic == expected_generic,
                "generic branch has noncollision support: "
                f"{sp.factor(actual_generic.as_expr())}",
            )
            modular_resultants = [
    ...
    for left_index in (args.left,):
        ...
        print(
            f"stage=exception_probe left={left_index} "
            f"factor={sp.factor(base_resultant.as_expr())}",
            flush=True,
        )
        if args.swap and left_index == 0:
            ...
            exceptional_data = (
                (sp.Rational(5, 7), sp.Rational(5), 7 * d - 5),
                (sp.Rational(53, 55), sp.Rational(-7, 5), 55 * d - 53),
            )
        ...
        if args.swap and left_index == 0:
            ...
            print(
                f"stage=exception_fiber_probe left={left_index} "
                f"c_gcd={sp.factor(c_gcd.as_expr())}",
                flush=True,
            )
    ...
    require(len(generic_rows) == expected_generic_rows
            ...
            "coverage accounting")
    print(
        "KB_C2_112_NEAR_TAU_XI_SQUARE_DIRECT_PASS "
