#!/usr/bin/env python3
"""Independent fraction-free audit of the moving-xi swapped square chart."""
...
def main() -> None:
    ...
    conditions = {
        ("c", "product"): residual_c[2] - residual_c[0] / d**2,
        ("c", "sum"): residual_c[1] + 2 * residual_c[0] / d,
        ("d", "product"): residual_d[2] - residual_d[0] / b**2,
        ("d", "sum"): residual_d[1] + 2 * residual_d[0] / b,
    }
    polynomials = {
    ...
    check(selected_c.degree(b) == 1, "c branch degree")
    check(selected_d.degree(b) == 2, "d branch degree")
    print("audit_stage=product_branches", flush=True)

    c_lead = sp.Poly(sp.diff(selected_c.as_expr(), b), c, d, domain=sp.QQ)
    c_constant = sp.Poly(
        selected_c.as_expr().subs(b, 0), c, d, domain=sp.QQ
    )
    ...
    if args.c_branch == 0:
        ...
        assert_basis(
            [
                selected_d.as_expr(), c_lead.as_expr(), c_constant.as_expr(),
    ...
    print(
        "KB_C2_112_NEAR_MOVING_XI_SQUARE_ELL_AUDIT_PASS "
