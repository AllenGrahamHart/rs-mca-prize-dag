#!/usr/bin/env python3
"""Direct normalized exclusion for the reciprocal-xi near mixed chart.

Here the common internal endpoint is ``a=2`` but ``xi=tau(a)=1/2``, so the
two mixed target roots are ``2`` and ``1/d``. The ``--support-gcd`` path is
the complete exclusion; the other modes expose intermediate certificates.
"""
...
def main() -> None:
    ...
    for root_name, root in (("c", c), ("d", d)):
        ...
        equations = (
            constant - 2 * leading / d,
            linear + (sp.Integer(2) + 1 / d) * leading,
        )
    ...
    if args.support_gcd:
        print("KB_C2_112_NEAR_TAU_XI_MIXED_PASS")
    else:
        print("KB_C2_112_NEAR_TAU_XI_MIXED_PROBE_INCOMPLETE")
