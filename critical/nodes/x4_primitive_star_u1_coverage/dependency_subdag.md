...
```text
star_pte_lemma [PROVED] ------------------------------ev--+
x4_maxfiber_local_shiftpair_equivalence [PROVED] ----ev--+--> x4_primitive_star_u1_coverage [TARGET]
...
The target now states the exact local primitive-column bound.  `u1` is an
optional minimal-record route, not a mandatory input.  This preserves the
...
split without estimating either part.
