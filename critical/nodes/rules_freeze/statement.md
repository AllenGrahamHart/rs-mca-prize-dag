...
- **status:** PROVED
- **closure:** citation
- **refs (legacy repo):** ['wp_detail/wp0_2_wp4_4_rules_freeze_and_dither.md#2']
...
**Corrections.** The historical upstream citation filename is
`rules_freeze.json`, not `rules_freeze_citation.json`; neither artifact is
vendored in this tree. The historical bonus claim that survey Lemma 4.7 gives
equality between interleaved and base MCA is retracted: the lemma gives only
the one-sided inequality
`epsilon_mca(C^{==t},delta) <= t epsilon_mca(C,delta)`. No consumer uses the
false equality; uniform-in-`m` list transport uses
