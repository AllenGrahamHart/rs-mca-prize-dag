# knife_edge_census

- **status:** see dag.json (single source of truth; dag status PROVED) [header retrofit 2026-07-10, catch #69 — was: CONDITIONAL]
- **closure:** proof
- **refs (legacy repo):** ['proof_sketch/s2_paid_ledger.md#3']

## Statement

REFINED: with bounded scales + exact counts, the census is exact Diophantine arithmetic — windows [L(n,A), K(n,A)) in B*-space per rate/candidate, undecided rows = admissible primes landing inside. Ties exist systematically (B* sweeps all integers as q varies); the residual undecided set shrinks exactly as certification strength L grows toward K. Full grand-challenge closure requires L within O(1) of K at the tie point OR per-prime typicality; exhibited-row partials dodge entirely (census_dodge_selection).

## Attack surface

needs acl_second_order first (the error band's width); then the census is mechanical over the descriptor family

## Falsifier

a dense knife-edge family (would mean the expansion's error model is wrong, not that adjacency fails)

## Ledger (migrated notes)

PROMOTED red -> amber (Codex flip packet, my replay 8/8; flip_packets/knife_edge_census.md): statement-level assembly over the wired kids.
