...
checks the banked `n = 2^41, log2(q) = 255.9` corridor table and verifies that
the computed clean-rate candidates have the intended adjacent margin signs.
For rate `1/2` it instead checks that the historical candidate lies inside
the now-proved simple-pole unsafe interval and is retired. This is a
self-consistency and regression check, not a proof of the remaining red
premises.
