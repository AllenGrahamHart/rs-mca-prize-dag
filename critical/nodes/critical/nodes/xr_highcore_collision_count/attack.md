...
The proved `xr_higher_rank_uniform_split_pencil_reduction` now supplies that
iteration on every uniform flat-nullity cell `u=v=0`, not only at rank five.
For selector rank `s=a+1`, an over-budget family contains a bounded Maxwell
core and a dual-product-code trade. Trade rank one is impossible; trade rank
two uses at most `2a` active coordinates and is a degree-at-most-`a-1`
polynomial pencil. This reaches the open prize ranks structurally, but it does
not count the trades. The remaining higher-rank work is the primitive
trade-rank-at-least-three census together with a compiler for the nonuniform
`u+v>0` cells.

The minimum-union rank-two branch is now classified by
`xr_higher_rank_minimal_face_syzygy_dichotomy`. At active union `a+2`, every
row is an `(a+1)`-point facet. Either the facet colors form the exact local
Plucker syzygy space, which can be quotiented, or both received directions
interpolate on all `a+2` points and create a common `k+2` near-tangent core.
There is no partially singular branch. The next count can therefore start at
rank-two union `a+3`, trade rank at least three, or the explicit tangent
deficit `h-2`; it need not classify minimum face supports again.
