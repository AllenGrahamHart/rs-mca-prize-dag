...
JOBS = [(p, ell, t) for p in (211,) for ell in (2, 3) for t in (3, 5)]
...
def rung(payload):
    ...
    import os
    import sys
    src, target, ell_only, t_only = payload
    src = src.replace("for ell in (2, 3):", f"for ell in ({ell_only},):")
    src = src.replace("for t in (3, 5):", f"for t in ({t_only},):")
...
def main():
    src = _src()
    payloads = [(src, p, e, tt) for (p, e, tt) in JOBS]
    results = [r for r in rung.map(payloads, return_exceptions=True)
               if isinstance(r, dict)]
    for r in sorted(results, key=lambda r: (r['target'], r['n_configs'])):
        s = r.get("summary") or {}
