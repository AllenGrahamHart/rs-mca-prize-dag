...
def main() -> None:
    ...
    if IMAGE_SUPPORT != EXPECTED_IMAGE["support"]:
        raise AssertionError((IMAGE_SUPPORT, EXPECTED_IMAGE["support"]))
    expected_image = dict(EXPECTED_IMAGE)
    expected_image.pop("support")
    if image != expected_image:
        raise AssertionError(("image", image, expected_image))

    print("h=8 exponent-unit falsifier")
    print(f"p={P}, exponent unit u={UNIT}")
    ...
    print(
        f"image obstructions: {image['obstructions']}, "
