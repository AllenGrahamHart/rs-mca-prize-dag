...
```text
n = 64, h = 5,
p = every admissible prime 1 mod 64 with 64^2 < p <= 48193,
...
`56.08s` with peak RSS below 90 MB.  Sixteen explicit prefix chunks cover the
remaining admissible primes through `48193`.  The `12289` chunks completed in
...
Thus the h=5 no-primitive evidence now has `149` complete `n=64` rows in
addition to the expanded `402`-row `n=32` bank.  In particular, every admissible
`n=64` prime through `48193` is certified.  The combined replay processes
`1,047,298,203` total right-side subsets.  This remains finite-row evidence, not
