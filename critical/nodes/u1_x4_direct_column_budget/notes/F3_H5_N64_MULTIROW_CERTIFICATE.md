...
```text
n = 64, h = 5,
p = every admissible prime 1 mod 64 with 64^2 < p <= 30977,
...
`56.08s` with peak RSS below 90 MB.  Nine explicit prefix chunks cover the
remaining admissible primes through `30977`.  The `12289` chunks completed in
...
chunk completed in `31.09s`.  For every listed prime:
...
Thus the h=5 no-primitive evidence now has `95` complete `n=64` rows in
addition to the expanded `402`-row `n=32` bank.  In particular, every admissible
`n=64` prime through `30977` is certified.  The combined replay processes
`667,740,465` total right-side subsets.  This remains finite-row evidence, not
