...
```text
n = 64, h = 5,
p = every admissible prime 1 mod 64 with 64^2 < p <= 57793,
...
`56.08s` with peak RSS below 90 MB.  Twenty-one explicit prefix chunks cover the
remaining admissible primes through `57793`.  The `12289` chunks completed in
...
Thus the h=5 no-primitive evidence now has `174` complete `n=64` rows in
addition to the expanded `402`-row `n=32` bank.  In particular, every admissible
`n=64` prime through `57793` is certified.  The combined replay processes
`1,223,019,378` total right-side subsets.  This remains finite-row evidence, not
