...
```text
n = 64, h = 5,
p = every admissible prime 1 mod 64 with 64^2 < p <= 44417,
...
`56.08s` with peak RSS below 90 MB.  Fifteen explicit prefix chunks cover the
remaining admissible primes through `44417`.  The `12289` chunks completed in
...
`4.85s`; the `44417` chunk completed in `39.65s`.
...
Thus the h=5 no-primitive evidence now has `137` complete `n=64` rows in
addition to the expanded `402`-row `n=32` bank.  In particular, every admissible
`n=64` prime through `44417` is certified.  The combined replay processes
`962,952,039` total right-side subsets.  This remains finite-row evidence, not
