...
```text
n = 64, h = 5,
p = every admissible prime 1 mod 64 with 64^2 < p <= 28097,
...
`56.08s` with peak RSS below 90 MB.  Eight explicit prefix chunks cover the
remaining admissible primes through `28097`.  The `12289` chunks completed in
...
completed in `36.51s`; the `28097` chunk completed in `33.21s`.  For every
listed prime:
...
Thus the h=5 no-primitive evidence now has `88` complete `n=64` rows in
addition to the expanded `402`-row `n=32` bank.  In particular, every admissible
`n=64` prime through `28097` is certified.  The combined replay processes
`618,538,536` total right-side subsets.  This remains finite-row evidence, not
