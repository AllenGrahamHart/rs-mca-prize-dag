...
n=64: 95 primes (contiguous admissible prefix through p=30977, plus high selected rows)
n=96: boundary prime 9601
n=128: boundary prime 17921 plus 6 nearby primes
```

The coverage audit is now in the aggregate replay.  It verifies `505` complete
zero rows and `2,573,607,631` total right-side probes.  It also prints the
remaining coverage limitation: up to the largest certified primes, the current
bank misses `0` admissible primes for `n=32` and `599` admissible primes for
