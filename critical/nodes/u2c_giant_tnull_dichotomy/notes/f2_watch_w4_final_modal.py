#!/usr/bin/env python3
"""F2 campaign (Modal): FALSIFIER WATCH W4 (FINAL) — the transition band at
(n=32, j=2) through the FULL teeth band, q to 65537 (cycle 121). Populations q^3 up
...
app = modal.App("rs-mca-f2-watch-w4")
image = modal.Image.debian_slim().pip_install("numpy")

PRIMES = [2273, 3041, 4001, 5153, 6337, 8161, 10273, 13729, 17921,
          24001, 32833, 44449, 57601, 65537]  # all = 1 mod 32
...
def one_prime(q):
    ...
    def keys_subsets(H):
        pw = np.array([[x % q, x * x % q] for x in H],
                      dtype=np.int64)
        m = np.arange(1 << 16, dtype=np.int64)
        v = np.zeros((1 << 16, 2), dtype=np.int64)
        for i in range(16):
    ...
    def keys_signed(H):
        ...
        v = np.zeros((3 ** 16, 2), dtype=np.int64)
...
def main():
    ...
    if fails:
        raise SystemExit(f"F2_WATCH_W4_FAIL ({fails})")
    print("\nF2_WATCH_W4_PASS")
