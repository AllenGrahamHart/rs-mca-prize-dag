...
In characteristic q > n: there is NO t-null block of size 1 <= b <= t,
...
Proof. p_1(S) = ... = p_t(S) = 0 with b = |S| <= t gives, by Newton's
identities in characteristic > b, e_1(S) = ... = e_b(S) = 0. But
...
1. The extreme bands are EMPTY (not merely under budget) out to width
   t ~ 7e10 ~ 2^36 on each side — vastly superseding the Weil-Newton
