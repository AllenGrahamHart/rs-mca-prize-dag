...
vanish. In the only deficient stratum not paid by the rational-direction
theorem, assume
...
This is now an owner-aware list count inside one primitive polynomial
direction. It is not the assertion that one conveniently chosen syzygy
has many roots, and it is not a one-parameter locator-pencil count: the
roots are forced on the entire left kernel while maximal locators still
range in the original window slice.

## Falsifier

One official deficient high-window system in the displayed primitive
normal form with more than `17n^2/25` maximal selected post-strip
parameters. A raw `tau` family without exact maximality and selected
liveness is not a falsifier.
