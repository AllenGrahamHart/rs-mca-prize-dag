...
named, charged by xr_graded_tangent_band_charge <= H_band(C), where
H_band(C) = s_lo(C)-16n^3 is the row-exact third-column headroom) plus
the post-band-column remainder with cores <= k, which is
