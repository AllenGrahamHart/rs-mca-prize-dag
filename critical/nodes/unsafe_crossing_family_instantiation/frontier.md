...
The remaining theorem is not the local occupancy inequality. It is the
row-by-row construction and exact arithmetic needed to trigger a proved lower-
bound mechanism at the proposed adjacent endpoint.

The identity-prefix plus flexible-budget theorem now supplies exact `V`
payloads for the two deployed MCA rows: KoalaBear at agreement `1116047` and
Mersenne-31 at agreement `1116023`. In both rows the prefix-fiber comparison,
the `B*+1` pair-root budget, ambient-field slope count, and strict endpoint are
...
cap. The raw binomial counts are above budget on every clean anchor, but their
...
`V<=46`. The fixed cubic-Hermite majorant remains dead from 48 down.
...
derive the exact cutoff-free `V=46` profile/parity/light router and price its
actual-vector residue.
Alternatively attack the still broad `(4,2,0)` profile through its 419 exact
cofactor classes. Do not rerun any `V>=48`
