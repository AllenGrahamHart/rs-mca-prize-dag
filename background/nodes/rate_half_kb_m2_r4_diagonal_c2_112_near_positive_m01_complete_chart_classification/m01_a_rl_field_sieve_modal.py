#!/usr/bin/env python3
"""Run the M01-A-RL exact field sieve on Modal."""
...
def sieve() -> dict[str, object]:
    ...
    completed = subprocess.run(
        ["sage", "/m01_a_rl_field_sieve.sage"],
        env=environment,
        capture_output=True,
        text=True,
        timeout=900,
    ...
    for line in completed.stdout.splitlines():
        if line.startswith("M01_A_RL_FIELD_SIEVE_JSON "):
...
def main() -> None:
    result = sieve.remote()
    output = {
        "schema": "kb-c2-112-near-positive-m01-a-rl-field-sieve-modal-v1",
        "result": result,
    }
    output_path = HERE / "m01_a_rl_field_sieve_output.json"
