#!/usr/bin/env python3
"""Run the M01-A-RL first quotient probe on Modal."""
...
def probe() -> dict[str, object]:
    ...
    completed = subprocess.run(
        ["sage", "/m01_a_rl_first_quotient_probe.sage"],
        env=environment,
        capture_output=True,
        text=True,
        timeout=150,
    ...
    for line in completed.stdout.splitlines():
        if line.startswith("M01_A_RL_FIRST_QUOTIENT_JSON "):
...
def main() -> None:
    result = probe.remote()
    output = {
        "schema": "kb-c2-112-near-positive-m01-a-rl-first-quotient-modal-v1",
        "result": result,
    }
    output_path = HERE / "m01_a_rl_first_quotient_output.json"
