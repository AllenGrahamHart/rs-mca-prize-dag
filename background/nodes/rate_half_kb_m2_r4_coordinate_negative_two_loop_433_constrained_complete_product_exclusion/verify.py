#!/usr/bin/env python3
"""Verify the complete X2/N1/L1 paired-product exclusion."""
...
ROOT = NODE.parents[2]
NODE_ID = "rate_half_kb_m2_r4_coordinate_negative_two_loop_433_constrained_complete_product_exclusion"
...
def main() -> None:
    ...
    require("full `(4,3,3)`" in statement and "skeleton is closed" in statement,
            "claim")
    require("does not close another coordinate skeleton" in statement and
    ...
    require("units=450" in output and "unit_check=norm" in output, "unit census")
    require("chain0=60 chain1=15 chain2=0" in output, "projection census")
    require(3*2*5*15 == 450, "cell count")
    print(
        "RATE_HALF_KB_M2_R4_COORDINATE_NEGATIVE_TWO_LOOP_433_CONSTRAINED_COMPLETE_PASS "
        "cells_deleted=30 universal_templates=75 unit_obstructions=450"
    )
