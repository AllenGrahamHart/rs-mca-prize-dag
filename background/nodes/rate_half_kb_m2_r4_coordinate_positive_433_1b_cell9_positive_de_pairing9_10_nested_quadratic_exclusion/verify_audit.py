#!/usr/bin/env python3
"""Cross-audit the cell-9 positive pairing-9 ledgers."""
...
def terminal(payload):
    require(payload["status"] == "PASS" and payload["rows"] == 96
            ...
            and payload["missing_relation_nonzero"] == 2752
            and payload["uf_candidate_count"] == payload["uf_checked"] == 384
...
def main():
    ...
    for path in (
        EXP / "rate_half_kb_positive_433_1b_cell9_positive_de_pairing9_chart_scout_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_positive_de_pairing9_frobenius_roots_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_positive_de_pairing9_direct_audit_modal.py",
    ):
    ...
    require("other 11 active" in contract and "Prize" in contract,
            "scope markers")
    print("PASS cell-9 positive pairing-9 cross-audit: charts=6 profiles=53 "
          "hostile=detected")
