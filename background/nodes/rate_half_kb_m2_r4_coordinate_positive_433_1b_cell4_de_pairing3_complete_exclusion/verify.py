#!/usr/bin/env python3
"""Verify the cell-4 DE-missing pairing-3 48-case exclusion."""
...
PINNED = {
    SCRIPT: "9e3b21a458b5405051fca1ad7cabd9f6b07b09bf1e44bed358e80f4254d69d5c",
    RESULT: "dd0f1c7b64de33c6872d74cccb6148417c63e805a08b4da5a3253d6eae5dd47c",
}
PARENTS = (
...
def verify_payload(payload):
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-cell4-de-pairing3-"
    ...
    for row_index, row in enumerate(payload["rows"]):
        ...
        for finite in row["finite_rows"]:
            ...
            require((finite["missing"], finite["de"], finite["source_sum"]) ==
                    ...
                    "missing row replay")
            u_coefficients = paired_coefficients(a_values, b_values, de_value)
    ...
    require(collections.Counter(candidate_counts) == {9: 16, 10: 8, 11: 8},
            "candidate-root census")
    require(collections.Counter(source_counts) == {6: 16, 8: 8, 14: 8},
            "source-point census")
    require(collections.Counter(uv_counts) == {1: 16, 2: 8, 4: 8},
            "uv-candidate census")
    require(collections.Counter(target_root_counts) == {8: 16, 9: 8, 10: 8},
            "target-root census")
    require(sum(candidate_counts) == 312 and sum(source_counts) == 272 and
            sum(uv_counts) == 64 and aggregate == {
                "h_nonzero": 752,
                "colored_nonzero": 96,
                "target_boundaries": 16,
            }, "global terminal census")
    require(free_shapes == {
        (2381, 2382, 1538, 1511): 16,
        (2385, 2386, 1541, 1513): 16,
        (2383, 2384, 1542, 1517): 16,
        (2387, 2388, 1545, 1519): 16,
        (2379, 2380, 1536, 1509): 16,
        (2383, 2384, 1539, 1511): 16,
        (2387, 2388, 1544, 1517): 16,
        (2391, 2392, 1547, 1519): 16,
    }, "target-free shape census")
    require(norm_shapes == {
        (0, 3796, 3345, 1720, 1721): 8,
        (2, 3844, 3425, 1736, 1737): 8,
        (0, 3804, 3365, 1716, 1717): 8,
        (2, 3824, 3385, 1736, 1737): 8,
    ...
    require((len(norm_numerator_hashes), len(norm_denominator_hashes)) ==
            (16, 12), "specialized norm diversity")
...
def main():
    ...
    verify_dag()
    print(
        "cell=4 pairing=3 DE_copies=3 raw_cases=48 computed=32 "
        "transported=16 candidates=312 source_points=272 "
        "colored_nonzero=96 f_zero=16 witnesses=0"
    )
