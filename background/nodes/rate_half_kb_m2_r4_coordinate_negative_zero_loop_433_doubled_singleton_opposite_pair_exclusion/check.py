#!/usr/bin/env python3
"""Shared exact row checker for zero-loop 433 cell 1."""
...
def check_row(epsilon_1, epsilon_2, expected_basis,
    ...
    route = ROUTER.compile_route(epsilon_1, epsilon_2, cell_index=1)
    y, b = route["variables"]
    ...
    require(basis_count == expected_basis, "generic basis count")
    require(eliminant.degree() == expected_eliminant_degree,
            "generic eliminant degree")
    ...
    require(ROUTER.reconstruct(
        epsilon_1, epsilon_2, b_roots, route=route, cell_index=1
