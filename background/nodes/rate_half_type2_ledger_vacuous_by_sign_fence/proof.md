...
belongs to `rate_half_far_ca_crossing_offset_value_ledger` (posed as
Statement U at drafting time, refuted round 37, priced
`r+1 + Theta(n/rho)`), and that node's cap is supplied by an independent
pigeonhole for exactly this reason.
