...
3. Spend the prize-field-floor exclusion first: all `S=16` profiles are zero.
   ...
   has 419 and receives no such prize-interval reduction).
...
6. Use common-prime ideal/resultant constraints to bound low-mass vectors at
   one row prime. For the leading profile, start with `m=514`, whose residual
   has `10<=V<=50` and `V=2 mod 8`, before the four broader windows. Never
