...
`D=log(18^64/(2^mu p))`. The proved character-diagonal router now converts
that body into one explicit finite integer ellipsoid and coordinate box using
the 63 nonzero Fourier eigenvalues of the cyclic group generated by `5`. The
certified preflight first proves `|xi_t|<=7` and `sum xi_t^2<=101`, and the
inverse-kernel contraction sharpens this to `|xi_t|<=3` and
`sum|xi_t|<=60`. The preflight exactly counts more than `2^143` zero-sum
vectors in its coarse envelope. Even the weighted ellipsoid contains at
least `38,482,585,013,041` explicit
`0,+-1` vectors. Thus generic lattice-first branch-and-bound is not a priced
route. What remains is an algebraic sparse-product/inverse classification or
a sparse-first generator with a new conservative projection. The exact
maximum-weight target is at most 367 log-lattice points across the four
cofactors; 368 already exceed the complete edge budget. Lower-weight profiles
