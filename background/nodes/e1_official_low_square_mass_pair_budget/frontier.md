...
the 63 nonzero Fourier eigenvalues of the cyclic group generated by `5`.
What remains is a certified eigenvalue interval table and exact sparse
branch-and-bound, not an unbounded lattice search. The exact maximum-weight
target is at most 367 log-lattice points across the four cofactors; 368
already exceed the complete edge budget. Lower-weight profiles still enter
the exact weighted sum, so 367 is necessary rather than sufficient.
