#!/usr/bin/env python3
"""Independent roots and direct residual audit for cell-11 DE pairing 3."""
...
def main():
    ...
    require(
        root_payload["schema"]
            == "rate-half-kb-positive-433-1b-cell11-de-pairing3-independent-roots-v1"
    ...
    require(sum(len(row["roots"]) for row in root_rows.values()) == ROOT_TOTAL
            and max(row["degree"] for row in root_rows.values()) == 4960,
    ...
    require((uv_count, f_count, colored_count, f_boundary_count)
            == (128, 256, 256, 0), "direct residual census")
    print(f"PASS cell-11 pairing-3 audit: unique=49 roots={ROOT_TOTAL} uv=128 f=256")
