#!/usr/bin/env python3
"""Verify the N=256 E=29 profile/parity/light reduction."""
...
NODE = "e1_n256_s16_e29_profile_parity_light_reduction"
DEPENDENCIES = {
    "e1_n256_s16_e30_endpoint_exclusion",
...
def main() -> None:
    ...
    spec = importlib.util.spec_from_file_location("variance_parent_e29", parent_path)
    assert spec is not None and spec.loader is not None
    parent = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(parent)
    slack = parent.relaxed_minimum_energy_by_slack(40)
    trace = [[l1, 29 + 66 - 4 * l1, slack[29 + 66 - 4 * l1]] for l1 in range(23, 14, -1)]
    assert trace == [
        [23, 3, 53], [22, 7, 49], [21, 11, 45], [20, 15, 41],
        [19, 19, 37], [18, 23, 33], [17, 27, 29], [16, 31, 25], [15, 35, 21],
    ]

    packet = json.loads((ROOT / pin["probe_result_file"]).read_text())
    assert packet["schema"] == "e1-e29-profile-parity-probe-v1" and packet["complete"] is True
    assert packet["source_sha256"] == pin["probe_source_file_sha256"]
    assert packet["slack_trace"] == trace and int(packet["l1_bound"]) == 17
    profiles = energy_profiles()
    assert len(profiles) == 17 and packet["profiles"] == profiles
    ...
    assert Fraction(-593209, 2544224) * u2 + forms[0][0] * l87 + forms[0][1] * l6457 - forms[0][2] > 0
    assert Fraction(-593081, 2544224) * l2 + forms[1][0] * u87 + forms[1][1] * u6457 - forms[1][2] < 0
    assert int(packet["cubic_cutoff"]) == 872

    above = [row for row in profiles if int(row["cap"]) > 872]
    ...
    assert packet["diameter_ledgers"] == [[1, -36], [5, -34], [9, -32], [17, -28], [21, -26]]
    ...
    assert "M_3=872" in nodes[NODE]["statement"] and "111" in nodes[NODE]["statement"]

    print("E1_N256_S16_E29_PROFILE_PARITY_LIGHT_REDUCTION_PASS profiles=17 above=13 survivors=8 supports=15624 orbits=119 router=111 ledgers=5")
