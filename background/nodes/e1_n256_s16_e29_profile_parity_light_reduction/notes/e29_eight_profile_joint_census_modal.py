#!/usr/bin/env python3
"""Run the exact E29 eight-profile joint census on Modal."""
...
SOURCE = HERE / "e29_eight_profile_joint_census.cpp"
REDUCTION = HERE / "e29_profile_parity_probe_result.json"
RESULT = HERE / "e29_eight_profile_joint_census_result.json"
REMOTE_SOURCE = "/root/e29_eight_profile_joint_census.cpp"
REMOTE_BINARY = "/root/e29_eight_profile_joint_census"

app = modal.App("e1-n256-e29-eight-profile-joint-census")
image = (
...
def main() -> None:
    ...
    if not reduction["complete"]:
        raise RuntimeError("E29 profile reduction is incomplete")
    geometry = reduction["light_geometry"]["orbit_representatives"]
    tasks = list(geometry["1"]) + list(geometry["5"])
    if len(tasks) != 111:
        raise RuntimeError("E29 light-template count mismatch")
    ...
    def write_checkpoint(complete: bool) -> dict[str, object]:
        packet = {
            "schema": "e1-e29-eight-profile-joint-census-v1",
            "complete": complete,
    ...
    try:
        ...
        print(f"E29_EIGHT_PROFILE_JOINT_INCOMPLETE completed={len(rows)}/{len(tasks)}")
        raise
    packet = write_checkpoint(
        len(rows) == len(tasks)
        and all(bool(row["complete"]) for row in rows)
        and sum(int(row["vectors"]) for row in rows) == 2_203_120_896
    )
    print("E29_EIGHT_PROFILE_JOINT " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E29_EIGHT_PROFILE_JOINT_COMPLETE {packet['complete']}")
    print(f"E29_EIGHT_PROFILE_JOINT_RESULT {RESULT}")
