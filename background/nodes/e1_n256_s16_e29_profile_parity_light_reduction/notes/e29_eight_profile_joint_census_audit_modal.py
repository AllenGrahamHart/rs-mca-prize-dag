#!/usr/bin/env python3
"""Independently audit the E29 census by direct negacyclic products."""
...
SOURCE = HERE / "e29_eight_profile_joint_census_audit.cpp"
PRODUCTION = HERE / "e29_eight_profile_joint_census_result.json"
RESULT = HERE / "e29_eight_profile_joint_census_audit_result.json"
REMOTE_SOURCE = "/root/e29_eight_profile_joint_census_audit.cpp"
REMOTE_BINARY = "/root/e29_eight_profile_joint_census_audit"

app = modal.App("e1-n256-e29-eight-profile-joint-census-audit")
image = (
...
def main() -> None:
    ...
    if not production["complete"]:
        raise RuntimeError("E29 production census is incomplete")
    ...
    if len(tasks) != int(production["expected_templates"]):
        raise RuntimeError("E29 production task count mismatch")
    ...
    if RESULT.exists():
        ...
        if (
            prior.get("schema") == "e1-e29-eight-profile-joint-census-audit-v1"
    ...
    def write_checkpoint(complete: bool) -> dict[str, object]:
        ...
        packet = {
            "schema": "e1-e29-eight-profile-joint-census-audit-v1",
            "complete": complete,
    ...
    try:
        ...
        write_checkpoint(False)
        print(f"E29_EIGHT_PROFILE_JOINT_AUDIT_INCOMPLETE completed={len(rows)}/{len(tasks)}")
        raise
    ...
    print("E29_EIGHT_PROFILE_JOINT_AUDIT " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E29_EIGHT_PROFILE_JOINT_AUDIT_AGREEMENT {packet['agreement']}")
    print(f"E29_EIGHT_PROFILE_JOINT_AUDIT_RESULT {RESULT}")
