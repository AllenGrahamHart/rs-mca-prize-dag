...
constexpr int kCutoff = 872;
constexpr std::array<std::array<int, 5>, 8> kProfiles{{
    {{5, 6, 0, 0, 0}},
    {{1, 7, 0, 0, 0}},
    {{4, 4, 1, 0, 0}},
    {{0, 5, 1, 0, 0}},
...
int main(int argc, char** argv) {
  ...
  std::array<std::uint64_t, 8> above_cutoff{};
  std::array<std::uint64_t, 8> full_above_cutoff{};
  std::array<int, 8> maximum_m3{};
  std::array<int, 8> maximum_full_m3{};
  maximum_m3.fill(-1);
    ...
    for (int second = first + 1; second < static_cast<int>(allowed.size()) - 1;
      ...
      for (int third = second + 1; third < static_cast<int>(allowed.size());
        ...
        for (int mask = 0; mask < 64; ++mask) {
          ...
          matches.push_back(
              {profile_index, positions, coefficients, conductor, m3});
        }
      }
    }
  }
