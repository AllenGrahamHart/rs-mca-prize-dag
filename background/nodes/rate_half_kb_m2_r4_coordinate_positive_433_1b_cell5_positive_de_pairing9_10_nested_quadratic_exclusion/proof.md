# Proof

Fix source signs and a target lane. On the proved cell-5 four-basis tower,
canonical matching `9` gives

```text
P_u(u)=Pair(-de,u),        P_f(f)=Pair(de,b*f),
```

where `u=df`, and both cuts are quadratic. Since `d=u/f` and
`e=de*f/u`, the deleted positive-`DE` squared-sum equation gives

```text
R(u,f)=(u^2+de*f^2)^2-S*f^2*u^2=0.                 (RS5-DE9)
```

The pinned compiler uses the division-free degree-eight construction,
pseudo-reduces modulo `P_f`, and norms the necessary cut through
`1,t,b,bt`, retaining all leading-coefficient drops.

The adapter runs all 16 source-sign/target-lane rows. Its complete root union
has 336 case-labeled candidates and lifts to 576 guarded source points. Of
1,184 Cartesian `(u,f)` rows, 1,088 fail (RS5-DE9). The other 96 reconstruct
`d,e,v` and have a nonzero final cut
`Pair(sigma_o*v,sigma_c*c*f)`. No zero-`f` survivor, colored solution,
witness, or unresolved branch occurs.

An external compiled Frobenius/gcd pass reconstructs all 144 roots of the 33
unique polynomial profiles and certifies that each squarefree root-part
degree equals its printed root count. The local audit validates each profile
and directly replays all 1,184 `(u,f)` rows.

Exact exchange of the identical positive `DE` copies supplies `xi=1`,
and the generic matching transport supplies pairing `10`, giving the orbit
printed in the statement. QED.
