# Claim contract

- **claim:** the final F2 extras-plus-trades budget on every official
  prize-max row/depth accepted by the generated-field consumer guard
- **status:** direct TARGET; no conditional proof is currently accepted
- **proved suppliers:** empty and edge bands, full-ladder dictionary,
  contraction, near tails, structural census, and the admissible direct-sum
  reduction on the plus branch; the generic pointwise collision floor; and
  the coupled negacyclic top-window reduction on the minus branch
- **open surfaces:** common mass control for the five signed generating
  types, PP5.0 normalization, and any separate coset-sensitive Frobenius
  antipodal step used by a proposed proof
- **closed surface:** at fixed depth all seven non-generating families
  descend exactly to generated-field rows for the final block census and
  kernel mass
- **scope theorem:** the `(C)` depth passes the guard exactly at `k=e`;
  every `(T*)` depth with `t log2(q)<n` fails it on all official types
- **nonclaim:** `(O1)` is not assumed on non-generating rows; growing-order
  Myerson is not claimed sufficient
- **route alarm:** the bounded-class direct-sum claim is refuted on the
  `p=3 mod 4` branch, including an official generating row
- **falsifier:** a guard-accepted official row with more than `n^3` extras
  and trades
