#!/usr/bin/env python3
"""Stream exact FLINT norms for the bounded m=4 residual."""
...
SOURCE = HERE / "m4_residual_candidates.cpp"
OUTPUT = HERE / "m4_candidate_norms_result.json"
SHARDS = 32
BUCKETS = 64
MODULUS = 1 << 256
...
app = modal.App("e1-prize-m4-candidate-norms")
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    packet = {
        "schema": "e1-prize-m4-candidate-norms-v1",
...
def main() -> None:
    ...
    for result in compute.map(range(SHARDS), order_outputs=False):
        ...
        print(f"M4_CANDIDATE_NORMS_PROGRESS returned={len(results)}/{SHARDS}")
    ...
    print(f"M4_CANDIDATE_NORMS complete={complete}/{SHARDS} interval={interval}")
