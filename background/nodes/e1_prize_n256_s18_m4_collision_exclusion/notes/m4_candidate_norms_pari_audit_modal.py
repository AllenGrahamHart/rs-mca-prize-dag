#!/usr/bin/env python3
"""Replay the streamed m=4 residual with PARI and an independent census."""
...
SOURCE = HERE / "m4_residual_candidates_audit.cpp"
PRIMARY = HERE / "m4_candidate_norms_result.json"
OUTPUT = HERE / "m4_candidate_norms_pari_audit_result.json"
SHARDS = 32
BUCKETS = 64
MODULUS = 1 << 256
...
app = modal.App("e1-prize-m4-candidate-norms-pari-audit")
...
def update_fingerprint(
    ...
    target = fingerprint[digest[0] & (BUCKETS - 1)]
    target["count"] += 1
    target["xor"] ^= value
...
def audit(shard: int) -> dict[str, object]:
    ...
    assert len(norms) == len(candidates)
    counts = {
    ...
    minimum_above = None
    interval_rows = []
    for (energy, positions, coefficients), norm in zip(candidates, norms):
        assert norm % 4 == 0
        assert (norm & -norm).bit_length() - 1 == 2
        candidate = norm // 4
        row: dict[str, object] = {
            "positions": positions,
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    packet = {
        "schema": "e1-prize-m4-candidate-norms-pari-audit-v1",
        ...
        "counts": counts,
        "fingerprint": fingerprint,
        "primary_match": len(complete) == SHARDS and counts == primary["counts"] and fingerprint == primary["fingerprint"],
        "interval_rows": interval_rows,
        ...
        "shards": [
            {"shard": row["shard"], "processed": row["processed"], "wall_seconds": row["wall_seconds"]}
            for row in complete
        ],
    }
...
def main() -> None:
    ...
    for result in audit.map(range(SHARDS), order_outputs=False):
        ...
        print(f"M4_NORM_PARI_AUDIT_PROGRESS returned={len(results)}/{SHARDS}")
    print(
        "M4_NORM_PARI_AUDIT "
