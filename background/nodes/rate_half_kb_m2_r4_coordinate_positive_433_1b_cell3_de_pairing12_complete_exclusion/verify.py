#!/usr/bin/env python3
"""Verify the cell-3 DE-missing pairing-12 48-case exclusion."""
...
PINNED = {
    SCRIPT: "ef1c3b37ff1d118c8ba5de133510b1416a62b0678fa94b75b22b23e5defb2226",
    RESULT: "f5bc1919a5bb14eca172a6a8bf682d05d5e13f06cbda46c196e72cb394e2c2f0",
}
...
def verify_payload(payload):
    require(
        payload["schema"] ==
        "rate-half-kb-positive-433-1b-cell3-de-pairing12-"
    ...
    for row in payload["rows"]:
        ...
        require(sigma_o == -1 and row["pairing_index"] == 12,
    ...
    require(set(rows) == expected and len(rows) == 16,
            "sixteen source rows")
    require(len(all_lane_rows) == 64, "64 nonzero final-pair evaluations")
    require(len(boundaries) == 16, "sixteen source-level f=0 boundaries")
    require(sum(row["direct_lift"]["candidate_r_count"]
                for row in rows.values()) == 184,
            "candidate-r aggregate")
    require(sum(row["direct_lift"]["source_point_count"]
                for row in rows.values()) == 272,
...
def main():
    ...
    verify_dag()
    print(
        "cell=3 pairing=12 DE_copies=3 raw_cases=48 "
