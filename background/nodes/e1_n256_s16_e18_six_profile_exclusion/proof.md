...
The E18 router exhausts every candidate with six profiles on 1,321 affine
...
profile and conductor of all 2,994 retained representatives.

The proper-conductor theorem removes 3,718 of the 6,712 actual vectors,
including every `(0,0,2)` vector. For the 2,994 full-conductor
representatives, FLINT and PARI/GP independently compute
`abs(Res(X^128+1,F))` and agree entry by entry. Every norm is positive.

The whole-norm shortcut stops here because six norms reach `2^250`. For each
norm `R`, write `R=2^mu R_odd`. The pair-feasible prime `p>2^250` is odd, so
`p|R` implies `p|R_odd`. The exact dual ledger and checker show that every
...
norm criterion excludes all six profiles.
