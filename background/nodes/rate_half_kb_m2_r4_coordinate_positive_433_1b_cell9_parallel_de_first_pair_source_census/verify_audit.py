#!/usr/bin/env python3
"""Independently substitute the cell-9 source-census survivors."""
    ...
    for row in replay["rows"]:
        ...
        for point in row["witnesses"]:
            ...
            cut = (paired(a_values, b_values, m, -m % P)
                   if row["cut_kind"] == "positive"
                   else paired(a_values, b_values, -m % P, -m % P))
            require(cut == point["cut"] == 0, "source-cut substitution")
        for point in row["missing_free"]:
            values = [value(item, point) for item in kernel]
            label = -point["t"]*point["t"] % P
            require(all(sum(values[offset+index]*pow(label, index, P)
                            for index in range(3)) % P == 0
                        for offset in (0, 3)), "A=B=0 base point")
...
def main():
    ...
    hostile["rows"][0]["witnesses"][0]["cut"] = 1
    try:
        check(hostile)
    except RuntimeError:
        pass
    ...
    print("PASS cell-9 source audit: ordinary=24 base_rows=16 hostile=detected")
