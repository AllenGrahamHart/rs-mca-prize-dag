#!/usr/bin/env python3
"""Compute exact norms of all primitive exceptional profile-(6,6) vectors."""
...
ACTUAL = HERE / "e30_profile66_exceptional_actual_result.json"
RESULT = HERE / "e30_profile66_exceptional_norm_result.json"
BATCH_SIZE = 16

app = modal.App("e1-n256-e30-profile66-exceptional-norm")
...
def main() -> None:
    ...
    if not actual["complete"]:
        raise RuntimeError("profile-(6,6) actual census is incomplete")
    vectors = [
        match
    ...
    if len(vectors) != int(actual["summary"]["full_above_cutoff"]):
        raise RuntimeError("primitive exception count mismatch")
    ...
    def write_checkpoint(complete: bool) -> dict[str, object]:
        ...
        packet: dict[str, object] = {
            "schema": "e1-e30-profile66-exceptional-norm-v1",
            "complete": complete,
            "agreement": agreement,
    ...
    try:
        ...
        write_checkpoint(False)
        print(
            "E30_PROFILE66_EXCEPTIONAL_NORM_INCOMPLETE "
    ...
    print("E30_PROFILE66_EXCEPTIONAL_NORM " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E30_PROFILE66_EXCEPTIONAL_NORM_AGREEMENT {packet['agreement']}")
    print(f"E30_PROFILE66_EXCEPTIONAL_NORM_RESULT {RESULT}")
