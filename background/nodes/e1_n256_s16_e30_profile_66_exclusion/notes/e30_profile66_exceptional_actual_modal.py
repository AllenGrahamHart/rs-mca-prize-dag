#!/usr/bin/env python3
"""Census actual profile-(6,6) vectors on cubic-exceptional light masks."""
...
SOURCE = HERE / "e30_profile66_exceptional_actual_census.cpp"
RELAXATION = HERE / "e30_profile66_odd_difference_scan_result.json"
RESULT = HERE / "e30_profile66_exceptional_actual_result.json"
REMOTE_SOURCE = "/root/e30_profile66_exceptional_actual_census.cpp"
REMOTE_BINARY = "/root/e30_profile66_exceptional_actual_census"

app = modal.App("e1-n256-e30-profile66-exceptional-actual")
image = (
...
def main() -> None:
    ...
    try:
        ...
        write_checkpoint(False)
        print(f"E30_PROFILE66_EXCEPTIONAL_ACTUAL_INCOMPLETE completed={len(rows)}/{len(tasks)}")
        raise
    packet = write_checkpoint(
        len(rows) == len(tasks) and all(bool(row["complete"]) for row in rows)
    )
    print("E30_PROFILE66_EXCEPTIONAL_ACTUAL " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E30_PROFILE66_EXCEPTIONAL_ACTUAL_COMPLETE {packet['complete']}")
    print(f"E30_PROFILE66_EXCEPTIONAL_ACTUAL_RESULT {RESULT}")
