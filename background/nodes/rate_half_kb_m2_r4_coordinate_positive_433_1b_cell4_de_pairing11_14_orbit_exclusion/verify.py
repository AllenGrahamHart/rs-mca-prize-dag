#!/usr/bin/env python3
"""Verify the cell-4 pairing-11/14 DE orbit composition."""
...
PAIRING11 = (
    ...
    "cell4_de_pairing11_complete_exclusion"
)
QUOTIENT = (
    "rate_half_kb_m2_r4_coordinate_positive_433_1b_"
...
PREVIOUS = (
    "rate_half_kb_m2_r4_coordinate_positive_433_1b_"
    "cell4_de_pairing8_13_complete_exclusion"
)
...
def verify_composition():
    ...
    require(len(previous) == 39 and len(previous_orbits) == 21,
            "previous complete ledger")

    direct = {(0, 11), (1, 11), (2, 11)}
    new_orbits = {orbit for orbit in orbits if orbit & direct}
    new_labels = set().union(*new_orbits)
    require(new_orbits == {
        frozenset(((0, 11), (1, 11))),
        frozenset(((2, 11), (2, 14))),
    } and new_labels == {(0, 11), (1, 11), (2, 11), (2, 14)},
            "honest pairing-11/14 orbit closure")
    require((0, 14) not in new_labels and (1, 14) not in new_labels,
            "positive pairing-14 scope fence")
    ...
    require(len(paid) == 43 and len(paid_orbits) == 23 and
            sum(map(len, paid_orbits)) == 43,
            "aggregate paid ledger")
    require(len(live_orbits) == 37 and sum(map(len, live_orbits)) == 62,
...
def verify_documents():
    ...
    require("= 64 raw cases" in compact_statement and
            "62 labels in 37 quotient orbits" in compact_statement,
            "printed aggregate")
    require("105-43 = 62" in proof and "60-23 = 37" in proof,
            "ledger subtraction")
    require("`(0,14)` and `(1,14)`" in frontier and
...
def main():
    ...
    print(
        ...
        "paid_labels=43 live_labels=62 live_orbits=37"
    )
