#!/usr/bin/env python3
"""Jointly census the three live E31 profiles on eight affine templates."""
...
SOURCE = HERE / "e31_three_profile_joint_census.cpp"
ORBITS = HERE.parent.parent / "e1_n256_s16_sparse_l1_variance_exclusion/notes/e31_three_odd_light_orbit_result.json"
RESULT = HERE / "e31_three_profile_joint_census_result.json"
REMOTE_SOURCE = "/root/e31_three_profile_joint_census.cpp"
REMOTE_BINARY = "/root/e31_three_profile_joint_census"

app = modal.App("e1-n256-e31-three-profile-joint-census")
image = (
...
def main() -> None:
    ...
    def write_checkpoint(complete: bool) -> dict[str, dict[str, int]]:
        ...
        packet = {
            "schema": "e1-e31-three-profile-joint-census-v1",
            "complete": complete,
    ...
    try:
        ...
        print(f"E31_THREE_PROFILE_JOINT_INCOMPLETE completed={len(rows)}/8 result={RESULT}")
        raise
    complete = len(rows) == 8 and all(bool(row["complete"]) for row in rows)
    summary = write_checkpoint(complete)
    print("E31_THREE_PROFILE_JOINT_CENSUS " + json.dumps(summary, sort_keys=True))
    print(f"E31_THREE_PROFILE_JOINT_COMPLETE {complete}")
    print(f"E31_THREE_PROFILE_JOINT_RESULT {RESULT}")
