...
- `pma_three_petal_projective_johnson_bound`, which pays every rate-quarter
  `M=4,t=3` cell and every positive-denominator rate-half three-petal module.
- `pma_ratehalf_core_triple_excess_reduction`, which turns three contributors
  in the remaining rate-half tail into a source-coupled positive
  `0/1/infinity` fiber-excess certificate and records why a generic
  smooth-core exclusion is false.
- `pma_ratehalf_source_aligned_gcd_excess`, which writes the source word as a
  three-dimensional CRT quotient code and turns the remaining excess into one
  exact polynomial-gcd inequality.
- `pma_ratehalf_source_crossratio_fiber_reduction`, which normalizes the three
  labels to one cross-ratio and forces `2b+3a-2` weighted nonexceptional votes
  for its source-label fiber.
- `pma_ratehalf_two_petal_support_fiber_reduction`, which reconstructs every
  exact contributor from its core agreement set and identifies the fixed-cell
  list with one guarded subset-map fiber.
- `pma_ratehalf_complement_linear_slice_reduction`, which complements the
  support and identifies that guarded fiber with split divisors in an exact
  codimension-`ell+a-1` linear slice modulo `L_2L_3`.
...
monic exact-defect locus in the four fixed mu-basis modules or routing it once
to an existing natural-scale owner, only in the explicit nonpositive-
denominator rate-half top-defect tail. Equivalently, it is enough to exclude or
own the source-coupled reduced rational maps whose three distinguished fibers
contain at least `2r+3a` core points. This final form must use
`y=-P_*/(L_1L_2L_3)`; arbitrary smooth-core list decoding is too weak.
Equivalently, exclude or own the tuples satisfying
`u_12+u_13+u_23-tau>=2(K_0-1)+3a` in the source quotient code.
After affine normalization, it is enough to bound or own the corresponding
positive weighted `lambda` fiber.
Equivalently, bound or profile-own the exact guarded fiber `Phi(S)=lambda` in
the two petal quotient rings.
Equivalently, bound or profile-own the monic degree-`2ell-a` divisors of
`L_C` in the complement linear slice, with its exactness gcd retained.
The aggregate theorem need only treat the residual
