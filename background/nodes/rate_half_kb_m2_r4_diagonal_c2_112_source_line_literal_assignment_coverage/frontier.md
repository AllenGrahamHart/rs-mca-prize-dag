...
`{F00,F01} x {R02,R11,R20}` block, the pinned GREEN #1141 import closes all
six `F02/F03` cells, the replayed subset of #1144 closes ten moving cells,
and the exact balanced fixed localization closes four `F04`--`F07-R11`
cells. Direct `F04-R02` and `F07-R02` compositions plus complete companion
inversion close all four fixed `R02` cells. Current aligned-positive coverage
is `30/36`.

The residual consists of balanced `M01-R11`, its literal `M02-R11`
companion, and all four `F04`--`F07-R20` cells. The #1149
...
All sixteen literal rank-drop factor branches in the fixed `R02/R20` cells
are PROVED empty. On the generic charts the resultant splits with degrees
`3,3,12`; literal replay and complete inversion close both cubic branches in
all eight cells. All four `R02` degree-12 branches are closed as well. The
fixed frontier is therefore exactly the four `R20` degree-12 resultant
branches. They have genuine nonnamed leading-drop
...
branchwise. The `s=0` and both literal degree-6 forms have already been
replayed and closed in all eight literal fixed cells, so the live `R20`
