#!/usr/bin/env python3
"""Verify the cell-12 reciprocal matching-3/6 packet."""
...
PINNED = {
    SCRIPT: "9f898b1c2fdcfd6e9e1f8115266d9b1946bb1c727f439fab22a51778742fd781",
    RESULT: "da0258841961221613a997c964143d4b94c9d6adfc8551842314280802632487",
...
def main():
    ...
    require(
        payload["schema"]
        == "rate-half-kb-positive-433-1b-cell12-xi3-pairing3-adapter-v1"
    ...
    require(
        sum(row["target_norm_root_count"] for row in payload["rows"]) == 68
        and sum(row["candidate_root_count"] for row in payload["rows"]) == 120
        and sum(row["source_point_count"] for row in payload["rows"]) == 176
        and sum(row["route_point_count"] for row in payload["rows"]) == 176
        and sum(row["z_candidate_count"] for row in payload["rows"]) == 40,
        "exact census totals",
    )
    require(
        len(all_lanes) == 80
    ...
    print("PASS cell-12 reciprocal pairing 3/6: rows=8 candidates=120 q_gcds=80")
