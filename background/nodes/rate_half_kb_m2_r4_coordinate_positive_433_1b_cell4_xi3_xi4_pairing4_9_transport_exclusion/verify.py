#!/usr/bin/env python3
"""Verify the cell-4 xi3/xi4 pairing-4/9 transport corollary."""
...
def main():
    ...
    require("role-cell-4" in direct_statement
            and "matching index 4" in direct_statement,
            "direct-parent scope")
    ...
    require("{4,9}" in matching_statement
    ...
    require("4 source signs * 4 target lanes * 4 labels = 64" in statement,
            "statement payment")
    require("transports `(3,4)` to `(3,9)`" in proof
            and "gives `(4,4)`" in proof
            and "gives `(4,9)`" in proof,
            "four-label composition")
    print("cell=4 xi=3,4 pairings=4,9 transport=exact raw_cases=64")
