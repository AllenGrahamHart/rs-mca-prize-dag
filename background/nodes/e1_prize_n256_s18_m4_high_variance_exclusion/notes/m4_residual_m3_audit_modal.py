#!/usr/bin/env python3
"""Folded-pair audit of the normalized m=4 third-moment frontier."""
...
SOURCE = HERE / "m4_residual_m3_audit.cpp"
OUTPUT = HERE / "m4_residual_m3_audit_result.json"
SHARDS = 32
...
app = modal.App("e1-prize-m4-residual-m3-audit")
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    packet = {
        "schema": "e1-prize-m4-residual-m3-audit-v1",
        "complete": len(complete) == SHARDS,
        "source_sha256": hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
        "normalization": {"singleton_positions": [0, 2]},
...
def main() -> None:
    ...
    for result in audit.map(range(SHARDS), order_outputs=False):
        ...
        print(f"M4_RESIDUAL_M3_AUDIT_PROGRESS returned={len(results)}/{SHARDS}")
    print(
        "M4_RESIDUAL_M3_AUDIT "
