#!/usr/bin/env python3
"""Verify the exact K'=73 full-carrier-atlas payment contract."""
...
CONTRACT_SHA256 = "dbc22b8c504a950b10be711ac8348617b4bef5cab40dc06a1a87a04f46d28bf8"
PROBE_PATH = ROOT / "experiments/prize_resolution/rate_half_mca_rank11_k72_two_step_probe.py"
...
PROBE = load_module("carrier_atlas_for_k73_contract", PROBE_PATH)
K71 = PROBE.K71
...
def active_conservative_caps():
    ...
    left = K71.base23_vector(kprime, baseline, 41, 33)
    exact45, _, _ = K71.exact45_rows(kprime, baseline)
    middle = next(
        vector for s4, s5, vector in exact45 if (s4, s5) == (33, 31)
    )
...
def validate(data: object) -> dict[str, int]:
    ...
    require(data["schema"].endswith("k73-full-carrier-atlas-payment-v1"), "schema")
    ...
    require(
        p["new_closed_prefix"] == [10, 73] and p["next_open_row"] == 74,
        "prefix",
    )
    ...
    require(
        plain["unsafe_distinct_defect_tuples"] == len(tuples) == 218,
        "unsafe count",
    )
    require(len(set(tuples)) == 218, "unsafe uniqueness")
    require(
        all(
            len(row) == 4 and all(0 <= value <= 63 for value in row)
            for row in tuples
        ),
        "unsafe range",
    )
    require(
        all(63 - row[1] > 63 - row[0] > 0 for row in tuples),
        "reroute positive support-three offset",
    )

    caps = active_conservative_caps()
    require(K71.premium(caps) == plain["safe_maximum"], "active premium")
    require(
    ...
    require(reroute["cells"] == 218 and reroute["all_safe"], "reroute")
    require(
        reroute["maximum"] + reroute["premium_margin"]
        == data["row"]["safe_premium_ceiling"],
        "reroute margin",
    )
    lanes = data["geometry_lanes"]
    require(len(lanes) == 7, "lane count")
    require(
        sum(row["evaluations"] for row in lanes)
        == data["geometry_total_evaluations"],
        "lane evaluations",
    )
    require(
        all(
            row["maximum"] < data["row"]["safe_premium_ceiling"]
            for row in lanes
        ),
        "lane safety",
    )
    require(
        max([reroute["maximum"]] + [row["maximum"] for row in lanes])
        < plain["safe_maximum"],
        "global maximum",
    )

    row = data["row"]
    old = K71.LEDGER.row(73)
    ...
    require(
        full + row["kernel_capacity"] == row["total_capacity"],
        "total capacity",
    )
...
def tamper_selftest(data: dict) -> int:
    mutations = (
        lambda item: item["parameters"].__setitem__("kprime", 74),
