#!/usr/bin/env python3
"""Run sequential saturation audits for all 48 literal boundary cells."""
...
app = modal.App("rs-mca-k3-near-positive-projective-boundary-literal-audit")
image = (
...
def audit(cell: str) -> dict[str, object]:
    ...
    environment["BOUNDARY_SATURATION_MODE"] = "sequential"
...
def main(cell: str = "") -> None:
    ...
    calls = {name: audit.spawn(name) for name in cells}
    results = {name: calls[name].get() for name in cells}
    output = {
        "schema": "kb-c2-112-near-positive-projective-boundary-literal-sequential-audit-modal-v1",
        "results": results,
    }
    output_path = HERE / "literal_boundary_sequential_audit_output.json"
