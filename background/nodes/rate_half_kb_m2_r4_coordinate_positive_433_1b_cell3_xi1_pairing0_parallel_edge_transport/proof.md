...
Thus the transposition of positions `0` and `1` fixes both complete record
rows pointwise.  It also fixes the target representatives `(1,b,c,d,e,f)`,
so all nonzero and pairwise-not-opposite guards are unchanged.
...
The missing product and missing squared-sum equations are also identical,
because positions `0` and `1` both carry `de` and `(d+e)^2`.  Common-curve
...
systems for `xi=0,pairing=0` and `xi=1,pairing=0` are identical after the
displayed transposition.

The first system has no guarded solution by `(KBP1B3-XI0P0-1)`.  Therefore
