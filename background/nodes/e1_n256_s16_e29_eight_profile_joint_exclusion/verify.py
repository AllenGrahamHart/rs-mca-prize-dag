#!/usr/bin/env python3
"""Verify the E29 eight-profile joint exclusion packet."""
...
NODE = "e1_n256_s16_e29_eight_profile_joint_exclusion"
ENDPOINT = "e1_n256_s16_e29_endpoint_exclusion"
REDUCTION = "e1_n256_s16_e29_profile_parity_light_reduction"
...
EXPECTED_SUMMARY = {
    ...
    "profile_counts": [34_810, 5_568, 11_468, 560, 4_252, 2_134, 148, 2_468],
    "above_cutoff": [3_090, 520, 604, 112, 486, 0, 0, 0],
    "full_above_cutoff": [544, 128, 148, 0, 0, 0, 0, 0],
    "maximum_m3": [1_332, 1_008, 1_038, 1_068, 1_110, 684, 648, 708],
    "maximum_full_m3": [1_164, 1_008, 984, 624, 732, 684, 216, 456],
}
...
def check_censuses(pin: dict[str, str]) -> list[dict[str, object]]:
    ...
    assert reduction["schema"] == "e1-e29-profile-parity-probe-v1" and reduction["complete"] is True
    geometry = reduction["light_geometry"]["orbit_representatives"]
    tasks = list(geometry["1"]) + list(geometry["5"])
    ...
    assert production["schema"] == "e1-e29-eight-profile-joint-census-v1"
    assert audit["schema"] == "e1-e29-eight-profile-joint-census-audit-v1"
    ...
    assert len(first) == len(second) == len(tasks) == 111
...
def check_norms(pin: dict[str, str], vectors: list[dict[str, object]]) -> None:
    ...
    assert packet["vectors"] == vectors and len(vectors) == 820
    assert packet["flint_norms"] == packet["pari_norms"] and len(packet["flint_norms"]) == 820
    assert packet["summary"] == {
        "distinct_norms": 242,
        "maximizing_indices": [244, 247, 249, 250],
        "maximum_norm": MAXIMUM_NORM,
        "maximum_norm_bits": 247,
        "norm_at_or_above_2_250": 0,
        "vectors": 820,
    }
    assert 9 * MAXIMUM_NORM < 2**250 < 10 * MAXIMUM_NORM
...
def main() -> None:
    ...
    assert "9*N_max<2^250<10*N_max" in nodes[NODE]["statement"]

    print("E1_N256_S16_E29_EIGHT_PROFILE_JOINT_EXCLUSION_PASS templates=111 vectors=2203120896 profile=61408 exceptions=4812 full=820 max_bits=247 engines=4")
