#!/usr/bin/env python3
"""Verify the corank-two projective-basis kernel capacity cut."""
...
def record_cap(kprime: int, d: int, p: dict[str, object]) -> int:
    ...
    if d == 2:
        return int(p["projective_corank2_record_cap"])
    if d == 9:
...
def certificate(kprime: int, p: dict[str, object]) -> dict[str, object]:
    ...
    require(optimum == cap_dual[1] * caps[0] + cap_dual[2] * caps[1], "duality")
    ...
    require(branches[:2] == ["ambient", "ambient"], "active branches")
    require(all(0 < value <= cap for value, cap in zip(allocation, caps)), "individual caps")
    require(allocation[0] == caps[0] and allocation[1] == caps[1], "root caps")
    require(all(allocation[i] < caps[i] for i in range(2, 9)), "nonroot caps")
...
def validate(data: object, files: bool) -> dict[str, int]:
    ...
    require(data.get("schema") == "rate-half-mca-rank11-kernel-corank2-projective-capacity-cut-v1", "schema")
    require(data.get("dependencies") == [
        "rate_half_mca_rank11_kernel_projective_pair_capacity_cut",
    ...
    require(p["active_shared_resources"] == [], "shared resources")
    require(p["positive_coranks"] == list(range(1, 10)), "positive coranks")
    require(p["dual_tree"] == [list(edge) for edge in TREE], "dual tree")
    require(len(p["tight_hierarchy_rows"]) == 17, "tight rows")
...
def main() -> None:
    ...
    mutations = (
        lambda item: item["parameters"].__setitem__("projective_corank2_record_cap", 84416264),
        lambda item: item["parameters"].__setitem__("closed_dimension_maximum", 568339),
        ...
        lambda item: item["parameters"].__setitem__("dual_tree", item["parameters"]["dual_tree"][:-1]),
        lambda item: item["parameters"].__setitem__("tight_hierarchy_rows", item["parameters"]["tight_hierarchy_rows"][:-1]),
    )
    caught = 0
    ...
    print(
        ...
        f"checked={result['checked']} wall={result['wall']} controls={caught}/{len(mutations)}"
    )
