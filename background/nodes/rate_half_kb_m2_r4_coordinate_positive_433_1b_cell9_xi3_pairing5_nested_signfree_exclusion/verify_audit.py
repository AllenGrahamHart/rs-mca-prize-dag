#!/usr/bin/env python3
"""Cross-audit the cell-9 xi3 pairing-5 ledgers."""
...
def terminal(payload):
    require(
        payload["status"] == "PASS"
        and payload["rows"] == 48
        and payload["profiles"] == 69
        and payload["profile_visits"] == 768
        and payload["target_norm_root_count"] == 576
        and payload["candidate_root_count"] == 1016
        and payload["source_point_count"] == payload["route_point_count"] == 1152
        and payload["missing_free"] == 96
        and payload["missing_impossible"] == 48
        and payload["product_boundaries"] == 48
        and payload["checked"] == 960
        and payload["missing_z_roots"] == payload["d_lifts"] == 2976
        and payload["q_intersections"] == 2976
        and payload["common_q_roots"] == payload["q_candidate_count"] == 288
        and payload["lane_checks"] == payload["third_pair_nonzero"] == 576
    ...
    require(
        ...
        and all(
            row["status"] == "PASS"
            and row["rows"] == 8
            and row["lane_checks"] == row["third_pair_nonzero"] == 96
...
def main():
    ...
    sources = (
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing5_chart_scout_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing5_frobenius_roots_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing5_direct_audit_modal.py",
    )
    for path in sources:
    ...
    require(
        "other three active" in (NODE / "claim_contract.md").read_text(),
        "scope marker",
    )
    require(
        "12 leading-open labels" in (NODE / "frontier.md").read_text(),
        "frontier marker",
    )
    print(
        "PASS cell-9 xi3 pairing-5 cross-audit: "
        "charts=6 profiles=69 lanes=576 hostile=detected"
    )
