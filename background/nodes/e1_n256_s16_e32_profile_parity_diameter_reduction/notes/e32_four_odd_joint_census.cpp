...
void update(ProfileLedger& ledger, const std::array<int, 7>& positions,
...
int main(int argc, char** argv) {
  ...
  ProfileLedger profile_47;
  ProfileLedger profile_351;
    ...
    for (int second = first + 1; second < static_cast<int>(allowed.size()) - 1;
      ...
      for (int third = second + 1; third < static_cast<int>(allowed.size());
        ...
        for (int mask = 0; mask < 64; ++mask) {
          ...
          if (!above_three && ones == 4 && twos == 7 && threes == 0) {
            update(profile_47, positions, coefficients, conductor, half);
          }
          if (!above_three && ones == 3 && twos == 5 && threes == 1) {
            update(profile_351, positions, coefficients, conductor, half);
          }
        }
      }
    }
  ...
  auto print_ledger = [&](const ProfileLedger& ledger) {
  ...
  std::cout << ",\"supports\":" << supports << ",\"vectors\":" << vectors
            << ",\"profile_47\":";
  print_ledger(profile_47);
  std::cout << ",\"profile_351\":";
  print_ledger(profile_351);
  std::cout << "}\n";
  return 0;
}
