#!/usr/bin/env python3
"""Cross-audit the cell-9 pairing-11 sharded and replay ledgers."""
...
def terminal(payload):
    require(payload["status"] == "PASS" and payload["rows"] == 192
            and payload["profiles"] == 73 and payload["profile_visits"] == 1920
            and payload["target_root_count"] == 1968
            and payload["candidate_root_count"] == 3152
            and payload["source_point_count"] == payload["route_point_count"]
            == 2496 and payload["missing_free"] == 384
            and payload["missing_impossible"] == 192
            and payload["product_boundaries"] == 192
            and payload["checked"] == 1728
            and payload["uf_candidate_count"] == 1152
            and payload["colored_nonzero"] == 1152
...
def main():
    for name in ("verify.py",):
        ast.parse((NODE / name).read_text())
    for path in (
        EXP / "rate_half_kb_positive_433_1b_cell9_de_pairing11_chart_scout_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_de_pairing11_frobenius_roots_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_de_pairing11_direct_audit_modal.py",
    ):
    ...
    hostile["colored_nonzero"] -= 1
    try:
        terminal(hostile)
    except RuntimeError:
        pass
    else:
        raise RuntimeError("hostile colored-cut mutation survived")
    contract = (NODE / "claim_contract.md").read_text()
    require("other 18 active" in contract and "Prize closure" in contract,
            "scope markers")
    print("PASS cell-9 pairing-11 cross-audit: charts=6 profiles=73 "
          "hostile=detected")
