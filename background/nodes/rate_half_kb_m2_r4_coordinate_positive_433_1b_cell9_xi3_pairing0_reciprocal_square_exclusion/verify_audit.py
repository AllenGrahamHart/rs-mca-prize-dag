#!/usr/bin/env python3
"""Cross-audit the cell-9 xi3 pairing-0 ledgers."""
...
def terminal(payload):
    require(
        ...
        and payload["profiles"] == 113
        and payload["profile_visits"] == 2016
        and payload["target_norm_root_count"] == 1392
        and payload["candidate_root_count"] == 2424
        and payload["source_point_count"] == payload["route_point_count"] == 1968
        ...
        and payload["checked"] == 1248
        and payload["common_y_roots"] == 288
        and payload["yd_candidate_count"] == payload["d_lifts"] == 576
        and payload["third_pair_nonzero"] == 1152
...
def main():
    ...
    sources = (
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing0_chart_scout_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing0_frobenius_roots_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing0_direct_audit_modal.py",
    )
    for path in sources:
    ...
    require(
        "other eight active" in contract and "Prize" in contract,
        "scope markers",
    )
    require("28 leading-open labels" in frontier, "frontier marker")
    print(
        "PASS cell-9 xi3 pairing-0 cross-audit: charts=6 profiles=113 "
        "hostile=detected"
    )
