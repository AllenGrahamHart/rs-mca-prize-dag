#!/usr/bin/env python3
"""Compute an exact R20 elimination curve at the KoalaBear prime on Modal."""
...
APP_NAME = "rs-mca-k3-degree12-r20-b0-target-elimination"
...
OUTPUT = HERE / "modal_r20_b0_target_elimination_output.json"
CASES = ({"cell": "F04-R20"},)
...
@app.function(image=image, cpu=4, memory=32768, timeout=900, max_containers=1)
def run_case(case: dict[str, str]) -> dict[str, object]:
    ...
    try:
        completed = subprocess.run(
            [
                "sage",
                "/r20_b0_function_field_probe.sage",
                "--cell",
                case["cell"],
                "--target-elimination",
            ],
...
def main() -> None:
    ...
    output = {
        "schema": "kb-c2-112-degree12-r20-b0-target-elimination-modal-v1",
        "app": APP_NAME,
