#!/usr/bin/env python3
"""Verify complete cell-4 pairing-4/9 composition."""
...
ORBIT = (
    ...
    "cell4_de_pairing4_9_orbit_exclusion"
)
POSITIVE = (
    "rate_half_kb_m2_r4_coordinate_positive_433_1b_"
    "cell4_positive_de_pairing9_complete_exclusion"
)
...
def verify_labels():
    ...
    orbit_parent = {(0, 4), (1, 4), (2, 4), (2, 9)}
    positive = {(0, 9), (1, 9)}
    complete = orbit_parent | positive
    require(complete == {(xi, pairing)
                         for xi in (0, 1, 2) for pairing in (4, 9)},
    ...
    require(len(paid) == 21 and len(paid_orbits) == 12,
            "cumulative paid ledger")
    require(sum(map(len, live_orbits)) == 84 and len(live_orbits) == 48,
...
def verify_documents():
    ...
    require("= 96 raw cases" in statement and
            "84 labels in 48 quotient orbits" in statement,
            "printed statement ledger")
    require("105-21=84" in proof and "60-12=48" in proof,
            "printed subtraction")
...
def main():
    ...
    verify_dag()
    verify_documents()
    print("cell=4 pairings=4,9 labels=6 raw_cases=96 live_labels=84 live_orbits=48")
