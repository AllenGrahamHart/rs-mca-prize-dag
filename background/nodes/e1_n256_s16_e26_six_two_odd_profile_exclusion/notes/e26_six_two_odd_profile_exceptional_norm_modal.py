#!/usr/bin/env python3
"""Compute exact norms of all primitive exceptional E26 two-odd vectors."""
...
AUDIT = HERE / "e26_six_two_odd_profile_census_audit_result.json"
RESULT = HERE / "e26_six_two_odd_profile_exceptional_norm_result.json"
BATCH_SIZE = 32

app = modal.App("e1-n256-e26-six-two-odd-profile-exceptional-norm")
...
def main() -> None:
    ...
    if not audit["complete"] or not audit["agreement"]:
        raise RuntimeError("E26 direct census audit is incomplete")
    if audit["production_sha256"] != production_sha256:
        raise RuntimeError("E26 direct census audit does not pin production")
    vectors = [
        match
    ...
    if len(vectors) != sum(int(value) for value in production["summary"]["full_above_cutoff"]):
        raise RuntimeError("primitive E26 exception count mismatch")
    ...
    def write_checkpoint(complete: bool) -> dict[str, object]:
        ...
        packet: dict[str, object] = {
            "schema": "e1-e26-six-two-odd-profile-exceptional-norm-v1",
            "complete": complete,
            "agreement": agreement,
    ...
    try:
        ...
        write_checkpoint(False)
        print(
            "E26_SIX_TWO_ODD_PROFILE_EXCEPTIONAL_NORM_INCOMPLETE "
    ...
    print("E26_SIX_TWO_ODD_PROFILE_EXCEPTIONAL_NORM " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E26_SIX_TWO_ODD_PROFILE_EXCEPTIONAL_NORM_AGREEMENT {packet['agreement']}")
    print(f"E26_SIX_TWO_ODD_PROFILE_EXCEPTIONAL_NORM_RESULT {RESULT}")
