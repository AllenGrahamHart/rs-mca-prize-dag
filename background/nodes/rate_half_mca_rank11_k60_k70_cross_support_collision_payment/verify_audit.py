#!/usr/bin/env python3
"""Independent audit of the K'=60..70 cross-support collision payments."""
...
PARENT_AUDIT = ROOT / "background/nodes/rate_half_mca_rank11_k54_k59_small_support_collision_payment/verify_audit.py"
...
def load_parent():
    spec = importlib.util.spec_from_file_location("small_audit_for_cross", PARENT_AUDIT)
...
def cross_count(K: int, m: int, c: int, d: int, s: int) -> int:
    q = K - 10
    need(0 <= s < q and c + d <= 11, "cross domain")
    b = q + c - 1 - s
    if s == 0:
        return comb(b, d)
    N = m - b
    ...
    for j in range(1, d + 1):
        total += comb(b, d - j) * comb(N, j - 1) * (s + d - j) // j
    return total


def cross_cap(K: int, m: int, c: int, d: int, s: int) -> int:
    return cross_count(K, m, c, d, s) * comb(m - d, 11 - d)


def exact_caps(K: int, c: int, s: int, baseline: dict[int, int]):
    q = K - 10
    m = 67472 + K
    caps = PARENT.exact_caps(K, c, s, baseline)
    if s < q:
        for d in SUPPORTS:
            if c + d <= 11:
                caps[d] = min(caps[d], cross_cap(K, m, c, d, s))
    return caps
    ...
    for s_left in range(q + 1):
        ...
        for s_right in range(q + 1):
            ...
            if (left, right) == (4, 5) and s_left + s_right < q:
                vector[2] = min(
                    vector[2],
                    BASE.joint_support4_cap(K, 67472 + K, s_left, s_right),
                )
...
def branch(K: int) -> dict[str, object]:
    ...
    raw69, front69 = PARENT.group_high(K, baseline)
    maximum = (-1, "", ())
    total = 0
    ...
    return {
        ...
        "active_small_defects": {
            str(c): int(parts[c - 2].split("=")[1]) for c in range(2, 6)
        },
        "active_branch": maximum[1],
        "active_caps": {
            str(t): maximum[2][i] for i, t in enumerate(SUPPORTS)
        },
...
def main() -> None:
    ...
    gaps = [audit_row(K, rows[str(K)], False) for K in range(60, 71)]
    wall = audit_row(71, rows["71"], True)
    need(data["parameters"]["remaining_rank9_interval"] == [71, 15528], "remaining")
    need(
        min(gaps)
    ...
    print(
        "RATE_HALF_MCA_RANK11_K60_K70_CROSS_SUPPORT_COLLISION_PAYMENT_AUDIT_PASS "
        f"rows=11 minimum_gap={min(gaps)} wall={-wall}"
    )
