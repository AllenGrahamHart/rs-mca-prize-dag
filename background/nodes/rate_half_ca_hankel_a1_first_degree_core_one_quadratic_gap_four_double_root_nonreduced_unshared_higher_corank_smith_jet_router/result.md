# Result

An unshared nonreduced nonzero-jet survivor is now routed to quotient-root
collision or one of only three noncollision profiles:

```text
[1,3] with kappa_2=0 and kappa_3!=0;
[2,2] with kappa_2!=0;
[1,1,2] with kappa_2!=0.
```

The corank-four profile `[1,1,1,1]` forces both jets to vanish. Thus the
formerly unrestricted higher-corank escape has only three exact Smith/jet
types plus one explicit locator collision.
