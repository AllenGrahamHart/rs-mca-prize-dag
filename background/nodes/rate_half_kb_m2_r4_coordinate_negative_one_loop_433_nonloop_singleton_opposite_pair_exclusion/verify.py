#!/usr/bin/env python3
"""Verify the one-loop 433 opposite-pair exclusion."""
...
ROUTER = ROOT / (
    "experiments/prize_resolution/rate_half_kb_one_loop_433_cell11_router.py"
)
...
def main():
    ...
    require(
        [router.ATLAS.BASE.cells()[index][0] for index in (11, 14)] == [3, 4],
        "cell orbit",
    )

    statement = (NODE / "statement.md").read_text()
    require("- **status:** PROVED" in statement, "status")
    require("orbit\n`[11,14]` is empty" in statement, "claim")
    ...
    print(
        "RATE_HALF_KB_ONE_LOOP_433_OPPOSITE_VERIFY_PASS "
        "cells=11,14 sign_rows=8 survivors=0"
    )
