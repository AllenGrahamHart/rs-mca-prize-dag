#!/usr/bin/env python3
"""Hostile controls for the uncolored DE exclusion."""
...
SPEC = importlib.util.spec_from_file_location("de_verify", NODE / "verify.py")
...
def main():
    ...
    mutation = list(copy.deepcopy(baseline))
    mutation[0]["rows"][0]["u_values"].pop()
    reject(mutation, "u cover")
    mutation = list(copy.deepcopy(baseline))
    mutation[0]["rows"][0]["fibers"][0]["status"] = "EMPTY_ENDPOINT_FIBERS"
    reject(mutation, "boundary status")
    mutation = list(copy.deepcopy(baseline))
    ...
    print("PASS uncolored DE hostile audit: 4/4")
