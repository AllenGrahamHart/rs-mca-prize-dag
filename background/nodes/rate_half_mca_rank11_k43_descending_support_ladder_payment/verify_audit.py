#!/usr/bin/env python3
"""Independent arithmetic audit for the K'=43 completion ladder payment."""
...
def branch_premiums(kprime: int) -> dict[str, int]:
    ...
    branches["__".join(prefixes)] = sum(
        DEFICITS[support] * inherited[support] for support in range(2, 10)
    )
    return branches
...
def audit_row(kprime: int, declared: dict[str, object], wall: bool) -> tuple[int, int]:
    ...
    require(declared["active_branch"] == active == "c5_defect_2", "active")
    ...
    if wall:
        ...
        require(raw // 55 == gap - 1 and raw % 55 == 38, "floor orientation")
    return gap, raw
...
def main() -> None:
    ...
    p = data["parameters"]
    gap43, _ = audit_row(43, p, False)
    gap44, _ = audit_row(44, p["K44_method_wall"], True)
    require(len(p["branch_premiums"]) == 27, "leaf count")
    print(
        "RATE_HALF_MCA_RANK11_K43_DESCENDING_SUPPORT_LADDER_PAYMENT_AUDIT_PASS "
        f"gap={gap43} wall={-gap44} branches=54"
    )
