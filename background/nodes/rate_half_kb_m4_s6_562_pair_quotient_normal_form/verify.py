#!/usr/bin/env python3
"""Verify the rigid S6 [5,6,2] pair-quotient certificate."""
...
ROOT = NODE.parents[2]
PRODUCER = ROOT / "experiments/prize_resolution/rate_half_kb_m4_s6_562_pair_quotient.py"
CERTIFICATE = ROOT / "experiments/prize_resolution/rate_half_kb_m4_s6_562_pair_quotient_result.json"
...
def load_producer():
    spec = importlib.util.spec_from_file_location("kb_m4_s6_562", PRODUCER)
...
def validate(data: dict[str, object], expected: dict[str, object]) -> None:
    if data.get("schema") != "rate_half_kb_m4_s6_562_pair_quotient_v1":
...
def hostile_mutations(data: dict[str, object]) -> list[dict[str, object]]:
    ...
    changed(lambda row: row["pair_curve"]["resultant_factor_degrees"].pop())
    changed(lambda row: row["pair_curve"].__setitem__("moving_factor", "0"))
    changed(lambda row: row["quotient"].__setitem__("degree", 14))
    changed(lambda row: row["quotient"]["fiber_zero"].pop())
    changed(lambda row: row["quotient"]["fiber_one"].__setitem__(0, 5))
...
def main() -> None:
    ...
    print("RATE_HALF_KB_M4_S6_562_PAIR_QUOTIENT_NORMAL_FORM_PASS")
    print("RATE_HALF_KB_M4_S6_562_PAIR_QUOTIENT_MUTATIONS_13_OF_13_REJECTED")
