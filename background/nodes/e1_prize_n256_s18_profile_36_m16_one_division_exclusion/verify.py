#!/usr/bin/env python3
"""Verify the once-divided profile-(3,6) cofactor-16 exclusion."""
...
NODE = "e1_prize_n256_s18_profile_36_m16_one_division_exclusion"
...
ORBIT_WEIGHTS = {
    "1": 8, "2": 14, "3": 57, "4": 9, "5": 164, "6": 94,
    "7": 704, "8": 354, "9": 1768, "10": 623, "11": 2209,
    "12": 465, "13": 1360, "14": 471, "15": 780,
}
Q_FRONTIERS = {
...
PRIMARY_COUNTS = {
    "orbits": 9080,
    "triple_syndromes": 2680779200,
    "distance_tests": 76819415040,
...
def main() -> None:
    ...
    assert branch["modulus"] == 64 and branch["lift"] == 2
    assert branch["quotient_multiplicity"] == 2
    assert branch["examined"] == comb(62, 4) == 557845
    assert branch["matching"] == 139360
    assert branch["orbit_weights"] == ORBIT_WEIGHTS
    orbits = [tuple(orbit) for orbit in branch["orbits"]]
    assert len(orbits) == len(set(orbits)) == 9080

    enumerated: set[tuple[int, ...]] = set()
    matching = 0
    for tail in combinations(range(2, 64), 4):
        quotient = (0, 1) + tail
        if multiplicity(quotient) != 2:
            continue
        matching += 1
        enumerated.add(canonical(quotient, 64))
    assert matching == 139360 and len(enumerated) == 9080
    quotients = {tuple(value // 2 for value in orbit) for orbit in orbits}
    assert quotients == enumerated
    for orbit in orbits:
        assert len(orbit) == 6 and orbit == tuple(sorted(orbit))
        assert all(value % 2 == 0 for value in orbit)
        assert any(value % 4 == 2 for value in orbit)
    ...
    assert primary["branch"] == audit["branch"] == "one_division"
    assert primary["completed_batches"] == primary["expected_batches"] == 2270
    assert audit["completed_batches"] == audit["expected_batches"] == 2270
    assert all(row["valid"] for row in primary["rows"])
    assert all(
        subrow["valid"] and subrow["returncode"] == 0 and not subrow["stderr"]
        for row in primary["rows"] for subrow in row["subrows"]
    )
    assert all(row["valid"] for row in audit["rows"])
    assert all(
        subrow["valid"] and subrow["primary_exact_match"]
        and subrow["returncode"] == 0 and not subrow["stderr"]
        for row in audit["rows"] for subrow in row["subrows"]
    )
    assert primary["orbit_file_sha256"] == audit["orbit_file_sha256"] == pin["atlas_sha256"]
    assert primary["base_engine_sha256"] == audit["primary_engine_sha256"] == pin["primary_engine_sha256"]
    assert primary["base_patch_sha256"] == pin["primary_patch_sha256"]
    assert primary["early_transform_sha256"] == pin["primary_transform_sha256"]
    assert audit["audit_engine_sha256"] == pin["audit_engine_sha256"]
    assert audit["audit_patch_sha256"] == pin["audit_patch_sha256"]
    ...
    assert PRIMARY_COUNTS["triple_syndromes"] == 9080 * comb(122, 3)
    assert comb(58, 3) == 30856
    assert PRIMARY_COUNTS["distance_tests"] == 9080 * 32 * (comb(122, 3) - comb(58, 3))
    ...
    for audit_key, primary_key in PRIMARY_COMPARISONS.items():
        assert audit["counts"][audit_key] == PRIMARY_COUNTS[primary_key]
    assert primary["counts"]["screen_below"] == 0
    assert primary["counts"]["screen_above"] == 0
    ...
    assert min(audit_high) == 15 and max(audit_high) == 27

    primary_by_orbit = {
        tuple(subrow["orbit"]): subrow["counts"]
        for row in primary["rows"] for subrow in row["subrows"]
    }
    assert len(primary_by_orbit) == 9080
    for row in audit["rows"]:
        for subrow in row["subrows"]:
            expected = primary_by_orbit[tuple(subrow["orbit"])]
            assert subrow["primary_counts"] == expected
            for audit_key, primary_key in PRIMARY_COMPARISONS.items():
                assert subrow["counts"][audit_key] == expected[primary_key]
    ...
    for energy_value, odd_weight, l1_norm, state in records:
        ...
        assert any(position % 2 for position, _ in state)
        ...
        assert any(position % 4 == 2 for position in support)
    ...
    for text in ("once-divided", "9,080", "1,816,625,504", "remain open"):
        assert text in statement
    for text in ("73175732492", "585405859936", "1816625308", "196"):
    ...
    print(
        "E1_PRIZE_N256_S18_PROFILE_36_M16_ONE_DIVISION_EXCLUSION_PASS "
        "orbits=9080 product_live=1816625504 separated=1816625504 witnesses=196"
    )
