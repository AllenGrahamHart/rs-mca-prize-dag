#!/usr/bin/env python3
"""Cross-audit the cell-9 xi3 pairing-7 ledgers."""
...
def main():
    ...
    sources = (
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing7_chart_scout_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing7_frobenius_roots_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing7_direct_audit_modal.py",
    )
    for path in sources:
    ...
    require(
        "other two active" in (NODE / "claim_contract.md").read_text(),
        "scope marker",
    )
    require(
        "eight leading-open labels" in (NODE / "frontier.md").read_text(),
        "frontier marker",
    )
    print(
        "PASS cell-9 xi3 pairing-7 cross-audit: "
