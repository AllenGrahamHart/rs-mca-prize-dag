...
Canonical matching 11 is `((0,4),(1,5),(2,3))`. The first two equations
are quadratics in the same variable `f`:

```text
P_b(f) = paired(de,bf),
P_c(f) = paired(second_de,sigma_c cf).
```
...
```text
(-1,0): 7 norm roots, 2 live, 9 candidate r, 10 source points,
...
(+1,0): 7 norm roots, 2 live, 9 candidate r, 8 source points,
         0 (u,f), 0 boundaries, 0 final-lane checks;
(+1,2): 7 norm roots, 2 live, 9 candidate r, 12 source points,
         0 (u,f), 2 f=0 boundaries, 0 final-lane checks.
```

Across all sixteen rows this is 148 candidate `r` values, 168 source points,
...
sum, matching 11, and all target guards value-for-value, so the 16 `xi=1`
