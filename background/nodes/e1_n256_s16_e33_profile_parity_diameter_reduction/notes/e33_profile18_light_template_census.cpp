...
std::array<int, 4> light_template(int index) {
  ...
  if (5 <= index && index < 10) {
    const int x = 1 << (index - 5);
    return {{0, 64, x, 64 - x}};
  }
  if (index == 10) return {{0, 16, 32, 64}};
  return {{-1, -1, -1, -1}};
}
...
int main(int argc, char** argv) {
  ...
  const int shard = std::atoi(argv[2]);
  const int shards = std::atoi(argv[3]);
  if (template_index < 0 || template_index >= 11 || shards <= 0 || shard < 0 ||
      shard >= shards) {
    return 2;
  }
  ...
  for (int first = 0; first < static_cast<int>(allowed.size()) - 2; ++first) {
    for (int second = first + 1; second < static_cast<int>(allowed.size()) - 1;
      ...
      for (int third = second + 1; third < static_cast<int>(allowed.size());
           ++third) {
        const std::uint64_t index_here = support_index++;
        if (index_here % static_cast<std::uint64_t>(shards) !=
        ...
        for (int mask = 0; mask < 64; ++mask) {
          ...
          if (ones != 1 || twos != 8 || other) continue;
          ++profile_18;
