#!/usr/bin/env python3
"""Exhaust the six actual light templates for the E33 profile (1,8)."""
...
RESULT = HERE / "e33_profile18_light_template_census_result.json"
REMOTE_SOURCE = "/root/e33_profile18_light_template_census.cpp"
REMOTE_BINARY = "/root/e33_profile18_light_template_census"
SHARDS = 8

app = modal.App("e1-n256-e33-profile18-light-template-census")
image = (
...
def main() -> None:
    templates = [template for template in range(11) for _ in range(SHARDS)]
    shards = list(range(SHARDS)) * 11
    ...
    summary = {
        template: {
            ...
            "profile_18": sum(
                int(row["profile_18"]) for row in rows if row["template"] == template
            ),
            "full_conductor": sum(
    ...
    packet = {
        "schema": "e1-e33-profile18-light-template-v1",
    ...
    print("E33_PROFILE18_LIGHT_TEMPLATE_CENSUS " + json.dumps(summary, sort_keys=True))
    print(f"E33_PROFILE18_LIGHT_TEMPLATE_RESULT {RESULT}")
