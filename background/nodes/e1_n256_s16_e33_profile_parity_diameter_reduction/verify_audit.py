#!/usr/bin/env python3
"""Independent audit of the E=33 profile and diameter frontier."""
...
def recursive_profiles() -> list[tuple[tuple[int, ...], int]]:
    ...
    def visit(magnitude: int, energy: int, l1_norm: int, counts: list[int]) -> None:
        if magnitude == 6:
            ...
            return
        maximum = (33 - energy) // (magnitude * magnitude)
...
def main() -> None:
    ...
    above = [(counts, cap) for counts, cap in profiles if cap > 1732]
    assert len(above) == 8
    survivors = [
        (counts, cap) for counts, cap in above if sum(counts[0::2]) <= 5
    ]
    assert [counts for counts, _ in survivors] == [
        (5,7,0,0,0), (1,8,0,0,0), (4,5,1,0,0), (0,6,1,0,0),
    ]
    ...
    for moment in (1732, 1733):
        raw_moments = (1, 16, 16**2 + 66, 16**3 + 3 * 16 * 66 + moment)
    ...
    assert (
        Fraction(-543033,2544224) * u2
        + Fraction(74749,79507) * l87
        + Fraction(4758,79507) * l6457
        + Fraction(601,38829)
    ) > 0
    assert (
        Fraction(-542905,2544224) * l2
        + Fraction(74751,79507) * u87
        + Fraction(4756,79507) * u6457
        + Fraction(7589,491834)
    ) < 0

    ledgers = matching_ledgers()
    assert ledgers == {(1,1), (5,1), (9,1), (17,1), (21,1)}
    for deleted in ((8,4,1,0,0), (11,1,2,0,0), (13,1,0,1,0), (7,2,2,0,0)):
        assert sum(deleted[0::2]) > 5

    print(
        "E1_N256_S16_E33_PROFILE_PARITY_DIAMETER_REDUCTION_AUDIT_PASS "
        "recursive_profiles=21 hermite=independent matchings=complete mutations=4"
    )
