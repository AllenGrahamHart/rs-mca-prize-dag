#!/usr/bin/env python3
"""Replay logarithmic-denominator intersections for three literal assignments."""
...
APP_NAME = "rs-mca-k3-fixed-literal-full-j-log-denominator-intersections"
...
FULL_IDENTITY = HERE / "modal_full_identity_literal_replay_output.json"
LOG_DERIVATIVE = HERE / "modal_log_derivative_literal_replay_output.json"
SCRIPT = HERE / "denominator_intersection_probe.sage"
OUTPUT = HERE / "modal_literal_denominator_intersections_output.json"
CASES = tuple(
    {"assignment": assignment, "target": target, "pair_index": pair_index}
...
image = (
    ...
    .add_local_file(LOG_DERIVATIVE, "/log_derivative.json")
    .add_local_file(SCRIPT, "/denominator_intersection_probe.sage")
)


@app.function(image=image, cpu=2, memory=16384, timeout=600, max_containers=12)
def run_case(case: dict[str, object]) -> dict[str, object]:
    ...
    try:
        completed = subprocess.run(
            [
                "sage",
                "/denominator_intersection_probe.sage",
                "--assignment",
                str(case["assignment"]),
                "--target",
                str(case["target"]),
...
def main() -> None:
    ...
    output = {
        "schema": "kb-c2-112-fixed-literal-log-denominator-intersections-modal-v1",
        "app": APP_NAME,
