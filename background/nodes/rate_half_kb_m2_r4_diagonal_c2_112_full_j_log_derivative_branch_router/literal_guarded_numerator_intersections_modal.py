#!/usr/bin/env python3
"""Replay guarded logarithmic-numerator intersections literally."""
...
APP_NAME = "rs-mca-k3-fixed-literal-full-j-log-guarded-numerator-intersections"
...
SCRIPT = HERE / "guarded_numerator_intersection_probe.sage"
OUTPUT = HERE / "modal_literal_guarded_numerator_intersections_output.json"
CASES = tuple(
...
image = (
    ...
    .add_local_file(SCRIPT, "/guarded_numerator_intersection_probe.sage")
)


@app.function(image=image, cpu=2, memory=16384, timeout=1020, max_containers=6)
def run_case(case: dict[str, object]) -> dict[str, object]:
    ...
    try:
        completed = subprocess.run(
            [
                "sage",
                "/guarded_numerator_intersection_probe.sage",
                "--assignment",
                str(case["assignment"]),
                "--target",
                str(case["target"]),
...
def main() -> None:
    ...
    output = {
        "schema": "kb-c2-112-fixed-literal-log-guarded-numerator-intersections-modal-v1",
        "app": APP_NAME,
