...
Canonical matching 7 is `((0,3),(1,4),(2,5))`. Put `u=ef`. For fixed
`sigma_o`, the first paired equation is the quadratic

```text
P_u(u) = paired(de,sigma_o u),
```

and the second is `P_f(f)=paired(second_de,b f)`. Neither depends on
`sigma_c`. Let `m,s` be the missing product and squared-sum values. Put
...
`P_f` leaves a linear remainder; its quadratic resultant gives a
...
For every source sign and `sigma_o`, an `xi=0` row has nine norm roots,
four live norm roots, 14 candidate `r` values, 20 guarded source points,
and no `(u,f)` candidate. An `xi=2` row has eleven norm roots, six live
roots, 15 candidate `r` values, 36 source points, and six `(u,f)`
candidates. Two have `f=0`, violating the target nonzero guard. Each of
the other four fails

```text
paired(df,sigma_c c f)=0
```

in both `sigma_c` lanes.

Across the sixteen source rows the exact ledger has 232 candidate `r`
values, 448 guarded source points, 48 `(u,f)` candidates, 16 terminal
`f=0` boundary records, and 64 nonboundary third-pair evaluations. Every
third-pair value is nonzero, with zero witnesses and zero unresolved
branches.

The sixteen computed source rows pay `16*2=32` raw cases. Deleting the
other positive parallel `DE` copy preserves the residual products,
missing squared sum, matching 7, and every target guard value-for-value, so
