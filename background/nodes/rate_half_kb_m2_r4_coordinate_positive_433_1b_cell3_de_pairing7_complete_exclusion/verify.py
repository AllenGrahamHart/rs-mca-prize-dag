#!/usr/bin/env python3
"""Verify the cell-3 DE-missing pairing-7 48-case exclusion."""
...
NODE_ID = NODE.name
SCRIPT = EXPERIMENTS / (
    "rate_half_kb_positive_433_1b_cell3_de_pairing7_"
...
PINNED = {
    SCRIPT: "856a8c1b5cafb75d1d1f155a45c813dd7f93fa8adef999a92161c2a0bd3b3d55",
    RESULT: "a7e72fa9039b4660f8e3f82cb26860c00cf3a5b14969376e6bb6e237bf2a17de",
}
SIGNS = set(itertools.product((-1, 1), repeat=2))
EXPECTED_MATCHING = ((0, 3), (1, 4), (2, 5))
...
def verify_payload(payload):
    require(
        payload["schema"] ==
        "rate-half-kb-positive-433-1b-cell3-de-pairing7-"
    ...
    expected = {
        (epsilon, sigma_o, xi)
        for epsilon in SIGNS for sigma_o in (-1, 1) for xi in (0, 2)
    }
    rows = {}
    all_lane_rows = []
    boundaries = []
    for row in payload["rows"]:
        ...
        key = (tuple(row["epsilon"]), sigma_o, row["xi_index"])
        require(key in expected and key not in rows, "source Cartesian row")
        rows[key] = row
        covered_lanes = {(-1, sigma_o), (1, sigma_o)}
        require(sigma_c == -1 and row["pairing_index"] == 7,
...
def main():
    ...
    verify_dag()
    print(
        "cell=3 pairing=7 DE_copies=3 raw_cases=48 "
