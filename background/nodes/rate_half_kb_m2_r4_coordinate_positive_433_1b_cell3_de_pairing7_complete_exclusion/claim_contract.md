...
- **Computed source rows:** `16`, each fixing `sigma_o` and covering
  both `sigma_c` lanes
- **Computed raw cases:** `32`
...
- **Elimination:** quadratic `P_u`, quadratic `P_f`, degree-eight
  ...
  norm
- **Terminal boundary ledger:** sixteen source-level records, all exactly
  `f=0` and each covering both `sigma_c` lanes at fixed `sigma_o`
