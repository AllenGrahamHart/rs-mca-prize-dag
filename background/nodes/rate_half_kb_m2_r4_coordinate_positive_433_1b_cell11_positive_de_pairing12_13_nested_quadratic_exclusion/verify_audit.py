#!/usr/bin/env python3
"""Direct residual replay for the cell-11 positive-DE pairing-12 packet."""
...
def main():
    ...
    leading = {
        tuple(row["epsilon"]): {
            "CELL11_B_LEADING": sp.sympify(row["b_leading"]["expression"]),
            "CELL11_C_LEADING": sp.sympify(row["c_leading"]["expression"]),
        }
    ...
    require(
        root_payload["schema"]
        == (
            "rate-half-kb-positive-433-1b-cell11-positive-de-pairing12-"
            "frobenius-roots-v1"
        )
    ...
    for row in payload["rows"]:
        ...
        for item in row["boundary_rows"]:
            if item["stage"] not in ("CELL11_B_LEADING", "CELL11_C_LEADING"):
                continue
            expression = leading[tuple(row["epsilon"])][item["stage"]]
    ...
    print(
        "PASS positive pairing-12 adapter audit: profiles=160 unique=33 "
        f"roots={ROOT_TOTAL} boundary=32 uf=256 lifts=32 colored=32"
    )
