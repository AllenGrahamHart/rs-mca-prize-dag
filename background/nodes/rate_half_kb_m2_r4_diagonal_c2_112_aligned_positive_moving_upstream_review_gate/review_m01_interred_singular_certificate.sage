#!/usr/bin/env sage
"""Replay M01-R11 after interreducing the exact J standard basis."""
...
SINGULAR_SCRIPT = Path("/tmp/m01_interred_certificate.sing")
...
def main():
    ...
    lines.extend(
        [
            ...
            'print("J_RAW_BASIS_BEGIN");',
            "ideal J_raw_basis=slimgb(J_input);",
            'print("J_RAW_BASIS_SIZE="+string(size(J_raw_basis)));',
            'print("J_RAW_DIMENSION="+string(dim(J_raw_basis)));',
            'print("J_INTERRED_BEGIN");',
            "ideal J_basis=interred(J_raw_basis);",
            'print("J_INTERRED_SIZE="+string(size(J_basis)));',
            'print("J_INTERRED_DIMENSION="+string(dim(J_basis)));',
            "poly I_remainder=0;",
        ]
    )
