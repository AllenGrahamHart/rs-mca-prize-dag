...
At matching `12=((0,5),(1,2),(3,4))`, put `u=df`. The first two paired cuts
are

```text
P_u(u)=Pair(-de,u),        P_f(f)=Pair(de,sigma_c*cf),
```
...
The division-free degree-eight construction in the matching-5 theorem
...
`e=de/d`, and checks `Pair(sigma_o*ef,bf)` and the target guards.

The direct ledger has 224 candidate `r` values and 320 guarded source
points. Of 1,088 `(u,f)` rows, 960 fail (1); all 128 survivors have a
