...
At matching `13=((0,5),(1,3),(2,4))`, put `u=ef`. The first two paired cuts
are

```text
P_u(u)=Pair(-de,sigma_o*u),        P_f(f)=Pair(de,sigma_c*cf),
```

and both are quadratic. Since `e=u/f` and `d=de*f/u`, the omitted squared-
sum equation is
...
The division-free degree-eight construction in the matching-8 theorem
...
source point it solves `P_u=P_f=0`, tests (1), reconstructs `e=u/f` and
`d=de/e`, and checks `Pair(df,bf)` and the target guards.

The direct ledger has 224 candidate `r` values, 208 target-norm roots, and
320 guarded source points. Of 1,088 `(u,f)` rows, 960 fail (1); all 128
survivors have a nonzero final colored-pair cut. There is no boundary,
colored solution, witness, or unresolved branch, proving `xi=0` empty.
