...
At matching 4, put `u=df`. The first two paired cuts are the quadratics

```text
P_u(u) = Pair(de,u),       P_f(f) = Pair(second_de,bf),
```
...
`Pair(sigma_o*ef,sigma_c*cf)` and all target guards.
