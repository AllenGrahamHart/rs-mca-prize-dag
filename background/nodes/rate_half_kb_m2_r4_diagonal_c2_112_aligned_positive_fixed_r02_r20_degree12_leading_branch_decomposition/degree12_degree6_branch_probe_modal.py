#!/usr/bin/env python3
"""Run common degree-6 leading-drop probes on Modal."""
...
MODE = os.environ.get("DEGREE12_DEGREE6_MODE", "f04")
APP_NAME = f"rs-mca-k3-degree12-degree6-{MODE}"
...
SCRIPT = HERE / "degree12_degree6_branch_probe.sage"
OUTPUT = HERE / f"modal_degree12_degree6_{MODE}_output.json"
...
image = (
    ...
    .add_local_file(SCRIPT, "/degree12_degree6_branch_probe.sage")
)


@app.function(image=image, cpu=4, memory=8192, timeout=900, max_containers=6)
def run_case(case: dict[str, object]) -> dict[str, object]:
    ...
    command = [
        "sage",
        "/degree12_degree6_branch_probe.sage",
        "--cell",
        str(case["cell"]),
    ]
    try:
...
def main() -> None:
    ...
    output = {
        "schema": "kb-c2-112-fixed-degree12-degree6-modal-v1",
        "app": APP_NAME,
        "mode": MODE,
