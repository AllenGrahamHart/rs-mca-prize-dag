#!/usr/bin/env python3
"""Run s=0 degree-12 branch probes on Modal."""
...
MODE = os.environ.get("DEGREE12_S_ZERO_MODE", "f04")
APP_NAME = f"rs-mca-k3-degree12-s-zero-{MODE}"
...
SCRIPT = HERE / "degree12_s_zero_probe.sage"
if MODE == "f06_f07":
    OUTPUT = HERE / "modal_degree12_s_zero_f06_f07_output.json"
    CASES = tuple(
        {"cell": f"{assignment}-{target}"}
        for assignment in ("F06", "F07")
        for target in ("R02", "R20")
    )
elif MODE == "f05":
    OUTPUT = HERE / "modal_degree12_s_zero_f05_output.json"
    CASES = ({"cell": "F05-R02"}, {"cell": "F05-R20"})
else:
    OUTPUT = HERE / "modal_degree12_s_zero_output.json"
    CASES = ({"cell": "F04-R02"}, {"cell": "F04-R20"})
...
image = (
    ...
    .add_local_file(SCRIPT, "/degree12_s_zero_probe.sage")
)


@app.function(image=image, cpu=2, memory=8192, timeout=360, max_containers=2)
def run_case(case: dict[str, object]) -> dict[str, object]:
    ...
    command = ["sage", "/degree12_s_zero_probe.sage", "--cell", str(case["cell"])]
    try:
...
def main() -> None:
    ...
    output = {
        "schema": "kb-c2-112-f04-degree12-s-zero-modal-v1",
        "app": APP_NAME,
        "mode": MODE,
