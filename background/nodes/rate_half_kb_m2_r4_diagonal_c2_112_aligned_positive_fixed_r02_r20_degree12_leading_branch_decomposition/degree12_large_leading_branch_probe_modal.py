#!/usr/bin/env python3
"""Run E2/E3 large leading-factor probes on Modal."""
...
MODE = os.environ.get("DEGREE12_LARGE_LEADING_MODE", "f04_r02")
APP_NAME = f"rs-mca-k3-degree12-large-leading-{MODE}"
...
SCRIPT = HERE / "degree12_large_leading_branch_probe.sage"
OUTPUT = HERE / f"modal_degree12_large_leading_{MODE}_output.json"

if MODE == "f04_r02":
    CASES = tuple({"cell": "F04-R02", "row": row} for row in ("E2", "E3"))
else:
...
image = (
    ...
    .add_local_file(SCRIPT, "/degree12_large_leading_branch_probe.sage")
)
...
def run_case(case: dict[str, object]) -> dict[str, object]:
    ...
    command = [
        "sage",
        "/degree12_large_leading_branch_probe.sage",
        "--cell",
        str(case["cell"]),
        "--row",
        str(case["row"]),
...
def main() -> None:
    ...
    output = {
        "schema": "kb-c2-112-fixed-degree12-large-leading-modal-v1",
        "app": APP_NAME,
        "mode": MODE,
