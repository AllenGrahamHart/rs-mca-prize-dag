#!/usr/bin/env python3
"""Run bounded degree-12 leading-syzygy probes on Modal."""
...
MODE = os.environ.get("DEGREE12_SYZYGY_MODE", "r02")
APP_NAME = f"rs-mca-k3-f04-degree12-leading-syzygy-{MODE}"
...
SCRIPT = HERE / "degree12_leading_syzygy_probe.sage"
OUTPUT = HERE / f"modal_degree12_leading_syzygy_{MODE}_output.json"
...
image = (
    ...
    .add_local_file(SCRIPT, "/degree12_leading_syzygy_probe.sage")
)


@app.function(image=image, cpu=4, memory=8192, timeout=240, max_containers=2)
def run_case(case: dict[str, object]) -> dict[str, object]:
    ...
    command = [
        "sage",
        "/degree12_leading_syzygy_probe.sage",
        "--cell",
        str(case["cell"]),
        "--steps",
        "3",
...
def main() -> None:
    ...
    output = {
        "schema": "kb-c2-112-f04-degree12-leading-syzygy-modal-v1",
        "app": APP_NAME,
        "mode": MODE,
