#!/usr/bin/env python3
"""Run degree-12 function-field route probes on Modal."""
...
MODE = os.environ.get("DEGREE12_FUNCTION_FIELD_MODE", "r02")
APP_NAME = f"rs-mca-k3-f04-degree12-function-field-{MODE}"
...
SCRIPT = HERE / "degree12_function_field_probe.sage"
OUTPUT = HERE / f"modal_degree12_function_field_{MODE}_output.json"
CELLS = ("F04-R02",) if MODE == "r02" else ("F04-R02", "F04-R20")
CASES = tuple({"cell": cell} for cell in CELLS)
...
image = (
    ...
    .add_local_file(SCRIPT, "/degree12_function_field_probe.sage")
)


@app.function(image=image, cpu=4, memory=16384, timeout=540, max_containers=2)
def run_case(case: dict[str, object]) -> dict[str, object]:
    ...
    command = ["sage", "/degree12_function_field_probe.sage", "--cell", str(case["cell"])]
    try:
...
def main() -> None:
    ...
    output = {
        "schema": "kb-c2-112-f04-degree12-function-field-modal-v1",
        "app": APP_NAME,
        "mode": MODE,
