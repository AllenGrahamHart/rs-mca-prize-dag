#!/usr/bin/env python3
"""Independent arithmetic and scope audit for the cell-4/cell-7 transport."""
...
def common4(b, c, r, t, iota, e1, e2):
    return {
        "LA": (-1, 0, 1),
        "AB": (b, t*(1+b), t*t),
        "AC": (c, r*(1+c), r*r),
        "BC+": (b*c, e1*iota*(b+c), -1),
        "BC-": (-b*c, e2*iota*r*(b-c), -r*r),
    }


def common7(b, c, r, t, iota, e1, e2):
    return {
        "LA": (-1, 0, 1),
        "AB": (b, r*(1+b), r*r),
        "AC": (c, t*(1+c), t*t),
...
def main():
    ...
    for b,c,r,t,iota in itertools.product(VALUES, repeat=5):
        for e1,e2 in itertools.product((-1,1), repeat=2):
            left = common4(b,c,r,t,iota,e1,e2)
            right = common7(c,b,r,t,iota,e1,-e2)
