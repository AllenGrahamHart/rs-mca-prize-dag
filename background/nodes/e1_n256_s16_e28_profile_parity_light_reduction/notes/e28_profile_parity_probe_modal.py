#!/usr/bin/env python3
"""Probe the exact E28 slack, profile, parity, and atlas frontier."""
...
RESULT = HERE / "e28_profile_parity_probe_result.json"
UNITS = tuple(range(1, 128, 2))

app = modal.App("e1-n256-e28-profile-parity-route-probe")
...
def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    for counts in product(range(29), range(8), range(4), range(2), range(2)):
        if sum((index + 1) ** 2 * count for index, count in enumerate(counts)) != 28:
            continue
...
def cubic_cutoff() -> tuple[int, list[dict[str, object]]]:
    ...
    for moment in range(5000):
        ...
        coefficient_2 = Fraction(-(689849 - 128 * moment), 2544224)
...
def matching_ledgers(light_diameters: int) -> list[list[int]]:
    ...
    visit(tuple(range(7)), ())
    return [[value, (value - 74) // 2] for value in sorted(square_masses)]
...
def compute() -> dict[str, object]:
    ...
    for l1_norm in range(23, 14, -1):
        delta = 28 + 66 - 4 * l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 28 and l1_bound is None:
            l1_bound = l1_norm
...
def main() -> None:
    ...
    packet["schema"] = "e1-e28-profile-parity-route-probe-v1"
    ...
    print("E28_PROFILE_PARITY_ROUTE_PROBE " + json.dumps({
    ...
    print(f"E28_PROFILE_PARITY_ROUTE_PROBE_RESULT {RESULT}")
