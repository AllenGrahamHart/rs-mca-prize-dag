...
parallel-edge transport have now proved all `384` cases with
`xi in {0,1,2}` and `pairing in {0,1,2,3,4,5,6,7}`.  The next route decision is
between `pairing=8` for the parallel-`DE` missing block and
...
pairing-3 through pairing-7 routes used FLINT-backed rational functions, direct
...
parametrization, and do not infer complete cell-3 closure from the eight paid
