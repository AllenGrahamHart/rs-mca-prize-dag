...
the paid cell-3 ledger to 768 cases. The opposite-DE parity theorem now also
pays `xi=3,pairing in {3,6}`, raising the ledger to 800 cases. The remaining
`xi=3` pairings form five two-element transport orbits; every pairing at
`xi in {4,5,6}` remains.
...
even quartic in `q`; parity descent closes pairings 3 and 6. The analogous
pairing-4/5 compiler still exceeds the 300-second row cap and must be
re-factored before reuse.
...
parallel-`DE` matching block or the five paid `xi=3` matchings.
