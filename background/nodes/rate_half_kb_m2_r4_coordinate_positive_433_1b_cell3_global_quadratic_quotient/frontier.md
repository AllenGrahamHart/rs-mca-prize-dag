...
parallel-edge transport have now proved all `240` cases with
`xi in {0,1,2}` and `pairing in {0,1,2,3,4}`.  The next route decision is
between `pairing=5` for the parallel-`DE` missing block and
...
pairing-3 and pairing-4 routes used FLINT-backed rational functions, direct
and tower norm cross-checks, and direct lifts of every norm and intermediate
exceptional root.  Before computing pairing 5, test whether pairing 4
transports under swapping the target labels `b,c` and lane signs.
...
parametrization, and do not infer complete cell-3 closure from the five paid
