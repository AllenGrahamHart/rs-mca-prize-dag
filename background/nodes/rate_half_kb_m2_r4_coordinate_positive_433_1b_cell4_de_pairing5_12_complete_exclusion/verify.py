#!/usr/bin/env python3
"""Verify complete cell-4 pairing-5/12 composition."""
...
ORBIT = (
    ...
    "cell4_de_pairing5_12_orbit_exclusion"
)
POSITIVE = (
    "rate_half_kb_m2_r4_coordinate_positive_433_1b_"
    "cell4_positive_de_pairing12_complete_exclusion"
)
...
def verify_labels():
    ...
    orbit_parent = {(0, 5), (1, 5), (2, 5), (2, 12)}
    positive = {(0, 12), (1, 12)}
    complete = orbit_parent | positive
    require(complete == {(xi, pairing)
                         for xi in (0, 1, 2) for pairing in (5, 12)},
    ...
    require(len(paid) == 27 and len(paid_orbits) == 15,
            "cumulative paid ledger")
    require(sum(map(len, live_orbits)) == 78 and len(live_orbits) == 45,
...
def verify_documents():
    ...
    require("= 96 raw cases" in statement and
            "78 labels in 45 quotient orbits" in statement,
            "printed statement ledger")
    require("105-27=78" in proof and "60-15=45" in proof,
            "printed subtraction")
...
def main():
    ...
    verify_dag()
    verify_documents()
    print("cell=4 pairings=5,12 labels=6 raw_cases=96 live_labels=78 live_orbits=45")
