#!/usr/bin/env python3
"""Verify the cell-3 xi3 fully mixed pairings 7, 8, 10, and 13 exclusion."""
...
PINNED = {
    SCRIPT: "17998157cf0bd624452a8404cfb2b288c1151147f20f8cba6a17431a06fef210",
    RESULT: "32045c3dec4f16791eb71b6354cac03670bad4f3ac9b593adb58d5d742ed7808",
}
...
def formal_transport():
    ...
    require(matchings[7] == ((0, 3), (1, 4), (2, 5)), "pairing 7")
    require(matchings[8] == ((0, 3), (1, 5), (2, 4)), "pairing 8")
    swap = {0: 1, 1: 0, 2: 2, 3: 3, 4: 4, 5: 5}
    for source, target in ((7, 10), (8, 13)):
...
def verify_payload(payload):
    require(
        ...
        "linear-pair-bezout-source-census-v1",
        "schema",
    )
    ...
    expected = {(epsilon, pairing, sigma_c) for epsilon in SIGNS
                for pairing in (7, 8) for sigma_c in (-1, 1)}
    rows = {}
    ...
    for row in payload["rows"]:
        ...
        rows[key] = row
        require(
            row["xi_index"] == 3 and row["sigma_o_anchor"] == 0 and
            row["target_lanes_covered"] == [[key[2], -1], [key[2], 1]],
            "scope and two-lane cover",
        )
        require(
        ...
        for b_row, f_row in current_f_rows:
            ...
            require(f_row["first_q_cut_degree"] <= 2 and
    ...
    require(len(all_f_rows) == 512, "512 missing-f rows")
    require(len(all_q_rows) == 64, "64 reconstructed targets")
    require(len(all_lanes) == 128, "128 final-pair evaluations")
    require(sum(row["direct_lift"]["candidate_r_count"]
                for row in rows.values()) == 152, "candidate-r aggregate")
    require(sum(row["direct_lift"]["source_point_count"]
                for row in rows.values()) == 176, "source-point aggregate")
...
def main():
    ...
    verify_dag()
    print(
        "cell=3 xi=3 pairings=7,8,10,13 raw_cases=64 computed_rows=16 "
        "f_rows=512 targets=64 final_checks=128 witnesses=0"
    )
