#!/usr/bin/env python3
"""Compute the exact E27 slack, profile, parity, and light-orbit frontier."""
...
RESULT = HERE / "e27_profile_parity_probe_result.json"
UNITS = tuple(range(1, 128, 2))

app = modal.App("e1-n256-e27-profile-parity-probe")
...
def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    for counts in product(range(28), range(8), range(4), range(2), range(2)):
        if sum((index + 1) ** 2 * count for index, count in enumerate(counts)) != 27:
            continue
...
def cubic_cutoff() -> tuple[int, list[dict[str, object]]]:
    ...
    for moment in range(0, 5000):
        form = add_forms(
            ...
            scale_form(Fraction(310), hermite[2]),
            scale_form(Fraction(6688 + moment), hermite[3]),
        )
        coefficient_2 = Fraction(-(674873 - 128 * moment), 2544224)
...
def compute() -> dict[str, object]:
    ...
    for l1_norm in range(23, 14, -1):
        delta = 27 + 66 - 4 * l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 27 and l1_bound is None:
            l1_bound = l1_norm
    ...
    for heavy_heavy in range(2):
        for heavy_light in range(4):
            ...
            diameter_ledgers.append([square_mass, (square_mass - 75) // 2])
    return {
        "complete": True,
        "variance": 54,
        "energy": 27,
...
def main() -> None:
    ...
    packet["schema"] = "e1-e27-profile-parity-probe-v1"
    ...
    print("E27_PROFILE_PARITY_PROBE " + json.dumps({
    ...
    print(f"E27_PROFILE_PARITY_PROBE_RESULT {RESULT}")
