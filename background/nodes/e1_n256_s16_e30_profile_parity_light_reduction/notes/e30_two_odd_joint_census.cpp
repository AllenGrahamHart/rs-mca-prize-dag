...
int main(int argc, char** argv) {
      ...
      for (int third = second + 1; third < static_cast<int>(allowed.size()); ++third) {
        ...
        for (int mask = 0; mask < 64; ++mask) {
          ...
          if (!above_three && ones == 2 && twos == 7 && threes == 0) {
            update(profile_27, positions, coefficients, conductor, half);
          }
          if (!above_three && ones == 1 && twos == 5 && threes == 1) {
            update(profile_151, positions, coefficients, conductor, half);
          }
        }
      }
    }
  ...
  auto print_witness = [&](const Witness& witness) {
    std::cout << "{\"positions\":";
    print_array(witness.positions);
    std::cout << ",\"coefficients\":";
    print_array(witness.coefficients);
