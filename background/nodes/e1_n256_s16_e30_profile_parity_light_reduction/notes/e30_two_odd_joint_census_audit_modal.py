#!/usr/bin/env python3
"""Audit the E30 two-odd census by direct negacyclic multiplication."""
...
SOURCE = HERE / "e30_two_odd_joint_census_audit.cpp"
ORBITS = HERE / "e30_two_six_odd_light_orbit_result.json"
PRODUCTION = HERE / "e30_two_odd_joint_census_result.json"
RESULT = HERE / "e30_two_odd_joint_census_audit_result.json"
REMOTE_SOURCE = "/root/e30_two_odd_joint_census_audit.cpp"
REMOTE_BINARY = "/root/e30_two_odd_joint_census_audit"
PROFILES = ("profile_27", "profile_151")

app = modal.App("e1-n256-e30-two-odd-joint-census-audit")
image = (
...
def main() -> None:
    ...
    def write_checkpoint(complete: bool) -> tuple[dict[str, dict[str, int]], bool]:
        ...
        packet = {
            "schema": "e1-e30-two-odd-joint-census-audit-v1",
            "complete": complete,
            "agreement": agreement,
    ...
    try:
        ...
        print(f"E30_TWO_ODD_JOINT_AUDIT_INCOMPLETE completed={len(rows)}/87 result={RESULT}")
        raise
    complete = len(rows) == 87 and all(bool(row["complete"]) for row in rows)
    summary, agreement = write_checkpoint(complete)
    print("E30_TWO_ODD_JOINT_CENSUS_AUDIT " + json.dumps(summary, sort_keys=True))
    print(f"E30_TWO_ODD_JOINT_AUDIT_COMPLETE {complete}")
    print(f"E30_TWO_ODD_JOINT_AGREEMENT {agreement}")
    print(f"E30_TWO_ODD_JOINT_AUDIT_RESULT {RESULT}")
