#!/usr/bin/env python3
"""Direct residual replay for the cell-12 positive-DE pairing-9 packet."""
...
def main():
    ...
    require(len(profiles) == 29, "unique polynomial census")
    root_payload = json.loads(ROOT_RESULT.read_text())
    require(
        root_payload["schema"]
        == (
            "rate-half-kb-positive-433-1b-cell12-positive-de-pairing9-"
            "frobenius-roots-v1"
        )
    ...
    require(leading_boundaries == 16, "leading-boundary count")
    require((uf_count, lift_count, colored_count) == (1056, 64, 64),
            "direct residual census")
    print(
        "PASS positive pairing-9 adapter audit: profiles=160 unique=29 "
        "boundary=16 uf=1056 lifts=64 colored=64"
    )
