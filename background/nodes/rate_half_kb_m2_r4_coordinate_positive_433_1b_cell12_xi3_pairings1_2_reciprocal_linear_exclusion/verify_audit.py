#!/usr/bin/env python3
"""Independent norm-root and adapter audit for cell-12 pairings 1-2."""
...
def main():
    ...
    require(len(profiles) == 125, "unique polynomial census")
    ...
    require(profile_visits == 576, "profile replay count")
    require(boundary_count == 36, "36 leading-boundary terminals")
    require(
        len(all_lanes) == 192
        and all(
            item["status"] == "THIRD_PAIR_NONZERO"
            and item["final_pair_cut"] % P
            for item in all_lanes
        ),
        "independent final-lane census",
    )
    print(
        "PASS reciprocal-linear adapter audit: "
        "profiles=576 unique=125 boundary=36 lanes=192"
    )
