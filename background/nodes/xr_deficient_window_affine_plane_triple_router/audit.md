# Audit

- The conics are formed only after a fixed `s-2` core cut; their direction
  evaluations therefore belong to one fixed two-dimensional code.
- Distinct `phi` is used twice: to make each determinant genuinely quadratic
  and to exclude the common radial line component.
- Distinct `psi_C` is not asserted universally.  Its failures are counted in
  `I_deg`, which remains open.
- Undefinedness and projective equality of `psi_C` are invariant under a
  basis change in the residual two-dimensional direction space.
- Bezout contributes at most three affine points, not four, because the two
  conics share one point at infinity.
- The official arithmetic uses the smallest local budget and therefore does
  not borrow active-defect mass that may be unavailable elsewhere.
- No upstream PR result is silently imported; PR #1148 is fixture-specific
  and supplies no uniform restriction-rank theorem for this plane.
