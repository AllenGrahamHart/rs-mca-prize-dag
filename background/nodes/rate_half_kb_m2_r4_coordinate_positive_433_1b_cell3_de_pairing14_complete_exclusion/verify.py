#!/usr/bin/env python3
"""Verify the cell-3 DE-missing pairing-14 48-case exclusion."""
...
NODE_ID = NODE.name
SCRIPT = EXPERIMENTS / (
    "rate_half_kb_positive_433_1b_cell3_de_pairing14_"
...
PINNED = {
    SCRIPT: "99e163807aa2020ff72afdf777f6be3f220962b5826bf697767ae11ac28d60ca",
    RESULT: "0c21b47cc5275efaffb90c4c04a05374012a910104d8a07acd2148e27d3d3cc1",
}
SIGNS = set(itertools.product((-1, 1), repeat=2))
EXPECTED_MATCHING = ((0, 5), (1, 4), (2, 3))
...
def verify_payload(payload):
    require(
        payload["schema"] ==
        "rate-half-kb-positive-433-1b-cell3-de-pairing14-"
        "common-f-resultant-source-census-v1",
        "schema",
    )
    ...
    expected = {
        (epsilon, sigma_c, xi)
        for epsilon in SIGNS for sigma_c in (-1, 1) for xi in (0, 2)
    }
    rows = {}
    all_lane_rows = []
    boundaries = []
    for row in payload["rows"]:
        ...
        key = (tuple(row["epsilon"]), sigma_c, row["xi_index"])
        require(key in expected and key not in rows, "source Cartesian row")
        rows[key] = row
        covered_lanes = {(sigma_c, -1), (sigma_c, 1)}
        require(sigma_o == -1 and row["pairing_index"] == 14,
                "source-census anchor")
        require({tuple(lane) for lane in row["target_lanes_covered"]} ==
                covered_lanes and len(row["target_lanes_covered"]) == 2,
                "two-lane coverage")
        require(
        ...
        require((row["p_b_degree"], row["p_c_degree"]) == (2, 2),
                "common-f quadratic degrees")
        ...
        require(lift["uf_candidate_count"] ==
        ...
        require(all(
            tuple(item["sigma"]) in covered_lanes and
        ...
        for boundary in lift["boundary_solutions"]:
            require(
                ...
                {tuple(lane) for lane in
                 boundary["target_lanes_covered"]} == covered_lanes,
                "f=0 two-lane boundary",
            )
        boundaries.extend(lift["boundary_solutions"])

        profile = (sigma_c, row["xi_index"])
        if profile == (-1, 0):
            require(
            ...
            require(
                (len(row["field_roots"]), lift["live_norm_root_count"],
                 ...
                 lift["boundary_solution_count"], len(current_lanes)) ==
                (7, 2, 9, 12, 0, 2, 0),
                "positive-c negative-DE census",
            )

    require(set(rows) == expected and len(rows) == 16,
            "sixteen source rows")
    require(len(all_lane_rows) == 64,
            "64 nonzero final-pair evaluations")
    require(len(boundaries) == 16,
            "sixteen source-level f=0 boundaries")
    require(sum(row["direct_lift"]["candidate_r_count"]
                for row in rows.values()) == 140,
            "candidate-r aggregate")
    require(sum(row["direct_lift"]["source_point_count"]
                for row in rows.values()) == 136,
            "source-point aggregate")
    require(sum(row["direct_lift"]["uf_candidate_count"]
                for row in rows.values()) == 32,
...
def main():
    ...
    verify_dag()
    print(
        "cell=3 pairing=14 DE_copies=3 raw_cases=48 "
        "source_rows=16 lane_checks=64 witnesses=0"
    )
