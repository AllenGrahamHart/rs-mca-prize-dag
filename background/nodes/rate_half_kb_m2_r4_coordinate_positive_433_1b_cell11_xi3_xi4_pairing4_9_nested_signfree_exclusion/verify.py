#!/usr/bin/env python3
"""Verify the cell-11 xi3/xi4 pairing-4/9 exclusion packet."""
...
SCRIPT = EXP / "rate_half_kb_positive_433_1b_cell11_xi3_pairing4_template_adapter_modal.py"
RESULT = EXP / "rate_half_kb_positive_433_1b_cell11_xi3_pairing4_template_adapter_result.json"
...
PINNED = {
    SCRIPT: "536eb389925747fc99d37e8d22ddca7ae1ead34f4dbcf135fa959d9754d370f7",
    RESULT: "0d54dc3a2ae985f4e24ebb87d917f7794a7ef6a2064352d8605477e6b2715e44",
...
def main():
    ...
    require(
        payload["schema"]
        == "rate-half-kb-positive-433-1b-cell11-xi3-pairing4-adapter-v1"
        and payload["field"] == P
    ...
    expected = set(signs)
    seen = {tuple(row["epsilon"]) for row in payload["rows"]}
    require(seen == expected and len(payload["rows"]) == 4,
            "exact row cover")
    ...
    require(totals == (28, 70, 120, 120, 8, 8, 0),
            "primary census")
    ...
    require(
        ...
        and roots["source_primary_sha256"][str(4)] == digest(RESULT)
    ...
    selected = [orbit for orbit in router.compile_orbits()
                if (3, 4) in orbit]
    require(selected == [[(3, 4), (3, 9), (4, 4), (4, 9)]],
    ...
    require(manifest["id"] == NODE.name and manifest["status"] == "PROVED",
            "node manifest")
    print("PASS cell-11 xi3/xi4 pairing 4/9: "
          "rows=4 candidates=70 labels=4")
