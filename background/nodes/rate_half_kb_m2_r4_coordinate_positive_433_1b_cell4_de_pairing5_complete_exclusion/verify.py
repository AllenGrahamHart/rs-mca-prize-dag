#!/usr/bin/env python3
"""Verify the cell-4 DE-missing pairing-5 48-case exclusion."""
...
PINNED = {
    SCRIPT: "047d9b65d0d6ccdfef71113c782b383be88fbdc9ba0e15492a543a890b299969",
    RESULT: "fa4b93a744541c624eed111a79419b8bb6c466cb9a052c3b21006a1078a8a917",
}
PARENTS = (
...
def verify_payload(payload):
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-cell4-de-pairing5-"
    ...
    for row_index, row in enumerate(payload["rows"]):
        ...
        for finite in row["finite_rows"]:
            ...
            f_coefficients = paired_coefficients(
                a_values, b_values, second_de, sigma_c*c_value % PRIME
            )
    ...
    require(collections.Counter(candidate_counts) == {10: 32},
            "candidate-root census")
    require(collections.Counter(source_counts) == {8: 16, 10: 16},
            "source-point census")
    require(collections.Counter(uf_counts) == {0: 16, 5: 16},
            "uf-candidate census")
    require(collections.Counter(target_root_counts) == {9: 32},
            "target-root census")
    require(sum(candidate_counts) == 320 and sum(source_counts) == 288 and
    ...
    require(free_shapes == {
        (5574, 5575, 3649, 3548): 32,
        (5578, 5579, 3652, 3550): 32,
        (5587, 5588, 3664, 3565): 32,
        (5591, 5592, 3667, 3567): 32,
    }, "target-free shape census")
    require(norm_shapes == {
        (0, 8518, 7493, 3848, 3849): 16,
        (2, 8638, 7693, 3888, 3889): 16,
...
def main():
    ...
    verify_dag()
    print(
        "cell=4 pairing=5 DE_copies=3 raw_cases=48 computed=32 "
        "transported=16 candidates=320 source_points=288 "
