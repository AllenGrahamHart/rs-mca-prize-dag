#!/usr/bin/env python3
"""Verify the exact kernel rank-stratified capacity interval."""
...
def capacity(p: dict[str, int], kprime: int) -> int:
    ...
    for dimension in range(1, p["correction_dimension"]):
        ...
        total += comb(nprime, rank) * record_cap(p, kprime, dimension) * extensions
    return total
...
def validate(data: object, exhaustive: bool = True) -> dict[str, int]:
    ...
    require(data.get("schema") == "rate-half-mca-rank11-kernel-rankstratified-capacity-cut-v1", "schema")
    ...
    require((p["closed_dimension_minimum"], p["closed_dimension_maximum"], p["first_open_dimension"]) == (10, 4598, 4599), "interval")
    ...
    require(wall_cap == p["wall_capacity"] and wall_demand <= wall_cap, "wall reversal")
    ...
    return {"checked": checked, "gap": end_demand - end_cap, "wall_gap": wall_cap - wall_demand}
...
def main() -> None:
    ...
    mutations = (
        ...
        lambda item: item["parameters"].__setitem__("closed_dimension_maximum", 4599),
        lambda item: item["parameters"].__setitem__("first_open_dimension", 4600),
        lambda item: item["parameters"].__setitem__("endpoint_gap", item["parameters"]["endpoint_gap"] - 1),
        lambda item: item["parameters"].__setitem__("wall_capacity", item["parameters"]["wall_capacity"] - 1),
    )
    caught = 0
    ...
    print(
        ...
        f"wall_gap={result['wall_gap']} controls={caught}/{len(mutations)}"
    )
