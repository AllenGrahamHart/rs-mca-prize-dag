...
The heavy-row quadratic residual theorem gives
...
The left side of `(2)` is divisible by `g_*S_B`. The scalar
`L_U0(x_*)` is nonzero, so
...
Cancel `g_*S_B` directly in `(2)` using `(1),(3)`. The result is `(DQQ6)`.
At `tau`, the first term vanishes, while
`Lambda(tau)T_2(tau)L_U0(x_*)` is nonzero. This proves the first assertion
in `(DQQ7)`. Evaluating recurrence `(DQQ4)` at `tau` gives the second by
induction. QED.
