...
3. **S3 (an exactness criterion — THEOREM, under the stated hypothesis).**
   ...
   *Two proved sufficient conditions for the hypothesis:* `U = {1}` (so the
   ...
   relation lane, is unconditional); and `1 in U` with `max(U)^2 < n` (the
   proof uses `b = b.1 in U`; official blocks all contain `1`).
...
- **Not** that `Stab(U_j) = {1}` for every official block. The two proved
  sufficient conditions cover `U = {1}` and blocks containing `1` with
  `max(U)^2 < n`; the official `U_j = {odd u : u 2^j <= t}` always contains
  `1` and satisfies the norm condition only for `j >= 26`. For the block family `U = {1,3,...,2L-1}` in `Z/2^m` the
