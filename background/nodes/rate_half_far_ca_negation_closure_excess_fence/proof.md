...
The draft closed here with "the symmetric-T variant at `rho >= 3` is
unmeasured". [WIRING UPDATE 2026-08-11: that gap is CLOSED — round 37
measured the symmetric carrier's death at `rho = 3` after correcting the
parity count to `ceil(rho/2)`, and round 38 proved the carrier family
`(X-x_0)P(X^2)` exhaustive by degree parity at odd `r`. The (NCE)
mechanism above is unaffected; it already counts `ceil(rho/2)`.]
