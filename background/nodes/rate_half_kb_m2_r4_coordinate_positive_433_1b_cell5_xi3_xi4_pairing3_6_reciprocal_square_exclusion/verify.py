#!/usr/bin/env python3
"""Verify the cell-5 xi3/xi4 pairing-3/6 exclusion packet."""
...
SCRIPT = EXP / "rate_half_kb_positive_433_1b_cell5_xi3_pairing3_template_adapter_modal.py"
RESULT = EXP / "rate_half_kb_positive_433_1b_cell5_xi3_pairing3_template_adapter_result.json"
...
PINNED = {
    SCRIPT: "cbd34f6d34ee8280f43453a5b50f07b5baee33349c28b47ab2c5c15de66a0cab",
    RESULT: "ac8903117be3e63fb5ddcec99a45dafe7b222b78cc65f41610cd3bba67ed771b",
    TEMPLATE: "ed1133214b5126f59279ccc75b91f4a572ef9cb62d6b24d8c84df8377da4ce5c",
...
def main():
    ...
    require(
        payload["schema"]
        == "rate-half-kb-positive-433-1b-cell5-xi3-pairing3-adapter-v1"
        and payload["field"] == P
    ...
    require(totals == (32, 88, 80, 80, 0, 0, 0),
            "primary census")
    ...
    require(
        ...
        and roots["source_primary_sha256"][str(3)] == digest(RESULT)
    ...
    selected = [orbit for orbit in router.compile_orbits()
                if (3, 3) in orbit]
    require(selected == [[(3, 3), (3, 6), (4, 3), (4, 6)]],
    ...
    require(manifest["id"] == NODE.name and manifest["status"] == "PROVED",
            "node manifest")
    print("PASS cell-5 xi3/xi4 pairing 3/6: "
          "rows=8 candidates=88 labels=4")
