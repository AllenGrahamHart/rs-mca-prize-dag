#!/usr/bin/env python3
"""Verify the cell-4 xi3/xi4 pairing-8/13 transport corollary."""
...
def main():
    ...
    require("role-cell-4" in direct_statement
            and "matching index 8" in direct_statement,
            "direct-parent scope")
    ...
    require("{8,13}" in matching_statement
    ...
    require("4 source signs * 4 target lanes * 4 labels = 64" in statement,
            "statement payment")
    require("transports `(3,8)` to `(3,13)`" in proof
            and "gives `(4,8)`" in proof
            and "gives `(4,13)`" in proof,
            "four-label composition")
    print("cell=4 xi=3,4 pairings=8,13 transport=exact raw_cases=64")
