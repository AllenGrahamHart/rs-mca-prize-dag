...
if any(R - j != e or any(not threshold < e for threshold in walls)
       or drop != walls[-1] for R, j, e, walls, drop in rows):
    raise ValueError("support conversion")
print("RATE_HALF_MCA_GLOBAL_CORE_RANK_SUPPORT_REPLACEMENT_TARGET_AUDIT_PASS gates=2 replacement_walls=8 top_splits=2")
