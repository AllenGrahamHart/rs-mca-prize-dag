#!/usr/bin/env python3
"""Independent arithmetic audit of the xi4-to-xi3 transport."""
...
def main():
    ...
    full_permutation = (0, 1, 2, 4, 3, 5, 6)
    ...
    for sigma_c, sigma_o in itertools.product((-1, 1), repeat=2):
        for b, c, d, e, f in itertools.product(VALUES, repeat=5):
            ...
            require(tuple(transported[i] for i in full_permutation) == original,
                    "record identity")
            require(tuple(transported_sums[i] for i in full_permutation)
    ...
    require("source role cell 3" in proof and "cell 6" in proof,
            "cell distinction")
    require("separately written audit" in audit and "primary polynomial" in audit,
