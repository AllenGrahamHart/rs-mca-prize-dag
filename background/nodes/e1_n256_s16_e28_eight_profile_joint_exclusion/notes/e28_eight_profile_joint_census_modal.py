#!/usr/bin/env python3
"""Run the exact E28 eight-profile joint census on Modal."""
...
SOURCE = HERE / "e28_eight_profile_joint_census.cpp"
PROBE = HERE / "e28_profile_parity_probe_result.json"
RESULT = HERE / "e28_eight_profile_joint_census_result.json"
REMOTE_SOURCE = "/root/e28_eight_profile_joint_census.cpp"
REMOTE_BINARY = "/root/e28_eight_profile_joint_census"

app = modal.App("e1-n256-e28-eight-profile-joint-census")
image = (
...
def main() -> None:
    ...
    if not probe["complete"]:
        raise RuntimeError("E28 profile probe is incomplete")
    root = HERE.parents[3]
    ...
    if not four["complete"] or int(four["orbits"]) != 148:
        raise RuntimeError("four-odd atlas is incomplete")
    tasks = list(probe["light_geometry"]["zero_odd_orbits"]) + [row["representative"] for row in four["rows"]]
    if len(tasks) != 154 or int(probe["relevant_affine_templates"]) != 154:
        raise RuntimeError("E28 light-template count mismatch")
    ...
    def write_checkpoint(complete: bool) -> dict[str, object]:
        packet = {
            "schema": "e1-e28-eight-profile-joint-census-v1",
            "complete": complete,
    ...
    try:
        ...
        print(f"E28_EIGHT_PROFILE_JOINT_INCOMPLETE completed={len(rows)}/{len(tasks)}")
        raise
    packet = write_checkpoint(
        len(rows) == len(tasks)
        and all(bool(row["complete"]) for row in rows)
        and sum(int(row["vectors"]) for row in rows) == int(probe["direct_vector_floor"])
    )
    print("E28_EIGHT_PROFILE_JOINT " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E28_EIGHT_PROFILE_JOINT_COMPLETE {packet['complete']}")
    print(f"E28_EIGHT_PROFILE_JOINT_RESULT {RESULT}")
