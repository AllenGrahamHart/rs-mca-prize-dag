#!/usr/bin/env python3
"""Verify complete cell-4 pairing-11/14 composition."""
...
ORBIT = (
    ...
    "cell4_de_pairing11_14_orbit_exclusion"
)
POSITIVE = (
    "rate_half_kb_m2_r4_coordinate_positive_433_1b_"
    "cell4_positive_de_pairing14_complete_exclusion"
)
...
def verify_labels():
    ...
    orbit_parent = {(0, 11), (1, 11), (2, 11), (2, 14)}
    positive = {(0, 14), (1, 14)}
    complete = orbit_parent | positive
    require(complete == {(xi, pairing)
                         for xi in (0, 1, 2) for pairing in (11, 14)},
    ...
    require(len(paid) == 45 and len(paid_orbits) == 24,
            "cumulative paid ledger")
    require(sum(map(len, live_orbits)) == 60 and len(live_orbits) == 36,
...
def verify_documents():
    ...
    require("= 96 raw cases" in statement and
            "60 labels in 36 quotient orbits" in statement,
            "printed statement ledger")
    require("105-45=60" in proof and "60-24=36" in proof,
            "printed subtraction")
...
def main():
    ...
    verify_dag()
    verify_documents()
    print("cell=4 pairings=11,14 labels=6 raw_cases=96 live_labels=60 live_orbits=36")
