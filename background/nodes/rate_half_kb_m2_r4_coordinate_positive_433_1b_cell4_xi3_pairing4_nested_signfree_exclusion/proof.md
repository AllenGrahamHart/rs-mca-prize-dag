...
source-sign row. The corresponding division-free common-root cut is normed
through the exact four-basis source tower.
...
degree-5108 norm rows yield 56 candidate `r` roots and 56 guarded source
points. Direct finite replay of the original equations leaves 32 compatible
`(z,q)` candidates.

For every candidate and all four target lanes, the third paired equation

```text
Pair(sigma_o ef,sigma_c cf)=0
```

is checked directly. All 128 evaluations are nonzero. The target-boundary,
