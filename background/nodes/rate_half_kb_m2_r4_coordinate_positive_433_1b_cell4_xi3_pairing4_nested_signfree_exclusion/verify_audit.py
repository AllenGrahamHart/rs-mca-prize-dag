#!/usr/bin/env python3
"""Audit cell-4 xi3 pairing-4 nested sign-free exclusion."""
...
def main():
    ...
    require(len(payload["rows"]) == 4, "4-row sign census")
    require(all(
        row["status"] == "COMPLETE"
    ...
    require("linear remainder" in proof
            and "All 128 evaluations are nonzero" in " ".join(proof.split()),
            "remainder and finite ledger")
    require("does not trust stored terminal statuses" in audit,
            "independent replay contract")
    require("four matching-exchange pairs" in frontier, "retained frontier")
    print(
        "audit=ok cell=4 xi=3 pairing=4 sign_rows=4 "
        "raw_cases=16 z=32 q=32 lane_checks=128"
    )
