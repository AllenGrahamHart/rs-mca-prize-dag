#!/usr/bin/env python3
"""Replay the exact E=35 mod-16 quotient census on Modal."""
...
CPP = HERE / "e35_mod16_quotient_census.cpp"
OUTPUT = HERE / "e35_mod16_quotient_census_result.json"
SHARDS = {(profile, order): 8 for profile in range(2) for order in (128, 64)}
base = modal.Image.debian_slim().apt_install("g++")
image = (
    base.add_local_file(str(CPP), "/root/census.cpp", copy=True)
    .run_commands("g++ -O3 -std=c++17 /root/census.cpp -o /usr/local/bin/e35-census")
)
app = modal.App("e1-e35-mod16-quotient-census")


@app.function(image=image, cpu=1.0, memory=256, timeout=180, max_containers=32)
def census_shard(task: tuple[int, int, int]) -> dict[str, object]:
    profile, order, shard = task
    completed = subprocess.run(
        [
            "/usr/local/bin/e35-census",
            str(profile),
            str(order),
            str(shard),
...
def main() -> None:
    ...
    packet = {
        "schema": "e1-e35-mod16-quotient-census-v1",
    ...
    print("E1_E35_MOD16_QUOTIENT_CENSUS " + repr(summaries))
