#!/usr/bin/env python3
"""Run the exact five-orbit E34 progression census on Modal."""
...
OUTPUT = HERE / "e34_progression_census_result.json"
TASKS = (1, 2, 4, 8, 16)
...
image = (
    ...
    .run_commands("g++ -O3 -std=c++17 /root/census.cpp -o /usr/local/bin/e34-progression")
)
app = modal.App("e1-e34-progression-census")
...
def census(t: int) -> dict[str, object]:
    ...
    try:
        completed = subprocess.run(
            ["/usr/local/bin/e34-progression", str(t)],
            capture_output=True,
            check=True,
            text=True,
            timeout=50,
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    packet = {
        "schema": "e1-e34-progression-census-v1",
...
def main() -> None:
    ...
    for result in census.map(TASKS, order_outputs=False):
        ...
        print(f"E1_E34_PROGRESSION_PROGRESS returned={len(results)}/{len(TASKS)}")
    complete = [result for result in results if result.get("complete") is True]
    print(
        "E1_E34_PROGRESSION_CENSUS "
