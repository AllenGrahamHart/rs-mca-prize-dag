...
int residue(int value) {
  value %= 128;
  return value < 0 ? value + 128 : value;
}
...
int main(int argc, char** argv) {
  ...
  for (int value = 0; value < 128; ++value) {
    if (value != 0 && value != t && value != 2 * t) allowed.push_back(value);
  }
  const std::array<int, 4> weld = {
      residue(-2 * t), residue(3 * t), residue(-t), residue(4 * t)};
    ...
    for (int j = i + 1; j < static_cast<int>(allowed.size()) - 2; ++j) {
      for (int k = j + 1; k < static_cast<int>(allowed.size()) - 1; ++k) {
        for (int l = k + 1; l < static_cast<int>(allowed.size()); ++l) {
          ...
          if (!contains(light, weld[0]) && !contains(light, weld[1]) &&
              !contains(light, weld[2]) && !contains(light, weld[3])) {
            continue;
          }
          ++supports;
