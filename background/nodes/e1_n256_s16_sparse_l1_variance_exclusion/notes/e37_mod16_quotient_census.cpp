...
struct Search {
  ...
  void evaluate(const std::array<int, kModulus>& outer, int aaa) {
    ...
    ++tested;
    const bool inner_in_4z =
        !(two[1] || two[2] || two[3] || two[5] || two[6] || two[7]);
    if (!inner_in_4z && objective > best_not4) {
      best_not4 = objective;
    ...
    if (inner_in_4z && inner_total && order == 128) {
  ...
  void run() {
    ...
    std::cout << ",\"components\":[" << best_components[0] << ','
              ...
              << best_components[3] << "],\"best_not4\":" << best_not4
              << ",\"not4_ones\":";
    print_array(best_not4_one);
    std::cout << ",\"not4_twos\":";
    print_array(best_not4_two);
    std::cout << ",\"not4_components\":[" << best_not4_components[0] << ','
              << best_not4_components[1] << ',' << best_not4_components[2]
              << ',' << best_not4_components[3]
              << "],\"best_inner4_refined\":" << best_inner4_refined
              << ",\"inner4_ones\":";
    print_array(best_inner4_one);
    std::cout << ",\"inner4_twos\":";
    print_array(best_inner4_two);
    std::cout << ",\"inner4_components\":[" << best_inner4_components[0]
