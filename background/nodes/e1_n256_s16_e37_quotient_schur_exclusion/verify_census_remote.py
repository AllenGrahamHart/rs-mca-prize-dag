#!/usr/bin/env python3
"""Replay the exact E=37 mod-16 quotient census on Modal."""
...
CPP = NOTES / "e37_mod16_quotient_census.cpp"
OUTPUT = NOTES / "e37_mod16_quotient_census_result.json"
...
image = (
    ...
    .run_commands("g++ -O3 -std=c++17 /root/census.cpp -o /usr/local/bin/e37-census")
)
app = modal.App("e1-e37-mod16-quotient-census")
...
def main() -> None:
    ...
    for profile, order in SHARDS:
        ...
        summaries[f"profile{profile}_order{order}"] = {
            ...
            "best_not4": max(
                selected, key=lambda result: int(result["best_not4"])
            )
            if selected and any(int(result["best_not4"]) >= 0 for result in selected)
            else None,
            "best_inner4_refined": max(
                selected,
                key=lambda result: int(result["best_inner4_refined"]),
            )
            if selected
            and any(int(result["best_inner4_refined"]) >= 0 for result in selected)
            else None,
        }
    packet = {
        "schema": "e1-e37-mod16-quotient-census-v1",
    ...
    print("E1_E37_MOD16_QUOTIENT_CENSUS " + repr(summaries))
