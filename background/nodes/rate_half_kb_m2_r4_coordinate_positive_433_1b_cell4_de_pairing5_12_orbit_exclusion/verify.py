#!/usr/bin/env python3
"""Verify the cell-4 pairing-5/12 DE orbit composition."""
...
PAIRING5 = (
    ...
    "cell4_de_pairing5_complete_exclusion"
)
QUOTIENT = (
    "rate_half_kb_m2_r4_coordinate_positive_433_1b_"
...
PREVIOUS = (
    "rate_half_kb_m2_r4_coordinate_positive_433_1b_"
    "cell4_de_pairing4_9_complete_exclusion"
)
...
def verify_composition():
    ...
    previous = {
        (xi, pairing)
        for xi in (0, 1, 2)
        for pairing in (0, 1, 2, 3, 4, 6, 9)
    }
    previous_orbits = {orbit for orbit in orbits if orbit <= previous}
    require(len(previous) == 21 and len(previous_orbits) == 12,
    ...
    require(new_orbits == {
        frozenset(((0, 5), (1, 5))),
        frozenset(((2, 5), (2, 12))),
    } and new_labels == {(0, 5), (1, 5), (2, 5), (2, 12)},
            "honest pairing-5/12 orbit closure")
    require((0, 12) not in new_labels and (1, 12) not in new_labels,
            "positive pairing-12 scope fence")
    ...
    require(len(paid) == 25 and len(paid_orbits) == 14 and
            sum(map(len, paid_orbits)) == 25,
            "aggregate paid ledger")
    require(len(live_orbits) == 46 and sum(map(len, live_orbits)) == 80,
...
def verify_documents():
    ...
    require("= 64 raw cases" in compact_statement and
            "80 labels in 46 quotient orbits" in compact_statement,
            "printed aggregate")
    require("105-25 = 80" in proof and "60-14 = 46" in proof,
            "ledger subtraction")
    require("`(0,12)` and `(1,12)`" in frontier and
...
def main():
    ...
    print(
        ...
        "paid_labels=25 live_labels=80 live_orbits=46"
    )
