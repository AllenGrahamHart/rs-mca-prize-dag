...
At matching `10=((0,4),(1,3),(2,5))`, put `u=ef`. The first two paired cuts
are

```text
P_u(u)=Pair(-de,sigma_o*u),        P_f(f)=Pair(de,bf),
```
...
The division-free degree-eight construction in the matching-7 theorem
...
`d=de/e`, and checks `Pair(df,sigma_c*cf)` and the target guards.
