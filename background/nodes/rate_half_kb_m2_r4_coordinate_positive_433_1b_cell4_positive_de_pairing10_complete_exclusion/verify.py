#!/usr/bin/env python3
"""Verify the cell-4 positive-DE pairing-10 32-case exclusion."""
...
NODE_ID = NODE.name
SCRIPT = EXPERIMENTS / (
    "rate_half_kb_positive_433_1b_cell4_positive_de_pairing10_"
...
PINNED = {
    SCRIPT: "c93e56d6d22043e8b1efce403caa6300d6818ce899c5ea2c48684b6ef74ecfc7",
    RESULT: "64fdf3f43cc83644a989c54bf34dc30676283993cc08cece0544d273876930b1",
}
PARENTS = (
...
def verify_payload(payload):
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-cell4-positive-de-pairing10-"
    ...
    require(collections.Counter(candidate_counts) == {18: 16},
            "candidate-root census")
    require(collections.Counter(source_counts) == {34: 16},
            "source-point census")
    require(collections.Counter(uf_counts) == {12: 16},
            "uf-candidate census")
    require(collections.Counter(target_root_counts) == {17: 16},
            "target-root census")
    require(sum(candidate_counts) == 288 and sum(source_counts) == 544 and
            sum(uf_counts) == 192 and aggregate == {
                "relation_nonzero": 1280,
                "colored_nonzero": 192,
                "target_boundaries": 0,
            }, "global terminal census")
    require(free_shapes == {
        (5524, 5525, 3599, 3498): 32,
        (5527, 5528, 3601, 3499): 32,
    }, "target-free shape census")
    require(norm_shapes == {
        (0, 8506, 7461, 3856, 3857): 16,
...
def main():
    ...
    verify_dag()
    print(
        "cell=4 pairing=10 positive_DE=2 raw_cases=32 computed=16 "
        "transported=16 candidates=288 source_points=544 "
        "colored_nonzero=192 boundaries=0 witnesses=0"
    )
