...
int third_moment(const std::array<int, 64>& half) {
  ...
  for (int left : support) {
    for (int right : support) {
      answer += weights[left] * weights[right] * weights[(256 - left - right) % 128];
    }
  }
  return answer;
}
...
int main(int argc, char** argv) {
    ...
    for (int second = first + 1; second < static_cast<int>(allowed.size()) - 1; ++second) {
      for (int third = second + 1; third < static_cast<int>(allowed.size()); ++third) {
        ...
        for (int mask = 0; mask < 64; ++mask) {
          ...
          if (above_three || ones != 5 || twos != 4 || threes != 1) continue;
          ++profile_count;
  ...
  for (std::size_t index = 0; index < matches.size(); ++index) {
    ...
    std::cout << ",\"conductor\":" << match.conductor << ",\"m3\":" << match.m3 << '}';
  }
  std::cout << "]}\n";
  return 0;
}
