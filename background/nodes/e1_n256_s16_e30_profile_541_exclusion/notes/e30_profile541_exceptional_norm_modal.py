#!/usr/bin/env python3
"""Compute exact norms of all full-conductor exceptional profile-(5,4,1) vectors."""
...
ACTUAL = HERE / "e30_profile541_exceptional_actual_result.json"
RESULT = HERE / "e30_profile541_exceptional_norm_result.json"

app = modal.App("e1-n256-e30-profile541-exceptional-norm")
...
def run_flint(vectors: list[dict[str, object]]) -> list[int]:
...
@app.function(image=image, cpu=1.0, memory=256, timeout=60)
def run_pari(vectors: list[dict[str, object]]) -> list[int]:
    import subprocess
...
def main() -> None:
    ...
    packet: dict[str, object] = {
        "schema": "e1-e30-profile541-exceptional-norm-v1",
        "complete": False,
        "agreement": False,
    ...
    try:
        ...
        print(f"E30_PROFILE541_EXCEPTIONAL_NORM_INCOMPLETE result={RESULT}")
        raise
    packet["agreement"] = packet["flint_norms"] == packet["pari_norms"]
    packet["complete"] = bool(packet["agreement"] and len(packet["flint_norms"]) == len(vectors) == 86)
    maximum = max(int(value) for value in packet["flint_norms"])
    packet["summary"] = {
        "vectors": len(vectors),
        "distinct_norms": len(set(packet["flint_norms"])),
        ...
        "maximizing_indices": [
            index for index, value in enumerate(packet["flint_norms"]) if int(value) == maximum
        ],
    }
    RESULT.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n")
    print("E30_PROFILE541_EXCEPTIONAL_NORM " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E30_PROFILE541_EXCEPTIONAL_NORM_AGREEMENT {packet['agreement']}")
    print(f"E30_PROFILE541_EXCEPTIONAL_NORM_RESULT {RESULT}")
