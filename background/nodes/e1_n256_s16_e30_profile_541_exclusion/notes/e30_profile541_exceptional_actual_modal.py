#!/usr/bin/env python3
"""Census actual vectors over exceptional profile-(5,4,1) odd masks."""
...
SOURCE = HERE / "e30_profile541_exceptional_actual_census.cpp"
RELAXATION = HERE / "e30_profile541_odd_difference_relaxation_result.json"
ORBITS = HERE / "e30_six_odd_mask_orbits_result.json"
RESULT = HERE / "e30_profile541_exceptional_actual_result.json"
REMOTE_SOURCE = "/root/e30_profile541_exceptional_actual_census.cpp"
REMOTE_BINARY = "/root/e30_profile541_exceptional_actual_census"

app = modal.App("e1-n256-e30-profile541-exceptional-actual")
image = (
...
def main() -> None:
    ...
    def write_checkpoint(complete: bool) -> dict[str, object]:
        packet = {
            "schema": "e1-e30-profile541-exceptional-actual-v1",
            "complete": complete,
    ...
    try:
        ...
        print(f"E30_PROFILE541_EXCEPTIONAL_ACTUAL_INCOMPLETE completed={len(rows)}/{len(tasks)}")
        raise
    packet = write_checkpoint(
        len(rows) == len(tasks) and all(bool(row["complete"]) for row in rows)
    )
    print("E30_PROFILE541_EXCEPTIONAL_ACTUAL " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E30_PROFILE541_EXCEPTIONAL_ACTUAL_COMPLETE {packet['complete']}")
    print(f"E30_PROFILE541_EXCEPTIONAL_ACTUAL_RESULT {RESULT}")
