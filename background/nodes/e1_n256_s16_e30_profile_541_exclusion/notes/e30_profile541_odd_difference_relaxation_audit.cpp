...
Kernel make_kernel() {
  ...
  for (int first = 1; first < 64; ++first) {
    for (int second = 1; second < 64; ++second) {
      std::array<unsigned char, 128> pair_sums{};
      ...
      for (int third = 1; third < 64; ++third) {
        kernel[first][second][third] =
            pair_sums[third] + pair_sums[kOrder - third];
      }
    }
  }
  return kernel;
...
int base_unit_unit(const std::vector<int>& odd, const std::array<int, 64>& weight,
  ...
  int answer = 0;
  for (int distance : odd) answer += weight[distance] * kernel[first][second][distance];
  return answer;
}
