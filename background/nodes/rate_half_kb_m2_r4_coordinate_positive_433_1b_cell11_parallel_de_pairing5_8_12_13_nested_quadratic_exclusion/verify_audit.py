#!/usr/bin/env python3
"""Direct residual replay for the cell-11 DE pairing-5 packet."""
...
def main():
    ...
    require(len(profiles) == 41, "unique polynomial census")
    root_payload = json.loads(ROOT_RESULT.read_text())
    require(
        root_payload["schema"]
        == "rate-half-kb-positive-433-1b-cell11-de-pairing5-frobenius-roots-v1"
    ...
    require(
        ...
        and max(row["degree"] for row in root_rows.values()) == 11240,
    ...
    require((uf_count, lift_count, colored_count, f_boundary_count)
            == (256, 32, 32, 0), "direct residual census")
    print(
        "PASS pairing-5 adapter audit: profiles=320 unique=41 "
