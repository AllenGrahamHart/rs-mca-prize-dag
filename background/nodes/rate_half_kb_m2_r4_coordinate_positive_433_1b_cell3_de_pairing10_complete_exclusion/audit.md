...
- Matching 10 is pinned as `((0,4),(1,3),(2,5))`: its first cuts are
  `paired(de,b f)` and `paired(second_de,sigma_o e f)`, and its final cut
  is `paired(d f,sigma_c c f)`.
- The missing-sum eliminant is derived with `u=ef`; direct replay recovers
  `e=u/f` and then `d=de/e`.
- The norm depends on `sigma_o` but not `sigma_c`. Each computed source row
  fixes the former and explicitly checks both latter lanes.
