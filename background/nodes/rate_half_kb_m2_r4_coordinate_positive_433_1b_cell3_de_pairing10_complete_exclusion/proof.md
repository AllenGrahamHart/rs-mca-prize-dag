...
Canonical matching 10 is `((0,4),(1,3),(2,5))`. Put `u=ef`. For fixed
`sigma_o`, the first two paired equations are
...
Neither depends on `sigma_c`. Let `m,s` be the missing product and
squared-sum values. Put `de=m, eta=1` for `xi=0`, and
`de=-m, eta=-1` for `xi=2`. Since `e=u/f` and `d=de f/u`, every target
satisfies
...
For every source sign and `sigma_o`, an `xi=0` row has ten norm roots, five
live norm roots, 15 candidate `r` values, 18 guarded source points, and four
`(u,f)` candidates. All four fail `paired(df,sigma_c cf)=0` in both
`sigma_c` lanes. An `xi=2` row has eleven norm roots, six live roots, 15
candidate `r` values, 36 source points, and six `(u,f)` candidates. Two
have `f=0`, violating the target nonzero guard, and the other four fail the
final paired equation in both `sigma_c` lanes.

Across the sixteen source rows the exact ledger has 240 candidate `r`
values, 432 guarded source points, 80 `(u,f)` candidates, 16 terminal
`f=0` boundary records, and 128 nonboundary final-pair evaluations. Every
...
other positive parallel `DE` copy preserves the residual products, missing
squared sum, matching 10, and every target guard value-for-value, so the 16
