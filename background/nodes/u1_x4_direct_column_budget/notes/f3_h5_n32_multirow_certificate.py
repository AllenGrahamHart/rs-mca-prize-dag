#!/usr/bin/env python3
"""Complete n=32,h=5 anchored certificates at several p>=n^2 primes."""
...
OUT = (
    ROOT
    / "background/nodes/u1_x4_direct_column_budget/notes"
...
int main(int argc, char** argv) {
    ...
    std::vector<int> tail;
    combinations(1, 31, H - 1, tail, [&](const std::vector<int>& t) {
        std::vector<int> exps;
        exps.reserve(H);
        exps.push_back(0);
    ...
    combinations(1, 31, H, right, [&](const std::vector<int>& r) {
        ++probed;
    ...
    if (hashed != choose_int(31, H - 1)) throw std::runtime_error("hash count mismatch");
    if (probed != choose_int(31, H)) throw std::runtime_error("probe count mismatch");

    std::cout
        << "{"
        << "\"name\":\"boundary_n32_h5_p" << P << "_FULL_CPP\","
        << "\"n\":32,\"h\":5,\"p\":" << P << ",\"W\":32,"
