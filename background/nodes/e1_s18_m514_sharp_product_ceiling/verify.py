#!/usr/bin/env python3
"""Verify the square-mass-18 cofactor-514 sharp product ceiling."""
...
NODE = "e1_s18_m514_sharp_product_ceiling"
...
def main() -> None:
    ...
    for lower_count in range(1, COUNT):
        if Fraction(36 * (COUNT - lower_count), lower_count) >= MEAN**2:
            continue
        _, upper = product_interval(36, lower_count)
        if not upper < target:
            raise RuntimeError(f"energy-18 chamber survives: j={lower_count}")
        rows.append((Fraction(target, 1) / upper, lower_count))
    if len(rows) != 57 or min(rows)[1] != 63:
        raise RuntimeError("energy-18 chamber census drift")

    boundary_lower, _ = product_interval(34, 63)
    if not boundary_lower > target:
        raise RuntimeError("energy-17 boundary guard drift")

    statement = (ROOT / "background/nodes" / NODE / "statement.md").read_text()
    proof = (ROOT / "background/nodes" / NODE / "proof.md").read_text()
    for text in ("E<=17", "57 feasible", "E=17"):
        if text not in statement:
            raise RuntimeError(f"statement pin missing: {text}")
    for text in ("2^192", "57 exact", "(V,j)=(34,63)"):
    ...
    print(
        "E1_S18_M514_SHARP_PRODUCT_CEILING_PASS "
        f"comparisons={len(rows)} closest_j={min(rows)[1]} boundary_E=17"
    )
