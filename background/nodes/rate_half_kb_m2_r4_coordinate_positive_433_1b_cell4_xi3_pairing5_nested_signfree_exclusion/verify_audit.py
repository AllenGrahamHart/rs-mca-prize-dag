#!/usr/bin/env python3
"""Audit cell-4 xi3 pairing-5 nested sign-free exclusion."""
...
def main():
    ...
    require("linear remainder" in proof
            and "All 64 evaluations are nonzero" in " ".join(proof.split()),
            "remainder and finite ledger")
    require("does not trust stored terminal statuses" in audit,
            "independent replay contract")
    require("three matching-exchange pairs" in frontier, "retained frontier")
    print(
        "audit=ok cell=4 xi=3 pairing=5 sign_lane_rows=8 "
        "raw_cases=16 z=32 q=32 lane_checks=64"
    )
