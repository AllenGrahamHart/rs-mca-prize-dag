#!/usr/bin/env python3
"""Independently verify the cell-4 xi3 pairing-3 exclusion."""
...
PINNED = {
    SCRIPT: "ed1133214b5126f59279ccc75b91f4a572ef9cb62d6b24d8c84df8377da4ce5c",
    RESULT: "f43378e5073b23b6b3540b4ec69a302df79971fe149992f746aaf79f55981aee",
    ...
    for r_value in row["candidate_roots"]:
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                colored_z_roots = roots_from_coefficients(
                    paired_both_coefficients(
                        a_values, b_values,
                        point["b"]*missing % PRIME,
                        sigma_c*point["c"]*missing % PRIME,
                    )
                )
                require(missing_z_roots is not None, "monic missing cut")
                z_roots = (
                    missing_z_roots if colored_z_roots is None
                ...
                for z_value in z_roots:
                    ...
                    d_row = {"d": d_value, "f": f_value, "lanes": []}
                    for sigma_o in (-1, 1):
                        outside_q_roots = roots_from_coefficients(
                            paired_both_coefficients(
                                a_values, b_values, 1,
                        ...
                        if antipodal_q_roots is None:
                            ...
                            q_roots = sorted(
                                set(antipodal_q_roots) & set(outside_q_roots)
                            )
                        if q_roots is None:
                            unresolved.append({
                                **point,
                        ...
                        for q_value in q_roots:
                            ...
                            equation_values = [
                                ...
                                paired(a_values, b_values, q_value, -q_value % PRIME),
                                paired(
...
def verify_payload(payload):
    ...
    for row_index, row in enumerate(payload["rows"]):
        ...
        require(
            (row["missing_cut_degree"], row["colored_z_cut_degree"],
             row["colored_sign_free_degree"], row["remainder_degree"])
            == (2, 4, 4, 1),
            "quadratic/quartic sign-free reciprocal-square cut",
        )
        ...
        for label, coefficient_count in (
            ("missing_cut_profiles", 3), ("colored_z_cut_profiles", 5)
        ):
    ...
    require(actual == expected and len(actual) == 8, "8 exact sign rows")
    ...
    require(
        ...
        and sum(row["q_candidate_count"] for row in payload["rows"]) == 0
...
def main():
    ...
    verify_dag()
    print(
        "cell=4 xi=3 pairing=3 raw_cases=16 sign_rows=8 candidate_r=60 "
        "source_points=16 z=8 q=0 lane_checks=16 witnesses=0"
    )
