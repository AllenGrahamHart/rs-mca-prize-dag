...
```text
R(u,f)=(u^2+eta*de*f^2)^2-S*f^2*u^2=0.           (RS12-DE5)
```
...
union has 672 case-labeled candidates and lifts to 1,088 guarded source
points. There are 208 compatible `(u,f)` rows satisfying (RS12-DE5): 48
have `f=0` and hit the explicit target boundary; the other 160 reconstruct
...
and directly replays all 1,680 `(u,f)` rows and all 208 zero-relation lifts.
