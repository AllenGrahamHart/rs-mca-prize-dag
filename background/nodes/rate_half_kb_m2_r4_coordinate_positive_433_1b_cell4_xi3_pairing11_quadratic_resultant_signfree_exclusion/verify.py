#!/usr/bin/env python3
"""Independently verify the cell-4 xi3 pairing-11 exclusion."""
...
PINNED = {
    SCRIPT: "8f2fe8ca53863b2220ae60558b3f8d64269eec0f3952138679f2bc3a7069698b",
    RESULT: "e6257ef93a029a96e0840ac774579349de588e3059c73db33b4e2fcad8f5f3d2",
...
PRIME = 2130706433
CASES = set(itertools.product((-1, 1), repeat=3))
...
def paired_left_coefficients(a_values, b_values, left_scale, right):
    p0, p1, p2 = (
        [b_value, -left_scale*a_value % PRIME]
        for a_value, b_value in zip(a_values, b_values)
    )
...
def reconstruct_row(
    row, sigma_c, base_expression, b_expression, c_expression, kernel
):
    route_guards = (
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                for z_value in missing_z_roots:
                    ...
                    bf_q_roots = roots_from_coefficients(
                        paired_left_coefficients(
                            a_values, b_values, 1,
                            point["b"]*f_value % PRIME,
                        )
                    )
                    cf_q_roots = roots_from_coefficients(
                        paired_left_coefficients(
                            a_values, b_values, 1,
                    ...
                    for q_value in q_roots:
                        ...
                        require(not bf_pair and not cf_pair,
...
def formal_elimination_identities():
    q_symbol = sp.symbols("q_symbol")
    a, b0, c0, d, e0, f0 = sp.symbols("a b0 c0 d e0 f0")
    first = a*q_symbol**2+b0*q_symbol+c0
    second = d*q_symbol**2+e0*q_symbol+f0
    expected_resultant = (
        (a*f0-c0*d)**2-(a*e0-b0*d)*(b0*f0-c0*e0)
    )
    require(
        sp.expand(sp.resultant(first, second, q_symbol)-expected_resultant) == 0,
        "formal quadratic resultant identity",
    )
...
def verify_payload(payload):
    ...
    for row_index, row in enumerate(payload["rows"]):
        ...
        require(
            (row["missing_cut_degree"], row["bf_q_z_degrees"],
             row["cf_q_z_degrees"], row["pair23_target_z_degree"],
             row["remainder_z_degree"], row["z_sign_free_degree"],
             row["remainder_degree"])
            == (2, [2, 2, 2], [2, 2, 2], 6, 3, 3, 1),
            "quadratic-resultant/z sign-free degree ledger",
        )
        ...
        expected_norm_degrees = (
            (3864, 1560) if sigma_c == -1 else (3868, 1560)
        )
        require(
            (norm["numerator"]["degree"], norm["denominator"]["degree"])
            == expected_norm_degrees,
            "exact norm degrees",
        )
        ...
        reconstruct_row(row, sigma_c, *expected_relations, kernel)
        lane_count = sum(
            len(q_row["lanes"])
    ...
    require(actual == expected and len(actual) == 8, "8 exact sign/lane rows")
    ...
    require(
        ...
        and final_lane_count == 16,
        "aggregate finite census",
    )
    return 8, 16, final_lane_count
...
def main():
    ...
    require((rows, raw_cases, lane_checks) == (8, 16, 16), "scope accounting")
    verify_dag()
    print(
        "cell=4 xi=3 pairing=11 raw_cases=16 sign_lane_rows=8 candidate_r=60 "
        "source_points=16 z=8 q=8 lane_checks=16 witnesses=0"
    )
