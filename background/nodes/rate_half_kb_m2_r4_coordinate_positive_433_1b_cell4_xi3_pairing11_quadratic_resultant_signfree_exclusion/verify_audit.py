#!/usr/bin/env python3
"""Audit cell-4 xi3 pairing-11 quadratic-resultant sign-free exclusion."""
...
def main():
    ...
    require(all(
        ...
        and row["q_candidate_count"] in (0, 2)
    ...
    require(
        replay["schema"]
        == "rate-half-kb-positive-433-1b-cell4-xi3-pairing11-independent-replay-v1"
        and replay["complete"] is True
        and replay["status"] == "PASS"
        and replay["app_id"] == "ap-74kOL7uM1Y7OHlIhM5mI1A",
        "independent replay terminal",
    )
    require(
        replay["compiler_sha256"] == sha256(SCRIPT)
        and replay["result_sha256"] == sha256(RESULT)
        and replay["verifier"] == str(VERIFIER.relative_to(ROOT))
        and replay["verifier_sha256"] == sha256(VERIFIER),
        "independent replay byte pins",
    )
    require(
        len(replay["manifest_sha256"]) == 64
        and all(character in "0123456789abcdef"
                for character in replay["manifest_sha256"])
        and 0 < replay["seconds"] <= 650,
        "independent replay execution metadata",
    )
    require(
        replay["stdout"]
        == (
            "cell=4 xi=3 pairing=11 raw_cases=16 sign_lane_rows=8 "
            "candidate_r=60 source_points=16 z=8 q=8 lane_checks=16 "
            "witnesses=0"
        ),
        "independent replay exact census",
    )
    ...
    require("linear remainder" in proof
            and "All 16 evaluations are nonzero" in " ".join(proof.split()),
            "remainder and finite ledger")
    require("does not trust stored terminal statuses" in audit,
            "independent replay contract")
    require("no positive cell-4 label remains" in frontier, "closed frontier")
    print(
        "audit=ok cell=4 xi=3 pairing=11 sign_lane_rows=8 "
        "raw_cases=16 z=8 q=8 lane_checks=16"
    )
