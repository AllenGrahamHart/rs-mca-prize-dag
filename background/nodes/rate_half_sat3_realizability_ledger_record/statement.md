...
- **status:** **HEURISTIC / RECORD** (node-level CONJECTURE, the
  DAG-schema bucket). Not one instrument here is a mechanism.
...
- **wired:** 2026-08-11 mint session (task #41), from the round-37 draft
  `notes/pilots_20260811/r37_mint_drafts/sat3_ledger_corrections/`,
  coordinator line-audited; **the D9 blocking flag is RESOLVED** — the gate
  formula is recovered and printed below, and the audit computes both
  calibration points from it.
...
is an exact identity (verified `m = 1..59`; polynomial identity in the
audit), but the two systems' shapes are incompatible and — decisively —
**the shared quantity governs NEITHER layer's existence**: `(L2)` is
nonempty at `m=2` despite `+4`, and (BIV-G) is realizable at `m=3` despite
`+17`.
...
- The `18` is `23 - 5`: (ERC2) (PROVED, on
  `rate_half_ca_hankel_exceptional_root_charge` — textual ingredient
  citation; the DAG schema keeps CONJECTURE nodes as leaves) forces
  `e = m` for (SAT3), so the curve must lie on the `18`-dimensional `(L2)`
  component, not the ambient `23`-dimensional space the round-33 ledger
  used.
...
## The C(16m, 4m-1) first-moment gate — WITH THE RECOVERED FORMULA (D9)
...
**THE GATE'S EXPRESSION (recovered per the D9 resolution; source
`notes/pilots_20260811/r35_rout_layer_a/REPORT.md`, section D3.3):**

```text
log2 E = [(m+1)(rho+1) - 4] log2 q  +  log2 C(q+1, T)
         +  T [ log2 C(16m, 4m-1)  -  rho log2 q ].
```

**The m=1 double calibration (the lane's only calibrated instrument), now
recomputable — the audit evaluates the formula at both points:**

- at `q = 17` (`T = 5`), `log2 E = +13.75` where **EXACTLY 16
  configurations exist and they ARE the 16 realized (SAT3) families** —
  two independent constructions. Since `16 = 2^4`, the gate
  **overestimates by `2^9.75`** (`~2^9.8` as banked), which is the SAFE
  direction;
- at `q = 97` (`T = 5`), `log2 E = -0.94`, where none are realized.
...
both still CARRIED, not recomputed).
...
(the `18-6T` exponent vanishes at `T = 3`), yet at drafting no exact solve
reached it. [WIRING NOTE 2026-08-11: round 38 then REACHED `T = 3` by scale
elimination (ten witnesses, two coordinator-certified) — the `+62.5`-bit
cell is now inhabited, a calibration point in the gate's favour at `T = 3`
and a fifth demonstration that the gate's negatives at OTHER cells still
carry no mechanism.]
...
- The recovered gate formula:
  `notes/pilots_20260811/r35_rout_layer_a/REPORT.md`, section D3.3
  (calibration table immediately below it).
