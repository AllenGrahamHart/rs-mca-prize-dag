#!/usr/bin/env python3
"""Mutation controls for the cell-14 guard-factorization verifier."""
...
SPEC = importlib.util.spec_from_file_location("cell14_guard_verify",
                                              NODE / "verify.py")
...
def main():
    keys = ("kernel_result", "plane_result", "main_result",
    ...
    mutation[0]["result"]["basis_size"] = 7
    ...
    mutation = copy.deepcopy(originals)
    mutation[2]["result"]["leading_exception_atlas"][
        "all_deployed_roots_guarded"
    ] = False
    must_fail(mutation, "leading atlas")

    mutation = copy.deepcopy(originals)
    mutation[2]["result"]["leading_exception_atlas"]["rows"][5][
        "b_rows"
    ][0]["deployed_w0_roots"][0]["guards"] = []
    must_fail(mutation, "finite root guard")
    ...
    mutation = copy.deepcopy(originals)
    mutation[4]["rows"][0]["unit"] = False
    must_fail(mutation, "exceptional unit ideal")
    print("positive 433-1a cell-14 guard factorization audit verified")
