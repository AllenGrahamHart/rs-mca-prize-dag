...
- **Falsifier:** two empty actual columns within deficit `2e-7`, an active
  normalization fiber not completed inside `Gamma`, a nontrivial common
  divisor of `m` and `3e-z`, or a shared root not corresponding to one
  branch at the associated frame vertex.
- **Status effect:** one PROVED rank-three router; no critical status change.
- **Nonclaims:** no rank-three exclusion, rank-four statement, or Lane-T
  close.
