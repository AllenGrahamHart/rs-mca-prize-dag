...
- **scope:** gate-clean zero-escape **block systems** `B(V,t,t_0,k)` with
  `V >= 5` and `dim Ann >= 1`. T0 is proved only outside the named
  residual band `t <= 2e-3`; P-SHARE is unconditional. The full T0
  statement remains open on that band, which includes the prize shapes.
  **T1 and T2 — the stronger forcings — are FALSE**, refuted
  constructively on 18 fixtures, and are stated here only as refutations.
...
### T0-NR (the proved nonresidual statement)

```text
(T0-NR)  For a gate-clean zero-escape system with V >= 5 blocks,
         dim Ann >= 1, and t >= 2e-2, the blocks CANNOT be covered
         by two DISTINCT pencils carrying >= 3 blocks each.
```

The complementary integer range `t <= 2e-3` is open. Thus this node
does not prove the unrestricted T0 assertion or `M<=1` at prize shape.
