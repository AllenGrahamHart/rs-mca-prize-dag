#!/usr/bin/env python3
"""Verify the cell-4 DE-missing pairing-7 48-case exclusion."""
...
NODE_ID = NODE.name
SCRIPT = EXPERIMENTS / (
    "rate_half_kb_positive_433_1b_cell4_de_pairing7_"
...
PINNED = {
    SCRIPT: "0c3c42ed564240fdea0537b796c0222e5841e384af53191097c586ccd536b425",
    RESULT: "30eb75cc89cd4d804a2feefa9b318c8ebab79c5a84e78f265628e880805d2960",
}
PARENTS = (
...
def verify_payload(payload):
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-cell4-de-pairing7-"
    ...
    for row_index, row in enumerate(payload["rows"]):
        ...
        for finite in row["finite_rows"]:
            ...
            f_coefficients = paired_coefficients(
                a_values, b_values, second_de, b_value
            )
            ...
            for uf_row in finite["uf_rows"]:
                ...
                require(e_value != 0, "nonzero reconstructed e")
    ...
    require(collections.Counter(candidate_counts) == {10: 16, 12: 16},
            "candidate-root census")
    require(collections.Counter(source_counts) == {12: 32},
            "source-point census")
    require(collections.Counter(uf_counts) == {0: 16, 5: 16},
            "uf-candidate census")
    require(collections.Counter(target_root_counts) == {9: 16, 11: 16},
            "target-root census")
    require(sum(candidate_counts) == 352 and sum(source_counts) == 384 and
            sum(uf_counts) == 80 and aggregate == {
                "relation_nonzero": 1088,
                "colored_nonzero": 64,
                "target_boundaries": 16,
            }, "global terminal census")
    require(free_shapes == {
        (5542, 5543, 3617, 3516): 32,
        (5545, 5546, 3619, 3517): 32,
        (5539, 5540, 3616, 3517): 32,
        (5543, 5544, 3619, 3519): 32,
    }, "target-free shape census")
    require(norm_shapes == {
        (0, 8558, 7533, 3888, 3889): 16,
        (2, 8658, 7713, 3908, 3909): 16,
    ...
    require((len(norm_numerator_hashes), len(norm_denominator_hashes)) ==
            (8, 8), "specialized norm diversity")
...
def main():
    ...
    verify_dag()
    print(
        "cell=4 pairing=7 DE_copies=3 raw_cases=48 computed=32 "
        "transported=16 candidates=352 source_points=384 "
        "colored_nonzero=64 f_zero=16 witnesses=0"
    )
