#!/usr/bin/env python3
"""Audit the pairing-7 eliminant identity and terminal scope."""
...
def main():
    ...
    require(sum(row["candidate_root_count"] for row in rows) == 352 and
            sum(row["source_point_count"] for row in rows) == 384 and
    ...
    require("Positive `DE` at matching 10 is not claimed" in
            (NODE / "audit.md").read_text(), "matching-10 fence")
    require("A missing sign/lane row" in statement and
            "degree-drop specializations" in proof,
            "falsifier and degree-drop disclosure")
    print("audit=ok degree=8 rows=32 candidates=352 witnesses=0")
