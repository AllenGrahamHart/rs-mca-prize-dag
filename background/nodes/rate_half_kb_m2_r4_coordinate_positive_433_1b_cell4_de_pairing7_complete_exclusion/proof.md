...
At matching 7, put `u=ef`. The first two paired cuts are the quadratics

```text
P_u(u) = Pair(de,sigma_o*u),       P_f(f) = Pair(second_de,bf),
```
...
(1), reconstructs `e=u/f`, `d=de/e`, and evaluates the third colored pair
`Pair(df,sigma_c*cf)` and all target guards.

The exact ledger has 352 candidate `r` values and 384 guarded source points.
Of 1,168 `(u,f)` rows, 1,088 fail (1). The remaining 80 consist of 16
