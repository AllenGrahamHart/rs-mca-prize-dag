#!/usr/bin/env python3
"""Cross-audit the cell-9 xi3 pairing-11 ledgers."""
...
def terminal(payload):
    require(
        ...
        and payload["target_norm_root_count"] == 528
        and payload["candidate_root_count"] == 824
        ...
        and payload["checked"] == 480
        and payload["missing_z_roots"] == payload["d_lifts"] == 1440
        and payload["q_intersections"] == 1440
        and payload["common_q_roots"] == payload["q_candidate_count"] == 144
        and payload["lane_checks"] == payload["third_pair_nonzero"] == 288
    ...
    require(
        ...
        and all(
            row["status"] == "PASS"
            and row["rows"] == 8
            and row["lane_checks"] == row["third_pair_nonzero"] == 48
...
def main():
    ...
    sources = (
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing11_chart_scout_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing11_frobenius_roots_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_xi3_pairing11_direct_audit_modal.py",
    )
    for path in sources:
    ...
    require(
        "separate aggregate composition"
        in (NODE / "claim_contract.md").read_text(),
        "scope marker",
    )
    require(
        "No cell-9 leading-open label orbit remains"
        in (NODE / "frontier.md").read_text(),
        "frontier marker",
    )
    print(
        "PASS cell-9 xi3 pairing-11 cross-audit: "
        "charts=6 profiles=69 lanes=288 hostile=detected"
    )
