#!/usr/bin/env python3
"""Stream exact FLINT norms for the analytic m=2 residual."""
...
SOURCE = HERE / "m2_residual_candidates.cpp"
OUTPUT = HERE / "m2_candidate_norms_result.json"
SHARDS = 32
BUCKETS = 64
MODULUS = 1 << 256
...
app = modal.App("e1-prize-m2-candidate-norms")
...
def compute(shard: int) -> dict[str, object]:
    ...
    for line in generated.stdout.splitlines():
        ...
        if candidate < lower:
            ...
            is_prime = bool(fmpz(candidate).is_prime())
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    packet = {
        "schema": "e1-prize-m2-candidate-norms-v1",
...
def main() -> None:
    ...
    for result in compute.map(range(SHARDS), order_outputs=False):
        ...
        print(f"M2_CANDIDATE_NORMS_PROGRESS returned={len(results)}/{SHARDS}")
    ...
    print(f"M2_CANDIDATE_NORMS complete={complete}/{SHARDS} inside={inside}")
