#!/usr/bin/env python3
"""Complete normalized m=256 census at the nine residual variances."""
...
SOURCE = HERE / "m256_residual_census.cpp"
OUTPUT = HERE / "m256_residual_census_result.json"
SHARDS = 32
...
app = modal.App("e1-prize-m256-residual-census")
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    witnesses = [
        {"shard": result["shard"], **witness}
        for result in complete
        for witness in result.get("witnesses", [])
    ]
    packet = {
        "schema": "e1-prize-m256-residual-census-v1",
        "complete": len(complete) == SHARDS,
        "source_sha256": hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
        "normalization": {
            "singleton_positions": [0, 8],
        ...
        "variances": [10, 18, 26, 34, 42, 50, 58, 66, 74],
...
def main() -> None:
    ...
    for result in census.map(range(SHARDS), order_outputs=False):
        ...
        print(f"M256_RESIDUAL_PROGRESS returned={len(results)}/{SHARDS}")
    print(
        "M256_RESIDUAL_CENSUS "
