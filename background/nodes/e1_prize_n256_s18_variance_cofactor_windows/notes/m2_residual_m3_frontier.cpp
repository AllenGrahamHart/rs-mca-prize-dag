...
constexpr int kEnergyCount = 24;

int EnergyIndex(int energy) {
  if (energy < 5 || energy > 97 || energy % 4 != 1) return -1;
  return (energy - 5) / 4;
}
...
bool Gate() {
  ...
  std::array<int, 128> correlation{};
  return Energy(positions, coefficients, correlation) == 89 &&
         ThirdMoment(correlation) == 732;
}
...
int main(int argc, char** argv) {
  ...
  if (argc == 2 && std::string(argv[1]) == "--gate") {
    std::cout << "M2_RESIDUAL_M3_FRONTIER_GATE_PASS\n";
    return 0;
  }
  if (argc != 2) return 2;
