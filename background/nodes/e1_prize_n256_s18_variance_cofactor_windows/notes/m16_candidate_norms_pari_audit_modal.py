#!/usr/bin/env python3
"""Replay the streamed m=16 residual with PARI and an independent census."""
...
SOURCE = HERE / "m16_residual_candidates_audit.cpp"
PRIMARY = HERE / "m16_candidate_norms_result.json"
OUTPUT = HERE / "m16_candidate_norms_pari_audit_result.json"
SHARDS = 32
BUCKETS = 64
MODULUS = 1 << 256
...
app = modal.App("e1-prize-m16-candidate-norms-pari-audit")
...
@app.function(image=image, cpu=1.0, memory=512, timeout=120, max_containers=32)
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    packet = {
        "schema": "e1-prize-m16-candidate-norms-pari-audit-v1",
...
def main() -> None:
    ...
    for result in audit.map(range(SHARDS), order_outputs=False):
        ...
        print(f"M16_NORM_PARI_AUDIT_PROGRESS returned={len(results)}/{SHARDS}")
    complete = sum(result.get("complete") is True for result in results)
    print(f"M16_NORM_PARI_AUDIT complete={complete}/{SHARDS}")
