#!/usr/bin/env python3
"""Independent direct-convolution audit of the m=256 residual census."""
...
SOURCE = HERE / "m256_residual_audit.cpp"
OUTPUT = HERE / "m256_residual_audit_result.json"
SHARDS = 32
...
app = modal.App("e1-prize-m256-residual-audit")
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    packet = {
        "schema": "e1-prize-m256-residual-audit-v1",
...
def main() -> None:
    ...
    for result in audit.map(range(SHARDS), order_outputs=False):
        ...
        print(f"M256_RESIDUAL_AUDIT_PROGRESS returned={len(results)}/{SHARDS}")
    print(
        "M256_RESIDUAL_AUDIT "
