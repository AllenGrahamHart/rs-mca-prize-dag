#!/usr/bin/env python3
"""Verify the cell-12 parallel-DE pairing-4/7/9/10 packet."""
...
SCRIPT = EXP / (
    "rate_half_kb_positive_433_1b_cell12_de_pairing4_"
...
PINNED = {
    SCRIPT: "9bb29583dee0396bf7934b786165b6649c35d7cfa17f2bfa669449350a7c2236",
    RESULT: "72dda518df89a0dbe0c3b6ae3509bf6e6982954a905ce028a7728dafb8813cd9",
    TEMPLATE: "86a51dc9a451980ff2f916fc7c249c7ab35c20a4a6a92d928577534df90c87c3",
    ROOT_SCRIPT: "0218e38f80b6e44548ac15f5f87b6bcf7501b38d1ec2abf8d52d9ae022dce9b5",
    ROOT_RESULT: "f5dbca636c31e86322f1c153b127207a777d819fa43d2012156c7360828a55a2",
}
P = 2130706433
...
def main():
    ...
    require(
        payload["schema"]
        == "rate-half-kb-positive-433-1b-cell12-de-pairing4-adapter-v1"
        and payload["field"] == P
    ...
    seen = set()
    row_census = {0: (16, 23, 34, 2), 2: (15, 22, 38, 9)}
    for row in payload["rows"]:
    ...
    require(totals == (496, 720, 1152, 1152, 176, 0), "exact totals")
    ...
    require(
        finite == {"CHECKED": 960, "MISSING_IMPOSSIBLE": 96,
    ...
    require(endings == {"MISSING_RELATION_NONZERO": 1504,
    ...
    print("PASS cell-12 parallel-DE pairing 4/7/9/10: rows=32 candidates=720 uf=1680")
