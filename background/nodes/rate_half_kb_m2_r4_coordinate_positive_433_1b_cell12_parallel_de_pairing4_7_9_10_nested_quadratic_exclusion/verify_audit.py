#!/usr/bin/env python3
"""Direct residual replay for the cell-12 DE pairing-4 packet."""
...
def main():
    ...
    require(len(profiles) == 37, "unique polynomial census")
    root_payload = json.loads(ROOT_RESULT.read_text())
    require(
        root_payload["schema"]
        == "rate-half-kb-positive-433-1b-cell12-de-pairing4-frobenius-roots-v1"
    ...
    for row in payload["rows"]:
        ...
        for item in row["boundary_rows"]:
            ...
            require(int(expression.subs({r: item["r"], t: item["t"]})) % P == 0,
                    "leading-boundary routing")
            leading_boundaries += 1
        ...
        for finite in row["finite_rows"]:
            ...
            for u_value in finite["u_roots"]:
                require(paired(a_values, b_values, de, u_value) == 0,
                        "u-pair root")
            for f_value in finite["f_roots"]:
                require(paired(a_values, b_values, second_de,
            ...
            for uf_row in finite["uf_rows"]:
                ...
                require(d_value != 0, "unexpected zero-d boundary")
    ...
    require(leading_boundaries == 32, "leading-boundary count")
    require((uf_count, lift_count, colored_count, f_boundary_count)
            == (1680, 176, 128, 48), "direct residual census")
    print(
        "PASS pairing-4 adapter audit: profiles=320 unique=37 "
        "boundary=32 uf=1680 lifts=176 colored=128"
    )
