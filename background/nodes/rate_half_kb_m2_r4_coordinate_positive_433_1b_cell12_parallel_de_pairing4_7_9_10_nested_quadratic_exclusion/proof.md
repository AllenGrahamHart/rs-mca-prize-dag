...
four-basis tower, canonical matching `4` gives quadratics `P_u(u)` and
...
```text
R(u,f)=(u^2+eta*de*f^2)^2-S*f^2*u^2=0.           (RS12-DE4)
```
...
union has 720 case-labeled candidates and lifts to 1,152 guarded source
points. There are 176 compatible `(u,f)` rows satisfying (RS12-DE4): 48
have `f=0` and hit the explicit target boundary; the other 128 reconstruct
`d,e,v` and have a nonzero third colored-pair cut. No colored solution,
witness, or unresolved branch survives.

An external compiled Frobenius/gcd pass reconstructs all roots of the 37
...
and directly replays all 1,680 `(u,f)` rows and all 176 zero-relation lifts.

The exact generic quotient gives the two disjoint orbits printed in the
statement, closing eight labels from the direct representatives `(0,4)` and
`(2,4)`. QED.
