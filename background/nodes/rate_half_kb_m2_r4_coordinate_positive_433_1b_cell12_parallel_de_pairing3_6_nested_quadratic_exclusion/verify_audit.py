#!/usr/bin/env python3
"""Independent roots and direct residual audit for cell-12 DE pairing 3."""
...
def main():
    ...
    for snippet in ("gf_pow_mod", "gf_gcd", "sp.factor_list",
                    "sorted(set(roots))"):
    ...
    require(len(profiles) == 49, "unique polynomial census")
    root_payload = json.loads(ROOT_RESULT.read_text())
    require(
        root_payload["schema"]
        == "rate-half-kb-positive-433-1b-cell12-de-pairing3-independent-roots-v1"
    ...
    for row in payload["rows"]:
        ...
        for finite in row["finite_rows"]:
            ...
            for u in finite["u_roots"]:
                require(paired(a_values, b_values, de, u) == 0,
                        "u-pair root")
            ...
            for uv in finite["uv_rows"]:
                ...
                for f_row in uv["f_rows"]:
                    ...
                    require(
                        (d_value, e_value) == (f_row["d"], f_row["e"])
    ...
    print(
        "PASS pairing-3 adapter audit: profiles=320 unique=49 "
        "boundary=32 uv=96 f=144 colored=96"
    )
