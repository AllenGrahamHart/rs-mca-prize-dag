#!/usr/bin/env python3
"""Verify the rigid A6 [5,4,2] pair-quotient certificate."""
...
def load_producer():
    spec = importlib.util.spec_from_file_location("kb_m4_a6_542", PRODUCER)
...
def validate(data: dict[str, object], expected: dict[str, object]) -> None:
    if data.get("schema") != "rate_half_kb_m4_a6_542_pair_quotient_v1":
...
def hostile_mutations(data: dict[str, object]) -> list[dict[str, object]]:
    ...
    changed(lambda row: row["source"].__setitem__("blob", "0" * 40))
    changed(lambda row: row["coefficient_field"].__setitem__("discriminant", -7))
    changed(lambda row: row["companion"].__setitem__("degree", 5))
    changed(lambda row: row["pair_curve"].__setitem__("adjoint_rank", 7))
    changed(lambda row: row["pair_curve"]["y_resultant_factor_degrees"].pop())
    changed(lambda row: row["pair_curve"].__setitem__("y_numerator", "0"))
    changed(lambda row: row["quotient"].__setitem__("degree", 14))
    changed(lambda row: row["quotient"].__setitem__("scalar", "1"))
    changed(
        lambda row: row["quotient"]["numerator_factors"][0].__setitem__("exponent", 4)
    )
    changed(lambda row: row["quotient"]["fiber_one"].__setitem__(0, 3))
    changed(lambda row: row["quotient"]["fiber_infinity"].pop())
    changed(lambda row: row["quotient"].__setitem__("total_branch_index", 27))
    changed(lambda row: row["challenge_field"]["nu_residues"].pop())
    changed(lambda row: row["challenge_field"].__setitem__("zero_fiber_splits", False))
    changed(lambda row: row["conclusion"].__setitem__("passport", "S6"))
...
def main() -> None:
    ...
    print("RATE_HALF_KB_M4_A6_542_PAIR_QUOTIENT_NORMAL_FORM_PASS")
    print("RATE_HALF_KB_M4_A6_542_PAIR_QUOTIENT_MUTATIONS_17_OF_17_REJECTED")
