#!/usr/bin/env python3
"""Cross-audit the cell-9 pairing-5 sharded and replay ledgers."""
...
def terminal(payload):
    require(payload["status"] == "PASS" and payload["rows"] == 192
            ...
            and payload["target_root_count"] == 2016
            and payload["candidate_root_count"] == 3200
            and payload["source_point_count"] == payload["route_point_count"]
            == 3072 and payload["missing_free"] == 384
            and payload["missing_impossible"] == 192
            and payload["product_boundaries"] == 192
            and payload["checked"] == 2304
            and payload["missing_relation_nonzero"] == 4928
            and payload["uf_candidate_count"] == payload["uf_checked"] == 576
            and payload["colored_nonzero"] == 576
            ...
            and payload["no_lifts"] == 1600
            and payload["r_boundaries"] == payload["t_boundaries"] == 960,
...
def main():
    ...
    for path in (
        EXP / "rate_half_kb_positive_433_1b_cell9_de_pairing5_chart_scout_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_de_pairing5_recovery_merge.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_de_pairing5_frobenius_roots_modal.py",
        EXP / "rate_half_kb_positive_433_1b_cell9_de_pairing5_direct_audit_modal.py",
    ):
        ast.parse(path.read_text())
    roots = json.loads(ROOTS.read_text())
    audit = json.loads(AUDIT.read_text())
    recovery = json.loads(RECOVERY_SUMMARY.read_text())
    ...
    require(roots["source_manifest_sha256"]
            == hashlib.sha256(PRIMARY.read_bytes()).hexdigest(),
            "root custody join")
    require(len(recovery["rows"]) == 3
            and {(tuple(row["epsilon"]), tuple(row["sigma"]),
                  row["xi_index"], row["b_row_index"], row["c_row_index"])
                 for row in recovery["rows"]}
            == {((1, 1), (1, -1), 2, 2, 5),
                ((1, 1), (1, -1), 2, 2, 6),
                ((1, 1), (1, -1), 2, 3, 4)}
            and all(row["status"] == "COMPLETE" and row["excluded"]
                    for row in recovery["rows"]), "recovery join")
    terminal(audit)
    ...
    require("other 12 active" in contract and "Prize" in contract,
            "scope markers")
    print("PASS cell-9 pairing-5 cross-audit: charts=6 profiles=61 "
          "recovery=3 hostile=detected")
