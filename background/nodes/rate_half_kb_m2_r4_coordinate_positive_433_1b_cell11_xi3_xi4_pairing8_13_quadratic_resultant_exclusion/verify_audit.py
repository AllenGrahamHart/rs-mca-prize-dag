#!/usr/bin/env python3
"""Independent root and direct-lane audit for cell-11 pairing 8/13."""
...
SCRIPT = EXP / (
    "rate_half_kb_positive_433_1b_cell11_xi3_pairing8_"
...
ROOT_SCRIPT = EXP / (
    "rate_half_kb_positive_433_1b_cell11_xi3_pairings7_8_11_"
...
def main():
    ...
    c_leading = {
        tuple(row["epsilon"]): sp.sympify(row["c_leading"]["expression"])
        for row in tower["rows"] if row["c_row_index"] == 6
    }
    ...
    require(len(profiles) == 45, "pairing-specific polynomial census")
    root_payload = json.loads(ROOT_RESULT.read_text())
    require(
        root_payload["schema"]
        == (
            "rate-half-kb-positive-433-1b-cell11-xi3-pairings7-8-11-"
            "independent-roots-v1"
        )
        and root_payload["field"] == P
        and root_payload["source_primary_sha256"]["8"]
        == hashlib.sha256(RESULT.read_bytes()).hexdigest(),
        "independent root custody",
    )
    root_rows = {row["sha256"]: row for row in root_payload["rows"]}
    require(set(profiles).issubset(root_rows), "independent root profile cover")
    ...
    for row in payload["rows"]:
        ...
        for item in row["boundary_rows"]:
            if item["stage"] == "CELL11_B_LEADING":
                expression = b_leading[tuple(row["epsilon"])]
            elif item["stage"] == "CELL11_C_LEADING":
                expression = c_leading[tuple(row["epsilon"])]
            else:
                continue
            require(
                int(expression.subs({r: item["r"], t: item["t"]})) % P == 0,
                "cell-11 leading-boundary replay",
            )
            boundary_count += 1
    ...
    require(boundary_count == 16, "leading-boundary census")
    require(lift_count == 64 and lane_count == 128,
            "direct lift/lane census")
    print(
        "PASS pairing-8 adapter audit: "
        "profiles=112 unique=45 boundary=16 lifts=64 lanes=128"
    )
