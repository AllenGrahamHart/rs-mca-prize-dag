#!/usr/bin/env python3
"""Independent root and direct-lane audit for cell-12 pairing 4/9."""
...
def main():
    ...
    require(
        root_payload["schema"]
        == (
            "rate-half-kb-positive-433-1b-cell12-xi3-pairing4-"
        ...
        for finite in row["finite_rows"]:
            for z_row in finite.get("z_rows", []):
                ...
                for q_row in z_row["q_rows"]:
                    ...
                    require(
                        paired_scalar(a_values, b_values, q_value, -q_value % P)
                        == 0,
                        "antipodal-pair replay",
                    )
                    require(
                        paired_scalar(
                    ...
                    for lane in q_row["lanes"]:
                        ...
                        lane_count += 1
    require(profile_visits == 64, "profile replay count")
    require(boundary_count == 4, "four leading-boundary terminals")
    require(lift_count == 8 and lane_count == 32, "direct lift/lane census")
    print(
        "PASS pairing-4 adapter audit: "
        "profiles=64 unique=45 boundary=4 lifts=8 lanes=32"
    )
