#!/usr/bin/env python3
"""Run the F04 full-J logarithmic-derivative compilation on Modal."""
...
APP_NAME = "rs-mca-k3-f04-full-j-log-derivative"
...
SCRIPT = HERE / "log_derivative_j_probe.sage"
OUTPUT = HERE / "modal_log_derivative_j_f04_output.json"
...
image = (
    ...
    .add_local_file(SCRIPT, "/log_derivative_j_probe.sage")
)


@app.function(image=image, cpu=2, memory=16384, timeout=720)
def run_probe() -> dict[str, object]:
    ...
    try:
        completed = subprocess.run(
            ["sage", "/log_derivative_j_probe.sage", "--assignment", "F04"],
            cwd="/repo",
            env=environment,
            capture_output=True,
            text=True,
...
def main() -> None:
    ...
    output = {
        "schema": "kb-c2-112-f04-full-j-log-derivative-modal-v1",
        "app": APP_NAME,
