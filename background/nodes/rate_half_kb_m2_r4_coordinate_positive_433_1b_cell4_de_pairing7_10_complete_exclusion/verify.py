#!/usr/bin/env python3
"""Verify complete cell-4 pairing-7/10 composition."""
...
ORBIT = (
    ...
    "cell4_de_pairing7_10_orbit_exclusion"
)
POSITIVE = (
    "rate_half_kb_m2_r4_coordinate_positive_433_1b_"
    "cell4_positive_de_pairing10_complete_exclusion"
)
...
def verify_labels():
    ...
    orbit_parent = {(0, 7), (1, 7), (2, 7), (2, 10)}
    positive = {(0, 10), (1, 10)}
    complete = orbit_parent | positive
    require(complete == {(xi, pairing)
                         for xi in (0, 1, 2) for pairing in (7, 10)},
    ...
    require(len(paid) == 33 and len(paid_orbits) == 18,
            "cumulative paid ledger")
    require(sum(map(len, live_orbits)) == 72 and len(live_orbits) == 42,
...
def verify_documents():
    ...
    require("= 96 raw cases" in statement and
            "72 labels in 42 quotient orbits" in statement,
            "printed statement ledger")
    require("105-33=72" in proof and "60-18=42" in proof,
            "printed subtraction")
...
def main():
    ...
    verify_dag()
    verify_documents()
    print("cell=4 pairings=7,10 labels=6 raw_cases=96 live_labels=72 live_orbits=42")
