#!/usr/bin/env python3
"""Count the 22 normalized m=16 residual-variance chambers."""
...
SOURCE = HERE / "m16_residual_counts.cpp"
OUTPUT = HERE / "m16_residual_counts_result.json"
SHARDS = 32
ENERGIES = list(range(5, 90, 4))
...
app = modal.App("e1-prize-m16-residual-counts")
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    packet = {
        "schema": "e1-prize-m16-residual-counts-v1",
        "complete": len(complete) == SHARDS,
        "source_sha256": hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
        "normalization": {
            "singleton_positions": [0, 4],
...
def main() -> None:
    ...
    for result in count.map(range(SHARDS), order_outputs=False):
        ...
        print(f"M16_RESIDUAL_COUNTS_PROGRESS returned={len(results)}/{SHARDS}")
    print(
        "M16_RESIDUAL_COUNTS "
