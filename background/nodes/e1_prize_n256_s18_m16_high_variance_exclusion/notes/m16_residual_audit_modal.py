#!/usr/bin/env python3
"""Independent direct-convolution audit of the m=16 chamber counts."""
...
SOURCE = HERE / "m16_residual_audit.cpp"
OUTPUT = HERE / "m16_residual_audit_result.json"
SHARDS = 32
...
app = modal.App("e1-prize-m16-residual-audit")
...
def write_packet(results: list[dict[str, object]]) -> None:
    ...
    packet = {
        "schema": "e1-prize-m16-residual-audit-v1",
...
def main() -> None:
    ...
    for result in audit.map(range(SHARDS), order_outputs=False):
        ...
        print(f"M16_RESIDUAL_AUDIT_PROGRESS returned={len(results)}/{SHARDS}")
    print(
        "M16_RESIDUAL_AUDIT "
