#!/usr/bin/env python3
"""Independently audit the E26 two-odd census by direct negacyclic products."""
...
SOURCE = HERE / "e26_six_two_odd_profile_census_audit.cpp"
PRODUCTION = HERE / "e26_six_two_odd_profile_census_result.json"
RESULT = HERE / "e26_six_two_odd_profile_census_audit_result.json"
REMOTE_SOURCE = "/root/e26_six_two_odd_profile_census_audit.cpp"
REMOTE_BINARY = "/root/e26_six_two_odd_profile_census_audit"

app = modal.App("e1-n256-e26-six-two-odd-profile-census-audit")
image = (
...
def main() -> None:
    ...
    if not production["complete"]:
        raise RuntimeError("E26 production census is incomplete")
    ...
    if len(tasks) != int(production["expected_templates"]):
        raise RuntimeError("E26 production task count mismatch")
    ...
    if RESULT.exists():
        ...
        if (
            prior.get("schema") == "e1-e26-six-two-odd-profile-census-audit-v1"
    ...
    def write_checkpoint(complete: bool) -> dict[str, object]:
        ...
        packet = {
            "schema": "e1-e26-six-two-odd-profile-census-audit-v1",
            "complete": complete,
    ...
    try:
        ...
        write_checkpoint(False)
        print(f"E26_SIX_TWO_ODD_PROFILE_AUDIT_INCOMPLETE completed={len(rows)}/{len(tasks)}")
        raise
    ...
    print("E26_SIX_TWO_ODD_PROFILE_AUDIT " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E26_SIX_TWO_ODD_PROFILE_AUDIT_AGREEMENT {packet['agreement']}")
    print(f"E26_SIX_TWO_ODD_PROFILE_AUDIT_RESULT {RESULT}")
