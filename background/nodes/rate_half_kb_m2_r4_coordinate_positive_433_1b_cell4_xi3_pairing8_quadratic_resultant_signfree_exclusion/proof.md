...
Applying (2) to `B` and `C` gives a degree-eight necessary cut `D(z)` for a
common `q` root.
...
degree-4068 norm rows yield 64 candidate `r` roots and 32 guarded source
points. Direct finite replay of the original equations leaves 16 compatible
`(z,q)` candidates.
...
is checked directly. All 32 evaluations are nonzero. The target-boundary,
