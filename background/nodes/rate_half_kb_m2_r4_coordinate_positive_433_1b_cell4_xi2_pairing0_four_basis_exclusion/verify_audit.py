#!/usr/bin/env python3
"""Independent root audit for the cell-4 xi2/pairing0 exclusion."""
...
def audit_profile(profile, candidates):
    ...
    found = sorted(point for point in candidates
                   if evaluate(polynomial, point) == 0)
    reconstructed = [1]
    for point in found:
...
def main():
    ...
    for row in payload["rows"]:
        ...
        union = set(target)
        for profile in [*row["inverse_guards"], row["target_norm"]]:
            for side in ("numerator", "denominator"):
                union.update(audit_profile(profile[side], candidates))
                audited_polynomials += 1
        require(sorted(union) == candidates, "candidate root union")
        terminal_roots = (
            {item["r"] for item in row["boundary_rows"]} |
            {item["r"] for item in row["no_lift_rows"]} |
            {item["r"] for item in row["finite_rows"]}
        )
        require(terminal_roots == set(union), "complete terminal cover")
        require(len(row["no_lift_rows"]) == 6 and
                len(row["finite_rows"]) == 4 and
                all(item["status"] == "NONZERO" and item["cut"] % PRIME
                    for item in row["finite_rows"]), "terminal ledgers")
        require(not row["unresolved"] and not row["witnesses"],
                "empty failure ledgers")
    statement = (NODE / "statement.md").read_text()
    proof = (NODE / "proof.md").read_text()
    audit = (NODE / "audit.md").read_text()
    require("P(-m,-m)=0" in statement and "DE=-m" in proof and
            "omitted record is `-DE=m`" in audit, "negative-DE sign")
    require("other `102`" in statement and "one of `7*15=105`" in audit,
            "scope fences")
    print(f"audit=ok rows=4 polynomials={audited_polynomials} "
          "candidates=40 finite=16")
