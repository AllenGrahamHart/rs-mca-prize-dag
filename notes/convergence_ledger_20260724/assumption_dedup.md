...
3. **No unwired hypothesis.** The failure mode that would invalidate (1) is a
   ...
   descendant (benign consumer back-reference). **13 mentions fell outside those
   classes; all 13 were read and are benign**, and are pinned by name in the
   verifier so a *new* one forces a re-audit.

## The 13 audited mentions
...
| `zone_b` | `e1_random_prime_model` | historical preferred-route evidence; the authoritative conditional consumes the wired `e1_fullness` requirement instead |
