...
Sources: upstream read-only at `origin/main = b13de811` via `git show`; `gh` read-only on `przchojecki/rs-mca` (open queue #1087–#1105; closed through #1040+ plus per-author sweeps); our docs `notes/lane_l_crosswalk_20260724/lnl_findings.md`, `notes/upstream_pr_proposals_20260720/upr_proposals.md`, `notes/correspondence/UPSTREAM_IMPORT_LEDGER.md`.
...
## PART 2 — OCCUPIED-TERRITORY BOUNDARY MAP (active contributors)

**holmbuar — three concurrent programs, the busiest contributor (13 of 18 open PRs):**
