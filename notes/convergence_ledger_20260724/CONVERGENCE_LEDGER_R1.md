...
**Baseline of record — CORRECTED 2026-07-27 (Q0 + E1, unsafe-crossing, and averaged-XR false-green audits), machine-pinned by `tools/verify_orbit_census.py`.** The two orbit definitions remain unchanged. The latest correction adds `averaged_xr` as a red leaf and propagates its consumers to amber. The current counts below supersede the earlier snapshots.

| name | root | closure | census |
|---|---|---|---|
| **MATH ORBIT** | {`mca_grand`, `list_grand`} | req-ancestry + alt-closure | **241 = 177 PROVED / 39 CONDITIONAL / 25 TARGET** |
| **SUBMISSION ORBIT** | `prize` | same | **256 = 189 / 41 / 26** |

The submission orbit is a strict superset: math orbit **+ a 15-node packaging spine** (12 PROVED — `bridge_ledger`, `compiler`, `dossier_partial`, `envelope`, `half_johnson_ca`, `harness`, `ld_bridge`, `ldsw_ld_separation`, `lean_existing`, `lean_tier1`, `mca_from_ca_reduction`, `pinned_row`; 2 CONDITIONAL — `prize` and `packaging` themselves; 1 TARGET — `submission_quality_paper_dossier`). **256 − 241 = 15 is definitional, not drift.**
