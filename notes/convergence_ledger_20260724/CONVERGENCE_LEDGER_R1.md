...
**Baseline of record — CORRECTED 2026-07-26 (Q0 + E1 false-green and quantifier audits), machine-pinned by `tools/verify_orbit_census.py`.** The two orbit definitions remain unchanged. The first E1 audit restored two certificate leaves to TARGET; the second removed their invalid named-exhibit-to-universal transport and re-posed one family-uniform target. The current counts below supersede the earlier snapshots.

| name | root | closure | census |
|---|---|---|---|
| **MATH ORBIT** | {`mca_grand`, `list_grand`} | req-ancestry + alt-closure | **246 = 183 PROVED / 39 CONDITIONAL / 24 TARGET** |
| **SUBMISSION ORBIT** | `prize` | same | **261 = 195 / 41 / 25** |

The submission orbit is a strict superset: math orbit **+ a 15-node packaging spine** (12 PROVED — `bridge_ledger`, `compiler`, `dossier_partial`, `envelope`, `half_johnson_ca`, `harness`, `ld_bridge`, `ldsw_ld_separation`, `lean_existing`, `lean_tier1`, `mca_from_ca_reduction`, `pinned_row`; 2 CONDITIONAL — `prize` and `packaging` themselves; 1 TARGET — `submission_quality_paper_dossier`). **261 − 246 = 15 is definitional, not drift.**
...
**Conditional dedup (C3-3) — OUR HALF CLOSED 2026-07-26 WITH A NEGATIVE.** `assumption_dedup.md` is written and the answer is that **there is no assumption set of ours to dedup**: the 39 mathematical CONDITIONALs carry **zero independent work** and all 39 discharge by pure gate propagation once the 24 TARGETs close (fixpoint in 8 rounds; 0 off-orbit blockers; all 13 open nodes named in their prose but not wired audited and found explicitly fenced as non-consumed, historical, or parenthetical). Pinned fail-closed by `tools/verify_conditional_propagation.py`. **The remaining mathematics is exactly the 24 TARGETs.** What survives of C3-3 is only the joint half: mapping our 24 against his six GF inputs.

**Ownership split (re-count before packaging; current submission orbit = 24 math leaves + dossier):** HE closes (A), (E), K1, K5, Lane-M rows, M31 Lane-L uppers. WE close the **12-leaf in-orbit DLI block** (13 counting the off-orbit factorization leaf), the now **3-leaf certificate/E1 group** (`integer_code_distance_cert`, `u2_per_row_certifier`, and the family-uniform `e1_official_prime_exception_control`), the F3 pair, `rate_half_band_closure`, and lane-adjacent list leaves under the crosswalk fences. The two named-field E1 certificate leaves are background exhibit work, not universal proof obligations. JOINT, double-key: the F2 wall, the L1 pair, (S) ↔ XR, list-completion assembly, the dossier ↔ his item (3).
