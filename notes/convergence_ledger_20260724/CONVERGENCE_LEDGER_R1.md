...
2. **Definition of DONE (all-green, submission orbit):** 24 TARGETs closed (23 math leaves + the dossier) **and** ~~38 CONDITIONALs discharged against a deduped joint hypothesis set~~ **and** his four terminal inputs closed. **C3-3 refinement (2026-07-26): the CONDITIONAL conjunct is STRUCK as redundant on our side** — all 36 mathematical CONDITIONALs discharge by pure gate propagation from the 23 TARGETs (`tools/verify_conditional_propagation.py`, `assumption_dedup.md`), and the other 2 (`prize`, `packaging`) are packaging spine nodes that discharge the same way. So **DONE = the 23 mathematical leaves + the dossier + his four inputs**, and the remaining mathematics is **23 units, not 59**. No plan item counts as convergence unless it moves *that* ledger.
...
- **H1 (TOP): affine-span / fixed-union list compilers → the 13-chamber bottleneck** (`thm:affine-span-list` :498, `thm:fixed-union-list-johnson` :439, `thm:single-mds-circuit-ray` :421, `thm:rank-flat-list` :583 — all verified at pin; arithmetic independently reproduced: s=1 pencil cap 2, s=2 cap exactly 4 with a rank-2 witness at equality). Surgeries S2 (mint suppliers PROVED-after-replay, ev into `rate_half_list_adjacent_crossing` + two secondaries) then **S3 immediately: ramguard pencil-cap check against each of the 13 chambers, s=2 rigidity second**. Promotion test: ev→req when chamber coordinates are bound to affine spans. **Any chamber kill is maintainer-surface before the DAG shows a reduced count.** The only harvest with new leverage on a wired bottleneck.
...
**Conditional dedup (C3-3) — OUR HALF CLOSED 2026-07-26 WITH A NEGATIVE.** `assumption_dedup.md` is written and the answer is that **there is no assumption set of ours to dedup**: the 36 mathematical CONDITIONALs carry **zero independent work** and all 36 discharge by pure gate propagation once the 23 TARGETs close (fixpoint in 8 rounds; 0 off-orbit blockers; all 12 open nodes named in their prose but not wired audited and found explicitly fenced as non-consumed, historical, or parenthetical). Pinned fail-closed by `tools/verify_conditional_propagation.py`. **The remaining mathematics is exactly the 23 TARGETs — a re-pricing from 59 to 23.** What survives of C3-3 is only the *joint* half: mapping our 23 against his six GF inputs, which is his-side work we cannot do unilaterally.
...
**Co-finish mechanics:** harvest pass per upstream wave; crosswalk diff per wave audit; snapshot regenerated per wave with the who's-on-it board (concurrent-close protection, R4 below); outbound only in his packet grammar naming live workboard items. The endgame sentence, honestly conditioned: one co-signed proof of the F2 identification's target — **once the scoped zero-prefix chain is extended to the pruned K2 atom** — discharges his `U_Q` atom and our summit red together, collapses the L1 pair via chain #2, and the 38 CONDITIONALs then discharge by gate propagation *to the extent the dedup ledger shows their hypotheses covered*; the resulting all-green DAG, carrying the crosswalk and the node-keyed Lean modules, IS the joint proof's dependency graph.
