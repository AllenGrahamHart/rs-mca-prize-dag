...
**Baseline of record — CORRECTED 2026-07-26 (Q0 + E1 false-green audit), machine-pinned by `tools/verify_orbit_census.py`.** The two orbit definitions remain unchanged. The E1 audit subsequently restored two certificate leaves to TARGET and regressed nine consumers; the current counts below supersede the earlier `201/36/23` and `213/38/24` status snapshots.

| name | root | closure | census |
|---|---|---|---|
| **MATH ORBIT** | {`mca_grand`, `list_grand`} | req-ancestry + alt-closure | **260 = 190 PROVED / 45 CONDITIONAL / 25 TARGET** |
| **SUBMISSION ORBIT** | `prize` | same | **275 = 202 / 47 / 26** |
...
The math orbit is what `orbit/critical_dag.json`, the radial SVG, the published site, `verify_prize_dag.py`'s partition law, and `verify_critical_harness_coverage.py` all measure; its 25 TARGETs are the roadmap's mathematical leaves. The submission orbit is this ledger's baseline and has 26 TARGETs, the 25 math leaves plus the dossier. **Burn-down of mathematics is measured on the math orbit; the all-green end state is the submission orbit, which owns the dossier leaf.** Every count below must now say which orbit it means.
...
2. **Definition of DONE (all-green, submission orbit):** 26 TARGETs closed (25 math leaves + the dossier) **and** his four terminal inputs closed. The conditional conjunct remains redundant on our side: all 45 mathematical CONDITIONALs discharge by pure gate propagation from the 25 TARGETs (`tools/verify_conditional_propagation.py`, `assumption_dedup.md`), and the other 2 (`prize`, `packaging`) are packaging spine nodes that discharge the same way. So **DONE = the 25 mathematical leaves + the dossier + his four inputs**. No plan item counts as convergence unless it moves that ledger.
...
**Coverage checklists (C3-2), standing in the snapshot:** a 4-row table over his terminal inputs — list-completion ↔ H1 (real route), (S) ↔ H3/XR (ev only), **(A) and (E): no route this cycle — his-side objects, we feed ev only** — and a 26-row table over our submission-orbit TARGETs; every uncovered row carries an explicit "no route — reason," never silence.

**Conditional dedup (C3-3) — OUR HALF CLOSED 2026-07-26 WITH A NEGATIVE.** `assumption_dedup.md` is written and the answer is that **there is no assumption set of ours to dedup**: the 45 mathematical CONDITIONALs carry **zero independent work** and all 45 discharge by pure gate propagation once the 25 TARGETs close (fixpoint in 8 rounds; 0 off-orbit blockers; all 13 open nodes named in their prose but not wired audited and found explicitly fenced as non-consumed, historical, or parenthetical). Pinned fail-closed by `tools/verify_conditional_propagation.py`. **The remaining mathematics is exactly the 25 TARGETs.** What survives of C3-3 is only the joint half: mapping our 25 against his six GF inputs.

**Ownership split (re-count before packaging; current submission orbit = 25 math leaves + dossier):** HE closes (A), (E), K1, K5, Lane-M rows, M31 Lane-L uppers. WE close the **12-leaf in-orbit DLI block** (13 counting the off-orbit factorization leaf), the now **4-leaf certificate/E1 group** (`integer_code_distance_cert`, `u2_per_row_certifier`, and the two restored E1 certificate leaves), the F3 pair, `rate_half_band_closure`, and lane-adjacent list leaves under the crosswalk fences. JOINT, double-key: the F2 wall, the L1 pair, (S) ↔ XR, list-completion assembly, the dossier ↔ his item (3).
