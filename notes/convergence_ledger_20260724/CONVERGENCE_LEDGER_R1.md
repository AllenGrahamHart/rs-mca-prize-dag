...
**Baseline of record — CORRECTED 2026-07-27 (Q0 + E1, unsafe-crossing, and exact averaged-XR repair), machine-pinned by `tools/verify_orbit_census.py`.** The two orbit definitions remain unchanged. The invalid averaged-XR auto-proof was first demoted; a separately sourced and independently replayed exact fixed-slope theorem now closes the required moment without claiming worst-case XR. The current counts below supersede the earlier snapshots.

| name | root | closure | census |
|---|---|---|---|
| **MATH ORBIT** | {`mca_grand`, `list_grand`} | req-ancestry + alt-closure | **241 = 179 PROVED / 38 CONDITIONAL / 24 TARGET** |
| **SUBMISSION ORBIT** | `prize` | same | **256 = 191 / 40 / 25** |
