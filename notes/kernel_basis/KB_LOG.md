# KERNEL-BASIS PROGRAM — LOG (append-only; entries after checks pass)

## 1 — 2026-07-10: WP0 DONE — RK imported as rk_rigidity_kernel
## (CONJECTURE), nine gaps flagged

Fresh-context extraction of Codex's rigidity_kernel.md: schema with
every defined term resolved (citations in the node's
WP0_IMPORT_PACKAGE.md), four faces verbatim with targets resolved,
proved-instance box label-checked row by row (one verifier
independently replayed: qx13 pair ledger, 164/164 PASS), and NINE
import-time gaps flagged — most critically: 'dihedral composites'
undefined, the (B, allowance) pair unpinned, per-face agreement
objects unpinned, and the flats instances incidence-shaped rather
than FM-shaped (a catch-#23-shaped conversion risk). Node banked
CONJECTURE with all consumption maps recorded as UNREPLAYED claims;
one context edge (ev -> u1_x4) for reachability, explicitly not a
reduction. Next per sequencing: U1 (F2-as-RK-instance comparison),
then WP2 (F5 -> Face 3, the pattern-setter).

## 2 — 2026-07-10: U1 DONE — F2 is an RK instance; catch #25

The comparison note (U1_F2_AS_RK_INSTANCE.md): mapping exact in all
clauses; RK (TR-quantifier reading) implies F2 at official rows
(FM-suppression makes clause (ii) predict zero extras); K1 demoted
to mechanism-for-instance; effective basis candidates now
{K2, K3, K4} (k = 3), pending face replays. En route, catch #25:
W4's 'sporadic primes observed' was a regime misattribution (the
j=2 boundary is q ~ 65536; the flagged rows are teeth-band) —
corrected on satellite 25; the q=24001 recount vindicated W4's
