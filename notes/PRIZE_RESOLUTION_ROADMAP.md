...
### Common-ray eliminant payment

The fixed-tuple owner constant is linear rather than exponential in the
residual affine dimension. After introducing the common error ray `rho`, the
`m+1` block conditions form an affine linear system in `m` parameters whose
coefficients are linear in `rho`. Generic consistency would create a
varying-ray component and is excluded by the distinct-pole argument. A
nonzero rank minor of degree at most `m+1` therefore contains every possible
ray, and the strip leaves at most one target per root.

At the full next dimensions the `ell=1` cap becomes

```text
|Tau|<=(s+1)binom(e,s+1)/(2binom(r,s+1)),
```

moving the paid endpoints to `8,500,560,263` at rates `1/4,1/8` and
`4,265,559,234` at rate `1/16`.

```text
node: xr_deficient_window_common_ray_eliminant_payment [PROVED]
verifier: 29 row-arithmetic and symbolic matrix-eliminant checks
target: xr_band_forced_commonroot_syzygy_count remains TARGET
unpaid: final ell=1 high-fiber tail; higher ell; split-pencil census
compute: exact local polynomial and integer arithmetic; no Modal
...
The optimization has a decisive endpoint. At
`ell_0=floor((h-4)/7)`, `r_0=2ell_0+1`, the packed profile
`(ell_0,ell_0,1)` satisfies every numerical active-defect constraint. Its
exact pair cap is above budget by factors about `13.74` and `23.55`; its
triple cap is above by about `1.13e8` and `4.84e7`; all tuple orders at least
four have zero incidence.

This is not a target counterexample because simultaneous realization of the
packed fibers by an official primitive pair has not been shown. It is a
proof that fiber caps, core incidence, and independent selected-block counts
cannot close the target by themselves.

```text
result: NARROWED plus route no-go
...
### Two-block kernel-slack router

Retaining the degree slack shows that positive nullity carries substantially
more structure than the full-rank endpoint. Put

```text
r=h-d,       sigma=d-ell-1-2r.
```

Every nonempty `D`-local target has

```text
2r<=|D|<=|G_d|<=2r+sigma.
```

When `sigma<r`, all selected blocks are disjoint `r`-subsets of `D`, so
there are exactly two. They cover all but `sigma` points of both `D` and
`G_d`. Every kernel multiplier is the forced-root locator times a polynomial
of degree below `sigma+1`.

At the tuple-incidence obstruction the exact official values are
`sigma=5,5,1`. Thus the deficient boundary is an almost complete two-ray
cover with kernel cofactor degree below `6,6,2`; it is not a generic
nullity-at-most-six affine window. Upstream PR `#1147` has exact smooth
quartic-trade currencies but no transport to this active-defect geometry,
and PR `#1148` remains a fixture-specific affine-hull classification without
maximal selection.

```text
starting pin: 88e12955; canonical c21ad24b; upstream main 93fba1be
open upstream frontier checked: #1145-#1148
node: xr_deficient_window_two_block_kernel_slack_router [PROVED]
result: NARROWED
DAG status delta: one proved structural router; no new red
upstream terminal delta: none
delta-star bracket movement: none
new assumptions: none
verifier: 2,001,348 exact general-range, official-row, and mutation checks
compute: exact local arithmetic under RAMguard; no Modal
next: derive a maximality-aware two-ray incidence or affine-plane census for
      the deficient branch; attack the full-rank branch separately through
      split-divisor/quotient-prefix anti-concentration
```

### Packed four-fiber plane payment

At the tuple-incidence boundary, an exact block profile `(ell,ell,1)` uses
two complete `phi` fibers and one tail point. The two blocks therefore use
four fixed full fibers, one of three pairings, and two distinct points from a
tail of size

```text
t=e-4ell,       2<=t<=sigma+2.
```

There are only `6t` individual selected-block geometries. Every target
contributes two blocks, while a fixed block owns at most three targets on an
affine plane by the common-ray eliminant. Hence

```text
|Tau_pack| B_(s-2)<=9t binom(n-e,s-2).
```

At the next dimensions, exact arithmetic puts this below the local allowance
for every possible tail on all three rows. The payment is profile-local; it
is not summed with a separate full allowance for mixed profiles, and it does
not cover higher affine dimensions.

```text
node: xr_deficient_window_packed_four_fiber_plane_payment [PROVED]
result: NARROWED
DAG status delta: one proved exact-profile payment; no new red
upstream terminal delta: none
delta-star bracket movement: none
new assumptions: none
compute: exact local combinatorics and official arithmetic; no Modal
...
### Predicate-filtered maximality inversion

The raw window-divisor census can retain maximal selection without treating
deeper cores as exact-depth candidates. For any predicate `Pi` of the
reconstructed maximal pair, including first-match liveness and strip
survival,
...
Truncation gives exact Bonferroni bounds

```text
S_(2a+1)^Pi(d)<=MAX_d^Pi<=S_(2a)^Pi(d).
```

This is a common maximality-aware interface for the deficient mixed-profile
and full-rank split-divisor branches. It is not yet an occupancy estimate:
the next theorem must control an even signed aggregate coherently. Independent
upper bounds on its raw moments are invalid because the aggregate has
negative coefficients.

```text
node: xr_window_divisor_maximality_filter [PROVED, strengthened]
result: NARROWED and exact promotion interface exposed
DAG status delta: no critical promotion
upstream terminal delta: none
delta-star bracket movement: none
new assumptions: none
verifier: 24,847 exhaustive spectrum, coefficient, and negative-control checks
compute: exact local combinatorics under RAMguard; no Modal
next: derive one signed support-moment or exchange-compression bound retaining
      the two-block profile; test whether the same theorem reaches the
      full-rank split-divisor branch
```
