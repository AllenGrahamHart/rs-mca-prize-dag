...
### Work-cycle theorem: positive 433-1b cell-4 DE first-pair block

```text
scope:                    xi in {0,1,2}, pairing in {0,1,2}
canonical matching fact:  pairings 0,1,2 are exactly the matchings whose
                          first pair is residual positions (0,1)
positive omissions:       first pair (DE,-DE), excluded by xi0 norm and
                          xi0-to-xi1 parallel-copy transport
negative omission:        first pair (DE,DE)=(-m,-m), excluded by xi2 norm
new computation:          none; exact composition of three PROVED parents
raw payment:              3 omissions x 3 matchings x 4 source signs x
                          4 target lanes = 144 cases
slice payment:            9 of 105
remaining cell-4 ledger:  96 slices
next route step:          matching 3 is the first new first-pair geometry;
                          quotient matchings 3..14 by parallel-DE exchange
                          and target-lane signs before selecting a norm
nonclaim:                 no xi>=3, pairing>=3, cell/orbit, K3, LIST, MCA,
                          or Prize closure
```
