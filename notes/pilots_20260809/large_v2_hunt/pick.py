#!/usr/bin/env python3
"""Select the max-v_2(p-1) witness across every round-25 shard, verify it
fail-closed (round-24 P5 protocol, reused verbatim), and emit a standalone
zero-import reproduction script whose norm computation is INDEPENDENT of the
tower recursion (Bareiss determinant of the multiplication matrix)."""
import glob
import json
import sys
sys.path.insert(0, "notes/pilots_20260808/kernel_window_hunt")
import klib as K  # noqa: E402
from hunt import verify_hit  # noqa: E402

D = "notes/pilots_20260809/large_v2_hunt/"
cands = []
for f in sorted(glob.glob(D + "state/lad_*.json") + glob.glob(D + "state/turbo_*.json")):
    st = json.load(open(f))
    for r in st.get("best", []):
        cands.append((r["v2"], r["w"], int(r["p"]), f))
    for r in st.get("wit", []):
        cands.append((r["v2"], r["w"], int(r["p"]), f))
cands.sort(key=lambda t: -t[0])
print("candidates: %d, top v_2 values: %s"
      % (len(cands), [c[0] for c in cands[:10]]))
if not cands:
    sys.exit("no candidates yet")

v2, w, p, src = cands[0]
rep = verify_hit(w, p, 128)
for k in sorted(rep):
    print("   %-20s %s" % (k, rep[k]))
assert rep["ok"], "top witness failed verification"
km = rep["kernel_membership"]
json.dump({"w": w, "p": str(p), "v2": v2, "rho": str(km[0]), "s": km[1],
           "src": src}, open(D + "state/best_witness_v2_r25.json", "w"))
...
VECTOR at (p, N' = 128) with v_2(p-1) = {V2}.
...
open(D + "repro_v2_r25.py", "w").write(
    TPL.replace("{W}", repr(w)).replace("{P}", str(p))
       .replace("{RHO}", str(km[0])).replace("{S}", str(km[1]))
       .replace("{V2}", str(v2)))
print("wrote %srepro_v2_r25.py  (v_2(p-1) = %d, p has %d bits)"
      % (D, v2, p.bit_length()))
