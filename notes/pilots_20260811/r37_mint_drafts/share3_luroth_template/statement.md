...
- **status:** MIXED. **PROVED:** the Lüroth degree arithmetic, the waste law,
  the demand calibrations, Lemma 1, the constant-norm existence mechanism.
...
plus `payment_completeness`'s Lüroth-lattice unification — the repo already
states that the lattice subsumes multiplicative and affine-involution as one
class. What is new is the *identification* of (BIV-CURVE) tuple-sharing as a
lattice member, together with the x-degree arithmetic:
...
separate calibration `D(3,3) = 11`; (iii) the flat-supply bound "`>= ~m-5`,
vacuous for `m <= 6`, binding from `m = 7`" is off by one as printed.
...
**THE GUARD AS THE FINDING:** the raw search reported a witness on the
coincidence currency (`C = 12` vs demand `11`) that the structural verifier
KILLED — one slope with hypergraph degree `8` (a common root of `Psi~`)
blowing the per-side cap by `8x`. The registered quantity was satisfied while
the configuration was infeasible, and it was caught BEFORE being reported.
...
- No `G` built, no completion, no bivariate system, nothing gated by bank
  2's verifier; `mu(x)`-at-middles unchecked.
- Sporadic (non-factoring) sharing unsearched (priced `< 1e-4`).
...
- `critical/nodes/rate_half_band_crossing_location/statement.md:4507-4574`
  (Round-36 (SHARE3-4) addendum, coordinator-audited; round 36 bank 4, pilot
  `r36_m4_nonsplit`). Coordinator hand-checks: ibid. :4509-4518, including
  the AUDIT NOTE that the closed-form demand display is the divisible-case
  idealization and the ceiling version is operative.
- (LUR): ibid. :4520-4529. (CN)/the m=4 gap: ibid. :4530-4545.
- The guard: ibid. :4546-4551. (DEG-m): ibid. :4552-4557.
- Flat supply: ibid. :4558-4565. Scope + censure: ibid. :4566-4574.
...
tools/ramguard local -- python3 \
  notes/pilots_20260811/r37_mint_drafts/share3_luroth_template/verify.py
```
