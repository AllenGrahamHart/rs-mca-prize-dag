...
cover the crossing offset either (sibling package
`negation_closure_excess_fence`). **One inequality, two fences.** It is also
why Statement U's cap must come from the fibre pigeonhole rather than from
any ledger import — the (FIB) cap in `statement_u` is explicitly a
from-scratch pigeonhole for this reason.
...
tools/ramguard tiny -- python3 \
  notes/pilots_20260811/r37_mint_drafts/type2_ledger_scope_fence/verify.py
```
