...
`6 = 2m`**, against count excess `3m^2-5m = +12`. The bare count is dead at
EVERY `m >= 2`.

**PRIOR ART — NOT re-claimed by this package.** The `m = 2` member is the
PROVED node `rate_half_layer_a_saturation_count_route_fence`, and **the
`Z^m - X^{2m}` generalization is already recorded in that node's coordinator
addendum** (`background/nodes/rate_half_layer_a_saturation_count_route_fence/
statement.md:73-89`, measured `m = 2,3,4,6` over five fields). This package
contributes an independent `m = 3` replay and should CITE, not duplicate.
...
`(RIC3)` (single-coefficient rational interpolation criterion), found by
subtraction after derivation. **NOT re-verified here** — carried at POSED.

**CROSS-POINTERS OF RECORD:** the saturation-count fence node, `(RIC3)`, and
the row-surplus fence (its `m=1` instance) are three faces of one mechanism;
**neither fence node cites `(RIC3)`** — recorded so the coordinator can wire
it.
...
- The generalized fence's `2m` is constructed-lower-bound plus
  measured-equality (ten cells in the source; one fresh cell here).
...
tools/ramguard local -- python3 \
  notes/pilots_20260811/r37_mint_drafts/la_eq_and_geometry_counterexamples/verify.py
```
