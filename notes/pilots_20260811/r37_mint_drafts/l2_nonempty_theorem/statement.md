...
`80` equations on `48` for fixed `B`; a new inversion is needed.
...
## CONVENTION FLAG (found by this package — coordinator action requested)
...
**The convention is not stated anywhere in the source addendum**, and the
round-35 F1 sentence ("`a*` over ALL slope pairs = 13 on 5 of 6 headline
witnesses, and 12 on one") is convention-sensitive. Any future F1/(NEWCAP)
exercise must fix the convention first.
...
- Blind (L2) search is banned at every `m` (`q^-5`); use the inversion — or,
  after round 36, the rate-1 parametrization (sibling package
  `l2_par_parametrization`).
...
tools/ramguard local -- python3 \
  notes/pilots_20260811/r37_mint_drafts/l2_nonempty_theorem/verify.py
```
