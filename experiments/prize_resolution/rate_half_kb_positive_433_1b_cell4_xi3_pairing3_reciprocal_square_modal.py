#!/usr/bin/env python3
"""Solve cell-4 xi3/pairing3 by reciprocal-square elimination."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell4_xi3_pairing3_reciprocal_square_result.json"
)
REMOTE_STRUCTURE = "/root/structure.json"
REMOTE_KERNEL = "/root/kernel.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell4-xi3-pairing3-reciprocal-square")
image = (
...
def evaluate_case(case):
    ...
    if sigma_c not in (-1, 1):
        raise ValueError("scope is xi=3, pairing=3, fixed sigma_c")
    ...
    p_colored_z = paired_polynomial(
        variable_polynomial * (common_b*missing_record),
        variable_polynomial * (sigma_c*c_pair*missing_record),
    )
    if p_colored_z.degree() != 4:
        raise ValueError("colored reciprocal cut is not quartic in z")
    p_colored_even = PairPolynomial(*(
        p_colored_z.coefficients[index] for index in (0, 2, 4)
    ))
    p_colored_odd = PairPolynomial(*(
        p_colored_z.coefficients[index] for index in (1, 3)
    ))
    p_colored_sign_free = (
        p_colored_even**2
        - variable_polynomial*p_colored_odd**2
    )
    if p_missing.degree() != 2 or p_colored_sign_free.degree() != 4:
        raise ValueError("reciprocal-square sign-free cuts have wrong degrees")
    remainder = polynomial_remainder(p_colored_sign_free, p_missing)
    if remainder.degree() == 1:
        ...
        raise ValueError("sign-free quartic reduction is identically zero")
    print(json.dumps({
        "phase": "reciprocal_square_resultant",
    ...
    for r_value in sorted(candidate_roots):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                colored_z_coefficients = paired_both_scalar_coefficients(
                    a_values,
                    b_values,
                    b_value*missing % PRIME,
                    sigma_c*c_value*missing % PRIME,
                )
                colored_z_coefficients.extend(
                    [0] * (5-len(colored_z_coefficients))
                )
                colored_z_roots = univariate_roots(colored_z_coefficients)
                if missing_z_roots is None:
                    raise ValueError("monic reciprocal quartic vanished")
                z_roots = (
                    missing_z_roots
                    if colored_z_roots is None
                    else sorted(set(missing_z_roots) & set(colored_z_roots))
                )
                ...
                branch_row.update({
                    ...
                    "colored_z_roots": colored_z_roots,
                ...
                for z_value in z_roots:
                    ...
                    for d_value in d_roots:
                        ...
                        for sigma_o in (-1, 1):
                            outside_q_coefficients = paired_both_scalar_coefficients(
                                a_values, b_values, 1,
                                sigma_o*missing*y_value % PRIME,
                            )
                            outside_q_roots = univariate_roots(
                                outside_q_coefficients
                            )
                            if antipodal_q_roots is None:
                                q_roots = outside_q_roots
                            elif outside_q_roots is None:
                            ...
                            lane_row = {
                                "sigma": [sigma_c, sigma_o],
                                "outside_q_roots": outside_q_roots,
                            ...
                            for q_value in q_roots:
                                ...
                                outside_pair_cut = paired_scalar(
                                    a_values, b_values, q_value,
                                    sigma_o*e_value*f_value % PRIME,
                                )
                                colored_pair_cut = paired_scalar(
                                    a_values, b_values,
    ...
    return {
        ...
        "timings": {
            ...
            "reciprocal_square_resultant": cut_seconds - kernel_seconds,
        ...
        "colored_z_cut_profiles": [
            [
                fraction_profile(value, include_expression=False)
                for value in coefficient.vector()
            ]
            for coefficient in p_colored_z.coefficients
        ],
        "missing_cut_degree": p_missing.degree(),
        "colored_z_cut_degree": p_colored_z.degree(),
        "colored_sign_free_degree": p_colored_sign_free.degree(),
...
def main(cases: str = "-1:-1:-1", all_cases: bool = False):
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell4-xi3-pairing3-"
            "reciprocal-square-v1"
        ),
        "field": PRIME,
        "scope": (
            "Exact reciprocal-square resultant norm and direct exceptional-"
            "root replay for cell 4 at xi=3 and pairing=3. Each row fixes "
            "sigma_c in the colored pair and checks both sigma_o lanes."
        ),
        ...
        "rows": [
            {
                key: row.get(key) for key in (
                    "epsilon", "xi_index", "pairing_index", "sigma_c",
