#!/usr/bin/env python3
"""Derive and price the cutoff-free E17 profile/parity/light router."""
...
RESULT = HERE / "e17_profile_parity_probe_result.json"

app = modal.App("e1-n256-e17-profile-parity-route-probe")
...
def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    rows = []
    for counts in product(range(18), range(5), range(2), range(2)):
        if sum((index + 1) ** 2 * count for index, count in enumerate(counts)) != 17:
            continue
...
def diameter_ledgers() -> list[list[int]]:
    ...
    visit(tuple(range(7)), ())
    return [[value, (value - 85) // 2] for value in sorted(square_masses)]
...
def compute() -> dict[str, object]:
    ...
    trace = []
    l1_bound = None
    for l1_norm in range(20, 7, -1):
        delta = 17 + 66 - 4 * l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 17 and l1_bound is None:
            l1_bound = l1_norm
...
def main() -> None:
    ...
    packet["schema"] = "e1-e17-profile-parity-route-probe-v1"
    ...
    print(
        "E17_PROFILE_PARITY_ROUTE_PROBE "
        + json.dumps(
            {
    ...
    print(f"E17_PROFILE_PARITY_ROUTE_PROBE_RESULT {RESULT}")
