#!/usr/bin/env python3
"""Outcome-neutral checker for the FFF q5 coefficient reductions."""
...
PROGRAM = HERE / "rate_half_kb_positive_433_1b_o0b_fff_q5_coefficients_program.py"
RESULT = HERE / "rate_half_kb_positive_433_1b_o0b_fff_q5_coefficients_result.json"
...
def verify(payload=None):
    ...
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-o0b-fff-q5-coefficients-v1",
            "schema")
    ...
    require(row["relation"] ==
            "exact q5 coefficients on admissible FFF base graph" and
            row["variable_count"] == 5 and
            row["variables"] == ["x", "t", "r", "c", "b"] and
            row["coefficient_order"] == [0, 1, 2] and
            row["s_degree"] == 2 and row["graph_basis_size"] == 48 and
    ...
    require(stage_indices == [0, 1, 2][:len(stage_indices)] and
            value_indices == [0, 1, 2][:len(value_indices)],
    ...
    require(row["stderr"] == "" and row["input_program"] == "" and
            len(row["coefficient_stages"]) == 3 and
            len(row["coefficients"]) == 3 and
            all(stage["term_count"] > 0 and stage["degree"] >= 0
                for stage in row["coefficient_stages"]),
            "complete coefficient ledger")
    return {"status": "COMPLETE", "coefficient_count": 3}
...
def main():
    ...
    print("RATE_HALF_KB_POSITIVE_433_1B_O0B_FFF_Q5_COEFFICIENTS_CHECK_PASS "
