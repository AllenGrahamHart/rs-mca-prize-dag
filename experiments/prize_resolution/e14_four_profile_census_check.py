#!/usr/bin/env python3
"""Check the dual complete E14 census and every retained vector."""
...
PROFILES = ((6,2,0,0),(2,3,0,0),(5,0,1,0),(1,1,1,0))
...
def direct_profile(match: dict[str, object]) -> tuple[int, ...]:
    ...
    for value in product[1:64]:
        magnitude = abs(value); assert magnitude <= 4
        if magnitude: profile[magnitude-1] += 1
    return tuple(profile)


def main() -> None:
    packet = json.loads(RESULT.read_text()); probe = json.loads(PROBE.read_text())
    assert packet["schema"] == "e1-e14-four-profile-census-v1"
    ...
    assert summary["vectors_per_engine"] == probe["direct_vector_floor"] == 1321 * comb(124,3) * 64
    profile = [sum(int(row["primary"]["profile_counts"][i]) for row in rows) for i in range(4)]
    full = [sum(int(row["primary"]["full_conductor_counts"][i]) for row in rows) for i in range(4)]
    ...
    for row in rows:
        ...
        for match in primary["matches"]:
            index = int(match["profile"]); positions = [int(value) for value in match["positions"]]
            assert 0 <= index < 4 and gcd(256,*positions) == 1
            assert direct_profile(match) == PROFILES[index]
            seen[index] += 1; matches.append(match)
    assert seen == full
    assert len(matches) == summary["collected_full_conductor"] == sum(full)
    assert all(profile)
    changed = list(matches[0]["positions"]); changed[0] = (changed[0]+1)%128
    assert changed != matches[0]["positions"]
    print("E14_FOUR_PROFILE_CENSUS_CHECK_PASS "
