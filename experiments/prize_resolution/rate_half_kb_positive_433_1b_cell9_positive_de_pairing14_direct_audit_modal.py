#!/usr/bin/env python3
"""Replay the cell-9 positive pairing-14 certificate independently."""
...
PRIMARY = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_positive_de_pairing14_chart_result"
)
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_positive_de_pairing14_"
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_positive_de_pairing14_direct_audit_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-positive-de-pairing14-direct-audit")
image = (
...
def audit(chart_case):
    ...
    expected_metadata = {
        "field": PRIME,
        "scope": (
            "Exact six-chart execution of the pinned common-f resultant "
            "positive-DE pairing-14 compiler for cell-9 xi=0."
        ),
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-positive-de-pairing14-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 14 and item["xi_index"] == 0
        ]
        core.require(
    ...
    expected = {
        ...
        for b_index in (selected_b,) for c_index in (selected_c,)
    }
    ...
    totals = {
        key: 0 for key in (
            ...
            "missing_free", "product_boundaries", "checked", "colored_nonzero",
            "chart_b_paid", "chart_c_paid", "regularized_paid",
            "target_boundaries",
        )
    }
    profile_visits = 0
    for row in iter_records(manifest):
        ...
        core.require(
            ...
            and row["common_f_resultant"] and not row["witnesses"]
            and not row["unresolved"] and not row["colored_solutions"],
            "complete result row",
        )
        ...
        for finite in row["finite_rows"]:
            ...
            totals["checked"] += 1
            b_roots = core.quadratic_roots(
                core.paired_coefficients(a_values, b_values, second_de, bv),
                "b paired f cut",
            )
            c_roots = core.quadratic_roots(
                core.paired_coefficients(
                    a_values, b_values, de, sigma_c * cv
                ),
                "c paired f cut",
            )
            core.require(b_roots == finite["b_pair_f_roots"]
                         and c_roots == finite["c_pair_f_roots"],
                         "paired f roots")
            common = sorted(set(b_roots) & set(c_roots))
            core.require(common == finite["common_f_roots"], "common-f roots")
            expected_pairs = set()
            ...
            for fv in common:
                ...
                square = fv * fv % PRIME
                quartic = [
                    de * de * square * square % PRIME,
                    0,
                    square * (2 * eta * de - source_sum) % PRIME,
                    0,
                    1,
                ]
                expected_pairs.update(
                    (uv, fv) for uv in core.even_quartic_roots(
                        quartic, "missing cut"
                    )
                )
            core.require({(item["u"], item["f"]) for item in finite["uf_rows"]}
                         == expected_pairs, "complete quartic lift")
            for uf_row in finite["uf_rows"]:
                uv, fv = uf_row["u"], uf_row["f"]
                if fv == 0:
                    core.require(
                        uv is None and uf_row["status"] == "TARGET_BOUNDARY"
                ...
                core.require(relation == uf_row["relation"] == 0,
                             "missing quartic relation")
                local_candidates.add((rv, tv, bv, cv, uv, fv))
                ev = uv * pow(fv, -1, PRIME) % PRIME
                if ev == 0:
                    core.require(
                    ...
                    local_boundaries.append({**finite, **uf_row})
                    continue
                dv = de * pow(ev, -1, PRIME) % PRIME
                vv = dv * fv % PRIME
                colored = core.paired(
                    a_values, b_values, vv, sigma_o * uv % PRIME
                )
                core.require(
    ...
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-positive-de-pairing14-"
            "direct-audit-v1"
        ),
        "field": PRIME,
