#!/usr/bin/env python3
"""Solve cell-4 positive-DE omissions at canonical pairing 14."""
...
app = modal.App(
    "rs-mca-positive-433-1b-cell4-positive-de-pairing14-common-f-resultant"
)
image = (
...
def evaluate_case(case):
    ...
    if xi_index != 0:
        raise ValueError("scope is xi=0, pairing=14; xi=1 transports")
    ...
    print(json.dumps({"phase": "common_f_quadratics",
                      "seconds": time.perf_counter() - started}), flush=True)

    beta_0, beta_1 = kernel[6:]
    eta = 1 if xi_index == 0 else -1
    p_b_0, p_b_1, p_b_2 = p_b.coefficients[:3]
    p_c_0, p_c_1, p_c_2 = p_c.coefficients[:3]
    target_free = (
        (p_b_2*p_c_0 - p_b_0*p_c_2)**2
        - (p_b_2*p_c_1 - p_b_1*p_c_2)
        * (p_b_1*p_c_0 - p_b_0*p_c_1)
    )
    print(json.dumps({"phase": "common_f_resultant",
                      "seconds": time.perf_counter() - started}), flush=True)
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                if de_value == 0:
                    ...
                    continue
                b_pair_f_roots = univariate_roots(
                    paired_scalar_coefficients(
                        a_values, b_values, second_de_value, b_value
                    )
                )
                c_pair_f_roots = univariate_roots(
                    paired_scalar_coefficients(
                        a_values, b_values,
                        de_value, sigma_c*c_value % PRIME,
                    )
                )
                point_row["b_pair_f_roots"] = b_pair_f_roots
                point_row["c_pair_f_roots"] = c_pair_f_roots
                if b_pair_f_roots is None or c_pair_f_roots is None:
                ...
                point_row["common_f_roots"] = f_roots
                for f_value in f_roots:
                    if f_value == 0:
                        ...
                        target_boundary_rows.append({**point, **uf_row})
                        point_row["uf_rows"].append(uf_row)
                        continue
                    f_squared = f_value*f_value % PRIME
                    u_roots = univariate_roots([
                    ...
                    if u_roots is None:
                        point_row["status"] = "ZERO_MISSING_RELATION"
                        unresolved.append({
                            **point, "f": f_value,
                            "reason": "ZERO_MISSING_RELATION",
                        })
                        continue
...
def main(cases: str = "-1:-1:-1:-1:0", all_cases: bool = False):
    ...
    for case, row in zip(selected, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell4-positive-de-pairing14-common-f-resultant-v1",
        "field": PRIME,
        "scope": (
            "Exact common-f resultant norm and direct finite replay for cell 4, "
            "xi=0, pairing 14; xi=1 transports from the identical copy."
        ),
