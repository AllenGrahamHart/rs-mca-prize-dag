...
TORUS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_o0b_common_repeat_cell3_"
    "bcplus_monomial_probe_result.json"
)
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_o0b_common_repeat_cell3_"
    "bcplus_uncolored_exceptional_fibers_result.json"
)
SHARD_RESULTS = {
    record: DIRECTORY / (
        "rate_half_kb_positive_433_1b_o0b_common_repeat_cell3_"
        f"bcplus_uncolored_exceptional_{record.replace('+', 'plus')}_result.json"
    )
...
app = modal.App("rs-mca-positive-433-1b-o0b-cell3-bcplus-uncolored-exceptional")
image = (
...
def pay(case):
    ...
    guard_tree = ast.parse(
        torus_row["transformed_guard"]["expression"], mode="eval"
    )

    def evaluate_guard(node, r_value, u_value):
        if isinstance(node, ast.Expression):
            return evaluate_guard(node.body, r_value, u_value)
        if isinstance(node, ast.Constant):
            return int(node.value) % PRIME
        if isinstance(node, ast.Name):
            return {"r": r_value, "u": u_value}[node.id]
        if isinstance(node, ast.UnaryOp) and isinstance(node.op, ast.USub):
            return -evaluate_guard(node.operand, r_value, u_value) % PRIME
        if isinstance(node, ast.BinOp):
            left = evaluate_guard(node.left, r_value, u_value)
            right = evaluate_guard(node.right, r_value, u_value)
        ...
        raise ValueError(f"unsupported guard AST: {ast.dump(node)}")
    ...
    for u_value in u_values:
        h = context([
            epsilon_2*IOTA*u_value,
            -epsilon_1*(IOTA+epsilon_2)*(u_value*u_value+1),
            u_value,
        ])
        r_values = field_roots(h)
        if r_values is None:
            unresolved.append({"u": u_value, "reason": "ZERO_TORUS_POLYNOMIAL"})
            continue
        for r_value in r_values:
            fiber = {"u": u_value, "r": r_value}
            if evaluate_guard(guard_tree, r_value, u_value) == 0:
                fiber["status"] = "COMMON_GUARD_BOUNDARY"
            ...
            for endpoint in endpoint_roots:
                ...
                if y_roots is None:
                    ...
                    unresolved.append({
                        "u": u_value, "r": r_value, "endpoint": endpoint,
...
def main():
    ...
    source_hashes = {
        ...
        "torus": hashlib.sha256(TORUS.read_bytes()).hexdigest(),
    }
    shards = {}
    for record, path in SHARD_RESULTS.items():
        shard_rows = [row for row in rows if row["missing_record"] == record]
        shard = {
            "schema": "rate-half-kb-positive-433-1b-o0b-cell3-bcplus-uncolored-exceptional-shard-v1",
    ...
    for row in rows:
        ...
        compact_rows.append({
            ...
            "status": row["status"],
            "u_count": len(row.get("u_values", [])),
            "u_sha256": hashlib.sha256(json.dumps(
                row.get("u_values", []), separators=(",", ":")
            ).encode()).hexdigest(),
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-o0b-cell3-bcplus-uncolored-exceptional-v1",
        "scope": (
        ...
        "source_torus_sha256": source_hashes["torus"],
        "case_count": len(rows),
    ...
    print(json.dumps({
        ...
        "u_incidence_count": sum(len(row.get("u_values", [])) for row in rows),
        "maximum_seconds": max(
