#!/usr/bin/env python3
"""Paired bounded completion wave for K'=85 best-single domination."""
...
PRIMARY = DIRECTORY / "rate_half_mca_rank11_k85_best_single_domination_falsifier.py"
AUDIT = DIRECTORY / "rate_half_mca_rank11_k85_best_single_domination_audit.py"
...
for path in (PRIMARY, AUDIT, BASE, CODE, DEPS):
    if not path.is_file():
        raise FileNotFoundError(path)


app = modal.App("rate-half-mca-rank11-k85-best-single-wave")
image = (
...
@app.function(image=image, cpu=1, memory=256, timeout=285)
...
def main() -> None:
    ...
    event = (
        "BATCH_COMPLETE"
        if completed == len(jobs) and infrastructure_failures == 0
