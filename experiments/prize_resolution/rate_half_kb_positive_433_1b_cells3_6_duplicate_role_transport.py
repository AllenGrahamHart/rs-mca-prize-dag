#!/usr/bin/env python3
"""Exact duplicate-common-role transport from complete cell 3 to cell 6."""
...
def common_cell3_rows(epsilon_1, epsilon_2, values):
    ...
    return {
        ...
        "AC": (
            c,
            scale(multiply(multiply(iota, add(one, c)), constant(1)), epsilon_1),
            constant(-1),
        ),
        "BC+": (multiply(b, c), multiply(r, add(b, c)), square(r)),
        "BC-": (
...
def common_cell6_rows(epsilon_1, epsilon_2, values):
    ...
    return {
        ...
        "AB": (
            b,
            scale(multiply(multiply(iota, add(one, b)), constant(1)), epsilon_1),
            constant(-1),
        ),
...
def verify_role_cell_action():
    ...
    require(indexed[permute_cell(cells[3], bc_swap)] == 6,
            "duplicate-role action cell 3 to cell 6")
...
def verify_transport():
    ...
    for epsilon_1, epsilon_2 in SOURCE_SIGNS:
        ...
        cell3 = common_cell3_rows(epsilon_1, epsilon_2, values)
        cell6 = common_cell6_rows(
        ...
        require(
            all(cell6[new_role] == cell3[old_role]
...
def main():
    ...
    print(
        "RATE_HALF_KB_POSITIVE_433_1B_CELLS3_6_TRANSPORT_PASS "
