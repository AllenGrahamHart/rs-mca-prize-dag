#!/usr/bin/env python3
"""Shared direct replay for cell-12 common-f resultant certificates."""
...
def load_root_cache(root_result, primary_paths):
    ...
    require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell12-de-pairing11-14-frobenius-roots-v1"
        and roots["field"] == P
        and roots["method"]
...
def audit_result(result, root_result, primary_paths, tower_path, kernel_path,
    ...
    for row in payload["rows"]:
        ...
        for item in row["boundary_rows"]:
            ...
            elif item["stage"] == "CELL12_B_LEADING":
