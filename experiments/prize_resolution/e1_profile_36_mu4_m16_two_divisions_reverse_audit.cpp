...
#include "e1_profile_36_mu4_m16_direct_radius.cpp"
...
struct ReverseAuditCounts {
    ...
    std::array<uint64_t, 90> live_by_energy{};
    std::array<uint64_t, 90> above_by_energy{};
};
        ...
        for (size_t first_index = 0; first_index < allowed.size(); ++first_index) {
            ...
            for (size_t second_index = first_index + 1; second_index < allowed.size(); ++second_index) {
                ...
                for (size_t third_index = second_index + 1; third_index < allowed.size(); ++third_index) {
                    ...
                    if (__builtin_popcountll((key ^ fixed) & even_mask) > radius) continue;
...
int main() {
    ...
    for (int candidate_energy = 2; candidate_energy <= 89; ++candidate_energy) {
