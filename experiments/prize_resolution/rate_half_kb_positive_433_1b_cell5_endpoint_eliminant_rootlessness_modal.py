#!/usr/bin/env python3
"""Independently certify that the eight cell-5 endpoint eliminants have no F_p root."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell5_endpoint_eliminant_rootlessness_result.json"
)
REMOTE_PILOT = "/root/pilot.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell5-endpoint-rootlessness")
image = (
...
def audit(row):
    ...
    expected_degree = 16 if row["endpoint"] == "b" else 11
...
def main():
    ...
    output = {
        ...
        "complete": len(rows) == 8 and all(row["root_gcd"] == [1] for row in rows),
        "rows": rows,
    }
