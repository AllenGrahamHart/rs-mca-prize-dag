#!/usr/bin/env python3
"""Evaluate cell-4 xi2/pairing0 in the exact four-basis common algebra."""
...
app = modal.App("rs-mca-positive-433-1b-cell4-xi2-pairing0-four-basis")
image = (
...
def evaluate_case(signs):
    ...
    # At xi=2 the omitted record is -DE.  Hence DE=-missing_record,
    # and pairing 0 joins the two surviving parallel DE records.
    ...
    for r_value in sorted(candidate_roots):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                cut_value = paired_scalar(de_value, de_value)
...
def main(signs: str = "-1:-1,-1:1,1:-1,1:1"):
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell4-xi2-pairing0-four-basis-v1",
        "field": PRIME,
        "scope": (
            "Exact four-basis norm and direct finite replay for cell 4, "
            "xi2, pairing0; no other pairing or full-cell claim."
        ),
        ...
        "rows": [
            {
                key: row.get(key) for key in (
                    ...
                    "boundary_rows", "no_lift_rows", "witnesses",
                    "unresolved", "excluded",
                )
            }
            for row in rows
