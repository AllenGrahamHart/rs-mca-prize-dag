#!/usr/bin/env python3
"""Scout the pinned pairing-3 compiler on the six cell-9 tower charts."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing3_chart_scout_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-de-pairing3-chart-scout")
image = (
...
@app.function(image=image, cpu=2.0, memory=4096, timeout=420, max_containers=24)
def evaluate(case):
    import time
...
def main(limit: int = 0, full: bool = False):
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell9-de-pairing3-chart-scout-v1",
        "field": PRIME,
    ...
    manifest = None
    if writer is not None:
        manifest = writer.close(complete=(len(rows) == 192 and not remote_errors))
    print(json.dumps({
