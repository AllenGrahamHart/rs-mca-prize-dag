#!/usr/bin/env python3
"""Independent deployed-field audit of cell-12 positive-DE residuals."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell12_parallel_de_first_pair_audit_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell12-parallel-de-audit")
image = (
...
def audit(case):
    ...
    for point_index, point in enumerate(points):
        ...
        for pairing_index, matching in enumerate(MATCHINGS):
            systems += 1
            substitution = {e: sp.Rational(missing, 1)/d}
            equations = []
            missing_sum, _ = sp.fraction(sp.cancel(
                ((d+e)**2-source_sum).subs(substitution)
            ))
            equations.append(sp.Poly(missing_sum, d, f, modulus=PRIME))
            ...
            guard_numerator = sp.Poly(guard_numerator, d, f, modulus=PRIME)
            basis = sp.groebner(
                [item.as_expr() for item in equations]
                + [z*d*guard_numerator.as_expr()-1],
            ...
            for f_value in roots(eliminant.as_expr(), f) or []:
                d_polynomials = [
                    ...
                    if not sp.Poly(item.as_expr().subs(f, f_value), d,
...
def main(limit: int = 0):
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell12-parallel-de-audit-v1",
        "field": PRIME,
        "scope": (
            "Independent two-variable lex and deployed-root audit for the "
            "cell-12 positive-DE first-pair residual systems."
        ),
