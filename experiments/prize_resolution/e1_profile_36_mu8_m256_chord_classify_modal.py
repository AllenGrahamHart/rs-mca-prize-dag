#!/usr/bin/env python3
"""Classify all affine six-singleton supports of exact multiplicity eight."""
...
app = modal.App("e1-profile-36-mu8-m256-chord-classify")
...
def main(
    ...
    output: str = (
        ...
        "e1_profile_36_mu8_m256_chord_orbits.json"
    ),
) -> None:
    ...
    payload = {
        "schema": "e1-profile-36-mu8-m256-chord-orbits-v1",
        "examined": sum(int(row["examined"]) for row in rows),
        "mu_eight": sum(int(row["mu_eight"]) for row in rows),
    ...
    assert payload["mu_eight"] == 87_856
    ...
    print(
        "E1_PROFILE_36_MU8_M256_CHORD_CLASSIFY_PASS "
        f"examined={payload['examined']} mu_eight={payload['mu_eight']} "
