...
def common_data(epsilon, b_value, c_value, r_value):
...
def validate_exceptional(missing_record, payload, generic, roots, lifts):
    require(missing_record in MISSING_RECORDS, "record scope")
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-o0b-cell3-bcminus-uncolored-exceptional-shard-v1",
            "shard schema")
    ...
    lifts_by_q = {row["q"]: row for row in lifts["rows"]}
    ...
    for key in sorted(rows):
        ...
        require(row["q_values"] == expected_q, "exceptional q cover")
        expected_points = {}
        for base_q in expected_q:
            lift_row = lifts_by_q[base_q]
            for y_row in lift_row.get("sign_rows", []):
                if y_row["status"] != "LIFTED":
                    continue
                sign_row = next(item for item in y_row["sign_rows"]
                                if tuple(item["epsilon"]) == epsilon)
                for point in sign_row["points"]:
                    if point["status"] == "GUARDED":
                        expected_points[(base_q, y_row["y"], point["r"])] = (
                            y_row["b"], y_row["c"]
                        )

        fibers = {(fiber["base_q"], fiber["curve_y"], fiber["r"]): fiber
                  for fiber in row["fibers"]}
        require(len(fibers) == len(row["fibers"]) == row["fiber_count"],
                "fiber uniqueness")
        require(set(fibers) == set(expected_points), "common lift support")
        ...
        for point_key, fiber in sorted(fibers.items()):
            _, _, r_value = point_key
            b_value, c_value = expected_points[point_key]
            data = common_data(epsilon, b_value, c_value, r_value)
    ...
    expected_profiles = {
        "DE+": (640, 512, {
            "EMPTY_ENDPOINT_FIBERS": 160,
            "MISSING_PRODUCT_INCONSISTENT": 240,
            "ZERO_MISSING_PRODUCT_BOUNDARY": 240,
        }),
        "DF+": (864, 1408, {
            "EMPTY_ENDPOINT_FIBERS": 384,
            "MISSING_PRODUCT_INCONSISTENT": 240,
            "ZERO_MISSING_PRODUCT_BOUNDARY": 240,
        }),
        "EF": (832, 1152, {
            "EMPTY_ENDPOINT_FIBERS": 352,
            "MISSING_PRODUCT_INCONSISTENT": 240,
            "ZERO_MISSING_PRODUCT_BOUNDARY": 240,
        }),
    }
