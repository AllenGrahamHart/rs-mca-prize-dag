#!/usr/bin/env python3
"""Check the dual complete E16 census and every retained vector."""
...
PROFILES = ([4, 3, 0, 0], [0, 4, 0, 0], [3, 1, 1, 0], [0, 0, 0, 1])
...
def main() -> None:
    ...
    assert packet["schema"] == "e1-e16-four-profile-census-v1"
    ...
    assert hashlib.sha256(FOUR_ATLAS.read_bytes()).hexdigest() == packet["four_atlas_sha256"]
    rows = packet["rows"]
    ...
    print(
        "E16_FOUR_PROFILE_CENSUS_CHECK_PASS "
        f"templates=154 vectors={summary['vectors_per_engine']} profile={sum(profile)} "
