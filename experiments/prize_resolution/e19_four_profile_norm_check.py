#!/usr/bin/env python3
"""Check the dual exact E19 norm ledger."""
...
SOURCE = HERE / "e19_four_profile_norm_modal.py"
...
def main() -> None:
    ...
    assert packet["schema"] == "e1-e19-four-profile-norm-v1"
    ...
    assert summary["norm_at_or_above_2_250"] == sum(
        value >= 2**250 for value in flint
    )
    ...
    print(
        "E19_FOUR_PROFILE_NORM_CHECK_PASS "
        f"vectors={len(vectors)} distinct={len(set(flint))} max={maximum} "
        f"bits={maximum.bit_length()} "
        f"hits={summary['norm_at_or_above_2_250']} engines=2 mutations=1"
    )
