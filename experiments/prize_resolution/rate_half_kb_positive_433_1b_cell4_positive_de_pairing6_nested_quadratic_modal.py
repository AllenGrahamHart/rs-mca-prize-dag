#!/usr/bin/env python3
"""Solve the positive-DE omissions in cell 4 at canonical pairing 6."""
...
app = modal.App("rs-mca-positive-433-1b-cell4-positive-de-pairing6")
image = (
...
def evaluate_case(case):
    ...
    if xi_index != 0:
        raise ValueError("scope is xi=0, pairing=6")
    ...
    second_de = -de_record
    p_u = paired_polynomial(PairPolynomial(second_de), variable_polynomial)
    ...
    for r_value in sorted(candidate_roots):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                de_value = missing
                second_de_value = -de_value % PRIME
                source_sum = (
                    label
                ...
                u_roots = univariate_roots(paired_scalar_coefficients(
                    a_values, b_values, second_de_value
                ))
...
def main(cases: str = "-1:-1:-1:-1:0", all_cases: bool = False):
    ...
    output = {
        "schema": (
            ...
            "pairing6-nested-quadratic-v1"
        ),
        "field": PRIME,
        "scope": (
            "Exact nested-quadratic norm and direct finite replay for cell 4, "
            "xi=0, pairing 6; xi1 transports from xi0."
        ),
