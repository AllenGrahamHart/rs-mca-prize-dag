#!/usr/bin/env python3
"""Outcome-neutral checker for the canonical O0b FFF square subsystem."""
...
PROGRAM = HERE / "rate_half_kb_positive_433_1b_o0b_fff_square_subsystem_program.py"
RATIO_TIMEOUT = HERE / "rate_half_kb_positive_433_1b_o0b_fff_ratio_reduction_result.json"
RESULT = HERE / "rate_half_kb_positive_433_1b_o0b_fff_square_subsystem_result.json"
...
def verify(payload=None):
    ...
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-o0b-fff-square-subsystem-v1",
            "schema")
    require(payload["collection_complete"] is True and
            ...
            payload["source_ratio_timeout_sha256"] == RATIO_TIMEOUT_SHA256 and
    ...
    require(row["relation"] == "necessary FFF square-subsystem superset" and
            row["omitted_finite_pair"] == "q4" and
            row["variable_count"] == 6 and
            row["variables"] == ["E", "s", "t", "r", "c", "b"] and
    ...
    require(row["stderr"] == "" and "END" in row["stdout"] and
            ...
            [7, 5, 6] and len(row["route_stages"]) == 16 and
...
def main():
    ...
    print("RATE_HALF_KB_POSITIVE_433_1B_O0B_FFF_SQUARE_SUBSYSTEM_CHECK_PASS "
