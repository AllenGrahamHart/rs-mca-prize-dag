#!/usr/bin/env python3
"""Triangularize the generic DE+/DE- pair on both cell-0 branches."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell0_generic_signed_pair_result.json"
REMOTE_KERNEL = "/root/cell0_kernel_reduction_result.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1a-cell0-generic-signed-pair")
image = (
...
def analyze_branch(branch_root):
    import sympy as sp
    ...
    d0, d1 = [evaluate("a2", root) for root in (z0, z1)]
    n0, n1 = [evaluate("a0", root) for root in (z0, z1)]
    q0, q1 = [
...
def main():
    ...
    rows = list(analyze_branch.map(roots, return_exceptions=True))
    normalized = []
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell0-generic-signed-pair-v1",
        "scope": (
            "Generic guard-saturated DE+/DE- pair on both cell-0 common "
            "branches; no colored BE, outside completion, route, K3, or Prize claim."
        ),
        "rows": normalized,
    }
