#!/usr/bin/env python3
"""Compile guarded four-basis presentations of the cell-5 common curve."""
...
app = modal.App("rs-mca-positive-433-1b-cell5-four-basis-tower")
image = (
...
def main():
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell5-four-basis-tower-v1",
        "field": PRIME,
        "scope": (
            "Exact quadratic-over-quadratic common tower with one linear c "
            "recovery on one guarded cell-5 chart; no outside claim."
        ),
