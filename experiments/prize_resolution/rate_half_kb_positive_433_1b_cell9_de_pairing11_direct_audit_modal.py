#!/usr/bin/env python3
"""Stream and independently replay the cell-9 pairing-11 certificate."""
...
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing11_"
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing11_direct_audit_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-de-pairing11-direct-audit")
image = (
...
def audit(chart_case):
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-de-pairing11-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 11 and item["xi_index"] in (0, 2)
        ]
        core.require(
        ...
        for finite in row["finite_rows"]:
            ...
            c_roots = core.quadratic_roots(
                core.paired_coefficients(
                    a_values, b_values, second_de, sigma_c * cv
                ),
                "c paired f cut",
            )
    ...
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-de-pairing11-"
            "direct-audit-v1"
        ),
        "field": PRIME,
