#!/usr/bin/env python3
"""Independent checker for the cutoff-free E23 router."""
...
ROOT = HERE.parents[1]
SOURCE = HERE / "e23_profile_parity_probe_modal.py"
RESULT = HERE / "e23_profile_parity_probe_result.json"
ATLAS = (
    ROOT
...
def recursive_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    def visit(magnitude: int, energy: int, l1_norm: int, counts: list[int]) -> None:
        if magnitude == 6:
            ...
            return
        for count in range((23-energy)//(magnitude*magnitude)+1):
        ...
        for count2 in range(13):
            for count1 in range(7):
                if count4+count2+count1 == 0:
                    continue
                totals = {
                    abs(4*(count4-2*n4)+2*(count2-2*n2)+count1-2*n1)
                    for n4 in range(count4+1) for n2 in range(count2+1)
                    for n1 in range(count1+1)
                }
                for total in totals:
...
def main() -> None:
    ...
    assert packet["schema"] == "e1-e23-profile-parity-route-probe-v1"
    assert packet["complete"] is True and packet["variance"] == 46 and packet["energy"] == 23
    ...
    trace = [[l1, 89-4*l1, minima[89-4*l1]] for l1 in range(22,10,-1)]
    assert packet["slack_trace"] == trace
    qualifying = [row for row in trace if row[2] is not None and int(row[2]) <= 23]
    ...
    expected_atlas = {
        key: {"normalized_supports": int(geometry["support_counts"][key]),
    ...
    print(
        "E23_PROFILE_PARITY_PROBE_CHECK_PASS "
