#!/usr/bin/env python3
"""Derive and price the cutoff-free E13 profile/parity/light router."""
...
RESULT = HERE / "e13_profile_parity_probe_result.json"
app = modal.App("e1-n256-e13-profile-parity-route-probe")
                ...
                for used_slack in range(class_target + 1):
                    ...
                    for (state_slack, used_2, used_1), energy in active:
                        for slack, count_2, count_1, class_sum in class_types:
                            ...
                            if (key[0] > class_target or key[1] > 12 - diameter_2
...
def layer_cap(counts: tuple[int, ...]) -> int:
    ...
    return sum(min(a * b - min(a, b), a * c - min(a, c), b * c - min(b, c))
               for a, b, c in product(sizes, repeat=3))


def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    rows = []
    for counts in product(range(14), range(4), range(2), range(2)):
        if sum((index + 1) ** 2 * count for index, count in enumerate(counts)) != 13:
            continue
        l1_norm = sum((index + 1) * count for index, count in enumerate(counts))
        if l1_norm <= l1_bound:
            rows.append({"cap": layer_cap(counts), "profile": list(counts),
                         "l1": l1_norm, "odd_classes": sum(counts[0::2])})
    return sorted(rows, key=lambda row: (int(row["cap"]), row["profile"]), reverse=True)


def matching_ledgers() -> list[list[int]]:
    ...
    def visit(available: tuple[int, ...], edges: tuple[tuple[int, int], ...]) -> None:
        if not available:
            if sum(weights[a] == weights[b] == 1 for a, b in edges) == 1:
                masses.add(sum((weights[a] * weights[b]) ** 2 for a, b in edges))
            return
        first = available[0]
        visit(available[1:], edges)
        for offset, second in enumerate(available[1:]):
            visit(available[1:offset + 1] + available[offset + 2:], edges + ((first, second),))

    visit(tuple(range(7)), ())
    return [[value, (value - 89) // 2] for value in sorted(masses)]


@app.function(cpu=1.0, memory=256, timeout=60)
def compute() -> dict[str, object]:
    slack = relaxed_minimum_energy_by_slack(55)
    trace = []
    l1_bound = None
    for l1_norm in range(13, 5, -1):
        delta = 13 + 66 - 4 * l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 13 and l1_bound is None:
            l1_bound = l1_norm
    assert l1_bound is not None
    profiles = energy_profiles(l1_bound)
    survivors = [row for row in profiles if int(row["odd_classes"]) <= 5]
    return {
        "complete": True, "variance": 26, "energy": 13,
        "slack_trace": trace, "l1_bound": l1_bound,
        ...
        "survivors_by_odd_count": {
            ...
            for odd in (1, 3, 5)},
        "diameter_ledgers": {"one_light_diameter": matching_ledgers()},
    }
...
def main() -> None:
    root = HERE.parents[1]
    atlas_path = root / "background/nodes/e1_n256_s16_e27_profile_parity_light_reduction/notes/e27_profile_parity_probe_result.json"
    atlas_source = json.loads(atlas_path.read_text())
    geometry = atlas_source["light_geometry"]
    if (not atlas_source["complete"]
            or set(geometry["orbit_counts"]) != {"1", "3", "5"}
            or set(geometry["support_counts"]) != {"1", "3", "5"}):
        raise RuntimeError("the proved odd-parity light atlas is incomplete")
    ...
    packet["relevant_normalized_supports"] = sum(int(atlas[key]["normalized_supports"]) for key in used)
    packet["relevant_affine_templates"] = sum(int(atlas[key]["affine_orbits"]) for key in used)
    packet["vectors_per_template"] = comb(124, 3) * 64
    packet["direct_vector_floor"] = packet["relevant_affine_templates"] * packet["vectors_per_template"]
    packet["schema"] = "e1-e13-profile-parity-route-probe-v1"
    ...
    print("E13_PROFILE_PARITY_ROUTE_PROBE " + json.dumps({
        "l1_bound": packet["l1_bound"], "profiles": packet["profile_count"],
        ...
        "direct_vector_floor": packet["direct_vector_floor"]}, sort_keys=True))
    print(f"E13_PROFILE_PARITY_ROUTE_PROBE_RESULT {RESULT}")
