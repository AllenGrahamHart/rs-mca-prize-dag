#!/usr/bin/env python3
"""Pilot xi3 opposite-DE pairings 4 and 5 via parity and missing f."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_opposite_de_parity_missing_f_bezout_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_opposite_de_parity_missing_f_bezout_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-xi3-opposite-de-parity-missing-f")
image = (
...
def profile_case(case):
    ...
    if (pairing_index not in (4, 5) or xi_index != 3 or sigma_o != 0 or
            (pairing_index == 4 and sigma_c != 0) or
            (pairing_index == 5 and sigma_c not in (-1, 1))):
        raise ValueError(
            "the pilot is scoped to xi=3 opposite-DE representatives 4 and 5"
        )
    ...
    # The opposite-DE equation is even in q.  For the other equation write
    # F(q,lambda*f)=a2(f)q^2+a1(f)q+a0(f), set x=q^2, and eliminate the sign
    # of q by (a2*x+a0)^2-a1^2*x.  Both x cuts are quadratic, so their exact
    # resultant has degree at most eight in f.
    opposite_q_cut = paired_polynomial(q_variable, -q_variable)
    if opposite_q_cut.degree() != 4:
        raise ValueError("opposite-DE cut is not quartic")
    if any(coefficient(opposite_q_cut, index) != Pair()
           for index in (1, 3)):
        raise ValueError("opposite-DE cut is not even")

    def q_coefficients_for_scaled_f(scale):
        output = []
        for q_degree in range(3):
            output.append(PairPolynomial(
                coefficient(paired_constant, q_degree),
                coefficient(paired_linear, q_degree)*scale,
                coefficient(paired_quadratic, q_degree)*scale**2,
            ))
        return tuple(output)

    scale = common_b if pairing_index == 4 else sigma_c*c_pair
    next_q0, next_q1, next_q2 = q_coefficients_for_scaled_f(scale)
    parity_x0 = next_q0**2
    parity_x1 = 2*next_q0*next_q2-next_q1**2
    parity_x2 = next_q2**2
    opposite_x0 = PairPolynomial(coefficient(opposite_q_cut, 0))
    opposite_x1 = PairPolynomial(coefficient(opposite_q_cut, 2))
    opposite_x2 = PairPolynomial(coefficient(opposite_q_cut, 4))
    parity_missing_f_resultant = (
        (opposite_x2*parity_x0-opposite_x0*parity_x2)**2
        - (opposite_x2*parity_x1-opposite_x1*parity_x2)
        * (opposite_x1*parity_x0-opposite_x0*parity_x1)
    )
    if parity_missing_f_resultant.degree() > 8:
        raise ValueError("parity x resultant exceeds degree eight in f")
    ...
    def monic_remainder(value, divisor):
        divisor_degree = divisor.degree()
        if divisor.coefficients[divisor_degree] != Pair(1):
            raise ValueError("missing-f divisor is not monic")
    ...
    reduced_parity_cut = monic_remainder(
        parity_missing_f_resultant, missing_f_cut
    )
    if reduced_parity_cut.degree() < 0:
        raise ValueError("parity x resultant vanishes modulo missing-f cut")
    print(json.dumps({
        "phase": "opposite_de_parity_missing_f_remainder",
        "parity_missing_f_resultant_degree": parity_missing_f_resultant.degree(),
        "missing_f_degree": missing_f_cut.degree(),
        "reduced_degree": reduced_parity_cut.degree(),
        "seconds": time.perf_counter()-started,
    }), flush=True)
    target_free = bezout_resultant(
        missing_f_cut, reduced_parity_cut
    )
    if target_free == Pair():
        raise ValueError("opposite-DE missing-f Bezout eliminant vanished")
    print(json.dumps({
        "phase": "opposite_de_parity_missing_f_bezout",
        "parity_missing_f_resultant_degree": parity_missing_f_resultant.degree(),
        "missing_f_degree": missing_f_cut.degree(),
        "reduced_degree": reduced_parity_cut.degree(),
    ...
    def paired_q_polynomial_at(right, left_sign=1):
        q_field = polynomial_context([0, left_sign % PRIME])
        p0, p1, p2 = (
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                if source_missing == 0:
                    ...
                    t_row["b_rows"].append(b_row)
                    continue
                missing_f_field = polynomial_context([
                ...
                if f_roots is None:
                    raise ValueError("monic missing-f cut vanished")
                b_row["missing_f_roots"] = f_roots
                b_row["f_rows"] = []
                for f_value in f_roots:
                    if f_value == 0:
                        raise ValueError("nonzero missing product admitted f=0")
                    next_right = (
                        b_value*f_value % PRIME if pairing_index == 4 else
                        sigma_c*c_value*f_value % PRIME
                    )
                    first_q_cut = opposite_q_polynomial_at()
                    second_q_cut = paired_q_polynomial_at(next_right, 1)
                    common_q_cut = first_q_cut.gcd(second_q_cut)
                    q_roots = field_roots(common_q_cut)
                    f_row = {
                        "f": f_value,
                        "first_q_cut_degree": int(first_q_cut.degree()),
                        "second_q_cut_degree": int(second_q_cut.degree()),
                        "common_q_cut_degree": int(common_q_cut.degree()),
                        "q_roots": q_roots,
                        "q_rows": [],
                    }
                    if q_roots is None:
                        f_row["status"] = "FREE_Q"
                        unresolved.append([point, f_value, "FREE_Q"])
                        b_row["f_rows"].append(f_row)
                        continue
                    for q_value in q_roots:
                        ...
                        second_pair_cut = paired_value_at(q_value, next_right)
                        if first_pair_cut or second_pair_cut:
                            raise ValueError(
                                "common q root violates a defining pair"
                            )
                        candidate = {
                            "point": point,
                        ...
                        for selected_sigma_c, selected_sigma_o in lane_signs:
                            if pairing_index == 4:
                                ...
                                final_right = (
                                    selected_sigma_c*c_value*f_value % PRIME
                                )
                            else:
                                final_left = (
                                    selected_sigma_o*e_value*f_value % PRIME
                                )
                                final_right = b_value*f_value % PRIME
                            final_pair_cut = paired_value_at(
                                final_left, final_right,
                            )
                            lane_row = {
                                "sigma": [selected_sigma_c, selected_sigma_o],
                                "final_pair_cut": final_pair_cut,
                            }
                            if final_pair_cut:
                                lane_row["status"] = "THIRD_PAIR_NONZERO"
                                q_row["lane_rows"].append(lane_row)
                                continue
                            representatives = (
                                1, b_value, c_value,
    ...
    return {
        ...
        "opposite_q_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in opposite_q_cut.coefficients
        ],
        "parity_missing_f_resultant_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in parity_missing_f_resultant.coefficients
        ],
        "missing_f_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in missing_f_cut.coefficients
        ],
        "reduced_parity_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in reduced_parity_cut.coefficients
        ],
        "target_free_profiles": [
            fraction_profile(value) for value in target_free.vector()
        ],
        "opposite_q_cut_degree": opposite_q_cut.degree(),
        "parity_missing_f_resultant_degree": parity_missing_f_resultant.degree(),
        "missing_f_cut_degree": missing_f_cut.degree(),
        "reduced_parity_cut_degree": reduced_parity_cut.degree(),
        "bezout_matrix_size": 4,
...
def main(
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-xi3-opposite-de-parity-missing-f-bezout-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-xi3-opposite-de-parity-missing-f-bezout-pilot-v1"
        ),
        "scope": (
            "Exact opposite-DE parity-x resultant, monic missing-f reduction, "
            "division-free Bezout eliminant, and exceptional-root census at xi=3 "
            "for representative pairings 4 and 5; no claim beyond the printed cases."
        ),
        ...
        "rows": [
            {
                ...
                "direct_lift": {
                    ...
                    for key in (
                        ...
                        "target_candidate_count", "final_pair_solution_count",
