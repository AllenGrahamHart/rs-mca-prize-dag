#!/usr/bin/env python3
"""Compute dual exact norms for the complete mu=9, E<=17 residue."""
...
PRIMARY = ROOT / "experiments/prize_resolution/e1_profile_36_mu9_m512_low_energy_exact_result.json"
AUDIT = ROOT / "experiments/prize_resolution/e1_profile_36_mu9_m512_low_energy_exact_audit_result.json"
OUTPUT = ROOT / "experiments/prize_resolution/e1_profile_36_mu9_m512_candidate_norms_result.json"
B_PRIZE = 317494674775468773183020924238786383963
P_MIN = B_PRIZE * 2**128
P_MAX = (B_PRIZE + 1) * 2**128 - 1
COFACTOR = 512

app = modal.App("e1-profile-36-mu9-m512-candidate-norms")
image = modal.Image.debian_slim().apt_install("pari-gp").pip_install("python-flint")


def candidates(packet: dict) -> list[tuple[int, tuple[tuple[int, int], ...]]]:
    found = []
...
def measure(vectors: list[tuple[int, tuple[tuple[int, int], ...]]]) -> list[dict[str, object]]:
    import subprocess
    ...
    rows = []
    for (energy, state), norm in zip(vectors, flint_norms):
...
def main() -> None:
    ...
    packet = {
        "schema": "e1-profile-36-mu9-m512-candidate-norms-v1",
        "complete": False,
        "agreement": False,
        "source_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        "primary_sha256": hashlib.sha256(PRIMARY.read_bytes()).hexdigest(),
        "audit_sha256": hashlib.sha256(AUDIT.read_bytes()).hexdigest(),
        "candidates": len(primary_candidates),
        "rows": [],
    }
    OUTPUT.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n")
    packet["rows"] = measure.remote(primary_candidates)
    ...
    print(
        "E1_PROFILE_36_MU9_M512_CANDIDATE_NORMS_PASS "
        f"candidates={len(primary_candidates)} relations={relations} output={OUTPUT}"
    )
