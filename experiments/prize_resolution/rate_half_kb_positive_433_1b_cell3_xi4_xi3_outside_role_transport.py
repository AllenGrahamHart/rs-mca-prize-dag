#!/usr/bin/env python3
"""Exact outside-role transport from cell-3 xi4 to the proved xi3 slice."""
...
def verify_role_cell_distinction():
    cells = role_cells()
    require(len(cells) == 15, "15 role cells")
    indexed = {canonical_cell(cell): index for index, cell in enumerate(cells)}
    cell3 = canonical_cell(cells[3])
    require(
        cell3 == canonical_cell((1, ((0, 2), (3, 4)))),
        "cell 3 role assignment",
    )
    ...
    require(indexed[permute_cell(cells[3], outside_swap)] == 3,
            "D/E outside transport fixes source role cell 3")
...
def verify_transport():
    ...
    verify_role_cell_distinction()
    return {
        "source_signs": len(SOURCE_SIGNS),
        "target_lanes": len(TARGET_LANES),
        "matchings": len(matching_rows),
        "raw_cases": len(SOURCE_SIGNS) * len(TARGET_LANES) * len(matching_rows),
    }
...
def main():
    ...
    print(
        "RATE_HALF_KB_POSITIVE_433_1B_CELL3_XI4_XI3_TRANSPORT_PASS "
        f"source_signs={census['source_signs']} "
        f"target_lanes={census['target_lanes']} "
        f"matchings={census['matchings']} raw_cases={census['raw_cases']}"
    )
