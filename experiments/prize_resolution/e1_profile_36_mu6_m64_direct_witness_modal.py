#!/usr/bin/env python3
"""Extract the rigorous above-threshold witnesses from affected census shards."""
...
SOURCE_SHARDS = 96
AFFECTED_SHARDS = (10, 16, 22, 38, 77, 80, 87)

app = modal.App("e1-profile-36-mu6-m64-direct-witness")
image = (
...
@app.function(image=image, cpu=2.0, memory=256, timeout=180, max_containers=7)
...
def main(
    output: str = (
        ...
        "e1_profile_36_mu6_m64_direct_witness_result.json"
    ),
) -> None:
    atlas = json.loads(ORBIT_PATH.read_text())
    tasks = [
        (shard, atlas["orbits"][shard::SOURCE_SHARDS])
    ...
    result = {
        "schema": "e1-profile-36-mu6-m64-direct-witness-result-v1",
        "complete": all(
    ...
    print(
        "E1_PROFILE_36_MU6_M64_DIRECT_WITNESS_PASS "
