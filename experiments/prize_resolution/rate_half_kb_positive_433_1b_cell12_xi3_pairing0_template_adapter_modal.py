#!/usr/bin/env python3
"""Apply the pinned cell-4 reciprocal-square compiler to cell 12."""
...
TEMPLATE = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell4_xi3_pairing0_reciprocal_square_modal.py"
)
TOWER = DIRECTORY / (
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell12_xi3_pairing0_template_adapter_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell12-xi3-pairing0-adapter")
image = (
...
@app.function(image=image, cpu=2.0, memory=4096, timeout=300, max_containers=24)
def evaluate(case):
    import time
    import sympy as sp
...
def main(limit: int = 0):
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            rows.append({
                ...
                "pairing_index": 0, "branch_index": case[2],
                "sigma_o": case[3], "status": "REMOTE_ERROR",
                "error": repr(row),
            })
        else:
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell12-xi3-pairing0-adapter-v1",
        "field": PRIME,
        "scope": (
            "Cell-12 execution of the pinned cell-4 reciprocal-square "
            "compiler for xi=3, pairing=0; adapter evidence until audited."
        ),
    ...
    print(json.dumps({
        "result": str(RESULT), "rows": [{key: row.get(key) for key in (
            "epsilon", "branch_index", "sigma_o", "status",
            "target_norm_root_count", "candidate_root_count",
            "route_point_count", "yd_candidate_count",
            "final_pair_solution_count", "witness_count",
            "target_excluded", "unresolved",
        )} for row in rows],
    }, sort_keys=True))
