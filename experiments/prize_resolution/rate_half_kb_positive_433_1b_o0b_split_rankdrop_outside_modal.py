#!/usr/bin/env python3
"""Exact O0b split-lane ideals at the 433-1b rank-drop common points."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_o0b_split_rankdrop_outside_result.json"
)
SHARDS = {
    stratum: DIRECTORY / (
        "rate_half_kb_positive_433_1b_o0b_split_rankdrop_outside_"
        f"{stratum}_result.json"
    )
...
app = modal.App("rs-mca-positive-433-1b-o0b-split-rankdrop-outside")
image = (
...
@app.function(image=image, cpu=1.0, memory=1024, timeout=210, max_containers=96)
def decide_lane(case):
    import sympy as sp

    point_id, stratum, sigma_o = case
    ...
    records = tuple(value for value, _ in signed_records)
    endpoint_squares = (
        (point["b"], e), (point["c"], f),
        (d, e), (d, e), (d, f), (d, f), (e, f),
    )
    squared_sums = tuple(
        left*left + right*right + 2*sign*left*right
        for (left, right), (_, sign) in zip(endpoint_squares, signed_records)
    )
...
def main(points: str = "0", shard_existing: bool = False):
    if shard_existing:
        previous = json.loads(RESULT.read_text())
        lanes = previous["lanes"]
        selected = tuple(sorted({row["point_id"] for row in lanes}))
    else:
        selected = tuple(int(value) for value in points.split(",") if value)
        cases = tuple(itertools.product(
            selected, ("S0", "SDE", "SDF"), (-1, 1)
        ))
        raw = list(decide_lane.map(
        ...
        for case, row in zip(cases, raw):
            if isinstance(row, BaseException):
                lanes.append({
                    "point_id": case[0], "stratum": case[1],
                    "sigma_o": case[2], "status": "REMOTE_ERROR",
                    "error": repr(row),
                })
            else:
                ...
                row["unit_count"] = sum(
                    item.get("unit", False) for item in row["rows"]
                )
                ...
                row["rational_candidate_cases"] = sum(
                    item.get("linear_factor_count", 0) > 0
                    for item in row["rows"]
                )
                lanes.append(row)

    shard_manifest = {}
    for stratum, path in SHARDS.items():
        shard_lanes = [row for row in lanes if row["stratum"] == stratum]
        shard = {
            "schema": (
                "rate-half-kb-positive-433-1b-o0b-split-rankdrop-"
                "outside-shard-v1"
            ),
            "stratum": stratum,
            "source_points_sha256": hashlib.sha256(POINTS.read_bytes()).hexdigest(),
            "lane_count": len(shard_lanes),
            "case_count": sum(len(row.get("rows", [])) for row in shard_lanes),
            "lanes": shard_lanes,
        }
        path.write_text(json.dumps(shard, indent=2, sort_keys=True) + "\n")
        shard_manifest[stratum] = {
            "file": path.name,
            "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
            "lane_count": shard["lane_count"],
            "case_count": shard["case_count"],
        }
    output = {
        "schema": "rate-half-kb-positive-433-1b-o0b-split-rankdrop-outside-v1",
        "app": "rs-mca-positive-433-1b-o0b-split-rankdrop-outside",
        "scope": (
            "Exact target-guarded O0b split-lane missing-product, three "
            "paired-product, and missing-mate squared-sum ideals at every "
            "deployed split-common product-rank-drop point."
        ),
        "source_points_sha256": hashlib.sha256(POINTS.read_bytes()).hexdigest(),
        ...
        "lane_summaries": [
            {
                "point_id": row["point_id"], "cell": row.get("cell"),
                "epsilon": row.get("epsilon"), "point_index": row.get("point_index"),
                "stratum": row["stratum"], "sigma_o": row["sigma_o"],
                "status": row["status"], "unit_count": row.get("unit_count"),
                "survivor_count": row.get("survivor_count"),
                "rational_candidate_cases": row.get("rational_candidate_cases"),
            }
            for row in lanes
        ],
    }
