#!/usr/bin/env python3
"""Check the dual complete E17 census and every retained vector."""
...
PROFILES = ([5, 3, 0, 0], [1, 4, 0, 0], [4, 1, 1, 0], [0, 2, 1, 0], [1, 0, 0, 1])
...
def main() -> None:
    ...
    assert packet["schema"] == "e1-e17-five-profile-census-v1"
    ...
    profile = [sum(row["primary"]["profile_counts"][i] for row in rows) for i in range(5)]
    full = [
    ...
    assert summary["proper_conductor_counts"] == [profile[i] - full[i] for i in range(5)]
    ...
    print(
        "E17_FIVE_PROFILE_CENSUS_CHECK_PASS "
