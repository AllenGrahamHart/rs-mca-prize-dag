#!/usr/bin/env python3
"""Solve the finite outside fibers left by the cell-3 xi0/pairing0 cut."""
...
REPLAY = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_six_basis_cut_root_replay_census_result.json"
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi0_pairing0_outside_solver_result.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_REPLAY = "/root/replay.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-xi0-pairing0-outside-solver")
image = (
...
def solve_sign(signs):
    ...
    for point in replay_row["target_free_zero_points"]:
        ...
        de_value = b_missing*pow(a_missing, -1, PRIME) % PRIME
...
def main(signs: str = "-1:-1", all_signs: bool = False):
    ...
    output = {
        "schema": (
            ...
            if all_signs else
            "rate-half-kb-positive-433-1b-cell3-xi0-pairing0-outside-solver-v1"
        ),
        "scope": (
            "All four target sign lanes above the exact xi0/pairing0 common "
