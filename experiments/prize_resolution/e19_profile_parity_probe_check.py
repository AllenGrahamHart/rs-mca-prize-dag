#!/usr/bin/env python3
"""Independent checker for the cutoff-free E19 profile/parity router."""
...
ROOT = HERE.parents[1]
SOURCE = HERE / "e19_profile_parity_probe_modal.py"
...
def recursive_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    def visit(magnitude: int, energy: int, l1_norm: int, counts: list[int]) -> None:
        if magnitude == 5:
            ...
            return
        for count in range((19 - energy) // (magnitude * magnitude) + 1):
...
def independent_diameter_ledgers() -> list[list[int]]:
    ...
    return [[mass, (mass - 83) // 2] for mass in sorted(masses)]


def main() -> None:
    packet = json.loads(RESULT.read_text())
    atlas = json.loads(ATLAS.read_text())
    assert packet["schema"] == "e1-e19-profile-parity-route-probe-v1"
    assert packet["complete"] is True and packet["variance"] == 38 and packet["energy"] == 19
    ...
    trace = [[l1, 85 - 4 * l1, minima[85 - 4 * l1]] for l1 in range(21, 8, -1)]
    assert packet["slack_trace"] == trace
    qualifying = [row for row in trace if row[2] is not None and int(row[2]) <= 19]
    ...
    print(
        "E19_PROFILE_PARITY_PROBE_CHECK_PASS "
