#!/usr/bin/env python3
"""Scout the pinned xi=3 pairing-2 compiler on cell-9 charts."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing2_chart_scout_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-xi3-pairing2-chart-scout")
image = (
...
def _evaluate_case(case):
    ...
    epsilon_1, epsilon_2, branch_index, sigma_c_anchor, b_index, c_index = case
    ...
    result = namespace["evaluate_case"](
        (epsilon_1, epsilon_2, branch_index, sigma_c_anchor, 2)
    )
...
def main(
    ...
    for envelope in evaluate.map(cases, order_outputs=False):
        ...
        if "error" in envelope:
            error = {
                ...
                "sigma_c_anchor": case[3], "xi_index": 3,
                "pairing_index": 2, "b_row_index": case[4],
        ...
        rows.append({
            ...
            "xi_index": row["xi_index"], "pairing_index": 2,
    ...
    rows.sort(key=lambda row: (
        tuple(row["epsilon"]), row["branch_index"], row["sigma_c_anchor"],
        row["b_row_index"], row["c_row_index"],
    ))
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell9-xi3-pairing2-chart-scout-v1",
        "field": PRIME,
