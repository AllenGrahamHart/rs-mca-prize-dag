#!/usr/bin/env python3
"""Check the dual exact E13 cyclotomic norm ledger."""
...
SOURCE = HERE / "e13_four_profile_norm_modal.py"
...
def main() -> None:
    ...
    assert packet["schema"] == "e1-e13-four-profile-norm-v1"
    ...
    assert summary["odd_part_maximizing_indices"] == [i for i, value in enumerate(odd_parts)
                                                      if value == max(odd_parts)]
    mutated = list(pari); mutated[0] += 1; assert mutated != flint
    print("E13_FOUR_PROFILE_NORM_LEDGER_CHECK_PASS "
          f"vectors={len(vectors)} distinct={len(set(flint))} max={maximum} "
          f"bits={maximum.bit_length()} hits={sum(value >= 2**250 for value in flint)} "
          f"odd_max={max(odd_parts)} odd_bits={max(odd_parts).bit_length()} "
          f"odd_hits={sum(value >= 2**250 for value in odd_parts)} engines=2 mutations=1")
