#!/usr/bin/env python3
"""Bounded paired Modal dispatcher for the K'=86 raw-threshold wave."""
...
PRIMARY = DIRECTORY / "rate_half_mca_rank11_k86_raw_threshold_offset_probe.py"
AUDIT = DIRECTORY / "rate_half_mca_rank11_k86_raw_threshold_offset_audit.py"
PRIMARY_BASE = DIRECTORY / "rate_half_mca_rank11_k85_raw_threshold_offset_probe.py"
AUDIT_BASE = DIRECTORY / "rate_half_mca_rank11_k85_raw_threshold_offset_audit.py"
CODE = Path("/tmp/k83-threshold-frontier-adjacent-code.tar.gz")
DEPS = Path("/tmp/k72-deps.tar.gz")
for path in (PRIMARY, AUDIT, PRIMARY_BASE, AUDIT_BASE, CODE, DEPS):
    if not path.is_file():
        raise FileNotFoundError(path)


app = modal.App("rate-half-mca-rank11-k86-raw-threshold-wave")
image = (
...
def main(smoke: bool = False) -> None:
    offsets = (1, 75) if smoke else range(1, 76)
    jobs = [
