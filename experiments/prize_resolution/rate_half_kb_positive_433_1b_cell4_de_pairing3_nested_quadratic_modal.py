#!/usr/bin/env python3
"""Solve cell-4 parallel-DE omissions at canonical pairing 3."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell4_de_pairing3_nested_quadratic_result.json"
)
REMOTE_STRUCTURE = "/root/structure.json"
REMOTE_KERNEL = "/root/kernel.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell4-de-pairing3-nested-quadratic")
image = (
...
def evaluate_case(case):
    ...
    if xi_index not in (0, 2):
        raise ValueError("scope is xi in {0,2}, pairing=3")
    ...
    print(json.dumps({"phase": "nested_quartic",
                      "seconds": time.perf_counter() - started}), flush=True)
    remainder = polynomial_pseudo_remainder(nested_quartic, p_v)
    ...
    if remainder_degree == 1:
        ...
        target_free = p_v_a * remainder.coefficients[0]
    else:
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                u_roots = univariate_roots(paired_scalar_coefficients(
                    a_values, b_values, de_value
                ))
                v_roots = univariate_roots(paired_scalar_coefficients(
                    a_values, b_values, second_de_value, sigma_o
                ))
                point_row["u_roots"] = u_roots
                point_row["v_roots"] = v_roots
                ...
                for u_value in u_roots:
                    for v_value in v_roots:
                        ...
                        if h_value:
                            ...
                            point_row["uv_rows"].append(uv_row)
                            continue
                        candidate = {**point, "u": u_value, "v": v_value}
                        uv_candidates.append(candidate)
                        f_squared = (
                            u_value*v_value*pow(de_value, -1, PRIME)
                        ) % PRIME
                        f_roots = univariate_roots([
                            -f_squared % PRIME, 0, 1
                        ])
                        ...
                        for f_value in f_roots:
                            if f_value == 0:
                                ...
                                target_boundary_rows.append({**candidate, **f_row})
...
def main(cases: str = "-1:-1:-1:-1:0", all_cases: bool = False):
    ...
    for case, row in zip(selected, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell4-de-pairing3-nested-quadratic-v1",
        "field": PRIME,
        "scope": (
            "Exact nested-quadratic norm and direct finite replay for cell 4, "
            "xi in {0,2}, pairing 3; xi1 transports from xi0."
        ),
