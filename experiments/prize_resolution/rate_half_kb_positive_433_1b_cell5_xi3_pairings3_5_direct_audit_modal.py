#!/usr/bin/env python3
"""Independently replay the cell-5 xi=3 pairings-3/4/5 packets."""
...
PRIMARIES = {
    str(pairing): DIRECTORY / (
        f"rate_half_kb_positive_433_1b_cell5_xi3_pairing{pairing}_"
        "template_adapter_result.json"
    )
    for pairing in (3, 4, 5)
}
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell5_xi3_pairings3_5_"
...
app = modal.App("rs-mca-positive-433-1b-cell5-xi3-pairings3-5-direct-audit")
...
def audit():
    ...
    for key, payload in payloads.items():
        core.require(
            payload["schema"]
            == f"rate-half-kb-positive-433-1b-cell5-xi3-pairing{key}-adapter-v1"
    ...
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell5-xi3-pairings3-5-"
            "direct-audit-v1"
        ),
        "field": PRIME,
