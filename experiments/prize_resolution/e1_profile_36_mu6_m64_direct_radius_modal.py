#!/usr/bin/env python3
"""Run the complete multiplicity-six direct radius census on Modal."""
...
ORBIT_PATH = ROOT / "experiments/prize_resolution/e1_profile_36_mu6_m64_chord_orbits.json"
...
app = modal.App("e1-profile-36-mu6-m64-direct-radius")
image = (
...
@app.function(image=image, cpu=2.0, memory=256, timeout=240, max_containers=96)
...
def main(
    ...
    output: str = (
        ...
        "e1_profile_36_mu6_m64_direct_radius_result.json"
    ),
) -> None:
    atlas = json.loads(ORBIT_PATH.read_text())
    orbits = atlas["orbits"]
    batches = [orbits[index::shards] for index in range(shards)]
    rows = list(classify_batch.map(batches, return_exceptions=True))
    normalized = []
    ...
    for index, row in enumerate(rows):
        ...
        summary = str(normalized[-1]["stdout"]).strip() or "NO_STDOUT"
        parsed = dict(re.findall(r"([a-z_]+)=([0-9]+)(?=\s|$)", summary))
        if summary.startswith("PASS ") and all(key in parsed for key in COUNT_KEYS):
    ...
    result = {
        "schema": "e1-profile-36-mu6-m64-direct-radius-result-v1",
        "complete": all(
    ...
    assert len(worker_seconds) == shards
    assert sum(int(row["orbits"]) for row in normalized) == atlas["affine_orbits"]
    assert counts["orbits"] == atlas["affine_orbits"]
    for key, expected in EXPECTED.items():
        assert counts[key] == expected, (key, counts[key], expected)
    print(
        "E1_PROFILE_36_MU6_M64_DIRECT_RADIUS_PASS "
        f"orbits={atlas['affine_orbits']} shards={shards} output={output}"
    )
