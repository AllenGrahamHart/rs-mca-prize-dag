#!/usr/bin/env python3
"""Externally reconstruct cell-12 pairing-11/14 roots by Frobenius/gcd."""
...
PRIMARY_11 = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell12_de_pairing11_"
    "template_adapter_result.json"
)
PRIMARY_14 = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell12_positive_de_pairing14_"
    "template_adapter_result.json"
)
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell12_de_pairing11_14_"
    "frobenius_roots_result.json"
)
REMOTE_PRIMARY_11 = "/root/primary11.json"
REMOTE_PRIMARY_14 = "/root/primary14.json"
PRIME = 2130706433

app = modal.App(
    "rs-mca-positive-433-1b-cell12-de-pairing11-14-frobenius-roots"
)
image = (
    modal.Image.debian_slim(python_version="3.12")
    .pip_install("python-flint==0.8.0")
    .add_local_file(PRIMARY_11, REMOTE_PRIMARY_11)
    .add_local_file(PRIMARY_14, REMOTE_PRIMARY_14)
)
...
def main():
    payloads = [json.loads(path.read_text()) for path in (PRIMARY_11, PRIMARY_14)]
    profiles = {}
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell12-de-pairing11-14-"
            "frobenius-roots-v1"
        ),
        "field": PRIME,
        "method": "external FLINT gcd(P,x^p-x), factor squarefree root part",
        "source_primary_sha256": {
            "pairing11": hashlib.sha256(PRIMARY_11.read_bytes()).hexdigest(),
            "pairing14": hashlib.sha256(PRIMARY_14.read_bytes()).hexdigest(),
        },
