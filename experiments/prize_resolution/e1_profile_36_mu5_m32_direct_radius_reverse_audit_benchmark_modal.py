#!/usr/bin/env python3
"""Benchmark the complete reverse-direct cofactor-32 audit on Modal."""
...
OUTPUT = Path(
    ...
    "e1_profile_36_mu5_m32_direct_radius_reverse_audit_benchmark_result.json"
)

app = modal.App("e1-profile-36-mu5-m32-radius-reverse-audit-benchmark")
image = (
...
def benchmark(item: tuple[int, list[int]]) -> dict[str, object]:
    ...
    return {
        "odd_weight": q, "orbit": orbit, "returncode": result.returncode,
...
def main() -> None:
    ...
    for task, returned in zip(tasks, rows, strict=True):
        ...
        for key in (
            ...
            assert audit_counts[key] == primary_counts[key], (
                task[0], key, audit_counts[key], primary_counts[key]
            )
    result = {
        "schema": "e1-profile-36-mu5-m32-direct-radius-reverse-audit-benchmark-v1",
    ...
    print(
        "E1_PROFILE_36_MU5_M32_RADIUS_REVERSE_AUDIT_BENCHMARK_PASS "
