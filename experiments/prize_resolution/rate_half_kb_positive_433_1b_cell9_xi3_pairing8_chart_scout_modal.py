#!/usr/bin/env python3
"""Scout the pinned xi=3 pairing-8 compiler on cell-9 charts."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing8_chart_scout_result.json"
)
FULL_RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing8_chart_result"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-xi3-pairing8-chart-scout")
image = (
...
def _evaluate_case(case):
    ...
    epsilon_1, epsilon_2, sigma_c, b_index, c_index = case
    ...
    result = namespace["evaluate_case"]((epsilon_1, epsilon_2, sigma_c))
...
def main(
    ...
    if full:
        ...
        writer = ShardedResultWriter(full_result, metadata={
            "field": PRIME,
            "scope": (
                "Exact six-chart execution of the pinned quadratic-resultant "
                "sign-free "
                "compiler for cell-9 xi=3, pairing=8."
            ),
    ...
    for envelope in evaluate.map(cases, order_outputs=False):
        ...
        if "error" in envelope:
            error = {
                "epsilon": list(case[:2]), "sigma_c": case[2],
                "xi_index": 3, "pairing_index": 8,
                "b_row_index": case[3], "c_row_index": case[4],
                "status": "REMOTE_ERROR",
        ...
        rows.append({
            "epsilon": row["epsilon"], "sigma_c": row["sigma_c"],
            "xi_index": row["xi_index"], "pairing_index": 8,
            ...
            "z_candidate_count": row["z_candidate_count"],
            "q_candidate_count": row["q_candidate_count"],
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell9-xi3-pairing8-chart-scout-v1",
        "field": PRIME,
    ...
    if writer is not None:
        expected_rows = (
            ...
            else 1 if recovery_index >= 0 else 48
        )
