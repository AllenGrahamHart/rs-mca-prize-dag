#!/usr/bin/env python3
"""Exact outside-edge census for the negative one-loop 433 skeleton."""
...
def verify():
    ...
    if [representative for representative, _ in orbits] != expected:
        raise RuntimeError(f"orbit representatives {orbits}")
    if sorted(size for _, size in orbits) != [3, 3, 6]:
        raise RuntimeError("orbit sizes")
...
def main():
    _, orbits = verify()
    print(
        "RATE_HALF_KB_ONE_LOOP_433_OUTSIDE_SKELETON_PASS "
        f"labeled=12 orbits={len(orbits)} sizes="
