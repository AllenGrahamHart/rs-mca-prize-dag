#!/usr/bin/env python3
"""Apply the pinned pairing-4 compiler to cell 12."""
...
TEMPLATE = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell4_xi3_pairing4_nested_signfree_modal.py"
)
TOWER = DIRECTORY / (
...
app = modal.App("rs-mca-positive-433-1b-cell12-xi3-pairing4-adapter")
image = (
...
@app.function(image=image, cpu=2.0, memory=4096, timeout=900, max_containers=4)
def evaluate(case):
    import time
...
def main(limit: int = 0):
    ...
    output = {
        ...
        "field": PRIME,
        "scope": (
            "Cell-12 execution of the pinned nested sign-free compiler for "
            "xi=3, pairing 4; adapter evidence until audited."
        ),
    ...
    print(json.dumps({
        ...
        "rows": [
            {key: row.get(key) for key in (
                "epsilon", "pairing_index", "status",
                "candidate_root_count", "source_point_count",
                "route_point_count", "d_candidate_count",
