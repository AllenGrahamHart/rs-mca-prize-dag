...
namespace {
constexpr std::array<std::array<int, 4>, 2> kProfiles{{
    {{3, 3, 0, 0}}, {{2, 1, 1, 0}},
}};
struct Match {
  int profile = -1;
...
int main(int argc, char** argv) {
  ...
  std::array<std::uint64_t, 2> profile_counts{}, full_conductor_counts{};
  ...
  for (int first = 0; first < static_cast<int>(allowed.size()) - 2; ++first) {
    for (int second = first + 1; second < static_cast<int>(allowed.size()) - 1;
      ...
      for (int third = second + 1; third < static_cast<int>(allowed.size());
        ...
        std::array<int, 21> chord_class{}, chord_orientation{}, chord_left{},
            chord_right{};
        int chord = 0;
        ...
        for (int mask = 0; mask < 64; ++mask) {
          ...
          const std::array<int, 7> coefficients{{
              2, (mask & 1) ? -2 : 2, (mask & 2) ? -2 : 2,
              (mask & 4) ? -1 : 1, (mask & 8) ? -1 : 1,
              (mask & 16) ? -1 : 1, (mask & 32) ? -1 : 1,
          }};
          std::array<int, 64> half{};
          for (int i = 0; i < 21; ++i) {
            if (chord_class[i] != 64) {
              half[chord_class[i]] += chord_orientation[i] *
                                      coefficients[chord_left[i]] *
                                      coefficients[chord_right[i]];
            }
          }
          std::array<int, 4> profile{};
          bool above_four = false;
          for (int d = 1; d < 64; ++d) {
            const int magnitude = std::abs(half[d]);
            if (magnitude > 4)
              above_four = true;
            else if (magnitude)
              ++profile[magnitude - 1];
