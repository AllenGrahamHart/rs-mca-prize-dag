#!/usr/bin/env python3
"""Shared direct replay for cell-5 common-f resultant certificates."""
...
def load_root_cache(root_result, primary_paths):
    ...
    require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell11-de-pairing11-frobenius-roots-v1"
        and roots["field"] == P
        and roots["method"]
        == "external FLINT gcd(P,x^p-x), factor squarefree root part"
        and roots["source_primary_sha256"]
        == {name: digest(path) for name, path in primary_paths.items()},
    ...
    for row in payload["rows"]:
        ...
        for item in row["boundary_rows"]:
            if item["stage"] == "R_GUARD":
                ...
                require(value(leading[epsilon][item["stage"]], item) == 0,
