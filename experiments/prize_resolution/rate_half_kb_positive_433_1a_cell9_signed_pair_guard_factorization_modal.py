#!/usr/bin/env python3
"""Certify the guarded factorization of the cell-9 signed-pair resultant."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell9_signed_pair_guard_factorization_result.json"
REMOTE_PLANE = "/root/plane.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1a-cell9-signed-pair-guard-factorization")
image = (
...
def certify():
    ...
    candidate, steps = reduce_by_plane(
        candidate * (w0 + 1) * (w0 - t * t) * (w0 - t * t) * r_minus
    )
    reductions.append({
        "operation": "times_source_label_guards", "steps": steps,
        "shape": shape(candidate),
    })
    ...
    return {
        ...
        "guard_identity": (
            "plane-reduced(N0*D0^5*(w0+1)*(w0-t^2)^2*"
...
def main():
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell9-signed-pair-guard-factorization-v1",
        "scope": (
            "Exact compact-plane quotient factorization of the complete "
            "necessary cell-9 signed-pair resultant and replay of every "
