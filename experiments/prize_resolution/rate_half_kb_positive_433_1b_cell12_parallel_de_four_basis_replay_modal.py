#!/usr/bin/env python3
"""Directly replay every candidate from the cell-12 parallel-DE norms."""
...
app = modal.App("rs-mca-positive-433-1b-cell12-parallel-de-replay")
image = (
...
def main():
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell12-parallel-de-replay-v1",
        "field": PRIME,
        "scope": (
            "Complete deployed direct replay of every four-basis norm and "
            "inverse candidate for the cell-12 first-pair parallel-DE cuts."
        ),
