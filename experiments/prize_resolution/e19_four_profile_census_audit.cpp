...
constexpr std::array<std::array<int,4>,4> kProfiles{{
    {{3,4,0,0}},{{2,2,1,0}},{{1,0,2,0}},{{3,0,0,1}},
}};
...
int main(int argc,char** argv) {
  ...
  std::vector<int> allowed; for (int position=0;position<128;++position) if (!occupied[position]) allowed.push_back(position);
    ...
    for (int second=first+1;second<static_cast<int>(allowed.size())-1;++second) {
      for (int third=second+1;third<static_cast<int>(allowed.size());++third) {
        ...
        for (int mask=0;mask<64;++mask) {
          ...
          for (int left=0;left<7;++left) for (int right=0;right<7;++right) {
          ...
          for (int d=1;d<64;++d) { const int magnitude=std::abs(product[d]); if (magnitude>4) above_four=true; else if (magnitude) ++profile[magnitude-1]; }
