#!/usr/bin/env python3
"""Apply the pinned reciprocal-linear compiler to cell-12 pairings 1-2."""
...
TEMPLATE = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell4_xi3_pairings1_2_reciprocal_linear_modal.py"
)
TOWER = DIRECTORY / (
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell12_xi3_pairings1_2_template_adapter_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell12-xi3-pairings1-2-adapter")
image = (
...
@app.function(image=image, cpu=2.0, memory=4096, timeout=300, max_containers=36)
def evaluate(case):
    import time
    import sympy as sp
    ...
    structure_path = Path("/tmp/cell12_structure.json")
    ...
    namespace = {
        "hashlib": hashlib, "json": json, "Path": Path, "time": time,
        "PRIME": PRIME, "REMOTE_STRUCTURE": str(structure_path),
...
def main(limit: int = 0):
    ...
    pairing_two = tuple(
        (*epsilon, branch_index, sigma_c, 2)
        for epsilon in signs for branch_index in range(3)
        for sigma_c in (-1, 1)
    )
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            rows.append({
                "epsilon": list(case[:2]), "xi_index": 3,
                "pairing_index": case[4], "branch_index": case[2],
                "sigma_c_anchor": case[3], "status": "REMOTE_ERROR",
                "error": repr(row),
            })
        else:
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell12-xi3-pairings1-2-adapter-v1",
        "field": PRIME,
        "scope": (
            "Cell-12 execution of the pinned reciprocal-linear compiler for "
            "xi=3, pairings 1-2; adapter evidence until audited."
        ),
    ...
    print(json.dumps({
        "result": str(RESULT), "rows": [{key: row.get(key) for key in (
            "epsilon", "pairing_index", "branch_index", "sigma_c_anchor",
            "status", "target_norm_root_count", "candidate_root_count",
            "route_point_count", "z_candidate_count",
            "final_pair_solution_count", "witness_count", "target_excluded",
            "unresolved",
        )} for row in rows],
    }, sort_keys=True))
