#!/usr/bin/env python3
"""Pilot the xi3 fully mixed pairing-11 orbit via a quartic-cubic cut."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_fully_mixed_pairing11_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_fully_mixed_pairing11_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-xi3-fully-mixed-pairing11-pilot")
image = (
...
def profile_case(case):
    ...
    if (pairing_index != 11 or xi_index != 3 or
            sigma_c not in (-1, 1) or sigma_o not in (-1, 1)):
        raise ValueError(
            "the pilot is scoped to xi=3 fully mixed representative 11"
        )
    ...
    def polynomial_remainder(dividend, divisor):
        ...
        divisor_leading_inverse = Pair(1)/divisor_leading
        while True:
    ...
    def bezout_resultant(left, right, degree=4):
        """Exact division-free resultant determinant in the coefficient ring."""
        def coefficient(polynomial, index):
            return (
                polynomial.coefficients[index]
                if index < len(polynomial.coefficients) else Pair()
            )

        numerator = [[Pair() for _ in range(degree+1)]
                     for _ in range(degree+1)]
        for i in range(degree+1):
            for j in range(i+1, degree+1):
                value = (
                    coefficient(left, i)*coefficient(right, j)
                    - coefficient(left, j)*coefficient(right, i)
                )
                numerator[i][j] = value
                numerator[j][i] = -value
        print(json.dumps({
            "phase": "bezout_numerator",
            "seconds": time.perf_counter()-started,
        }), flush=True)
        matrix = [[
            -sum((
                numerator[row-offset][column+1+offset]
                for offset in range(row+1)
                if column+1+offset <= degree
            ), Pair())
            for column in range(degree)
        ] for row in range(degree)]
        for i in range(degree+1):
            for j in range(degree+1):
                left_q = matrix[i-1][j] if i and j < degree else Pair()
                right_q = matrix[i][j-1] if j and i < degree else Pair()
                if left_q-right_q != numerator[i][j]:
        ...
        if degree != 4:
            raise ValueError("the shared-minor determinant is specialized to 4x4")
        bottom_minors = {}
        for left_column in range(4):
            for right_column in range(left_column+1, 4):
                bottom_minors[left_column, right_column] = (
                    matrix[2][left_column]*matrix[3][right_column]
                    - matrix[2][right_column]*matrix[3][left_column]
                )
        print(json.dumps({
            "phase": "bezout_bottom_minors",
            "seconds": time.perf_counter()-started,
        }), flush=True)

        def bottom_minor(left_column, right_column):
            if left_column < right_column:
                return bottom_minors[left_column, right_column]
            return -bottom_minors[right_column, left_column]

        def trailing_minor(columns):
            first, second, third = columns
            return (
                matrix[1][first]*bottom_minor(second, third)
                - matrix[1][second]*bottom_minor(first, third)
                + matrix[1][third]*bottom_minor(first, second)
            )

        output = Pair()
        for column in range(4):
            columns = tuple(index for index in range(4) if index != column)
            term = matrix[0][column]*trailing_minor(columns)
            output += -term if column % 2 else term
            print(json.dumps({
                "phase": "bezout_cofactor",
                "column": column,
                "seconds": time.perf_counter()-started,
            }), flush=True)
        return output
    ...
    half = Pair(1)/Pair(2)
    paired_constant = paired_zero
    paired_linear = (paired_plus-paired_minus)*half
    paired_quadratic = (
        paired_plus+paired_minus-2*paired_zero
    )*half

    # Pairing 11 is (de,bf),(de,sigma_c*cf),(-de,sigma_o*ef).
    # With q=de, m=df, z=1/d, put U=b*m and V=sigma_c*c*m.
    # Subtracting F(q,Uz)=F(q,Vz)=0 gives
    #   ((U+V) A(q)) z + B(q) = 0.
    # Compatibility of the two equations is the quartic G below.  Clearing
    # the missing-record equation after z=-B/D gives the degree-eight K.
    source_u = common_b*missing_record
    source_v = sigma_c*c_pair*missing_record
    source_uv_sum = source_u+source_v
    source_uv_product = source_u*source_v
    linear_z_denominator = paired_quadratic*source_uv_sum
    compatibility_cut = (
    ...
    if compatibility_cut.degree() != 4:
        raise ValueError("fully mixed compatibility cut is not quartic")
    compatibility_leading_inverse = Pair(1)/compatibility_cut.coefficients[4]

    def cached_remainder(value, divisor):
        divisor_degree = divisor.degree()
        if divisor is not compatibility_cut:
            return polynomial_remainder(value, divisor)
        ...
        while True:
            ...
            if degree < divisor_degree:
                return PairPolynomial(*work[:degree+1])
            shift = degree-divisor_degree
            factor = work[degree]*compatibility_leading_inverse
            for index in range(divisor_degree+1):
                work[shift+index] = (
                    work[shift+index]-factor*divisor.coefficients[index]
                )

    def compatibility_multiply(*values):
        output = PairPolynomial(1)
        for value in values:
            output = cached_remainder(output*value, compatibility_cut)
        return output

    denominator_square = compatibility_multiply(
        linear_z_denominator, linear_z_denominator
    )
    denominator_fourth = compatibility_multiply(
        denominator_square, denominator_square
    )
    linear_square = compatibility_multiply(paired_linear, paired_linear)
    linear_fourth = compatibility_multiply(linear_square, linear_square)
    reduced_missing_cut = cached_remainder(
        denominator_fourth
        + compatibility_multiply(linear_square, denominator_square)
        * missing_second
        + linear_fourth*missing_fourth,
        compatibility_cut,
    )
    if reduced_missing_cut.degree() < 0:
        raise ValueError("missing substitution vanishes modulo compatibility")
    print(json.dumps({
        "phase": "fully_mixed_remainder",
        "compatibility_degree": compatibility_cut.degree(),
        "missing_substitution_degree_bound": 8,
        "reduced_missing_degree": reduced_missing_cut.degree(),
        "seconds": time.perf_counter()-started,
    }), flush=True)
    target_free = bezout_resultant(
        compatibility_cut, reduced_missing_cut
    )
    if target_free == Pair():
        raise ValueError("fully mixed quartic-cubic resultant vanished")
    print(json.dumps({
        "phase": "fully_mixed_bezout_resultant",
        "compatibility_degree": compatibility_cut.degree(),
        "missing_substitution_degree_bound": 8,
        "reduced_missing_degree": reduced_missing_cut.degree(),
        "bezout_matrix_size": 4,
        "seconds": time.perf_counter()-started,
    }), flush=True)
    ...
    def paired_q_polynomial_at(right):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                if source_missing == 0:
                    ...
                    t_row["b_rows"].append(b_row)
                    continue
                paired_zero_field = paired_q_polynomial_at(0)
                paired_plus_field = paired_q_polynomial_at(1)
                paired_minus_field = paired_q_polynomial_at(-1 % PRIME)
                half_value = pow(2, -1, PRIME)
                paired_constant_field = paired_zero_field
                paired_linear_field = (
                    paired_plus_field-paired_minus_field
                )*half_value
                paired_quadratic_field = (
                    paired_plus_field+paired_minus_field
                    - 2*paired_zero_field
                )*half_value
                source_u_value = b_value*source_missing % PRIME
                source_v_value = (
                    sigma_c*c_value*source_missing % PRIME
                )
                source_uv_sum_value = (
                    source_u_value+source_v_value
                ) % PRIME
                source_uv_product_value = (
                    source_u_value*source_v_value
                ) % PRIME
                compatibility_field = (
                    paired_constant_field*paired_quadratic_field
                    * pow(source_uv_sum_value, 2, PRIME)
                    - paired_linear_field**2*source_uv_product_value
                )
                q_roots = field_roots(compatibility_field)
                b_row["compatibility_q_roots"] = q_roots
                b_row["q_rows"] = []
                if q_roots is None:
                    b_row["status"] = "FREE_COMPATIBILITY_Q"
                    unresolved.append([point, "FREE_COMPATIBILITY_Q"])
                    t_row["b_rows"].append(b_row)
                    continue
                ...
                if missing_roots is None:
                    raise ValueError("monic missing-record cut vanished")
                for q_value in q_roots:
                    a_q = evaluate_polynomial(
                        paired_quadratic_field, q_value
                    )
                    b_q = evaluate_polynomial(paired_linear_field, q_value)
                    c_q = evaluate_polynomial(
                        paired_constant_field, q_value
                    )
                    linear_denominator = (
                        a_q*source_uv_sum_value
                    ) % PRIME
                    if linear_denominator:
                        possible_z = [
                            -b_q*pow(linear_denominator, -1, PRIME) % PRIME
                        ]
                        reconstruction_mode = "LINEAR_Z"
                    elif b_q:
                        possible_z = []
                        reconstruction_mode = "INCONSISTENT_LINEAR_Z"
                    else:
                        possible_z = missing_roots
                        reconstruction_mode = "DEGENERATE_A_B"
                    q_row = {
                        "q": q_value,
                        "a_q": a_q,
                        "b_q": b_q,
                        "c_q": c_q,
                        "linear_denominator": linear_denominator,
                        "reconstruction_mode": reconstruction_mode,
                        "possible_z": possible_z,
                        "root_rows": [],
                    }
                    for z_value in possible_z:
                        if z_value == 0:
                            continue
                        ...
                        first_pair_cut = paired_value_at(
                            q_value, b_value*f_value % PRIME
                        )
                        second_pair_cut = paired_value_at(
                            q_value, sigma_c*c_value*f_value % PRIME
                        )
                        if first_pair_cut or second_pair_cut:
                            if reconstruction_mode == "LINEAR_Z":
                                raise ValueError(
                                    "compatibility root violates a defining pair"
                                )
                            continue
                        candidate = {
                            "point": point,
                        ...
                        root_row = {
                            ...
                            "e": e_value,
                            "f": f_value,
                            "first_pair_cut": first_pair_cut,
                            "second_pair_cut": second_pair_cut,
                        }
                        final_pair_cut = paired_value_at(
                            -q_value % PRIME,
                            sigma_o*e_value*f_value % PRIME,
                        )
                        root_row["final_pair_cut"] = final_pair_cut
                        if final_pair_cut:
                            root_row["status"] = "THIRD_PAIR_NONZERO"
                        ...
                        record = {**candidate, **root_row}
                        final_pair_solutions.append(record)
                        (
                            boundary_solutions if failed_guards else witnesses
                        ).append(record)
                        q_row["root_rows"].append(root_row)
    ...
    return {
        ...
        "pairing_index": pairing_index,
        "status": "COMPLETE",
        "target_lanes_covered": [[sigma_c, sigma_o]],
        ...
        "compatibility_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in compatibility_cut.coefficients
        ],
        "reduced_missing_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in reduced_missing_cut.coefficients
        ],
        "target_free_profiles": [
            fraction_profile(value) for value in target_free.vector()
        ],
        "compatibility_cut_degree": compatibility_cut.degree(),
        "missing_substitution_cut_degree_bound": 8,
        "reduced_missing_cut_degree": reduced_missing_cut.degree(),
        "bezout_matrix_size": 4,
...
def main(
    ...
    if source_census:
        cases = tuple(
            (*epsilon, selected_sigma_c, selected_sigma_o, 3, 11)
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-xi3-fully-mixed-pairing11-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-xi3-fully-mixed-pairing11-pilot-v1"
        ),
        "scope": (
            "Exact fully mixed division-free Bezout resultant and exceptional-root "
            "census at xi=3, representative pairing 11; no claim beyond the "
            "printed cases."
        ),
