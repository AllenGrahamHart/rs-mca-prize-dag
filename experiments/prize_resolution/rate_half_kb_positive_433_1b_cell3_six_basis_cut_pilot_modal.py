#!/usr/bin/env python3
"""Pilot a cell-3 target-free outside cut in the six-dimensional function field."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_six_basis_cut_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_six_basis_cut_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-six-basis-cut-pilot")
image = (
...
def profile_case(case):
    ...
    if xi_index != 0 or pairing_index != 0:
        raise ValueError("the pilot is scoped exactly to xi=0, pairing=0")
    ...
    # For xi=0 and matching=0, the first residual pair is (de,-de).
...
def main(
    ...
    output = {
        "schema": (
            ...
            if all_signs else
            "rate-half-kb-positive-433-1b-cell3-six-basis-cut-pilot-v1"
        ),
        "scope": (
            "Exact target-free outside cuts in the six-dimensional common "
            "function field at xi0/pairing0; no wider outside or cell claim."
        ),
