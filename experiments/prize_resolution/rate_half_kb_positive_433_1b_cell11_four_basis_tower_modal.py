#!/usr/bin/env python3
"""Compile guarded four-basis presentations of the cell-11 common curve."""
...
app = modal.App("rs-mca-positive-433-1b-cell11-four-basis-tower")
image = (
...
def compile_tower(case):
    ...
    if not payload.get("complete") or len(rows) != 6 or len(signatures) != 1:
        raise RuntimeError("invalid cell-11 structure payload")
...
def main():
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell11-four-basis-tower-v1",
        "field": PRIME,
        "scope": (
            "Exact quadratic-over-quadratic common tower with one linear c "
            "recovery on the guarded cell-11 chart cover; no outside claim."
        ),
