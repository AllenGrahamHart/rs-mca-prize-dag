#!/usr/bin/env python3
"""Classify all affine six-singleton supports of exact multiplicity five."""
...
app = modal.App("e1-profile-36-mu5-m32-chords")
...
def main(
    shards: int = 16,
    output: str = (
        "experiments/prize_resolution/e1_profile_36_mu5_m32_chord_orbits.json"
    ),
) -> None:
    ...
    payload = {
        "schema": "e1-profile-36-mu5-m32-chord-orbits-v1",
        "examined": sum(int(row["examined"]) for row in rows),
        "mu_five": sum(int(row["mu_five"]) for row in rows),
    ...
    assert payload["examined"] == 10009125
    path = Path(output)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n")
    print(
        "E1_PROFILE_36_MU5_M32_CHORDS_PASS "
        f"examined={payload['examined']} mu5={payload['mu_five']} "
