#!/usr/bin/env python3
"""Check the dual exact E20 cyclotomic norm ledger."""
...
SOURCE = HERE / "e20_six_profile_norm_modal.py"
...
def main() -> None:
    ...
    assert packet["schema"] == "e1-e20-six-profile-norm-v1"
    ...
    vectors = [match for row in collection["rows"] for match in row["primary"]["matches"]]
    flint = [
        int(value)
    ...
    assert summary["maximizing_indices"] == [index for index, value in enumerate(flint) if value == maximum]
    expected_profile_maxima = [
        max(
            (value for value, vector in zip(flint, vectors) if int(vector["profile"]) == profile),
            default=0,
        )
    ...
    print(
        "E20_SIX_PROFILE_NORM_CHECK_PASS "
        f"vectors={len(vectors)} distinct={len(set(flint))} max={maximum} "
        f"bits={maximum.bit_length()} hits=0 engines=2 mutations=1"
    )
