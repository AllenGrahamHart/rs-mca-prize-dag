#!/usr/bin/env python3
"""Run dual complete E15 two-profile censuses on Modal."""
...
REMOTE_PRIMARY_SOURCE = "/root/e15_two_profile_census.cpp"
REMOTE_AUDIT_SOURCE = "/root/e15_two_profile_census_audit.cpp"
REMOTE_PRIMARY = "/root/e15_two_profile_census"
REMOTE_AUDIT = "/root/e15_two_profile_census_audit"

app = modal.App("e1-n256-e15-two-profile-census")
image = (
...
def main() -> None:
    ...
    if not probe["complete"] or int(probe["relevant_affine_templates"]) != 8:
        raise RuntimeError("E15 router is incomplete or changed")
    root = HERE.parents[1]
    atlas_path = (
        root
    ...
    def vector_sum(key: str) -> list[int]:
        return [sum(int(row["primary"][key][i]) for row in rows) for i in range(2)]
    ...
    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        ...
        packet = {
            "schema": "e1-e15-two-profile-census-v1",
            "complete": complete,
            ...
            "summary": {
                ...
                "full_conductor_counts": full,
                "proper_conductor_counts": [profile[i] - full[i] for i in range(2)],
    ...
    try:
        ...
        print(f"E15_TWO_PROFILE_CENSUS_INCOMPLETE completed={len(rows)}/8")
        raise
    ...
    print("E15_TWO_PROFILE_CENSUS " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E15_TWO_PROFILE_CENSUS_COMPLETE {packet['complete']}")
    print(f"E15_TWO_PROFILE_CENSUS_RESULT {RESULT}")
