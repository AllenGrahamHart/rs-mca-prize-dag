#!/usr/bin/env python3
"""Compute dual exact cyclotomic norms for the full E20 residue."""
...
COLLECT = HERE / "e20_six_profile_collect_result.json"
RESULT = HERE / "e20_six_profile_norm_result.json"
BATCH_SIZE = 1000

app = modal.App("e1-n256-e20-six-profile-norm")
...
def load_vectors() -> list[dict[str, object]]:
    ...
    if not packet["complete"]:
        raise RuntimeError("E20 collector is incomplete")
    ...
    if len(vectors) != int(packet["summary"]["collected_full_conductor"]):
        raise RuntimeError("E20 full-conductor collection count mismatch")
    return vectors
...
def main() -> None:
    ...
    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        packet = {
            "schema": "e1-e20-six-profile-norm-v1",
            "complete": complete,
            "agreement": False,
            "vectors": len(vectors),
    ...
    try:
        ...
        print(
            "E20_SIX_PROFILE_NORM_INCOMPLETE "
    ...
    if len(flint_norms) != len(vectors) or flint_norms != pari_norms:
        raise RuntimeError("FLINT/PARI E20 norm mismatch")
    profile_maxima = [
        max(
    ...
    packet["summary"] = {
        ...
        "maximizing_indices": [index for index, value in enumerate(flint_norms) if value == max(flint_norms)],
    ...
    RESULT.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n")
    print("E20_SIX_PROFILE_NORM " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E20_SIX_PROFILE_NORM_AGREEMENT {packet['agreement']}")
    print(f"E20_SIX_PROFILE_NORM_RESULT {RESULT}")
