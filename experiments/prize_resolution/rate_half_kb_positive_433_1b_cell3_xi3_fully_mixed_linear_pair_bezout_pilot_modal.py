#!/usr/bin/env python3
"""Pilot xi3 fully mixed pairings 7 and 8 via the two linear-f pairs."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_fully_mixed_linear_pair_bezout_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_fully_mixed_linear_pair_bezout_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-xi3-fully-mixed-linear-pair-bezout")
image = (
...
def profile_case(case):
    ...
    if (pairing_index not in (7, 8) or xi_index != 3 or
            sigma_c not in (-1, 1) or sigma_o != 0):
        raise ValueError(
            "the pilot is scoped to xi=3 fully mixed representatives 7 and 8"
        )
    ...
    # In pairings 7 and 8, two equations have second arguments linear in f.
    # Their quadratic-q resultant has degree at most eight in f.  Any target
    # also obeys (m+f^2)^2-s f^2=0, the monic missing-f quartic.
    def q_coefficients_for_scaled_f(scale, left_sign):
        output = []
        for q_degree in range(3):
            q_coefficient = PairPolynomial(
            ...
            output.append(
                -q_coefficient if left_sign == -1 and q_degree == 1
                else q_coefficient
            )
        return tuple(output)

    if pairing_index == 7:
        first_q = q_coefficients_for_scaled_f(common_b, 1)
        second_q = q_coefficients_for_scaled_f(sigma_c*c_pair, -1)
    else:
        first_q = q_coefficients_for_scaled_f(sigma_c*c_pair, 1)
        second_q = q_coefficients_for_scaled_f(common_b, -1)
    first_q0, first_q1, first_q2 = first_q
    second_q0, second_q1, second_q2 = second_q
    linear_pair_resultant = (
        (first_q2*second_q0-first_q0*second_q2)**2
        - (first_q2*second_q1-first_q1*second_q2)
        * (first_q1*second_q0-first_q0*second_q1)
    )
    if linear_pair_resultant.degree() > 8:
        raise ValueError("linear-pair q resultant exceeds degree eight in f")
    ...
    reduced_linear_pair_cut = monic_remainder(
        linear_pair_resultant, missing_f_cut
    )
    if reduced_linear_pair_cut.degree() < 0:
        raise ValueError("linear-pair resultant vanishes modulo missing-f cut")
    print(json.dumps({
        "phase": "fully_mixed_linear_pair_remainder",
        "linear_pair_resultant_degree": linear_pair_resultant.degree(),
        "missing_f_degree": missing_f_cut.degree(),
        "reduced_degree": reduced_linear_pair_cut.degree(),
        "seconds": time.perf_counter()-started,
    }), flush=True)
    target_free = bezout_resultant(
        missing_f_cut, reduced_linear_pair_cut
    )
    if target_free == Pair():
        raise ValueError("fully mixed missing-f Bezout eliminant vanished")
    print(json.dumps({
        "phase": "fully_mixed_linear_pair_bezout",
        "linear_pair_resultant_degree": linear_pair_resultant.degree(),
        "missing_f_degree": missing_f_cut.degree(),
        "reduced_degree": reduced_linear_pair_cut.degree(),
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                for f_value in f_roots:
                    ...
                    if pairing_index == 7:
                        first_right = b_value*f_value % PRIME
                        second_right = sigma_c*c_value*f_value % PRIME
                    else:
                        first_right = sigma_c*c_value*f_value % PRIME
                        second_right = b_value*f_value % PRIME
                    first_q_cut = paired_q_polynomial_at(first_right, 1)
                    second_q_cut = paired_q_polynomial_at(second_right, -1)
                    ...
                    for q_value in q_roots:
                        ...
                        for selected_sigma_o in (-1, 1):
                            ...
                            lane_row = {
                                "sigma": [sigma_c, selected_sigma_o],
    ...
    return {
        ...
        "pairing_index": pairing_index,
        "status": "COMPLETE",
        "target_lanes_covered": [[sigma_c, -1], [sigma_c, 1]],
        ...
        "linear_pair_resultant_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in linear_pair_resultant.coefficients
        ],
        "missing_f_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in missing_f_cut.coefficients
        ],
        "reduced_linear_pair_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in reduced_linear_pair_cut.coefficients
        ],
        ...
        "reduced_linear_pair_cut_degree": reduced_linear_pair_cut.degree(),
        "bezout_matrix_size": 4,
...
def main(
    ...
    if source_census:
        cases = tuple(
            (*epsilon, selected_sigma_c, 0, 3, selected_pairing)
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-xi3-fully-mixed-linear-pair-bezout-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-xi3-fully-mixed-linear-pair-bezout-pilot-v1"
        ),
        "scope": (
            "Exact fully mixed linear-pair q resultant, monic missing-f reduction, "
            "division-free Bezout eliminant, and exceptional-root census at xi=3 "
            "for representative pairings 7 and 8; no claim beyond the printed cases."
        ),
