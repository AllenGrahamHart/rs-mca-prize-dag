#!/usr/bin/env python3
"""Apply the pinned positive-DE pairing-9 compiler to cell 12."""
...
app = modal.App("rs-mca-positive-433-1b-cell12-positive-de-pairing9-adapter")
image = (
...
@app.function(image=image, cpu=2.0, memory=4096, timeout=900, max_containers=16)
def evaluate(case):
    import time
...
def main(limit: int = 0):
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell12-positive-de-"
            "pairing9-adapter-v1"
        ),
        "field": PRIME,
        "scope": (
            "Cell-12 execution of the pinned nested-quadratic compiler for "
            "positive-DE xi=0, pairing 9; adapter evidence until audited."
        ),
    ...
    print(json.dumps({
        ...
        "rows": [
            {key: row.get(key) for key in (
                ...
                "route_point_count", "uf_candidate_count",
                "colored_solution_count", "excluded", "unresolved",
            )}
            for row in rows
        ],
    }, sort_keys=True))
