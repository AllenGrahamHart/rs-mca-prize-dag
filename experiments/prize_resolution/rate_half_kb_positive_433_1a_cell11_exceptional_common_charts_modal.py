#!/usr/bin/env python3
"""Decide every deployed cell-11 linear exceptional scale in the common ideal."""
...
FACTORS = DIRECTORY / "rate_half_kb_positive_433_1a_cell11_exceptional_scale_factor_result.json"
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell11_exceptional_common_charts_result.json"
REMOTE_SCOUT = "/root/remaining_lex_scout.json"
REMOTE_FACTORS = "/root/cell11_exceptional_scales.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1a-cell11-exceptional-common-charts")
image = (
...
@app.function(image=image, cpu=1.0, memory=2048, timeout=120, max_containers=7)
...
"""
    ...
    return {
        "t": t_value, "status": "COMPLETE" if valid else "ERROR",
        "unit": "UNIT=1" in stdout,
        "guard_trivial": (
            t_value == 0 or pow(t_value, 2, PRIME) in (1, PRIME - 1)
        ),
        "program_sha256": digest(program),
        "stdout": stdout[-20000:], "stderr": process.stderr[-4000:],
    }
...
def main():
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell11-exceptional-common-charts-v1",
        "scope": (
            "Exact original-common-ideal decisions at every deployed linear "
            "root of the cell-11 chart scales; no outside, orbit, or Prize claim."
        ),
    ...
    print(json.dumps({
        ...
        "rows": [
            {
                "t": row.get("t"), "status": row.get("status"),
                "unit": row.get("unit"),
