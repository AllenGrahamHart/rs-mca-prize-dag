#!/usr/bin/env python3
"""Classify all affine six-singleton supports of exact multiplicity six."""
...
app = modal.App("e1-profile-36-mu6-m64-chord-classify")
...
def main(
    ...
    output: str = (
        ...
        "e1_profile_36_mu6_m64_chord_orbits.json"
    ),
) -> None:
    ...
    payload = {
        "schema": "e1-profile-36-mu6-m64-chord-orbits-v1",
        "examined": sum(int(row["examined"]) for row in rows),
        "mu_six": sum(int(row["mu_six"]) for row in rows),
    ...
    assert payload["mu_six"] == 122_880
    assert payload["weights"] == {
        2: 64, 3: 448, 4: 16, 5: 128, 6: 176, 7: 2_080,
        8: 416, 9: 5_856, 10: 2_080, 11: 24_672, 12: 1_360,
        13: 32_544, 14: 4_080, 15: 48_960,
    }
    output_path = Path(output)
    output_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n")
    print(
        "E1_PROFILE_36_MU6_M64_CHORD_CLASSIFY_PASS "
        f"examined={payload['examined']} mu_six={payload['mu_six']} "
