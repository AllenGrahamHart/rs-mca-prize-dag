#!/usr/bin/env python3
"""Pilot the cell-3 DE-missing pairing-4 nested quadratic cut."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing4_nested_quadratic_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing4_nested_quadratic_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-de-pairing4-nested-quadratic-pilot")
image = (
...
def profile_case(case):
    ...
    if xi_index not in (0, 2) or pairing_index != 4:
        raise ValueError("the pilot is scoped to xi in {0,2}, pairing=4")
    ...
    p_f = paired_polynomial(
        PairPolynomial(second_de), variable_polynomial*common_b
    )
    ...
    middle_coefficient = (
        2*eta*de_record*a_missing_squared
        - source_squared_sum_numerator
    )
    ...
    relation_linear = (
        PairPolynomial(
            a_missing_squared
            * (u_linear**3+2*u_linear*u_constant)
        )
        + variable_polynomial**2*(middle_coefficient*u_linear)
    )
    relation_constant = (
        PairPolynomial(
            a_missing_squared
            * (u_linear*u_linear*u_constant+u_constant*u_constant)
        )
        + variable_polynomial**2*(middle_coefficient*u_constant)
        + variable_polynomial**4*(de_record**2*a_missing_squared)
    )
    uf_eliminant = (
        PairPolynomial(p_u_a)*relation_constant**2
        - PairPolynomial(p_u_b)*relation_linear*relation_constant
        + PairPolynomial(p_u_c)*relation_linear**2
    )
    ...
    for r_value in sorted(candidate_r_values):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                p_f_field = paired_polynomial_at(second_de_value, b_value)
                u_roots = field_roots(p_u_field)
                f_roots = field_roots(p_f_field)
                b_row["u_roots"] = u_roots
                b_row["f_roots"] = f_roots
...
def main(
    ...
    if full_census:
        cases = tuple(
            (*epsilon, sigma_c, sigma_o, selected_xi, 4)
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-de-pairing4-nested-quadratic-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-de-pairing4-nested-quadratic-census-v1"
            if all_signs or full_census else
            "rate-half-kb-positive-433-1b-cell3-de-pairing4-nested-quadratic-pilot-v1"
        ),
        "scope": (
            "Exact nested-quadratic and exceptional-root census at DE-missing "
            "pairing 4; no claim beyond the printed cases."
        ),
