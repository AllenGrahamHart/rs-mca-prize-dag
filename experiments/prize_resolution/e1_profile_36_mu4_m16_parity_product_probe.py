#!/usr/bin/env python3
"""Certify parity-adaptive product chambers for cofactor 16."""
...
CHORD_SLACK = 117
OUTPUT = Path(
    "experiments/prize_resolution/e1_profile_36_mu4_m16_parity_product_result.json"
)
HEADER = Path(
    "experiments/prize_resolution/e1_profile_36_mu4_m16_product_live.hpp"
)

app = modal.App("e1-profile-36-mu4-m16-parity-product")
...
def compute() -> dict[str, object]:
    ...
    chord_excluded: list[tuple[int, int, int]] = []
    ambiguous: list[tuple[int, int, int]] = []
    comparisons = 0
    for energy in range(2, 109):
        for odd_weight, l1_values in sorted(parity_l1_values(energy).items()):
            for l1_norm in sorted(l1_values):
                ...
                if 4 * l1_norm > energy + CHORD_SLACK:
                    excluded.append(record)
                    chord_excluded.append(record)
                    continue
                key = (energy, l1_norm)
                if key not in cache:
    ...
    return {
        "schema": "e1-profile-36-mu4-m16-parity-product-v1",
        "complete": True,
        "cofactor": COFACTOR,
        "energy_range": [2, 108],
        "maximum_classes": MAX_CLASSES,
        "chord_inequality": "4L<=E+117",
        "chord_excluded": len(chord_excluded),
...
@app.function(image=image, cpu=1.0, memory=256, timeout=240, max_containers=1)
...
def main() -> None:
    ...
    print(
        "E1_PROFILE_36_MU4_M16_PARITY_PRODUCT_PASS "
