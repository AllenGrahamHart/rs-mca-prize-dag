#!/usr/bin/env python3
"""Clear cell-11 plane-chart kernel denominators with FLINT."""
...
KERNEL = DIRECTORY / "rate_half_kb_positive_433_1a_cell11_kernel_reduction_result.json"
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell11_plane_kernel_flint_result.json"
REMOTE_SCOUT = "/root/remaining_lex_scout.json"
REMOTE_KERNEL = "/root/cell11_kernel_reduction.json"
PRIME = 2130706433
NAMES = ("a20", "a21", "a22", "a00", "a01", "a02", "b10", "b11")

app = modal.App("rs-mca-positive-433-1a-cell11-plane-kernel-flint")
image = (
...
def clear_denominators():
    ...
    if len(basis) != 10:
        raise RuntimeError("unexpected cell-11 lex basis")
    ...
    def from_terms(terms, predicate, transform, scalar=1):
        ...
        return context.from_dict({
            key: coefficient for key, coefficient in payload.items()
            if coefficient
        })
    ...
    cleared = {
        name: substitute_cleared(reduced_terms[name]) for name in NAMES
    }
    common_gcd = functools.reduce(
        lambda left, right: left.gcd(right), cleared.values()
    )
    preprojection = {}
    ...
    projected_gcd = functools.reduce(
        lambda left, right: left.gcd(right), aligned.values()
    )
    normalized = {}
    ...
    return {
        "status": "COMPLETE", "field": PRIME, "cell": 11,
        "epsilon": [-1, -1],
        "source_scout_sha256": hashlib.sha256(scout_bytes).hexdigest(),
        "source_kernel_sha256": hashlib.sha256(kernel_bytes).hexdigest(),
        "basis_indices": {"plane": 0, "r_linear": 1, "c_linear": 4},
        "kernel_degree_bounds": {"c": max_c_degree, "r": max_r_degree},
        "plane_polynomial": plane.str(), "plane_shape": shape(plane),
        "plane_leading_coefficient": plane_leading.str(),
        "plane_leading_shape": shape(plane_leading),
        "r_numerator": r_numerator.str(), "r_denominator": r_denominator.str(),
        "c_numerator": c_numerator.str(), "c_denominator": c_denominator.str(),
        ...
        "normalized_coefficients": normalized, "b1_opposite": True,
    }
...
def main():
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell11-plane-kernel-flint-v1",
        "scope": (
            "Exact denominator clearing and plane reduction of the cell-11 "
