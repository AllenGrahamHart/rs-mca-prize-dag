#!/usr/bin/env python3
"""Replay the cell-9 xi=3 pairing-1 certificate independently."""
...
PRIMARY = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing1_chart_result"
)
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing1_"
...
app = modal.App("rs-mca-positive-433-1b-cell9-xi3-pairing1-direct-audit")
image = (
...
def audit(chart_case):
    ...
    expected_metadata = {
        ...
        "scope": (
            ...
            "compiler for cell-9 xi=3, pairing=1."
        ),
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-xi3-pairing1-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 1 and item["xi_index"] == 3
        ]
        core.require(
            row["status"] == "COMPLETE"
    ...
    expected = {
        (epsilon, branch_index, selected_b, selected_c)
    ...
    for row in iter_records(manifest):
        ...
        seen.add(key)
        epsilon, branch_index, b_index, c_index = key
    ...
    core.require(
        seen == expected and counts["records"] == 72,
        "complete Cartesian cover",
    )
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-xi3-pairing1-"
            "direct-audit-v1"
        ),
        "field": PRIME,
