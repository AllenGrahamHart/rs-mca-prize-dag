#!/usr/bin/env python3
"""Pilot the cell-3 DE-missing pairing-3 nested quadratic cut."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing3_nested_quadratic_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing3_nested_quadratic_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-de-pairing3-nested-quadratic-pilot")
image = (
...
def profile_case(case):
    ...
    if xi_index not in (0, 2) or pairing_index != 3:
        raise ValueError("the pilot is scoped to xi in {0,2}, pairing=3")
    ...
    p_u = paired_polynomial(PairPolynomial(de_record), variable_polynomial)
    p_v = paired_polynomial(
        PairPolynomial(second_de), variable_polynomial*sigma_o
    )
    if p_u.degree() != 2 or p_v.degree() != 2:
        raise ValueError("paired DE cuts are not quadratic")

    source_squared_sum_numerator = (
        missing_label*(beta_0+beta_1*missing_label)**2
    )
    eta = 1 if xi_index == 0 else -1
    a_missing_squared = a_missing**2
    h_a = PairPolynomial(de_record*a_missing_squared)
    h_b = variable_polynomial*(
        2*eta*de_record*a_missing_squared-source_squared_sum_numerator
    )
    h_c = variable_polynomial**2*(de_record*a_missing_squared)
    p_u_c, p_u_b, p_u_a = p_u.coefficients[:3]
    nested_quartic = (
        (PairPolynomial(p_u_a)*h_c-PairPolynomial(p_u_c)*h_a)**2
        - (PairPolynomial(p_u_a)*h_b-PairPolynomial(p_u_b)*h_a)
        * (PairPolynomial(p_u_b)*h_c-PairPolynomial(p_u_c)*h_b)
    )
    remainder = polynomial_remainder(nested_quartic, p_v)
    remainder_degree = remainder.degree()
    p_v_c, p_v_b, p_v_a = p_v.coefficients[:3]
    if remainder_degree == 1:
        remainder_constant, remainder_linear = remainder.coefficients
        target_free = (
            remainder_linear**2*p_v_c
            - remainder_linear*remainder_constant*p_v_b
            + p_v_a*remainder_constant**2
        )
    ...
    print(json.dumps({
        ...
        "p_v_degree": p_v.degree(),
        "nested_quartic_degree": nested_quartic.degree(),
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                if de_value == 0:
                    ...
                    t_row["b_rows"].append(b_row)
                    continue
                p_u_field = paired_polynomial_at(de_value)
                p_v_field = paired_polynomial_at(second_de_value, sigma_o)
                u_roots = field_roots(p_u_field)
                v_roots = field_roots(p_v_field)
                b_row["u_roots"] = u_roots
                b_row["v_roots"] = v_roots
                if u_roots is None or v_roots is None:
                ...
                for u_value in u_roots:
                    for v_value in v_roots:
                        ...
                        uv_row = {
                            "u": u_value, "v": v_value, "h": h_value,
                            "f_rows": [],
                        }
                        if h_value:
                            uv_row["status"] = "MISSING_SUM_NONZERO"
                            b_row["uv_rows"].append(uv_row)
                            continue
                        candidate = {
                            "point": point, "u": u_value, "v": v_value,
                        }
                        uv_candidates.append(candidate)
                        f_squared = (
                            u_value*v_value*pow(de_value, -1, PRIME)
                        ) % PRIME
                        f_roots = field_roots(polynomial_context([
                            -f_squared, 0, 1
                        ]))
                        uv_row["f_squared"] = f_squared
                        uv_row["f_roots"] = f_roots
                        if f_roots is None:
                            uv_row["status"] = "ZERO_F_EQUATION"
                            unresolved.append([candidate, "ZERO_F_EQUATION"])
                            b_row["uv_rows"].append(uv_row)
                            continue
                        for f_value in f_roots:
                            if f_value == 0:
                                ...
                                boundary_solutions.append({**candidate, **f_row})
                                uv_row["f_rows"].append(f_row)
                                continue
                            ...
                            if colored_cut:
                                ...
                                uv_row["f_rows"].append(f_row)
                                continue
                            colored_record = {**candidate, **f_row}
                            colored_solutions.append(colored_record)
                            representatives = (
                            ...
                            equation_values = [
                                ...
                                (pow(d_value+eta*e_value, 2, PRIME)-source_sum)
                                % PRIME,
                                paired_value_at(de_value, u_value),
                                paired_value_at(
                                    second_de_value, sigma_o*v_value % PRIME
                                ),
                                colored_cut,
                            ]
                            ...
                            record = {**candidate, **f_row}
                            (boundary_solutions if failed_guards else witnesses).append(
                                record
                            )
                            uv_row["f_rows"].append(f_row)
                        uv_row["status"] = "CHECKED"
                        b_row["uv_rows"].append(uv_row)
    ...
    direct_lift = {
        ...
        "uv_candidate_count": len(uv_candidates),
        "uv_candidates": uv_candidates,
    ...
    print(json.dumps({
        ...
        "uv_candidates": len(uv_candidates),
        "colored_solutions": len(colored_solutions),
...
def main(
    ...
    if full_census:
        cases = tuple(
            (*epsilon, sigma_c, sigma_o, selected_xi, 3)
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-de-pairing3-nested-quadratic-census-v1"
            if all_signs or full_census else
            "rate-half-kb-positive-433-1b-cell3-de-pairing3-nested-quadratic-pilot-v1"
        ),
        "scope": (
            "Exact nested-quadratic and exceptional-root census at DE-missing "
            "pairing 3; no claim beyond the printed cases."
        ),
    ...
    output_path = CENSUS_RESULT if all_signs or full_census else RESULT
