#!/usr/bin/env python3
"""Compute dual exact cyclotomic norms for the full E16 residue."""
...
app = modal.App("e1-n256-e16-four-profile-norm")
...
def main() -> None:
    ...
    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        packet = {
            "schema": "e1-e16-four-profile-norm-v1",
            "complete": complete,
            "agreement": False,
            "vectors": len(vectors),
    ...
    try:
        ...
        print(
            "E16_FOUR_PROFILE_NORM_INCOMPLETE "
    ...
    if len(flint_norms) != len(vectors) or flint_norms != pari_norms:
        raise RuntimeError("FLINT/PARI E16 norm mismatch")
    ...
    RESULT.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n")
    print("E16_FOUR_PROFILE_NORM " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E16_FOUR_PROFILE_NORM_AGREEMENT {packet['agreement']}")
    print(f"E16_FOUR_PROFILE_NORM_RESULT {RESULT}")
