#!/usr/bin/env python3
"""Independently audit the cell-5 first-pair residual systems."""
...
REPLAY = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell5_parallel_de_four_basis_replay_result.json"
)
KERNEL = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell5_compact_kernel_result.json"
)
PRIMARY = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell5_parallel_de_first_pair_residual_result.json"
)
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell5_parallel_de_first_pair_audit_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell5-parallel-de-audit")
image = (
...
def audit(case):
    ...
    source_row = next(
        ...
        and row["cut_kind"] == "equal_negative"
    )
    ...
    for point_index, point in enumerate(points):
        ...
        if not a_missing:
            raise RuntimeError("ordinary survivor has zero missing denominator")
        source_sum = (
    ...
    return {
        ...
        "sigma": [sigma_c, sigma_o], "cut_kind": "equal_negative",
...
def main(limit: int = 0):
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            rows.append({
                ...
                "cut_kind": "equal_negative", "status": "REMOTE_ERROR",
                "error": repr(row),
            })
        else:
            rows.append(row)
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell5-parallel-de-audit-v1",
        "field": PRIME,
        "scope": (
            "Independent two-variable lex and deployed-root audit for the "
            "cell-5 negative-DE first-pair residual systems."
        ),
