#!/usr/bin/env python3
"""Derive and price the exact E24 profile/parity/light-atlas router."""
...
RESULT = HERE / "e24_profile_parity_probe_result.json"

app = modal.App("e1-n256-e24-profile-parity-route-probe")
...
def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    rows = []
    for counts in product(range(25), range(7), range(3), range(2), range(1)):
        if sum((index+1)**2 * count for index, count in enumerate(counts)) != 24:
            continue
...
def matching_ledgers(light_diameters: int) -> list[list[int]]:
    ...
    visit(tuple(range(7)), ())
    return [[value, (value-78)//2] for value in sorted(square_masses)]
...
def compute() -> dict[str, object]:
    ...
    for l1_norm in range(22, 10, -1):
        delta = 24 + 66 - 4*l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 24 and l1_bound is None:
            l1_bound = l1_norm
    ...
    return {
        ...
        "majorant_policy": "not invoked: the fixed cubic-Hermite margin is negative at M_3=0",
        "parity_survivors": survivors,
        "parity_rejected": [row for row in profiles if int(row["odd_classes"]) > 6],
...
def main() -> None:
    ...
    prior = json.loads(e26_atlas.read_text())
    if not prior["complete"]:
        raise RuntimeError("the proved even-parity light atlas is incomplete")
    atlas = prior["atlas_inputs"]
    assert set(atlas) == {"0", "2", "4", "6"}

    packet = compute.remote()
    used = {str(int(row["odd_classes"])) for row in packet["parity_survivors"]}
    packet["atlas_inputs"] = atlas
    packet["atlas_source_sha256"] = hashlib.sha256(e26_atlas.read_bytes()).hexdigest()
    ...
    packet["schema"] = "e1-e24-profile-parity-route-probe-v1"
    ...
    print("E24_PROFILE_PARITY_ROUTE_PROBE " + json.dumps({
    ...
    print(f"E24_PROFILE_PARITY_ROUTE_PROBE_RESULT {RESULT}")
