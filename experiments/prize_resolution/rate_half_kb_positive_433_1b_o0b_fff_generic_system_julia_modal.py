#!/usr/bin/env python3
"""Compute the generic FFF necessary subsystem with Groebner.jl."""
...
PROGRAM = DIRECTORY / "rate_half_kb_positive_433_1b_o0b_fff_generic_system_julia_program.py"
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_o0b_fff_generic_system_julia_result.json"
REMOTE_CACHE = "/root/cache.json"
REMOTE_GENERIC = "/root/generic.json"
REMOTE_PROGRAM = "/root/fff_generic_system_program.py"
PRIME = 2130706433
...
app = modal.App("rs-mca-positive-433-1b-o0b-fff-generic-system-julia")
image = (
...
def compute_system():
    core = load("fff_generic_system", REMOTE_PROGRAM)
    cache = json.loads(Path(REMOTE_CACHE).read_text())
    generic = json.loads(Path(REMOTE_GENERIC).read_text())
    built = core.build(cache, generic)
    ...
    profile = re.search(
        r"(?:^|\n)SYSTEM_COMPLETE ([01]) (-?\d+) (\d+) (\d+)", stdout
    )
    certified = "SYSTEM_CERTIFIED" in stdout
    basis_path = Path("/tmp/fff_generic_system_basis.txt")
    coefficient_path = Path("/tmp/fff_generic_system_coefficients.txt")
    ...
    valid = (
        ...
        and len(basis) == int(profile.group(3)) and entries
    )
    return {
        **common,
...
def write_checkpoint(row, collection_complete):
    RESULT.write_text(json.dumps({
        "schema": "rate-half-kb-positive-433-1b-o0b-fff-generic-system-julia-v1",
        "app": "rs-mca-positive-433-1b-o0b-fff-generic-system-julia",
