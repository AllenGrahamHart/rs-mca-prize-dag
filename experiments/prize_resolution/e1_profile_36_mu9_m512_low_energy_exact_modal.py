#!/usr/bin/env python3
"""Run the exact mu=9, E<=17 radius classifier on Modal."""
...
ORBIT_PATH = ROOT / "experiments/prize_resolution/e1_profile_36_mu9_chord_orbits.json"
CPP_PATH = ROOT / "experiments/prize_resolution/e1_profile_36_mu9_m512_low_energy_exact.cpp"
BASE_CPP_PATH = ROOT / "experiments/prize_resolution/e1_profile_36_mu1_low_energy_exact.cpp"

app = modal.App("e1-profile-36-mu9-m512-low-energy-exact")
image = (
...
def classify_batch(batch: list[list[int]], engine: str) -> dict[str, object]:
    ...
    argv = ["/worker"] + (["triple"] if engine == "triple-radius" else [])
...
def main(
    shards: int = 24,
    engine: str = "pair-radius-plus-third",
    output: str = "experiments/prize_resolution/e1_profile_36_mu9_m512_low_energy_exact_result.json",
) -> None:
    ...
    result = {
        "schema": "e1-profile-36-mu9-m512-low-energy-exact-result-v1",
        "engine": engine,
    ...
    print(
        "E1_PROFILE_36_MU9_M512_LOW_ENERGY_EXACT_PASS "
