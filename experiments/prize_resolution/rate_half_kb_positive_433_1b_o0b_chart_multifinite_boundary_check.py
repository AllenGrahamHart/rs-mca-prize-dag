#!/usr/bin/env python3
"""Outcome-neutral checker for four O0b multi-finite boundary tests."""
...
PROGRAM = HERE / "rate_half_kb_positive_433_1b_o0b_projective_chart_boundary_program.py"
PILOT = HERE / "rate_half_kb_positive_433_1b_o0b_chart_remaining_pilot_result.json"
RESULT = HERE / "rate_half_kb_positive_433_1b_o0b_chart_multifinite_boundary_result.json"
...
def verify(payload=None):
    ...
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-o0b-chart-multifinite-boundary-v1",
            "schema")
    require(payload["collection_complete"] is True and
            payload["field"] == 2130706433 and payload["case"] == CASE,
            "scope")
    require(payload["chart_masks"] == [list(mask) for mask in MASKS],
            "chart scope")
    require(payload["source_cache_sha256"] == CACHE_SHA256 and
            ...
            payload["source_seven_chart_pilot_sha256"] == PILOT_SHA256,
            "source fields")
    rows = payload["rows"]
    require(payload["expected_row_count"] == 4 and
            payload["processed_row_count"] == 4 and
            payload["remote_errors"] == [] and len(rows) == 4,
            "complete collection")
    require([row["index"] for row in rows] == list(range(4)) and
    ...
    for row in rows:
        ...
        require(row["variable_count"] == 8 + finite_count and
                ...
                row["rabinowitsch_variable"] == "w" and
                row["boundary_guard"] == "b+1" and
                row["generator_count"] == 30,
                "input ledger")
    ...
    return {
        "rows": 4, "unit": unit_count, "nonunit": nonunit_count,
...
def hostile_audit():
    ...
    mutation = deepcopy(payload)
    mutation["rows"][0]["boundary_guard"] = "b-1"
    expect_rejected(mutation, "wrong boundary")
    mutation = deepcopy(payload)
    mutation["rows"][0]["rabinowitsch_equation_count"] = 0
    expect_rejected(mutation, "missing inversion equation")
    return 3
...
def main():
    ...
    print("RATE_HALF_KB_POSITIVE_433_1B_O0B_CHART_MULTIFINITE_BOUNDARY_CHECK_PASS "
