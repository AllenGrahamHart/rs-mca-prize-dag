#!/usr/bin/env python3
"""Exact product-block rank compiler for the positive 433-1a route."""
...
def compile_cell(cell_index, dump=False):
    ...
    stripped = tuple(value for value, ledger in stripped_records)
    stripped_ledgers = tuple(ledger for value, ledger in stripped_records)
    output = {
        "cell": cell_index,
    ...
    if cell_index in (0, 14):
        ...
        specialized = tuple(value for value, ledger in specialized_records)
        specialized_ledgers = tuple(
            ledger for value, ledger in specialized_records
        )
        output["rank_drop_substitution"] = str(substitution[b])
        output["rank_drop_specialized"] = [
            summary(value, variables) for value in specialized
        ]
        output["rank_drop_guard_only_minor_columns"] = [
            index for index, value in enumerate(specialized)
            if not sp.Poly(value, *variables, modulus=PRIME).is_zero
            and sp.Poly(value, *variables, modulus=PRIME).total_degree() == 0
        ]
        if dump:
            output["rank_drop_specialized_expressions"] = [
                str(value) for value in specialized
            ]
            output["rank_drop_specialized_ledgers"] = specialized_ledgers
    if dump:
