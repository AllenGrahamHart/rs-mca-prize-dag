#!/usr/bin/env python3
"""Row-generic residual traversal for the K'=87 best-single adapters."""
...
def scan(base, offset: int) -> dict[str, object]:
    assert 1 <= offset <= 43
    baseline = base.K71.PARENT.PARENT.PARENT.CAPS.baseline_caps(
        base.Q, base.M
    )
        ...
        for s4, s5, middle_vector in middle:
            ...
            for case, charges, fixed, adjacent in base.geometry_profiles(cases):
                ...
                after = max(
                    (
                        base.edge4_price(
                            base.K71.combine(candidate, high_vector), adjacent
                        ),
                        high_name,
