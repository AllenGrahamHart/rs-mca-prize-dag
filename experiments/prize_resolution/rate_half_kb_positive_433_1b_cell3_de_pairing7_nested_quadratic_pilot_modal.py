#!/usr/bin/env python3
"""Pilot the cell-3 DE-missing pairing-7 nested quadratic cut."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing7_nested_quadratic_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing7_nested_quadratic_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-de-pairing7-nested-quadratic-pilot")
image = (
...
def profile_case(case):
    ...
    if xi_index not in (0, 2) or pairing_index != 7:
        raise ValueError("the pilot is scoped to xi in {0,2}, pairing=7")
...
def main(
    ...
    if full_census:
        cases = tuple(
            (*epsilon, sigma_c, sigma_o, selected_xi, 7)
        ...
        cases = tuple(
            (*epsilon, -1, sigma_o, selected_xi, 7)
            for epsilon in sign_pairs
            for sigma_o in (-1, 1)
            for selected_xi in (0, 2)
        )
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-de-pairing7-nested-quadratic-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-de-pairing7-nested-quadratic-census-v1"
            if all_signs or full_census else
            "rate-half-kb-positive-433-1b-cell3-de-pairing7-nested-quadratic-pilot-v1"
        ),
        "scope": (
            "Exact nested-quadratic and exceptional-root census at DE-missing "
            "pairing 7; no claim beyond the printed cases."
        ),
