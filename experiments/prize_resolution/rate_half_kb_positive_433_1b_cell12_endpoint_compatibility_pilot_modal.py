#!/usr/bin/env python3
"""Test source-only endpoint cuts on the cell-12 common curve."""
...
app = modal.App("rs-mca-positive-433-1b-cell12-endpoint-pilot")
image = (
...
def main():
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell12-endpoint-pilot-v1",
        "field": PRIME,
        "scope": (
            "Exact source-only endpoint compatibility ideals on cell 12; "
