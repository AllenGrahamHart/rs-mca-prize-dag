#!/usr/bin/env python3
"""Classify every E14 odd norm part above the pair-feasible floor."""
...
app = modal.App("e1-n256-e14-large-odd-candidate")
...
def classify_pari(values: list[int]) -> list[bool]:
    ...
    completed = subprocess.run(
        ["gp", "-q"], input=script, capture_output=True, check=True,
        text=True, timeout=55)
    flags = [line.strip() == "1" for line in completed.stdout.splitlines()
             if line.strip()]
...
def load_candidates() -> list[dict[str, object]]:
    ...
    if not census["complete"] or not norms["complete"] or not norms["agreement"]:
        raise RuntimeError("E14 source packets are incomplete")
    vectors = []
    ...
    if not len(vectors) == len(exact_norms) == int(norms["vectors"]):
        raise RuntimeError("E14 vector/norm cardinality mismatch")
    candidates = []
    for index, (vector, norm) in enumerate(zip(vectors, exact_norms)):
        ...
        if odd_part >= 2**250:
            candidates.append({
                "index": index, "norm": norm, "valuation": valuation,
...
def main() -> None:
    ...
    values = [int(row["odd_part"]) for row in candidates]
    flint_flags = classify_flint.remote(values)
    pari_flags = classify_pari.remote(values)
    if flint_flags != pari_flags:
        raise RuntimeError("FLINT/PARI E14 primality mismatch")
    ...
    packet = {
        "schema": "e1-e14-large-odd-candidate-v1", "complete": True,
        "agreement": True,
        ...
        "threshold": 2**250, "modulus": 256,
        "candidates": candidates,
        "summary": {
            "candidates": len(candidates),
            "distinct_odd_parts": len(set(values)),
            "prime_candidates": sum(flag for flag in flint_flags),
            "congruence_candidates": sum(value % 256 == 1 for value in values),
            "pair_feasible_prime_candidates": sum(
                bool(row["pair_feasible_prime"]) for row in candidates),
        },
    }
    RESULT.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n")
    print("E14_LARGE_ODD_CANDIDATE " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E14_LARGE_ODD_CANDIDATE_RESULT {RESULT}")
