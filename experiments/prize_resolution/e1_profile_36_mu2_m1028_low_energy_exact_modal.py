#!/usr/bin/env python3
"""Run the exact mu=2, E<=6, factor-257 classifier on Modal."""
...
CPP_PATH = ROOT / "experiments/prize_resolution/e1_profile_36_mu2_m1028_low_energy_exact.cpp"
BASE_CPP_PATH = ROOT / "experiments/prize_resolution/e1_profile_36_mu1_low_energy_exact.cpp"

app = modal.App("e1-profile-36-mu2-m1028-low-energy-exact")
image = (
...
def main(
    ...
    output: str = "experiments/prize_resolution/e1_profile_36_mu2_m1028_low_energy_exact_result.json",
) -> None:
    orbit_payload = json.loads(ORBIT_PATH.read_text())
    target = orbit_payload["targets"]["2"]
    ...
    assert sum(int(row["orbits"]) for row in rows) == target["affine_orbits"]

    result = {
        "schema": "e1-profile-36-mu2-m1028-low-energy-exact-result-v1",
        "engine": engine,
        ...
        "affine_orbits": target["affine_orbits"],
        "shards": shards,
        "passed": len(rows),
        "rows": rows,
    }
    ...
    print(
        "E1_PROFILE_36_MU2_M1028_LOW_ENERGY_EXACT_PASS "
        f"engine={engine} orbits={target['affine_orbits']} output={output}"
    )
