#!/usr/bin/env python3
"""Test all residual matchings at generic cell-9 endpoint candidates."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_endpoint_residual_result.json"
)
REMOTE_REPLAY = "/root/replay.json"
REMOTE_KERNEL = "/root/kernel.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell9-endpoint-residual")
image = (
...
def decide(case):
    ...
    if len(points) != 4:
        raise RuntimeError("unexpected endpoint point count")
...
def main(limit: int = 0):
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell9-endpoint-residual-v1",
        "field": PRIME,
        "scope": (
            "Exact all-matching target census at every generic source point "
            "surviving the cell-9 BF/CF compatibility cuts."
        ),
