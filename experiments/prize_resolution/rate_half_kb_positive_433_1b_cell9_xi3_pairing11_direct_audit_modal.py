#!/usr/bin/env python3
"""Replay the cell-9 xi=3 pairing-11 certificate independently."""
...
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing11_"
...
app = modal.App("rs-mca-positive-433-1b-cell9-xi3-pairing11-direct-audit")
image = (
...
def audit(chart_case):
    ...
    expected_metadata = {
        ...
        "scope": (
            ...
            "sign-free compiler for cell-9 xi=3, pairing=11."
        ),
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-xi3-pairing11-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 11 and item["xi_index"] == 3
        ]
        core.require(
            row["status"] == "COMPLETE"
    ...
    for row in iter_records(manifest):
        ...
        for finite in row["finite_rows"]:
            ...
            for z_value in missing_roots:
                ...
                common_q = intersect(bf_roots, cf_roots)
                core.require(
                    common_q is not None
                    and z_row["bf_q_roots"] == bf_roots
                    and z_row["cf_q_roots"] == cf_roots
                ...
                for q_value in common_q:
                    ...
                    core.require(
                        q_row["e"] == e_value
                        and q_row["bf_pair_cut"] == 0
                        and q_row["cf_pair_cut"] == 0
                        and core.paired(
    ...
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-xi3-pairing11-direct-audit-v1"
        ),
        "field": PRIME,
