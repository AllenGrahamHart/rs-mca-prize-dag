#!/usr/bin/env python3
"""Stream and independently replay the cell-9 pairing-3 certificate."""
...
PRIMARY = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing3_chart_result"
)
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing3_"
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing3_direct_audit_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-de-pairing3-direct-audit")
image = (
...
@app.function(
    image=image, cpu=1.0, memory=4096, timeout=300, max_containers=6
)
def audit(chart_case):
    ...
    expected_metadata = {
        ...
        "scope": (
            ...
            "pairing-3 compiler for cell-9 xi in {0,2}."
        ),
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-de-pairing3-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 3 and item["xi_index"] in (0, 2)
        ]
        core.require(
    ...
    totals = {
        key: 0 for key in (
            ...
            "missing_sum_nonzero", "uv_checked", "f_rows", "f_boundaries",
            "colored_nonzero", "chart_b_paid", "chart_c_paid",
    ...
    for row in iter_records(manifest):
        ...
        for finite in row["finite_rows"]:
            ...
            if finite["status"] == "TARGET_PRODUCT_BOUNDARY":
                core.require(de == 0 and finite["uv_rows"] == [],
                             "product boundary")
            ...
            u_roots = core.quadratic_roots(
                core.paired_coefficients(a_values, b_values, de, 1),
                "u paired cut")
            v_roots = core.quadratic_roots(
                core.paired_coefficients(
                    a_values, b_values, second_de, sigma_o
                ), "v paired cut")
            core.require(u_roots == finite["u_roots"]
                         and v_roots == finite["v_roots"],
                         "paired quadratic roots")
            uv_rows = {(item["u"], item["v"]): item for item in finite["uv_rows"]}
            core.require(set(uv_rows) == {(u, v) for u in u_roots for v in v_roots},
                         "Cartesian uv replay")
            eta = 1 if xi == 0 else -1
            for (uv, vv), uv_row in uv_rows.items():
                h_value = (
                    de * pow(uv + eta * vv, 2, PRIME)
                    - source_sum * uv * vv
                ) % PRIME
                core.require(h_value == uv_row["h"], "missing-sum replay")
                if h_value:
                    core.require(uv_row["status"] == "MISSING_SUM_NONZERO"
                                 and not uv_row["f_rows"],
                                 "nonzero missing-sum terminal")
                    totals["missing_sum_nonzero"] += 1
                    continue
                totals["uv_checked"] += 1
                local_candidates.add((rv, tv, bv, cv, uv, vv))
                f_squared = uv * vv * pow(de, -1, PRIME) % PRIME
                core.require(uv_row["status"] == "CHECKED"
                             and uv_row["f_squared"] == f_squared,
                             "zero missing-sum terminal")
                expected_f = sorted(
                    int(value) for value in sp.sqrt_mod(
                        f_squared, PRIME, all_roots=True
                    )
                )
                core.require(sorted(item["f"] for item in uv_row["f_rows"])
                             == expected_f, "complete f lift")
                for f_row in uv_row["f_rows"]:
                    ...
                    if fv == 0:
                        core.require(
                            ...
                            "zero-f boundary")
                        local_boundaries.append({**finite, **uv_row, **f_row})
                        totals["f_boundaries"] += 1
                        continue
                    dv = uv * pow(fv, -1, PRIME) % PRIME
                    ev = vv * pow(fv, -1, PRIME) % PRIME
                    colored = core.paired(
                        a_values, b_values, bv * fv % PRIME,
                        sigma_c * cv * fv % PRIME,
                    )
                    core.require(
                        (f_row["d"], f_row["e"], f_row["colored_cut"])
                        == (dv, ev, colored)
                        and colored != 0
        ...
        reported_candidates = {
            tuple(item[name] for name in ("r", "t", "b", "c", "u", "v"))
            for item in row["uv_candidates"]
        }
    ...
    core.require(seen == expected and counts["records"] == 192,
                 "complete Cartesian cover")
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-de-pairing3-"
            "direct-audit-v1"
        ),
        "field": PRIME,
...
def main():
    ...
    summed = (
        ...
        "source_point_count", "route_point_count", "uv_candidate_count",
