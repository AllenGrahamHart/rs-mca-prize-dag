#!/usr/bin/env python3
"""Scout the pinned xi=3 pairing-7 compiler on cell-9 charts."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing7_chart_scout_result.json"
)
FULL_RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing7_chart_result"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-xi3-pairing7-chart-scout")
image = (
...
def main(
    ...
    if full:
        ...
        writer = ShardedResultWriter(full_result, metadata={
            ...
            "scope": (
                ...
                "sign-free "
                "compiler for cell-9 xi=3, pairing=7."
            ),
    ...
    for envelope in evaluate.map(cases, order_outputs=False):
        ...
        rows.append({
            ...
            "xi_index": row["xi_index"], "pairing_index": 7,
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell9-xi3-pairing7-chart-scout-v1",
        "field": PRIME,
