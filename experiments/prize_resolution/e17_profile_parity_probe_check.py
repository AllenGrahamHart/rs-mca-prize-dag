#!/usr/bin/env python3
"""Independent checker for the cutoff-free E17 profile/parity router."""
...
ROOT = HERE.parents[1]
SOURCE = HERE / "e17_profile_parity_probe_modal.py"
...
def recursive_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    def visit(magnitude: int, energy: int, l1_norm: int, counts: list[int]) -> None:
        if magnitude == 5:
            ...
            return
        for count in range((17 - energy) // (magnitude * magnitude) + 1):
...
def independent_diameter_ledgers() -> list[list[int]]:
    ...
    return [[mass, (mass - 85) // 2] for mass in sorted(masses)]


def main() -> None:
    packet = json.loads(RESULT.read_text())
    atlas = json.loads(ATLAS.read_text())
    assert packet["schema"] == "e1-e17-profile-parity-route-probe-v1"
    assert packet["complete"] is True and packet["variance"] == 34 and packet["energy"] == 17
    ...
    trace = [[l1, 83 - 4 * l1, minima[83 - 4 * l1]] for l1 in range(20, 7, -1)]
    assert packet["slack_trace"] == trace
    qualifying = [row for row in trace if row[2] is not None and int(row[2]) <= 17]
    ...
    assert packet["parity_rejected"] == [
        row for row in profiles if int(row["odd_classes"]) > 5
    ]
    ...
    print(
        "E17_PROFILE_PARITY_PROBE_CHECK_PASS "
