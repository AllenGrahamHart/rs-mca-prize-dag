#!/usr/bin/env python3
"""Pilot xi3 pairings 1-2 via q branches and reciprocal-linear cuts."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_pairings1_2_reciprocal_linear_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_pairings1_2_reciprocal_linear_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-xi3-pairings1-2-reciprocal-linear-pilot")
image = (
...
def profile_case(case):
    ...
    epsilon_1, epsilon_2, branch_index, sigma_c, xi_index, pairing_index = case
    if (branch_index not in (0, 1, 2) or pairing_index not in (1, 2) or
            xi_index != 3 or
            (pairing_index == 1 and sigma_c != 0) or
            (pairing_index == 2 and sigma_c not in (-1, 1))):
        raise ValueError(
            "the pilot is scoped to xi=3, pairings 1-2, three q branches"
        )
    ...
    p_next = paired_polynomial(
        ...
        variable_polynomial*next_scale,
    )
    if p_missing.degree() != 4 or p_next.degree() != 2:
        raise ValueError("reciprocal-linear cuts have wrong degrees")
    ...
    if remainder.degree() == 1:
        ...
        p_next_c, p_next_b, p_next_a = p_next.coefficients[:3]
        target_free = (
        ...
        raise ValueError("quartic reduction is identically zero")
    print(json.dumps({
        "phase": "reciprocal_linear_resultant_cut",
        "p_missing_degree": p_missing.degree(),
        "p_next_degree": p_next.degree(),
        "remainder_degree": remainder.degree(),
        "seconds": time.perf_counter()-started,
    }), flush=True)
    ...
    for r_value in sorted(candidate_r_values):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                b_row["a_branch"] = a_branch
                b_row["b_branch"] = b_branch
                if a_branch == 0:
                    if b_branch == 0:
                        b_row["status"] = "FREE_Q_BRANCH"
                        unresolved.append([point, branch_index, "FREE_Q_BRANCH"])
                    else:
                        b_row["status"] = "EMPTY_Q_BRANCH"
                    t_row["b_rows"].append(b_row)
                    continue
                ...
                if same_pair_cut:
                    raise ValueError("q branch does not satisfy paired(q,q)")
                ...
                next_z_field = paired_polynomial_at(
                    -q_value % PRIME, next_scale_value
                )
                missing_z_roots = field_roots(missing_z_field)
                next_z_roots = field_roots(next_z_field)
                if missing_z_roots is None:
                    raise ValueError("monic reciprocal-linear cut vanished")
                z_roots = (
                    missing_z_roots if next_z_roots is None else
                    sorted(set(missing_z_roots) & set(next_z_roots))
                )
                b_row["missing_z_roots"] = missing_z_roots
                b_row["next_z_roots"] = next_z_roots
                b_row["common_z_roots"] = z_roots
                b_row["z_rows"] = []
                for z_value in z_roots:
                    ...
                    if next_pair_cut:
                        raise ValueError("common-z root violates next pair")
                    lanes = (
                    ...
                    for lane_c, lane_o in lanes:
                        ...
                        z_row["lanes"].append(lane_row)
                    z_row["status"] = "CHECKED"
                    b_row["z_rows"].append(z_row)
    ...
    return {
        ...
        "q_branch_profiles": [
            fraction_profile(value) for value in q_record.vector()
        ],
...
def main(
    ...
    if source_census:
        pairing_one = tuple(
            (*epsilon, selected_branch, 0, 3, 1)
            for epsilon in sign_pairs
            for selected_branch in (0, 1, 2)
        )
        pairing_two = tuple(
            (*epsilon, selected_branch, selected_sigma_c, 3, 2)
            for epsilon in sign_pairs
            for selected_branch in (0, 1, 2)
            for selected_sigma_c in (-1, 1)
        )
        cases = pairing_one+pairing_two
    else:
        selected_signs = (
        ...
        cases = tuple(
            (*epsilon, branch_index, sigma_c, 3, pairing_index)
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-xi3-pairings1-2-reciprocal-linear-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-xi3-pairings1-2-reciprocal-linear-pilot-v1"
        ),
        "scope": (
            "Exact three-branch reciprocal-linear resultant and "
            "exceptional-root census at xi=3, pairings 1-2; no claim beyond "
            "the printed cases."
        ),
