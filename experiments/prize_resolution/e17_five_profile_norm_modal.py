#!/usr/bin/env python3
"""Compute dual exact cyclotomic norms for the full E17 residue."""
...
CENSUS = HERE / "e17_five_profile_census_result.json"
RESULT = HERE / "e17_five_profile_norm_result.json"
BATCH_SIZE = 500

app = modal.App("e1-n256-e17-five-profile-norm")
...
def main() -> None:
    ...
    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        packet = {
            "schema": "e1-e17-five-profile-norm-v1",
            "complete": complete,
            "agreement": False,
            "vectors": len(vectors),
    ...
    try:
        ...
        print(
            "E17_FIVE_PROFILE_NORM_INCOMPLETE "
    ...
    if len(flint_norms) != len(vectors) or flint_norms != pari_norms:
        raise RuntimeError("FLINT/PARI E17 norm mismatch")
    ...
    RESULT.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n")
    print("E17_FIVE_PROFILE_NORM " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E17_FIVE_PROFILE_NORM_AGREEMENT {packet['agreement']}")
    print(f"E17_FIVE_PROFILE_NORM_RESULT {RESULT}")
