#!/usr/bin/env python3
"""Solve cell-4 xi3/pairing0 by the reciprocal-square branch cut."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell4_xi3_pairing0_reciprocal_square_result.json"
)
REMOTE_STRUCTURE = "/root/structure.json"
REMOTE_KERNEL = "/root/kernel.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell4-xi3-pairing0-reciprocal-square")
image = (
...
def evaluate_case(case):
    ...
    if branch_index not in (0, 1, 2) or sigma_o not in (-1, 1):
        raise ValueError("scope is xi=3, pairing=0, three q branches")
    ...
    p_o_0, p_o_1, p_o_2 = p_outside.coefficients[:3]
    target_free = (
        (p_m_2*p_o_0 - p_m_0*p_o_2)**2
        - (p_m_2*p_o_1 - p_m_1*p_o_2)
        * (p_m_1*p_o_0 - p_m_0*p_o_1)
    )
    print(json.dumps({
    ...
    for r_value in sorted(candidate_roots):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                missing_y_roots = univariate_roots([
                    1,
                    (2*missing-source_sum) % PRIME,
                    missing*missing % PRIME,
                ])
                outside_y_roots = univariate_roots(
                ...
                for y_value in y_roots:
                    ...
                    d_roots = univariate_roots([-d_squared % PRIME, 0, 1])
                    if d_roots is None:
                    ...
                    for d_value in d_roots:
                        ...
                        d_row = {
                            ...
                            "e": e_value,
                            "f": f_value,
                            "outside_pair_cut": outside_pair_cut,
                            "lanes": [],
                        }
                        for sigma_c in (-1, 1):
    ...
    return {
        ...
        "outside_cut_profiles": [
            [
                fraction_profile(value, include_expression=False)
                for value in coefficient.vector()
            ]
            for coefficient in p_outside.coefficients
        ],
        "missing_cut_degree": p_missing.degree(),
        "outside_cut_degree": p_outside.degree(),
...
def main(cases: str = "-1:-1:0:-1", all_cases: bool = False):
    if all_cases:
        selected = tuple(
    ...
    output = {
        ...
        "field": PRIME,
        "scope": (
            "Exact three-branch reciprocal-square resultant norm and direct "
            "exceptional-root replay for cell 4 at xi=3 and pairing=0. Each "
            "row fixes the outside sign sigma_o and checks both remaining "
            "sigma_c target lanes."
        ),
        ...
        "rows": [
            {
                key: row.get(key) for key in (
                    ...
                    "sigma_o", "status", "timings", "target_norm_root_count",
