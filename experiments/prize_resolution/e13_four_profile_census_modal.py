#!/usr/bin/env python3
"""Run dual complete E13 four-profile censuses on Modal."""
...
PRIMARY = HERE / "e13_four_profile_census.cpp"
AUDIT = HERE / "e13_four_profile_census_audit.cpp"
PROBE = HERE / "e13_profile_parity_probe_result.json"
ATLAS = ROOT / "background/nodes/e1_n256_s16_e27_profile_parity_light_reduction/notes/e27_profile_parity_probe_result.json"
RESULT = HERE / "e13_four_profile_census_result.json"
REMOTE_PRIMARY_SOURCE = "/root/e13_four_profile_census.cpp"
REMOTE_AUDIT_SOURCE = "/root/e13_four_profile_census_audit.cpp"
REMOTE_PRIMARY = "/root/e13_four_profile_census"
REMOTE_AUDIT = "/root/e13_four_profile_census_audit"

app = modal.App("e1-n256-e13-four-profile-census")
...
def run_template(template: int, light: list[int]) -> dict[str, object]:
    ...
    primary = json.loads(subprocess.run(
...
def main() -> None:
    ...
    if (not probe["complete"] or not atlas["complete"] or len(tasks) != 111
            or int(probe["relevant_affine_templates"]) != len(tasks)):
        raise RuntimeError("E13 router or one-diameter atlas mismatch")
    ...
    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        ...
        packet = {
            "schema": "e1-e13-four-profile-census-v1", "complete": complete,
            "completed_templates": len(rows), "expected_templates": len(tasks),
            "error": error,
            ...
            "summary": {
                ...
                "proper_conductor_counts": [profile_counts[i] - full_counts[i]
                                             for i in range(4)],
                "collected_full_conductor": sum(len(row["primary"]["matches"])
                                                for row in rows),
    ...
    try:
        ...
        print(f"E13_FOUR_PROFILE_CENSUS_INCOMPLETE completed={len(rows)}/{len(tasks)}")
        raise
    ...
    print("E13_FOUR_PROFILE_CENSUS " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E13_FOUR_PROFILE_CENSUS_COMPLETE {packet['complete']}")
    print(f"E13_FOUR_PROFILE_CENSUS_RESULT {RESULT}")
