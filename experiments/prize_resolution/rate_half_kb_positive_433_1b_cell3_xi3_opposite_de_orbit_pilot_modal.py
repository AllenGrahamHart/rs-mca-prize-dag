#!/usr/bin/env python3
"""Pilot the xi3 opposite-DE matching orbit via nested resultants."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_opposite_de_orbit_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_opposite_de_orbit_census_result.json"
PAIRING3_CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_opposite_de_pairing3_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-xi3-opposite-de-orbit-pilot")
image = (
...
def profile_case(case):
    ...
    if (pairing_index not in (3, 4, 5) or xi_index != 3 or
            (pairing_index == 3 and
             (sigma_c != 0 or sigma_o not in (-1, 1))) or
            (pairing_index == 4 and (sigma_c != 0 or sigma_o != 0)) or
            (pairing_index == 5 and
             (sigma_c not in (-1, 1) or sigma_o != 0))):
        raise ValueError(
            "the pilot is scoped to xi=3 opposite-DE representatives 3-5"
        )
    ...
    q_variable = PairPolynomial(0, 1)
    q_cut = paired_polynomial(q_variable, -q_variable)
    if q_cut.degree() != 4:
        raise ValueError("opposite-DE cut is not quartic")
    if any(q_cut.coefficients[index] != Pair() for index in (1, 3)):
        raise ValueError("opposite-DE cut is not even")
    q_leading_inverse = Pair(1)/q_cut.coefficients[4]

    def cached_remainder(value, divisor):
        divisor_degree = divisor.degree()
        work = list(value.coefficients)
        while True:
            degree = next((
    ...
    def q_reduce(value):
        return cached_remainder(value, q_cut)

    def q_multiply(*values):
        output = PairPolynomial(1)
        for value in values:
            output = q_reduce(output*value)
        return output

    # F(q,y) is quadratic in y. Recover its q-polynomial coefficients from
    # the exact values at y=0,+1,-1, avoiding a nested polynomial class.
    paired_zero = paired_polynomial(q_variable, PairPolynomial(0))
    paired_plus = paired_polynomial(q_variable, PairPolynomial(1))
    paired_minus = paired_polynomial(q_variable, PairPolynomial(-1))
    half = Pair(1)/Pair(2)
    paired_constant = q_reduce(paired_zero)
    paired_linear = q_reduce((paired_plus-paired_minus)*half)
    paired_quadratic = q_reduce((
        paired_plus+paired_minus-2*paired_zero
    )*half)

    missing_second = 2*missing_record-source_sum_record
    missing_fourth = missing_record**2
    if pairing_index == 3:
        p_missing_degree = 2
        next_scale = q_variable*(sigma_o*missing_record)
        p_next_coefficients = (
            paired_constant,
            q_multiply(paired_linear, next_scale),
            q_multiply(paired_quadratic, next_scale, next_scale),
        )
        # Resultant of A y^2+B y+C and D y^2+E y+F.
        c_left, b_left, a_left = (
            PairPolynomial(1),
            PairPolynomial(missing_second),
            PairPolynomial(missing_fourth),
        )
        c_right, b_right, a_right = p_next_coefficients
        af_cd = q_reduce(
            q_multiply(a_left, c_right)-q_multiply(c_left, a_right)
        )
        ae_bd = q_reduce(
            q_multiply(a_left, b_right)-q_multiply(b_left, a_right)
        )
        bf_ce = q_reduce(
            q_multiply(b_left, c_right)-q_multiply(c_left, b_right)
        )
        inner_cut = q_reduce(
            q_multiply(af_cd, af_cd)-q_multiply(ae_bd, bf_ce)
        )
    else:
        p_missing_degree = 4
        next_scale = PairPolynomial(
            common_b*missing_record if pairing_index == 4 else
        ...
        c_next, b_next, a_next = p_next_coefficients
        # Clear a_next^3 after reducing the even quartic missing relation
        # modulo a_next*z^2+b_next*z+c_next.
        remainder_linear = q_reduce(
            (
                -q_multiply(b_next, b_next, b_next)
                + 2*q_multiply(a_next, b_next, c_next)
            )*missing_fourth
            - q_multiply(b_next, a_next, a_next)*missing_second
        )
        remainder_constant = q_reduce(
            (
                -q_multiply(b_next, b_next, c_next)
                + q_multiply(a_next, c_next, c_next)
            )*missing_fourth
            - q_multiply(c_next, a_next, a_next)*missing_second
            + q_multiply(a_next, a_next, a_next)
        )
        inner_cut = q_reduce(
            q_multiply(c_next, remainder_linear, remainder_linear)
            - q_multiply(
                b_next, remainder_constant, remainder_linear
            )
            + q_multiply(a_next, remainder_constant, remainder_constant)
        )
    if p_next_coefficients[2].degree() < 0:
        raise ValueError("next paired cut is not quadratic")
    q_remainder = q_reduce(inner_cut)
    if q_remainder.degree() < 0:
        raise ValueError("inner cut vanishes modulo the opposite-DE cut")

    def coefficient(value, index):
        return (
            value.coefficients[index]
            if index < len(value.coefficients) else Pair()
        )

    # Write the reduced cut as E(x)+q O(x), x=q^2. Existence of a common q
    # forces E(x)^2-x O(x)^2=0, while the opposite-DE cut is quadratic in x.
    x_cut = PairPolynomial(
        coefficient(q_cut, 0), coefficient(q_cut, 2), coefficient(q_cut, 4)
    )
    even_part = PairPolynomial(
        coefficient(q_remainder, 0), coefficient(q_remainder, 2)
    )
    odd_part = PairPolynomial(
        coefficient(q_remainder, 1), coefficient(q_remainder, 3)
    )
    x_variable = PairPolynomial(0, 1)
    parity_condition = even_part**2-x_variable*odd_part**2
    x_remainder = cached_remainder(parity_condition, x_cut)
    if x_remainder.degree() < 0:
        raise ValueError("parity condition vanishes modulo the x cut")
    if x_remainder.degree() == 0:
        target_free = x_remainder.coefficients[0]
    elif x_remainder.degree() == 1:
        remainder_constant, remainder_linear = x_remainder.coefficients
        x_constant, x_linear, x_quadratic = x_cut.coefficients
        target_free = (
            x_constant*remainder_linear**2
            - x_linear*remainder_constant*remainder_linear
            + x_quadratic*remainder_constant**2
        )
    else:
        raise ValueError("x remainder is not linear")
    if target_free == Pair():
        raise ValueError("nested x cut vanished")
    print(json.dumps({
        "phase": "opposite_de_nested_resultant_cut",
        "q_cut_degree": q_cut.degree(),
        "p_missing_degree": p_missing_degree,
        "p_next_degree": 2,
        "inner_q_degree": inner_cut.degree(),
        "q_remainder_degree": q_remainder.degree(),
        "parity_condition_degree": parity_condition.degree(),
        "x_remainder_degree": x_remainder.degree(),
        "seconds": time.perf_counter()-started,
    }), flush=True)
    ...
    def opposite_q_polynomial_at():
        q_field = polynomial_context([0, 1])
        p0, p1, p2 = (
            polynomial_context([b_value])-q_field*a_value
            for a_value, b_value in zip(a_values, b_values)
        )
        q0 = polynomial_context([b_values[0]])+q_field*a_values[0]
        q1 = polynomial_context([-b_values[1]])-q_field*a_values[1]
        q2 = polynomial_context([b_values[2]])+q_field*a_values[2]
        return (p2*q0-p0*q2)**2-(p2*q1-p1*q2)*(p1*q0-p0*q1)
    ...
    for r_value in sorted(candidate_r_values):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                b_row["opposite_q_roots"] = q_roots
                b_row["q_rows"] = []
                if q_roots is None:
                    b_row["status"] = "FREE_OPPOSITE_Q"
                    unresolved.append([point, "FREE_OPPOSITE_Q"])
                    t_row["b_rows"].append(b_row)
                    continue
                for q_value in q_roots:
                    opposite_pair_cut = paired_value_at(q_value, -q_value % PRIME)
                    if opposite_pair_cut:
                        raise ValueError("opposite-q root violates paired(q,-q)")
                    q_row = {
                        "q": q_value,
                        "opposite_pair_cut": opposite_pair_cut,
                        "root_rows": [],
                    }
                    reconstruction_rows = []
                    if pairing_index == 3:
                        missing_root_field = polynomial_context([
                            1,
                            (2*source_missing-source_sum_value) % PRIME,
                            source_missing*source_missing % PRIME,
                        ])
                        next_root_field = paired_polynomial_at(
                            q_value,
                            sigma_o*q_value*source_missing % PRIME,
                        )
                        missing_roots = field_roots(missing_root_field)
                        next_roots = field_roots(next_root_field)
                        if missing_roots is None:
                            raise ValueError("monic reciprocal-square cut vanished")
                        common_roots = (
                            missing_roots if next_roots is None else
                        ...
                        for y_value in common_roots:
                            if y_value == 0:
                                raise ValueError("reciprocal-square root cannot vanish")
                            d_square = pow(y_value, -1, PRIME)
                            d_roots = field_roots(polynomial_context([
                                -d_square % PRIME, 0, 1,
                            ]))
                            if d_roots is None:
                                raise ValueError("square-lift polynomial vanished")
                            for d_value in d_roots:
                                reconstruction_rows.append({
                                    "root": y_value,
                                    "z": pow(d_value, -1, PRIME),
                                    "d": d_value,
                                })
                    else:
                        missing_root_field = polynomial_context([
                            1, 0,
                            (2*source_missing-source_sum_value) % PRIME, 0,
                            source_missing*source_missing % PRIME,
                        ])
                        next_scale_value = (
                            b_value*source_missing % PRIME
                            if pairing_index == 4 else
                            sigma_c*c_value*source_missing % PRIME
                        )
                        next_root_field = paired_polynomial_at(
                            q_value, next_scale_value
                        )
                        missing_roots = field_roots(missing_root_field)
                        next_roots = field_roots(next_root_field)
                        if missing_roots is None:
                            raise ValueError("monic reciprocal-linear cut vanished")
                        common_roots = (
                            missing_roots if next_roots is None else
                            sorted(set(missing_roots) & set(next_roots))
                        )
                        ...
                        for z_value in common_roots:
                            if z_value == 0:
                                raise ValueError("reciprocal-linear root cannot vanish")
                            reconstruction_rows.append({
                                "root": z_value,
                                "z": z_value,
                                "d": pow(z_value, -1, PRIME),
                            })

                    for reconstruction in reconstruction_rows:
                        z_value = reconstruction["z"]
                        d_value = reconstruction["d"]
                        e_value = q_value*z_value % PRIME
                        f_value = source_missing*z_value % PRIME
                        relation_value = (
                            1
                            + (2*source_missing-source_sum_value)*z_value*z_value
                            + source_missing*source_missing*pow(z_value, 4, PRIME)
                        ) % PRIME
                        if relation_value:
                        ...
                        next_pair_cut = paired_value_at(
                            q_value,
                            (
                                sigma_o*e_value*f_value % PRIME
                                if pairing_index == 3 else
                        ...
                        if next_pair_cut:
                            raise ValueError("common root violates next pair")
                        lanes = (
                        ...
                        for lane_c, lane_o in lanes:
                            ...
                            if final_pair_cut:
                                ...
                                root_row["lanes"].append(lane_row)
                                continue
                            representatives = (
                                1, b_value, c_value,
                            ...
                            equation_values = [
                                ...
                                (d_value*f_value-source_missing) % PRIME,
                                (pow(d_value+f_value, 2, PRIME)-source_sum_value)
                                % PRIME,
                                opposite_pair_cut,
                                next_pair_cut,
                                final_pair_cut,
                            ...
                            record = {**candidate, **lane_row}
                            final_pair_solutions.append(record)
                            (
                                boundary_solutions
                        ...
                        q_row["root_rows"].append(root_row)
                    q_row["status"] = "CHECKED"
                    b_row["q_rows"].append(q_row)
    ...
    print(json.dumps({
        ...
        "final_pair_solutions": len(final_pair_solutions),
    ...
    return {
        ...
        "target_lanes_covered": (
            ...
            if pairing_index == 3 else
            [[lane_c, lane_o]
             for lane_c in (-1, 1) for lane_o in (-1, 1)]
            if pairing_index == 4 else
            [[sigma_c, lane_o] for lane_o in (-1, 1)]
        ),
        ...
        "q_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in q_cut.coefficients
        ],
        "inner_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in inner_cut.coefficients
        ],
        "q_remainder_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in q_remainder.coefficients
        ],
        "x_cut_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in x_cut.coefficients
        ],
        "x_remainder_profiles": [
            [fraction_profile(value) for value in coefficient.vector()]
            for coefficient in x_remainder.coefficients
        ],
...
def main(
    ...
    pairing_three_census: bool = False,
):
    sign_pairs = ((-1, -1), (-1, 1), (1, -1), (1, 1))
    if source_census and pairing_three_census:
        raise ValueError("choose one census mode")
    if pairing_three_census:
        cases = tuple(
            (*epsilon, 0, selected_sigma_o, 3, 3)
        ...
        cases = pairing_three+pairing_four+pairing_five
    else:
        selected_signs = (
        ...
        cases = tuple(
            (*epsilon, sigma_c, sigma_o, 3, pairing_index)
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-xi3-opposite-de-pairing3-source-census-v1"
            if pairing_three_census else
            "rate-half-kb-positive-433-1b-cell3-xi3-opposite-de-orbit-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-xi3-opposite-de-orbit-pilot-v1"
        ),
        "scope": (
            "Exact opposite-DE nested-resultant and exceptional-root census "
            "at xi=3, representative pairing 3; no claim beyond the printed "
            "cases."
            if pairing_three_census else
            "Exact opposite-DE nested-resultant and exceptional-root census "
            "at xi=3, representative pairings 3-5; no claim beyond the "
            "printed cases."
        ),
    ...
    output_path = (
        PAIRING3_CENSUS_RESULT if pairing_three_census else
        CENSUS_RESULT if source_census else RESULT
    )
        ...
        "rows": [
            {
                ...
                "direct_lift": {
                    ...
                    for key in (
                        ...
                        "z_candidate_count", "final_pair_solution_count",
