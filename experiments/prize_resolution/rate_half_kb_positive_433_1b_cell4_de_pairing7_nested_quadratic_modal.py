#!/usr/bin/env python3
"""Solve cell-4 parallel-DE omissions at canonical pairing 7."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell4_de_pairing7_nested_quadratic_result.json"
)
REMOTE_STRUCTURE = "/root/structure.json"
REMOTE_KERNEL = "/root/kernel.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell4-de-pairing7-nested-quadratic")
image = (
...
def evaluate_case(case):
    ...
    if xi_index not in (0, 2):
        raise ValueError("scope is xi in {0,2}, pairing=7")
...
def main(cases: str = "-1:-1:-1:-1:0", all_cases: bool = False):
    ...
    for case, row in zip(selected, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell4-de-pairing7-nested-quadratic-v1",
        "field": PRIME,
        "scope": (
            "Exact nested-quadratic norm and direct finite replay for cell 4, "
            "xi in {0,2}, pairing 7; xi1 transports from xi0."
        ),
