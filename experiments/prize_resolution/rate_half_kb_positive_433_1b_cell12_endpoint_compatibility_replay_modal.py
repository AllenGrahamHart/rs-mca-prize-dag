#!/usr/bin/env python3
"""Replay deployed roots of the cell-12 endpoint compatibility cuts."""
...
PILOT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell12_endpoint_compatibility_pilot_result.json"
)
TOWER = DIRECTORY / (
...
app = modal.App("rs-mca-positive-433-1b-cell12-endpoint-replay")
image = (
...
def main():
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell12-endpoint-replay-v1",
        "field": PRIME,
        "scope": (
            "Complete deployed-root replay of cell-12 source-only endpoint "
