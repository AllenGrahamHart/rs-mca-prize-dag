#!/usr/bin/env python3
"""Independent checker for the exact E18 profile/parity router."""
...
ROOT = HERE.parents[1]
SOURCE = HERE / "e18_profile_parity_probe_modal.py"
...
def recursive_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    def visit(magnitude: int, energy: int, l1_norm: int, counts: list[int]) -> None:
        if magnitude == 5:
            ...
            return
        for count in range((18 - energy) // (magnitude * magnitude) + 1):
...
def independent_matching_ledgers(light_diameters: int) -> list[list[int]]:
    ...
    return [[mass, (mass - 84) // 2] for mass in sorted(masses)]


def main() -> None:
    packet = json.loads(RESULT.read_text())
    atlas_source = json.loads(ATLAS.read_text())
    assert packet["schema"] == "e1-e18-profile-parity-route-probe-v1"
    assert packet["complete"] is True and packet["variance"] == 36
    assert packet["energy"] == 18 and packet["majorant_filter"] is None
    ...
    trace = [[l1, 84 - 4 * l1, minima[84 - 4 * l1]] for l1 in range(18, 8, -1)]
    assert packet["slack_trace"] == trace
    qualifying = [row for row in trace if row[2] is not None and int(row[2]) <= 18]
    ...
    assert packet["parity_rejected"] == rejected
    ...
    mutated = [row for row in profiles if int(row["odd_classes"]) <= 4]
    assert mutated != survivors
    print(
        "E18_PROFILE_PARITY_PROBE_CHECK_PASS "
