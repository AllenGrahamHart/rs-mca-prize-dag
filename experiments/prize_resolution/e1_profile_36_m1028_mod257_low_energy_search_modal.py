#!/usr/bin/env python3
"""Search the exact mod-257 cofactor chamber for low-energy profile-(3,6) vectors."""
...
app = modal.App("e1-profile-36-m1028-mod257-low-energy-search")
...
def reparameterize(
    ...
    singletons = [(position, value) for position, value in state.items() if abs(value) == 1]
    ...
    heavies = {position: value for position, value in state.items() if abs(value) == 2}
...
def mutate(
    ...
    mutate_heavy = rng.random() < 3 / 8
    selected = candidate_heavies if mutate_heavy else candidate_singletons
    ...
    if new in occupied:
        new = rng.choice([position for position in range(128) if position not in occupied])
    selected[new] = coefficient
    return candidate_singletons, candidate_heavies


@app.function(image=image, cpu=1.0, memory=256, timeout=75, max_containers=16)
...
def main(shards: int = 16, seconds: float = 60.0) -> None:
    ...
    print(
        "E1_PROFILE_36_M1028_MOD257_LOW_ENERGY_SEARCH_DONE "
        f"shards={shards} seconds={seconds} found={found} "
        f"best_E={rows[0]['energy']} iterations={total_iterations}"
    )
