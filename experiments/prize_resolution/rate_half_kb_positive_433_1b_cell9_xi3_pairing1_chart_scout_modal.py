#!/usr/bin/env python3
"""Scout the pinned xi=3 pairing-1 compiler on cell-9 charts."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing1_chart_scout_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-xi3-pairing1-chart-scout")
image = (
...
def _evaluate_case(case):
    ...
    epsilon_1, epsilon_2, branch_index, b_index, c_index = case
...
def main(
    ...
    for envelope in evaluate.map(cases, order_outputs=False):
        ...
        if "error" in envelope:
            error = {
                ...
                "sigma_c_anchor": 0, "xi_index": 3,
                "pairing_index": 1, "b_row_index": case[3],
                "c_row_index": case[4], "status": "REMOTE_ERROR",
        ...
        rows.append({
            ...
            "xi_index": row["xi_index"], "pairing_index": 1,
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell9-xi3-pairing1-chart-scout-v1",
        "field": PRIME,
    ...
    if writer is not None:
        expected_rows = (
            ...
            else 1 if recovery_index >= 0 else 72
        )
