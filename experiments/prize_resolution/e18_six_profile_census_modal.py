#!/usr/bin/env python3
"""Run dual complete E18 six-profile censuses on Modal."""
...
REMOTE_PRIMARY_SOURCE = "/root/e18_six_profile_census.cpp"
REMOTE_AUDIT_SOURCE = "/root/e18_six_profile_census_audit.cpp"
REMOTE_PRIMARY = "/root/e18_six_profile_census"
REMOTE_AUDIT = "/root/e18_six_profile_census_audit"

app = modal.App("e1-n256-e18-six-profile-census")
image = (
...
def atlas_tasks() -> list[list[int]]:
    ...
    if not two["complete"] or not six["complete"]:
        raise RuntimeError("an E18 light atlas is incomplete")
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("E18 router is incomplete or changed")
    ...
    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        ...
        packet = {
            "schema": "e1-e18-six-profile-census-v1",
            "complete": complete,
            ...
            "summary": {
                ...
                "proper_conductor_counts": [profile[i] - full[i] for i in range(6)],
    ...
    try:
        ...
        print(f"E18_SIX_PROFILE_CENSUS_INCOMPLETE completed={len(rows)}/{len(tasks)}")
        raise
    ...
    print("E18_SIX_PROFILE_CENSUS " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E18_SIX_PROFILE_CENSUS_COMPLETE {packet['complete']}")
    print(f"E18_SIX_PROFILE_CENSUS_RESULT {RESULT}")
