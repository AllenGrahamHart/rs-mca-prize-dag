#!/usr/bin/env python3
"""Independent checker for the exact E20 profile/parity router."""
...
ROOT = HERE.parents[1]
SOURCE = HERE / "e20_profile_parity_probe_modal.py"
...
def recursive_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    def visit(magnitude: int, energy: int, l1_norm: int, counts: list[int]) -> None:
        if magnitude == 5:
            ...
            return
        for count in range((20 - energy) // (magnitude * magnitude) + 1):
...
def main() -> None:
    ...
    assert packet["schema"] == "e1-e20-profile-parity-route-probe-v1"
    assert packet["complete"] is True and packet["variance"] == 40 and packet["energy"] == 20
    ...
    minima = independent_relaxed_minima(52)
    expected_trace = [[l1, 86 - 4 * l1, minima[86 - 4 * l1]] for l1 in range(20, 8, -1)]
    assert packet["slack_trace"] == expected_trace
    qualifying = [
        row for row in expected_trace if row[2] is not None and int(row[2]) <= 20
    ]
    ...
    print(
        "E20_PROFILE_PARITY_PROBE_CHECK_PASS "
