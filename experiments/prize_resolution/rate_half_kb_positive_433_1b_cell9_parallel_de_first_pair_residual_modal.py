#!/usr/bin/env python3
"""Census residual targets for cell-9 first-pair parallel-DE survivors."""
...
app = modal.App("rs-mca-positive-433-1b-cell9-parallel-de-residual")
image = (
...
def decide(case):
    ...
    expected = 4 if cut_kind == "positive" else 2
    ...
    for point_index, point in enumerate(points):
        ...
        de_value = missing if cut_kind == "positive" else -missing
        for pairing_index, matching in enumerate(MATCHINGS):
            equations = [
                d*e-de_value,
                (d+eta*e)**2-source_sum,
            ]
            equations.extend(
...
"""
    try:
        ...
        return {
            ...
            "status": "TIMEOUT", "partial_stdout": stdout[-3000:],
...
def main(limit: int = 0):
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell9-parallel-de-residual-v1",
        "field": PRIME,
        "scope": (
            "Exact guarded residual census for xi=0 and xi=2, first-pair "
            "matchings 0,1,2, at every ordinary surviving source point."
        ),
