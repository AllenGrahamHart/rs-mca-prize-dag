#!/usr/bin/env python3
"""Build the exact z2=z5 collapsed O0b FFI boundary system."""
...
def build(packet_row, basis_row, compiler_core):
    ...
    chart_definitions = (
        "poly x4p=m4p0+m4p1*u4;",
        "poly x4q=m4q0+m4q1*u4;",
        "poly x5p=m5p0+m5p1*u5;",
        "poly x5q=m5q0+m5q1*u5;",
    )
    generators = (
        *(f"g{index}" for index in range(len(basis_row["basis"]))),
        *(f"l{index}" for index in range(8)),
        "q3", "q7", "x4p", "x4q", "x5p", "x5q", "rb",
    )
    program = f"""
...
"""
    return {
        ...
        "finite_chart_equation_count": len(chart_definitions),
...
if __name__ == "__main__":
    require(collapse_kernel("k0+k2+k5+k7") == "z0+0+0+z7",
            "kernel collapse")
    print("RATE_HALF_KB_POSITIVE_433_1B_O0B_FFI_COLLAPSED_PROGRAM_PASS "
          "variables=16 generators=36")
