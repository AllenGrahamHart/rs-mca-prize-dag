#!/usr/bin/env python3
"""Directly replay every candidate from the cell-5 parallel-DE norms."""
...
NORM = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell5_parallel_de_four_basis_norm_result.json"
)
TOWER = DIRECTORY / (
...
app = modal.App("rs-mca-positive-433-1b-cell5-parallel-de-replay")
image = (
...
def main():
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell5-parallel-de-replay-v1",
        "field": PRIME,
        "scope": (
            "Complete deployed direct replay of every four-basis norm and "
            "inverse candidate for the cell-5 first-pair parallel-DE cuts."
        ),
