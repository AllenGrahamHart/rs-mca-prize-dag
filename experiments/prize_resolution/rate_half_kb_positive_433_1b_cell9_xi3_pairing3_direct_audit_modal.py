#!/usr/bin/env python3
"""Replay the cell-9 xi=3 pairing-3 certificate independently."""
...
PRIMARY = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing3_chart_result"
)
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing3_"
...
app = modal.App("rs-mca-positive-433-1b-cell9-xi3-pairing3-direct-audit")
image = (
...
def audit(chart_case):
    ...
    expected_metadata = {
        ...
        "scope": (
            ...
            "compiler for cell-9 xi=3, pairing=3."
        ),
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-xi3-pairing3-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 3 and item["xi_index"] == 3
        ]
        core.require(
            row["status"] == "COMPLETE"
    ...
    for row in iter_records(manifest):
        ...
        for finite in row["finite_rows"]:
            ...
            core.require(
                ...
                "reciprocal-square root replay",
            )
            totals["common_z_roots"] += len(common_z)
            reported_z = {item["z"]: item for item in finite["z_rows"]}
            core.require(set(reported_z) == set(common_z), "complete z ledger")
            for z_value in common_z:
                ...
                core.require(
                    (d_row["d"], d_row["f"]) == (d_value, f_value),
                    "d/f replay",
                )
                ...
                for sigma_o in (-1, 1):
                    ...
                    core.require(
                        ...
                        and lane["outside_q_roots"] == outside_roots
                        and lane["common_q_roots"] == common_q
                        and common_q == []
                        and lane["q_rows"] == [],
                        "empty q intersection",
                    )
                    totals["lane_records"] += 1
    ...
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-xi3-pairing3-direct-audit-v1"
        ),
        "field": PRIME,
