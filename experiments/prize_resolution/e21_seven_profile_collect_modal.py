#!/usr/bin/env python3
"""Collect the full-conductor E21 residue with two exact Modal engines."""
...
PRIMARY_SOURCE = HERE / "e21_seven_profile_collect.cpp"
AUDIT_SOURCE = HERE / "e21_seven_profile_collect_audit.cpp"
...
REMOTE_PRIMARY_SOURCE = "/root/e21_seven_profile_collect.cpp"
REMOTE_AUDIT_SOURCE = "/root/e21_seven_profile_collect_audit.cpp"
REMOTE_PRIMARY = "/root/e21_seven_profile_collect"
REMOTE_AUDIT = "/root/e21_seven_profile_collect_audit"

app = modal.App("e1-n256-e21-seven-profile-collect")
image = (
...
def main() -> None:
    ...
    if (
        not probe["complete"]
        or not count["complete"]
        or not atlas["complete"]
        or len(tasks) != 111
    ):
        raise RuntimeError("E21 router, count packet, or atlas is incomplete")
    ...
    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        packet = {
            "schema": "e1-e21-seven-profile-collect-v1",
            "complete": complete,
    ...
    try:
        ...
        print(f"E21_SEVEN_PROFILE_COLLECT_INCOMPLETE completed={len(rows)}/{len(tasks)}")
        raise
    ...
    print("E21_SEVEN_PROFILE_COLLECT " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E21_SEVEN_PROFILE_COLLECT_COMPLETE {packet['complete']}")
    print(f"E21_SEVEN_PROFILE_COLLECT_RESULT {RESULT}")
