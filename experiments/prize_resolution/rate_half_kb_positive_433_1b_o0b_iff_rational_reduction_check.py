#!/usr/bin/env python3
"""Outcome-neutral checker for the O0b IFF rational reduction."""
...
PROGRAM = HERE / "rate_half_kb_positive_433_1b_o0b_iff_rational_reduction_program.py"
COLLAPSED_UNIT = HERE / "rate_half_kb_positive_433_1b_o0b_collapsed_common_admissible_result.json"
RESULT = HERE / "rate_half_kb_positive_433_1b_o0b_iff_rational_reduction_result.json"
...
COLLAPSED_UNIT_SHA256 = "38a44a30aa3421a67161acf5268d4bbfbe9e33903547e50259fc3f0da77efd03"
...
def verify(payload=None):
    ...
    require(hashlib.sha256(BASIS.read_bytes()).hexdigest() == BASIS_SHA256,
            "basis custody")
    require(hashlib.sha256(COLLAPSED_UNIT.read_bytes()).hexdigest() ==
            COLLAPSED_UNIT_SHA256, "collapsed-unit custody")
    payload = payload or json.loads(RESULT.read_text())
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-o0b-iff-rational-reduction-v1",
            "schema")
    require(payload["collection_complete"] is True and
            ...
            payload["source_collapsed_unit_sha256"] == COLLAPSED_UNIT_SHA256 and
    ...
    require(row["relation"] == "necessary IFF rational-reduction superset" and
            row["variable_count"] == 4 and row["common_basis_size"] == 21 and
            row["outside_equation_order"] == [7, 5, 6] and
            row["route_guard_count"] == 16 and
            row["extra_guards"] == ["k2", "k5", "a2m"] and
    ...
    require(row["stderr"] == "" and "END" in row["stdout"] and
            ...
            [stage["equation"] for stage in row["equation_stages"]] == [7, 5, 6] and
            len(row["route_stages"]) == 16 and
            len(row["extra_stages"]) == 3,
...
def hostile_audit():
    ...
    mutation = deepcopy(payload)
    mutation["row"]["outside_equation_order"] = [7, 5]
    expect_rejected(mutation, "missing q6")
    mutation = deepcopy(payload)
    mutation["row"]["extra_guards"] = ["k2", "a2m"]
    expect_rejected(mutation, "missing k5 denominator")
    return 3
...
def main():
    ...
    print("RATE_HALF_KB_POSITIVE_433_1B_O0B_IFF_RATIONAL_REDUCTION_CHECK_PASS "
