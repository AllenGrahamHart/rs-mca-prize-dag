#!/usr/bin/env python3
"""Independent checker for the cutoff-free E15 profile/parity router."""
...
ROOT = HERE.parents[1]
SOURCE = HERE / "e15_profile_parity_probe_modal.py"
...
def recursive_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    def visit(magnitude: int, energy: int, l1_norm: int, counts: list[int]) -> None:
        if magnitude == 5:
            ...
            return
        for count in range((15 - energy) // (magnitude * magnitude) + 1):
...
def independent_diameter_ledgers() -> list[list[int]]:
    ...
    return [[mass, (mass - 87) // 2] for mass in sorted(masses)]


def main() -> None:
    packet = json.loads(RESULT.read_text())
    atlas = json.loads(ATLAS.read_text())
    assert packet["schema"] == "e1-e15-profile-parity-route-probe-v1"
    assert packet["complete"] is True and packet["variance"] == 30 and packet["energy"] == 15
    ...
    trace = [[l1, 81 - 4 * l1, minima[81 - 4 * l1]] for l1 in range(15, 7, -1)]
    assert packet["slack_trace"] == trace
    qualifying = [row for row in trace if row[2] is not None and int(row[2]) <= 15]
    ...
    print(
        "E15_PROFILE_PARITY_PROBE_CHECK_PASS "
