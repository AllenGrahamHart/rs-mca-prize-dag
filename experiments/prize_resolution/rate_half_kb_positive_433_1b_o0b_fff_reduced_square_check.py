#!/usr/bin/env python3
"""Outcome-neutral checker for the normal-form-first FFF subsystem."""
...
SQUARE = HERE / "rate_half_kb_positive_433_1b_o0b_fff_square_subsystem_program.py"
PROGRAM = HERE / "rate_half_kb_positive_433_1b_o0b_fff_reduced_square_program.py"
SQUARE_TIMEOUT = HERE / "rate_half_kb_positive_433_1b_o0b_fff_square_subsystem_result.json"
RESULT = HERE / "rate_half_kb_positive_433_1b_o0b_fff_reduced_square_result.json"
...
SQUARE_SHA256 = "c5eca188068083699e94ba321858710f5225f423380a71821f9cea90135c4e72"
SQUARE_TIMEOUT_SHA256 = "cefc9fc49863ab0d20291c7cc009553bc45b8eb2946550c97c3daca154b595af"
...
def verify(payload=None):
    ...
    require(hashlib.sha256(BASIS.read_bytes()).hexdigest() == BASIS_SHA256,
            "basis custody")
    require(hashlib.sha256(SQUARE.read_bytes()).hexdigest() == SQUARE_SHA256,
            "square-core custody")
    require(hashlib.sha256(SQUARE_TIMEOUT.read_bytes()).hexdigest() ==
            SQUARE_TIMEOUT_SHA256, "square-timeout custody")
    payload = payload or json.loads(RESULT.read_text())
    require(payload["schema"] ==
            "rate-half-kb-positive-433-1b-o0b-fff-reduced-square-v1",
            "schema")
    require(payload["collection_complete"] is True and
            ...
            payload["source_square_sha256"] == SQUARE_SHA256 and
            payload["source_square_timeout_sha256"] == SQUARE_TIMEOUT_SHA256 and
    ...
    require(row["relation"] ==
            "necessary FFF reduced square-subsystem superset" and
            row["source_square_relation"] ==
            "necessary FFF square-subsystem superset" and
            row["omitted_finite_pair"] == "q4" and
            row["outside_equation_order"] == [5, 7, 6] and
            row["normal_form_order"] == [5, 7, 6] and
            row["variable_count"] == 6 and row["common_basis_size"] == 21 and
    ...
    require(row["stderr"] == "" and "END" in row["stdout"] and
            ...
            [stage["equation"] for stage in row["normal_stages"]] == [5, 7, 6] and
            [stage["equation"] for stage in row["equation_stages"]] == [5, 7, 6] and
            len(row["route_stages"]) == 16 and len(row["extra_stages"]) == 4,
...
def hostile_audit():
    ...
    mutation = deepcopy(payload)
    mutation["row"]["normal_form_order"] = [7, 5, 6]
    expect_rejected(mutation, "wrong normal-form order")
    mutation = deepcopy(payload)
...
def main():
    ...
    print("RATE_HALF_KB_POSITIVE_433_1B_O0B_FFF_REDUCED_SQUARE_CHECK_PASS "
