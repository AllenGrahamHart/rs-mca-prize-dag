#!/usr/bin/env python3
"""Replay the cell-9 xi=3 pairing-2 certificate independently."""
...
PRIMARY = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing2_chart_result"
)
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing2_"
...
app = modal.App("rs-mca-positive-433-1b-cell9-xi3-pairing2-direct-audit")
image = (
...
def audit(chart_case):
    ...
    expected_metadata = {
        "field": PRIME,
        "scope": (
            "Exact six-chart execution of the pinned reciprocal-linear "
            "compiler for cell-9 xi=3, pairing=2."
        ),
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-xi3-pairing2-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 2 and item["xi_index"] == 3
        ]
        core.require(
            row["status"] == "COMPLETE"
    ...
    expected = {
        (epsilon, branch_index, sigma_c_anchor, selected_b, selected_c)
    ...
    for row in iter_records(manifest):
        ...
        seen.add(key)
        epsilon, branch_index, sigma_c_anchor, b_index, c_index = key
        ...
        core.require(
            ...
            and row["sigma_c_anchor"] == sigma_c_anchor
            and row["target_lanes_covered"]
            == [[sigma_c_anchor, -1], [sigma_c_anchor, 1]]
        ...
        for finite in row["finite_rows"]:
            ...
            if finite["status"] == "TARGET_PRODUCT_BOUNDARY":
                core.require(
                    missing == 0 and finite["z_rows"] == [],
                    "product boundary",
                )
                local_boundaries.add(
                    (rv, tv, bv, cv, branch_index, finite["status"])
                )
            ...
            if finite["status"] == "EMPTY_Q_BRANCH":
                core.require(
                    a_branch == 0 and b_branch != 0 and finite["z_rows"] == [],
                    "empty q branch",
                )
            ...
            next_coefficients = core.paired_coefficients(
                ...
                b_values,
                -q_value % PRIME,
                sigma_c_anchor * cv * missing % PRIME,
            )
            next_roots = (
                None
                if not any(next_coefficients)
            ...
            totals["common_z_roots"] += len(common_roots)
            reported_z = {item["z"]: item for item in finite["z_rows"]}
            core.require(
                set(reported_z) == set(common_roots), "complete common-z ledger"
            )
            for z_value in common_roots:
                z_row = reported_z[z_value]
                relation = (
                    1
                    + (2 * missing - source_sum) * z_value * z_value
                    + missing * missing * pow(z_value, 4, PRIME)
                ) % PRIME
                core.require(relation == 0 and z_value != 0, "missing z relation")
                ...
                next_pair_cut = core.paired(
                    ...
                    b_values,
                    -q_value % PRIME,
                    sigma_c_anchor * cv * f_value % PRIME,
                )
                core.require(
                    (z_row["d"], z_row["e"], z_row["f"], z_row["next_pair_cut"])
                ...
                totals["z_lifts"] += 1
                lanes = {tuple(item["sigma"]): item for item in z_row["lanes"]}
                core.require(
                    set(lanes)
                    == {(sigma_c_anchor, -1), (sigma_c_anchor, 1)},
                ...
                for sigma_o in (-1, 1):
                    lane = lanes[(sigma_c_anchor, sigma_o)]
    ...
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-xi3-pairing2-"
            "direct-audit-v1"
        ),
        "field": PRIME,
