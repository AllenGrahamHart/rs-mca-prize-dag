#!/usr/bin/env python3
"""Scout the pinned pairing-4 compiler on the six cell-9 tower charts."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing4_chart_scout_result.json"
)
FULL_RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing4_chart_result"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-de-pairing4-chart-scout")
image = (
...
@app.function(image=image, cpu=2.0, memory=4096, timeout=900, max_containers=24)
def evaluate(case):
    import time
    ...
    result["excluded"] = (
        not unresolved and not result["witnesses"]
        and not result["colored_solutions"]
    )
...
def main(limit: int = 0, full: bool = False, recovery_index: int = -1):
    cases = tuple(
    ...
    if full:
        ...
        writer = ShardedResultWriter(full_result, metadata={
            ...
            "scope": (
                ...
                "pairing-4 compiler for cell-9 xi in {0,2}."
            ),
    ...
    raw = evaluate.map(cases, order_outputs=True, return_exceptions=True)
    remote_errors = 0
    for case, row in zip(cases, raw):
        ...
        rows.append({
            ...
            "xi_index": row["xi_index"], "pairing_index": 4,
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell9-de-pairing4-chart-scout-v1",
        "field": PRIME,
    ...
    manifest = None
    if writer is not None:
        expected_rows = 1 if recovery_index >= 0 else 192
