#!/usr/bin/env python3
"""Reduce and retain q5 coefficients on the admissible FFF base graph."""
...
PROGRAM = DIRECTORY / "rate_half_kb_positive_433_1b_o0b_fff_q5_coefficients_program.py"
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_o0b_fff_q5_coefficients_result.json"
REMOTE_CACHE = "/root/cache.json"
REMOTE_GRAPH = "/root/graph.json"
REMOTE_PROGRAM = "/root/fff_q5_coefficients_program.py"
PRIME = 2130706433
...
app = modal.App("rs-mca-positive-433-1b-o0b-fff-q5-coefficients")
image = (
...
def reduce_coefficients():
    core = load("fff_q5_coefficients_program", REMOTE_PROGRAM)
    ...
    valid = (
        ...
        and len(stages) == 3 and len(values) == 3
    )
    return {
        **common,
...
def write_checkpoint(row, collection_complete):
    payload = {
        "schema": "rate-half-kb-positive-433-1b-o0b-fff-q5-coefficients-v1",
        "app": "rs-mca-positive-433-1b-o0b-fff-q5-coefficients",
