#!/usr/bin/env python3
"""Reduce the cell-3 common coefficient kernel modulo its lex curve basis."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell3_kernel_reduction_result.json"
...
app = modal.App("rs-mca-positive-433-1a-cell3-kernel-reduction")
image = (
...
def reduce_kernel():
    ...
    a2, a0, b1, common_gcd, _ = common_kernel(3, -1, -1)
    ...
    def singular(expression):
        return str(
            sp.Poly(expression, c, r, b, t, modulus=PRIME).as_expr()
        ).replace("**", "^")
...
"""
    ...
    valid = (
        ...
        and all(f"END_{name}" in stdout for name in names)
        and "?" not in stdout
    )
    return {
...
def main():
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell3-kernel-reduction-v1",
        "scope": (
            "Exact reduction of the unique common coefficient kernel modulo "
            "the cell-3 lex curve basis; no outside, route, K3, or Prize claim."
        ),
