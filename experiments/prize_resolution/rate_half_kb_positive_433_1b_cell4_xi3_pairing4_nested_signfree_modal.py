#!/usr/bin/env python3
"""Solve cell-4 xi3/pairing4 by nested sign-free elimination."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell4_xi3_pairing4_nested_signfree_result.json"
)
REMOTE_STRUCTURE = "/root/structure.json"
REMOTE_KERNEL = "/root/kernel.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell4-xi3-pairing4-nested-signfree")
image = (
...
def evaluate_case(case):
    ...
    q_polynomial = PairPolynomial(0, 1)
    p_antipodal_q = paired_polynomial(q_polynomial, -q_polynomial)
    if p_antipodal_q.degree() != 4 or any(
        p_antipodal_q.coefficients[index] != Pair() for index in (1, 3)
    ):
        raise ValueError("antipodal pair is not biquadratic in q")
    p_antipodal_u = tuple(
        p_antipodal_q.coefficients[index] for index in (0, 2, 4)
    )

    q_bivariate = PairBivariate({(1, 0): 1})
    z_bivariate = PairBivariate({(0, 1): 1})
    p_second_qz = paired_bivariate(
        q_bivariate,
        z_bivariate*(common_b*missing_record),
    )
    p_second_sign_free = p_second_qz*p_second_qz.negate_q()
    if any(q_degree & 1 for q_degree, _ in p_second_sign_free.terms):
        raise ValueError("q sign-free cut retains odd q degree")
    maximum_u = max(
        (q_degree//2 for q_degree, _ in p_second_sign_free.terms),
        default=0,
    )
    p_second_u = [PairPolynomial() for _ in range(maximum_u+1)]
    for (q_degree, z_degree), coefficient in p_second_sign_free.terms.items():
        u_degree = q_degree//2
        coefficients = list(p_second_u[u_degree].coefficients)
        coefficients.extend([Pair()] * (z_degree+1-len(coefficients)))
        coefficients[z_degree] = coefficients[z_degree]+coefficient
        p_second_u[u_degree] = PairPolynomial(*coefficients)
    remainder_u = nested_remainder(p_second_u, p_antipodal_u)
    if len(remainder_u) != 2 or remainder_u[1].degree() < 0:
        raise ValueError("q-sign-free reduction is not linear in u")
    remainder_u_constant, remainder_u_linear = remainder_u
    p_a_0, p_a_1, p_a_2 = p_antipodal_u
    p_second_target_z = (
        remainder_u_linear**2*p_a_0
        - remainder_u_linear*remainder_u_constant*p_a_1
        + remainder_u_constant**2*p_a_2
    )
    remainder_z = polynomial_remainder(p_second_target_z, p_missing_z)
    ...
    print(json.dumps({
        "phase": "nested_signfree_resultant",
    ...
    def paired_left_scalar_coefficients(a_values, b_values, right):
        p0, p1, p2 = (
            b_value-x_symbol*a_value
            for a_value, b_value in zip(a_values, b_values)
        )
    ...
    for r_value in sorted(candidate_roots):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                antipodal_q_coefficients = paired_both_scalar_coefficients(
                    a_values, b_values, 1, -1
                )
                antipodal_q_roots = univariate_roots(antipodal_q_coefficients)
                branch_row.update({
                ...
                for z_value in missing_z_roots:
                    ...
                    second_q_roots = univariate_roots(
                        paired_left_scalar_coefficients(
                            a_values, b_values, b_value*f_value % PRIME
                        )
                    )
                    ...
                    for q_value in q_roots:
                        ...
                        antipodal_pair_cut = paired_scalar(
                            a_values, b_values, q_value, -q_value % PRIME
                        )
                        second_pair_cut = paired_scalar(
                            a_values, b_values, q_value,
                            b_value*f_value % PRIME,
                        )
                        if antipodal_pair_cut or second_pair_cut:
                            raise ValueError("common-q root violates paired cuts")
                        q_row = {
                            "q": q_value,
                            "e": e_value,
                            "antipodal_pair_cut": antipodal_pair_cut,
    ...
    return {
        ...
        "timings": {
            ...
            "nested_signfree_resultant": cut_seconds - kernel_seconds,
        ...
        "antipodal_u_profiles": [
            [
                fraction_profile(value, include_expression=False)
                for value in coefficient.vector()
            ]
            for coefficient in p_antipodal_u
        ],
        "second_target_z_profiles": [
            [
                fraction_profile(value, include_expression=False)
                for value in coefficient.vector()
            ]
            for coefficient in p_second_target_z.coefficients
        ],
        "missing_cut_degree": p_missing.degree(),
        "antipodal_u_degree": len(p_antipodal_u)-1,
        "second_target_z_degree": p_second_target_z.degree(),
        "remainder_u_z_degrees": [value.degree() for value in remainder_u],
...
def main(cases: str = "-1:-1", all_cases: bool = False):
    if all_cases:
        selected = tuple(
            (epsilon_1, epsilon_2)
    ...
    output = {
        ...
        "field": PRIME,
        "scope": (
            "Exact nested q/z sign-free resultant norm and direct exceptional-"
            "root replay for cell 4 at xi=3 and pairing=4. Each source-sign "
            "row checks all four target lanes."
        ),
        ...
        "rows": [
            {
                key: row.get(key) for key in (
                    "epsilon", "xi_index", "pairing_index",
