#!/usr/bin/env python3
"""Scout the pinned positive-DE pairing-14 compiler on cell-9 charts."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_positive_de_pairing14_chart_scout_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-positive-de-pairing14-chart-scout")
image = (
...
def main(
    ...
    for envelope in evaluate.map(cases, order_outputs=False):
        ...
        rows.append({
            ...
            "xi_index": row["xi_index"], "pairing_index": 14,
    ...
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell9-positive-de-pairing14-chart-scout-v1",
        "field": PRIME,
    ...
    if writer is not None:
        expected_rows = (
            ...
            else 1 if recovery_index >= 0 else 96
        )
