#!/usr/bin/env python3
"""Solve cell-4 xi3 pairings 1-2 by the reciprocal-linear cut."""
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell4_xi3_pairings1_2_reciprocal_linear_result.json"
)
REMOTE_STRUCTURE = "/root/structure.json"
REMOTE_KERNEL = "/root/kernel.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell4-xi3-pairings1-2-reciprocal-linear")
image = (
...
def evaluate_case(case):
    ...
    if (
        branch_index not in (0, 1, 2)
        or pairing_index not in (1, 2)
        or (pairing_index == 1 and sigma_c_anchor != 0)
        or (pairing_index == 2 and sigma_c_anchor not in (-1, 1))
    ):
        raise ValueError("scope is xi=3, pairings 1-2, three q branches")
    ...
    next_scale = (
        ...
        else sigma_c_anchor*c_pair*missing_record
    )
    ...
    if remainder.degree() == 1:
        ...
        p_next_0, p_next_1, p_next_2 = p_next.coefficients[:3]
        target_free = (
            remainder_linear**2*p_next_0
            - remainder_linear*remainder_constant*p_next_1
            + p_next_2*remainder_constant**2
        )
    ...
    print(json.dumps({
        "phase": "reciprocal_linear_remainder_cut",
    ...
    def paired_scalar_coefficients(a_values, b_values, left, right_scale):
        p0, p1, p2 = (
    ...
    for r_value in sorted(candidate_roots):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                branch_row["q"] = q_value
                same_pair_cut = paired_scalar(
                    a_values, b_values, q_value, q_value
                )
                if same_pair_cut:
                    raise ValueError("q branch violates paired(q,q)")
                ...
                next_scale_value = (
                    b_value*missing % PRIME if pairing_index == 1
                    else sigma_c_anchor*c_value*missing % PRIME
                )
                next_z_roots = univariate_roots(
                    paired_scalar_coefficients(
                        a_values,
                        b_values,
                        -q_value % PRIME,
                        next_scale_value,
                ...
                z_roots = (
                    missing_z_roots
                    if next_z_roots is None
                    else sorted(set(missing_z_roots) & set(next_z_roots))
                )
                branch_row.update({
                    "same_pair_cut": same_pair_cut,
                ...
                for z_value in z_roots:
                    ...
                    next_pair_cut = paired_scalar(
                        a_values,
                        b_values,
                        -q_value % PRIME,
                        (
                            b_value*f_value % PRIME if pairing_index == 1
                            else sigma_c_anchor*c_value*f_value % PRIME
                        ),
                    )
                    if next_pair_cut:
    ...
    return {
        ...
        "timings": {
            ...
            "reciprocal_linear_remainder_cut": cut_seconds - kernel_seconds,
        ...
        "next_cut_profiles": [
            ...
            for coefficient in p_next.coefficients
        ],
        "missing_cut_degree": p_missing.degree(),
        "next_cut_degree": p_next.degree(),
        "remainder_degree": remainder.degree(),
...
def main(cases: str = "-1:-1:0:0:1", all_cases: bool = False):
    if all_cases:
        sign_pairs = tuple(
            (epsilon_1, epsilon_2)
        ...
        selected = pairing_one + pairing_two
    else:
        selected = tuple(
    ...
    output = {
        ...
        "field": PRIME,
        "scope": (
            "Exact three-branch reciprocal-linear remainder norm and direct "
            "exceptional-root replay for cell 4 at xi=3 and pairings 1-2. "
            "Pairing-1 rows check all four target lanes; pairing-2 rows fix "
            "sigma_c and check both sigma_o lanes."
        ),
        ...
        "rows": [
            {
                key: row.get(key) for key in (
                    "epsilon", "xi_index", "pairing_index", "branch_index",
                    "sigma_c_anchor", "status", "timings",
                    "target_norm_root_count",
