#!/usr/bin/env python3
"""Derive and price the cutoff-free E16 profile/parity/light router."""
...
RESULT = HERE / "e16_profile_parity_probe_result.json"

app = modal.App("e1-n256-e16-profile-parity-route-probe")
...
def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    rows = []
    for counts in product(range(17), range(5), range(2), range(2)):
        if sum((index + 1) ** 2 * count for index, count in enumerate(counts)) != 16:
            continue
...
def matching_ledgers(light_diameters: int) -> list[list[int]]:
    ...
    visit(tuple(range(7)), ())
    return [[value, (value - 86) // 2] for value in sorted(square_masses)]
...
def compute() -> dict[str, object]:
    ...
    trace = []
    l1_bound = None
    for l1_norm in range(16, 7, -1):
        delta = 16 + 66 - 4 * l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 16 and l1_bound is None:
            l1_bound = l1_norm
...
def main() -> None:
    ...
    packet["schema"] = "e1-e16-profile-parity-route-probe-v1"
    ...
    print(
        "E16_PROFILE_PARITY_ROUTE_PROBE "
        + json.dumps(
            {
    ...
    print(f"E16_PROFILE_PARITY_ROUTE_PROBE_RESULT {RESULT}")
