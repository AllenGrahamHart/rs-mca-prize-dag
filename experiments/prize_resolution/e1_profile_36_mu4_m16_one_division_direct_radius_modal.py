#!/usr/bin/env python3
"""Run the complete once-divided cofactor-16 direct census on Modal."""
...
BASE_PATCH_PATH = PREFIX / "e1_profile_36_mu4_m16_even_square_noscreen.patch"
EARLY_TRANSFORM_PATH = PREFIX / "e1_profile_36_mu4_m16_early_cap_transform.py"
...
OUTPUT = Path(
    ...
    "e1_profile_36_mu4_m16_one_division_direct_radius_result.json"
)
...
app = modal.App("e1-profile-36-mu4-m16-one-division-direct-radius")
image = (
    modal.Image.debian_slim()
    .apt_install("g++", "libboost-dev", "patch")
    .add_local_file(str(CPP_PATH), "/repo/experiments/prize_resolution/worker.cpp", copy=True)
    .add_local_file(str(BASE_PATCH_PATH), "/repo/experiments/prize_resolution/base.patch", copy=True)
    .add_local_file(str(EARLY_TRANSFORM_PATH), "/repo/experiments/prize_resolution/early_transform.py", copy=True)
    ...
    .run_commands(
        "cd /repo/experiments/prize_resolution && patch worker.cpp base.patch",
        "python3 /repo/experiments/prize_resolution/early_transform.py /repo/experiments/prize_resolution/worker.cpp",
...
def write_packet(
    ...
    packet = {
        "schema": "e1-profile-36-mu4-m16-one-division-direct-radius-result-v1",
        ...
        "base_patch_sha256": hashlib.sha256(BASE_PATCH_PATH.read_bytes()).hexdigest(),
        "early_transform_sha256": hashlib.sha256(EARLY_TRANSFORM_PATH.read_bytes()).hexdigest(),
...
def main(batch_size: int = 4) -> None:
    ...
    if OUTPUT.exists():
        ...
        assert previous["schema"] == "e1-profile-36-mu4-m16-one-division-direct-radius-result-v1"
        assert previous["orbit_file_sha256"] == hashlib.sha256(ORBIT_PATH.read_bytes()).hexdigest()
        assert previous["base_engine_sha256"] == hashlib.sha256(CPP_PATH.read_bytes()).hexdigest()
        assert previous["base_patch_sha256"] == hashlib.sha256(BASE_PATCH_PATH.read_bytes()).hexdigest()
        assert previous["early_transform_sha256"] == hashlib.sha256(EARLY_TRANSFORM_PATH.read_bytes()).hexdigest()
    ...
    assert counts["orbits"] == branch["affine_orbits"] == 9080
    assert counts["triple_syndromes"] == counts["orbits"] * 295240
    assert counts["distance_tests"] == counts["orbits"] * 32 * (295240 - 30856)
    ...
    print(
        "E1_PROFILE_36_MU4_M16_ONE_DIVISION_DIRECT_RADIUS_PASS "
