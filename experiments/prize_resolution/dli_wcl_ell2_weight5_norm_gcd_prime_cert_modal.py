#!/usr/bin/env python3
"""Build recursive Pocklington certificates for the weight-5 norm-gcd roots."""
...
app = modal.App("rs-mca-dli-wcl-ell2-weight5-norm-gcd-prime-cert")
...
def main(shard_size: int = 24, resume: bool = True) -> None:
    factor_result = json.loads(FACTOR_RESULT.read_text())
    if (
        factor_result.get("status") != "COMPLETE"
        or factor_result.get("factor_status") != "COMPLETE"
    ):
        raise AssertionError("factor ledger incomplete")
    roots = sorted(
        {
            factor["prime"]
            for row in factor_result["factor_rows"]
    ...
    if resume and OUTPUT.exists():
        ...
        if result.get("schema") != "dli-wcl-ell2-weight5-prime-cert-v1":
            raise AssertionError("resume schema mismatch")
        completed_roots = {
            root
            for shard in result.get("completed_shards", [])
            for root in shard["roots"]
        }
        result["worker_errors"] = []
    else:
        result = {
            "schema": "dli-wcl-ell2-weight5-prime-cert-v1",
            "status": "IN_PROGRESS",
    ...
    write_result(result)
    print(
        "DLI_WCL_ELL2_WEIGHT5_CERT_START "
    ...
    if result["worker_errors"]:
        ...
        write_result(result)
        print(
            "DLI_WCL_ELL2_WEIGHT5_CERT_RESULT "
