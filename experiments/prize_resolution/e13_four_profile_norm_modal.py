#!/usr/bin/env python3
"""Compute dual exact cyclotomic norms for the full E13 residue."""
...
RESULT = HERE / "e13_four_profile_norm_result.json"
BATCH_SIZE = 200
app = modal.App("e1-n256-e13-four-profile-norm")
...
def run_flint(batch: tuple[int, list[dict[str, object]]]) -> dict[str, object]:
    ...
    return {"batch": batch_index, "norms": norms,
...
def run_pari(batch: tuple[int, list[dict[str, object]]]) -> dict[str, object]:
    ...
    script = []
    for vector in vectors:
        terms = "+".join(f"({int(coefficient)})*x^{int(position)}"
                         for position, coefficient in zip(vector["positions"], vector["coefficients"]))
        script.append(f"print(abs(polresultant(x^128+1,{terms})));" )
    completed = subprocess.run(["gp", "-q"], input="\n".join(script) + "\n",
                               capture_output=True, check=True, text=True, timeout=55)
...
def load_vectors() -> list[dict[str, object]]:
    ...
    if not packet["complete"]:
        raise RuntimeError("E13 census is incomplete")
    vectors = [match for row in packet["rows"] for match in row["primary"]["matches"]]
    if len(vectors) != int(packet["summary"]["collected_full_conductor"]):
        raise RuntimeError("E13 full-conductor collection count mismatch")
    return vectors
...
def main() -> None:
    ...
    batches = [(index // BATCH_SIZE, vectors[index:index + BATCH_SIZE])
               for index in range(0, len(vectors), BATCH_SIZE)]
    flint_rows = []; pari_rows = []

    def flatten(rows: list[dict[str, object]]) -> list[int]:
        return [int(value) for row in sorted(rows, key=lambda item: int(item["batch"]))
                for value in row["norms"]]

    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        packet = {
            "schema": "e1-e13-four-profile-norm-v1", "complete": complete,
            "agreement": False, "vectors": len(vectors), "batch_size": BATCH_SIZE,
            "expected_batches": len(batches), "completed_flint": len(flint_rows),
    ...
    try:
        for row in run_flint.map(batches):
            flint_rows.append(row); write_checkpoint(False)
        for row in run_pari.map(batches):
            pari_rows.append(row); write_checkpoint(False)
    except BaseException as error:
        write_checkpoint(False, f"{type(error).__name__}: {error}")
        print(f"E13_FOUR_PROFILE_NORM_INCOMPLETE flint={len(flint_rows)}/{len(batches)} pari={len(pari_rows)}/{len(batches)}")
        raise
    flint_norms = flatten(flint_rows); pari_norms = flatten(pari_rows)
    if len(flint_norms) != len(vectors) or flint_norms != pari_norms:
        raise RuntimeError("FLINT/PARI E13 norm mismatch")
    ...
    profile_maxima = [max((norm for norm, vector in zip(flint_norms, vectors)
                           if int(vector["profile"]) == profile), default=0)
                      for profile in range(4)]
    profile_odd_maxima = [max((odd for odd, vector in zip(odd_parts, vectors)
                               if int(vector["profile"]) == profile), default=0)
                          for profile in range(4)]
    packet = write_checkpoint(True); packet["agreement"] = True
    packet["summary"] = {
        "vectors": len(vectors), "distinct_norms": len(set(flint_norms)),
        "maximum_norm": max(flint_norms), "maximum_norm_bits": max(flint_norms).bit_length(),
        ...
        "maximizing_indices": [index for index, value in enumerate(flint_norms)
                               if value == max(flint_norms)],
        "odd_part_maximizing_indices": [index for index, value in enumerate(odd_parts)
                                        if value == max(odd_parts)],
    ...
    RESULT.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n")
    print("E13_FOUR_PROFILE_NORM " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E13_FOUR_PROFILE_NORM_AGREEMENT {packet['agreement']}")
    print(f"E13_FOUR_PROFILE_NORM_RESULT {RESULT}")
