#!/usr/bin/env python3
"""Reduce the cell-4 common coefficient kernel modulo its lex curve basis."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell4_kernel_reduction_result.json"
...
app = modal.App("rs-mca-positive-433-1a-cell4-kernel-reduction")
image = (
...
def reduce_kernel():
    ...
    a2, a0, b1, common_gcd, _ = common_kernel(4, -1, -1)
...
"""
    ...
    return {
        ...
        "input_shape": {
            name: {"degree": sp.Poly(expression, c, r, b, t,
...
def main():
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell4-kernel-reduction-v1",
        "scope": (
            "Exact reduction of the unique common coefficient kernel modulo "
            "the cell-4 lex curve basis; no outside, route, K3, or Prize claim."
        ),
    ...
    print(json.dumps({
        ...
        "reduced_lengths": {name: len(value) for name, value in
