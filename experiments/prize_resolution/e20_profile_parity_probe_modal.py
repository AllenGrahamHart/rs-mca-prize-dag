#!/usr/bin/env python3
"""Derive and price the exact E20 profile/parity/light-atlas router."""
...
RESULT = HERE / "e20_profile_parity_probe_result.json"

app = modal.App("e1-n256-e20-profile-parity-route-probe")
...
def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    rows = []
    for counts in product(range(21), range(6), range(3), range(2)):
        if sum((index + 1) ** 2 * count for index, count in enumerate(counts)) != 20:
            continue
...
def matching_ledgers(light_diameters: int) -> list[list[int]]:
    ...
    visit(tuple(range(7)), ())
    return [[value, (value - 82) // 2] for value in sorted(square_masses)]
...
def compute() -> dict[str, object]:
    ...
    trace = []
    l1_bound = None
    for l1_norm in range(20, 8, -1):
        delta = 20 + 66 - 4 * l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 20 and l1_bound is None:
            l1_bound = l1_norm
...
def main() -> None:
    ...
    packet["schema"] = "e1-e20-profile-parity-route-probe-v1"
    ...
    print(
        "E20_PROFILE_PARITY_ROUTE_PROBE "
        + json.dumps(
            {
    ...
    print(f"E20_PROFILE_PARITY_ROUTE_PROBE_RESULT {RESULT}")
