#!/usr/bin/env python3
"""Merge and compare a complete paired K'=88 raw-threshold capture."""
...
def main() -> None:
    ...
    expected = {
        ...
        for offset in range(1, 78)
    }
    assert set(jobs) == expected
    results: dict[str, dict[int, dict[str, object]]] = {"primary": {}, "audit": {}}
    peaks = []
    ...
    assert total_units == 18_741_723
    assert total_safe + total_unsafe == total_units
    assert unsafe_offsets == list(range(min(unsafe_offsets), max(unsafe_offsets) + 1))
    print(json.dumps({
        "status": "PASS",
        ...
        "unsafe_offsets": [min(unsafe_offsets), max(unsafe_offsets)],
