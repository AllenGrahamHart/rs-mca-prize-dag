#!/usr/bin/env python3
"""Pilot the cell-3 DE-missing pairing-12 nested quadratic cut."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing12_nested_quadratic_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing12_nested_quadratic_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-de-pairing12-nested-quadratic-pilot")
image = (
...
def profile_case(case):
    ...
    if xi_index not in (0, 2) or pairing_index != 12:
        raise ValueError("the pilot is scoped to xi in {0,2}, pairing=12")
...
def main(
    ...
    if full_census:
        cases = tuple(
            (*epsilon, sigma_c, sigma_o, selected_xi, 12)
        ...
        cases = tuple(
            (*epsilon, sigma_c, -1, selected_xi, 12)
            for epsilon in sign_pairs
            for sigma_c in (-1, 1)
            for selected_xi in (0, 2)
        )
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-de-pairing12-nested-quadratic-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-de-pairing12-nested-quadratic-census-v1"
            if all_signs or full_census else
            "rate-half-kb-positive-433-1b-cell3-de-pairing12-nested-quadratic-pilot-v1"
        ),
        "scope": (
            "Exact nested-quadratic and exceptional-root census at DE-missing "
            "pairing 12; no claim beyond the printed cases."
        ),
