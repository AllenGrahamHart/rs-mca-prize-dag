#!/usr/bin/env python3
"""Compute dual exact cyclotomic norms for the full E14 residue."""
...
RESULT = HERE / "e14_four_profile_norm_result.json"
BATCH_SIZE = 200
app = modal.App("e1-n256-e14-four-profile-norm")
...
def load_vectors() -> list[dict[str, object]]:
    ...
    if not packet["complete"]:
        raise RuntimeError("E14 census is incomplete")
    ...
    if len(vectors) != int(packet["summary"]["collected_full_conductor"]):
        raise RuntimeError("E14 full-conductor collection count mismatch")
    return vectors
...
def main() -> None:
    ...
    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        packet = {
            "schema": "e1-e14-four-profile-norm-v1", "complete": complete,
    ...
    try:
        ...
        print(f"E14_FOUR_PROFILE_NORM_INCOMPLETE flint={len(flint_rows)}/{len(batches)} pari={len(pari_rows)}/{len(batches)}")
        raise
    flint_norms = flatten(flint_rows); pari_norms = flatten(pari_rows)
    if len(flint_norms) != len(vectors) or flint_norms != pari_norms:
        raise RuntimeError("FLINT/PARI E14 norm mismatch")
    ...
    RESULT.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n")
    print("E14_FOUR_PROFILE_NORM " + json.dumps(packet["summary"], sort_keys=True))
    print(f"E14_FOUR_PROFILE_NORM_AGREEMENT {packet['agreement']}")
    print(f"E14_FOUR_PROFILE_NORM_RESULT {RESULT}")
