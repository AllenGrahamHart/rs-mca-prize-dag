#!/usr/bin/env python3
"""Derive and price the cutoff-free E15 profile/parity/light router."""
...
RESULT = HERE / "e15_profile_parity_probe_result.json"

app = modal.App("e1-n256-e15-profile-parity-route-probe")
...
def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    rows = []
    for counts in product(range(16), range(4), range(2), range(2)):
        if sum((index + 1) ** 2 * count for index, count in enumerate(counts)) != 15:
            continue
...
def diameter_ledgers() -> list[list[int]]:
    ...
    visit(tuple(range(7)), ())
    return [[value, (value - 87) // 2] for value in sorted(square_masses)]
...
def compute() -> dict[str, object]:
    ...
    trace = []
    l1_bound = None
    for l1_norm in range(15, 7, -1):
        delta = 15 + 66 - 4 * l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 15 and l1_bound is None:
            l1_bound = l1_norm
...
def main() -> None:
    ...
    packet["schema"] = "e1-e15-profile-parity-route-probe-v1"
    ...
    print(
        "E15_PROFILE_PARITY_ROUTE_PROBE "
        + json.dumps(
            {
    ...
    print(f"E15_PROFILE_PARITY_ROUTE_PROBE_RESULT {RESULT}")
