#!/usr/bin/env python3
"""Certify the guarded factorization of the cell-14 signed-pair resultant."""
...
PLANE = DIRECTORY / "rate_half_kb_positive_433_1a_cell14_plane_kernel_flint_result.json"
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell14_signed_pair_guard_factorization_result.json"
REMOTE_PLANE = "/root/plane.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1a-cell14-signed-pair-guard-factorization")
image = (
...
def certify():
    ...
    def form(prefix, variable):
        return sum(normalized[f"{prefix}{index}"]*variable**index
                   for index in range(3))

    d0, d1 = form("a2", w0), form("a2", w1)
    n0, n1 = form("a0", w0), form("a0", w1)
    k = normalized["b10"]
    ...
    def reduce_by_plane(polynomial):
        ...
        while int(reduced.degrees()[2]) >= 4:
            ...
            reduced = plane_leading*reduced-coefficient*b**(old_degree-4)*plane
    ...
    if int(resultant.degrees()[0]) != 0:
        raise RuntimeError("pair resultant retained w1")
    projected, projected_steps = reduce_by_plane(resultant)
    degree = int(projected.degrees()[1])
    if degree != 16:
        raise RuntimeError(f"unexpected projected degree {degree}")

    reductions = []
    d0_fifth = d0
    for exponent in range(2, 6):
        d0_fifth, steps = reduce_by_plane(d0_fifth*d0)
        reductions.append({"operation": f"d0_power_{exponent}",
                           "steps": steps, "shape": shape(d0_fifth)})
    r_minus, steps = reduce_by_plane(rd*rd*w0-rn*rn)
    reductions.append({"operation": "r_minus", "steps": steps,
                       "shape": shape(r_minus)})
    r_plus, steps = reduce_by_plane(rd*rd*w0+rn*rn)
    reductions.append({"operation": "r_plus", "steps": steps,
                       "shape": shape(r_plus)})
    candidate, steps = reduce_by_plane(n0*d0_fifth)
    reductions.append({"operation": "n0_times_d0_fifth", "steps": steps,
                       "shape": shape(candidate)})
    candidate = candidate*(w0+1)*(w0+1)
    candidate, steps = reduce_by_plane(candidate*r_minus)
    reductions.append({"operation": "times_minus_labels", "steps": steps,
                       "shape": shape(candidate)})
    candidate, steps = reduce_by_plane(candidate*r_plus)
    reductions.append({"operation": "times_plus_label", "steps": steps,
                       "shape": shape(candidate)})
    if int(candidate.degrees()[1]) != degree:
        raise RuntimeError("candidate degree mismatch")

    projected_leading = coefficient_at_degree(projected, 1, degree)
    candidate_leading = coefficient_at_degree(candidate, 1, degree)
    ...
    for factor, multiplicity in factors:
        ...
        factor_rows.append({
            "multiplicity": int(multiplicity), "shape": shape(factor),
            "sha256": hashlib.sha256(factor.str().encode()).hexdigest(),
            "text": factor.str(),
        })
    if reconstruction != leading_norm:
        raise RuntimeError("norm reconstruction failed")
    ...
    def specialize(polynomial, variable_index, values):
        ...
        for monomial, source_coefficient in polynomial.to_dict().items():
            ...
            coefficients[exponent] = (coefficients.get(exponent, 0)+value) % PRIME
        maximum = max(coefficients, default=0)
        return univariate([coefficients.get(index, 0)
    ...
    def evaluate(polynomial, value):
        return sum(int(polynomial[index])*pow(value, index, PRIME)
                   for index in range(int(polynomial.degree())+1)) % PRIME

    norm_roots = []
    for factor, _ in factors:
        if int(factor.degrees()[3]) == 1:
            coefficients = {monomial[3]: int(coefficient)
                            for monomial, coefficient in factor.to_dict().items()}
            norm_roots.append((-coefficients.get(0, 0)
                               *pow(coefficients[1], -1, PRIME)) % PRIME)
    norm_roots = sorted(set(norm_roots))
    atlas_rows = []
    all_guarded = True
    for t_value in norm_roots:
        plane_b = specialize(plane, 2, {3: t_value})
        leading_b = specialize(projected_leading, 2, {3: t_value})
        common_b = plane_b.gcd(leading_b)
        _, b_factors = common_b.factor()
        b_roots = sorted({linear_root(factor) for factor, _ in b_factors
                          if int(factor.degree()) == 1})
        row = {"t": t_value, "common_b_degree": int(common_b.degree()),
               "deployed_b_roots": b_roots, "b_rows": []}
        for b_value in b_roots:
            values = {2: b_value, 3: t_value}
            projected_w0 = specialize(projected, 1, values)
            if projected_w0.is_zero():
                ...
                continue
            known = {
                "N0": specialize(n0, 1, values),
                "D0": specialize(d0, 1, values),
                "w0+1": univariate([1, 1]),
                "w0-r^2": specialize(rd*rd*w0-rn*rn, 1, values),
                "w0+r^2": specialize(rd*rd*w0+rn*rn, 1, values),
            }
            ...
            for factor, multiplicity in w0_factors:
                if int(factor.degree()) != 1:
                    nonlinear.append({"degree": int(factor.degree()),
                                      "multiplicity": int(multiplicity)})
                    continue
                root = linear_root(factor)
                guards = [name for name, polynomial in known.items()
                          if evaluate(polynomial, root) == 0]
                if not guards:
                    all_guarded = False
                deployed.append({"w0": root, "multiplicity": int(multiplicity),
                                 "guards": guards})
            row["b_rows"].append({
                "b": b_value, "zero_projected_polynomial": False,
                "projected_degree": int(projected_w0.degree()),
                "deployed_w0_roots": deployed,
                "nonlinear_factors": nonlinear,
                "all_deployed_roots_guarded": all(item["guards"]
    ...
    return {
        ...
        "plane_shape": shape(plane), "plane_leading": plane_leading.str(),
        "raw_product_shape": shape(raw_product),
        "raw_square_shape": shape(raw_square),
        "product_steps": product_steps, "square_steps": square_steps,
        ...
        "guard_identity": (
            "plane-reduced(N0*D0^5*(w0+1)^2*(rd^2*w0-rn^2)*"
            "(rd^2*w0+rn^2)) is proportional to pair resultant"
        ),
        "guard_reductions": reductions,
        "guard_plane_leading_exponent": sum(row["steps"] for row in reductions),
...
def main():
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell14-signed-pair-guard-factorization-v1",
        "scope": (
            "Exact compact-plane quotient factorization of the complete "
            "necessary cell-14 signed-pair resultant and replay of every "
            "leading-norm base-field root. Exceptional compact-kernel scales, "
            "orbit, route, and Prize claims remain separate."
        ),
