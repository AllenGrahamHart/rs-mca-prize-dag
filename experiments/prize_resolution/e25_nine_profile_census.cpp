...
constexpr int kCutoff = 13;
constexpr int kProfileCount = 9;
constexpr std::array<std::array<int, 5>, kProfileCount> kProfiles{{
...
int main(int argc, char** argv) {
  ...
  std::array<std::uint64_t, kProfileCount> profile_counts{};
  std::array<std::uint64_t, kProfileCount> above_cutoff{};
  std::array<std::uint64_t, kProfileCount> full_above_cutoff{};
  std::array<int, kProfileCount> minimum_m3{};
  std::array<int, kProfileCount> minimum_full_m3{};
  std::array<int, kProfileCount> maximum_m3{};
  std::array<int, kProfileCount> maximum_full_m3{};
  minimum_m3.fill(std::numeric_limits<int>::max());
  minimum_full_m3.fill(std::numeric_limits<int>::max());
  maximum_m3.fill(-1);
    ...
    for (int second = first + 1; second < static_cast<int>(allowed.size()) - 1;
      ...
      for (int third = second + 1; third < static_cast<int>(allowed.size());
        ...
        for (int mask = 0; mask < 64; ++mask) {
          ...
          minimum_m3[profile_index] = std::min(minimum_m3[profile_index], m3);
          maximum_m3[profile_index] = std::max(maximum_m3[profile_index], m3);
          if (conductor == 1) {
            minimum_full_m3[profile_index] =
                std::min(minimum_full_m3[profile_index], m3);
  ...
  for (int index = 0; index < kProfileCount; ++index) {
    if (minimum_m3[index] == std::numeric_limits<int>::max()) minimum_m3[index] = -1;
    if (minimum_full_m3[index] == std::numeric_limits<int>::max()) minimum_full_m3[index] = -1;
  }
  ...
  std::cout << ",\"minimum_m3\":";
  print_array(minimum_m3);
  std::cout << ",\"minimum_full_m3\":";
  print_array(minimum_full_m3);
  std::cout << ",\"maximum_m3\":";
