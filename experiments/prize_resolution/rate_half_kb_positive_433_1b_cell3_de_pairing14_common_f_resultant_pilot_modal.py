#!/usr/bin/env python3
"""Pilot the cell-3 DE-missing pairing-14 common-f resultant cut."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing14_common_f_resultant_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_de_pairing14_common_f_resultant_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-de-pairing14-common-f-resultant-pilot")
image = (
...
def profile_case(case):
    ...
    if xi_index not in (0, 2) or pairing_index != 14:
        raise ValueError("the pilot is scoped to xi in {0,2}, pairing=14")
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                if de_value == 0:
                    ...
                    t_row["b_rows"].append(b_row)
                    continue
                p_b_field = paired_polynomial_at(second_de_value, b_value)
...
def main(
    ...
    if full_census:
        cases = tuple(
            (*epsilon, sigma_c, sigma_o, selected_xi, 14)
        ...
        cases = tuple(
            (*epsilon, sigma_c, -1, selected_xi, 14)
            for epsilon in sign_pairs
            for sigma_c in (-1, 1)
            for selected_xi in (0, 2)
        )
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-de-pairing14-common-f-resultant-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-de-pairing14-common-f-resultant-census-v1"
            if all_signs or full_census else
            "rate-half-kb-positive-433-1b-cell3-de-pairing14-common-f-resultant-pilot-v1"
        ),
        "scope": (
            "Exact common-f resultant and exceptional-root census at "
            "DE-missing pairing 14; no claim beyond the printed cases."
        ),
