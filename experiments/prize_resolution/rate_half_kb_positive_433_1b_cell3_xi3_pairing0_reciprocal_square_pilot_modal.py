#!/usr/bin/env python3
"""Pilot the cell-3 xi3/pairing0 reciprocal-square branch cut."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_pairing0_reciprocal_square_pilot_result.json"
CENSUS_RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_xi3_pairing0_reciprocal_square_census_result.json"
REMOTE_QUOTIENT = "/root/quotient.json"
REMOTE_KERNEL = "/root/kernel.json"
REMOTE_PRODUCT = "/root/product.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1b-cell3-xi3-pairing0-reciprocal-square-pilot")
image = (
...
def profile_case(case):
    ...
    epsilon_1, epsilon_2, branch_index, sigma_o, xi_index, pairing_index = case
    if (branch_index not in (0, 1, 2) or sigma_o not in (-1, 1) or
            xi_index != 3 or pairing_index != 0):
        raise ValueError(
            "the pilot is scoped to xi=3, pairing=0, three q branches"
        )
    ...
    p_outside = paired_polynomial(
        PairPolynomial(-q_record),
        variable_polynomial*(sigma_o*q_record*missing_record),
    )
    if p_missing.degree() != 2 or p_outside.degree() != 2:
        raise ValueError("reciprocal-square cuts are not quadratic")

    p_b_c, p_b_b, p_b_a = p_missing.coefficients[:3]
    p_c_c, p_c_b, p_c_a = p_outside.coefficients[:3]
    target_free = (
    ...
    print(json.dumps({
        "phase": "reciprocal_square_resultant_cut",
        "p_missing_degree": p_missing.degree(),
        "p_outside_degree": p_outside.degree(),
        "seconds": time.perf_counter()-started,
    }), flush=True)
    ...
    for r_value in sorted(candidate_r_values):
        ...
        for t_value in t_roots:
            ...
            for b_value in b_roots:
                ...
                missing_y_roots = field_roots(missing_y_field)
                outside_y_roots = field_roots(outside_y_field)
                if missing_y_roots is None:
                    raise ValueError("monic reciprocal-square cut vanished")
                y_roots = (
                    missing_y_roots if outside_y_roots is None else
                    sorted(set(missing_y_roots) & set(outside_y_roots))
                )
                b_row["missing_y_roots"] = missing_y_roots
                b_row["outside_y_roots"] = outside_y_roots
                b_row["common_y_roots"] = y_roots
                b_row["yd_rows"] = []
                for y_value in y_roots:
                    relation_value = (
                        1
                        + (2*source_missing-source_sum_value)*y_value
                        + source_missing*source_missing*y_value*y_value
                    ) % PRIME
                    if relation_value:
                    ...
                    if d_roots is None:
                        raise ValueError("monic square lift vanished")
                    y_row = {
                        "y": y_value,
                        "x": x_value,
                        "d_roots": d_roots,
                    ...
                    for d_value in d_roots:
                        if d_value == 0:
                            raise ValueError("nonzero square has zero root")
                        ...
                        outside_pair_cut = paired_value_at(
                            -q_value % PRIME,
                            sigma_o*e_value*f_value % PRIME,
                        )
                        if outside_pair_cut:
                            raise ValueError("common-y root violates outside pair")
                        for lane_c in (-1, 1):
                    ...
                    y_row["status"] = "CHECKED"
                    b_row["yd_rows"].append(y_row)
    ...
    print(json.dumps({
        ...
        "yd_candidates": len(yd_candidates),
...
def main(
    ...
    if source_census:
        cases = tuple(
            (*epsilon, selected_branch, selected_sigma_o, 3, 0)
        ...
        cases = tuple(
            (*epsilon, branch_index, sigma_o, 3, 0)
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell3-xi3-pairing0-reciprocal-square-source-census-v1"
            if source_census else
            "rate-half-kb-positive-433-1b-cell3-xi3-pairing0-reciprocal-square-pilot-v1"
        ),
        "scope": (
            "Exact three-branch reciprocal-square resultant and "
            "exceptional-root census at xi=3, pairing=0; no claim beyond "
            "the printed cases."
        ),
        ...
        "rows": [
            {
                ...
                "direct_lift": {
                    ...
                    for key in (
                        ...
                        "yd_candidate_count", "final_pair_solution_count",
