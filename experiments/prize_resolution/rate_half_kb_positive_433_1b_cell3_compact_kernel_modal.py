#!/usr/bin/env python3
"""Compile a global polynomial coefficient kernel on 433-1b cell 3."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell3_compact_kernel_result.json"
REMOTE_PRODUCT = "/root/product.json"
REMOTE_STRUCTURE = "/root/structure.json"
PRIME = 2130706433
IOTA = 16711679

app = modal.App("rs-mca-positive-433-1b-cell3-compact-kernel")
image = (
...
def compile_kernel(signs):
    ...
    ab_label = labels[1]
    ab_scale = sp.expand(ab_label * (1 - ab_label))
    a_at_ab = sp.expand(sum(
        cofactors[index] * ab_label**index for index in range(3)
    ))
    gamma = sp.expand(q_values[1] * a_at_ab)
    kernel = [
        *(sp.expand(ab_scale * value) for value in cofactors),
        -gamma,
        gamma,
    ]
    kernel_polynomials = [
    ...
    base_rows = [*reduced_product_rows, reduced_sum_rows[1]]
    equations = [
        sp.expand(
            sp.Matrix([*base_rows, reduced_sum_rows[index]]).det(
                method="domain-ge"
            )
        )
        for index in (2, 3, 4)
...
"""
    ...
    for index in range(len(row_dots)):
        match = re.search(
            rf"ROW={index},BEGIN\n(.*?)\nROW={index},END", stdout, re.DOTALL
        )
...
def main():
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell3-compact-kernel-v1",
        "field": PRIME,
        "scope": (
            "Exact global polynomial coefficient kernels on the guarded "
            "cell-3 principal common curve; no outside or route claim."
        ),
        ...
        "rows": [
            {
                ...
                "all_rows_zero_mod_common": row.get("all_rows_zero_mod_common"),
