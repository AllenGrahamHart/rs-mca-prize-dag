...
constexpr std::array<std::array<int, 4>, 7> kProfiles{{
    {{5, 4, 0, 0}}, {{1, 5, 0, 0}}, {{4, 2, 1, 0}}, {{0, 3, 1, 0}},
    {{3, 0, 2, 0}}, {{5, 0, 0, 1}}, {{1, 1, 0, 1}},
}};
...
int main(int argc, char** argv) {
  ...
  std::array<std::uint64_t, 7> profile_counts{};
  std::array<std::uint64_t, 7> full_conductor_counts{};
  std::array<std::uint64_t, 7> hash_sums{};
  std::array<std::uint64_t, 7> hash_xors{};
    ...
    for (int second = first + 1; second < static_cast<int>(allowed.size()) - 1;
      ...
      for (int third = second + 1; third < static_cast<int>(allowed.size());
        ...
        for (int mask = 0; mask < 64; ++mask) {
          ...
          if (conductor == 1) {
            matches.push_back({profile_index, positions, coefficients});
          }
        }
      }
    }
  ...
  for (std::size_t index = 0; index < matches.size(); ++index) {
    if (index) std::cout << ',';
    std::cout << "{\"profile\":" << matches[index].profile
              << ",\"positions\":";
