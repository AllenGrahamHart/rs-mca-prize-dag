#!/usr/bin/env python3
"""Compute four-basis norms for the cell-9 parallel-DE source cuts."""
...
app = modal.App("rs-mca-positive-433-1b-cell9-parallel-de-four-basis")
image = (
...
def evaluate(case):
    ...
    epsilon_1, epsilon_2, b_row_index, c_row_index, cut_kind = case
    ...
    row = next(
        ...
        and item["b_row_index"] == b_row_index
        and item["c_row_index"] == c_row_index
    )
    if not all((
        row["status"] == "COMPLETE", row["exact"],
        row["b_cover_complete"], row["c_cover_complete"],
    )):
        raise RuntimeError("invalid tower chart")
    ...
    if cut_kind == "positive":
        source_cut = paired(missing, -missing)
    elif cut_kind == "negative":
        source_cut = paired(-missing, -missing)
    else:
        raise RuntimeError(f"unknown cut kind {cut_kind}")
    cut_seconds = time.perf_counter()-started
    source_norm = source_cut.norm()
    ...
    roots = field_roots(source_norm.numer)
    guard_roots = set()
    guard_profiles = []
    seen = set()
    ...
    return {
        ...
        "source_cut_profiles": [fraction_profile(value)
                                for value in source_cut.vector()],
        "source_norm": fraction_profile(source_norm),
        "source_roots": roots,
        "source_root_count": None if roots is None else len(roots),
...
def main(limit: int = 0):
    cases = tuple(
        (*epsilon, b_row_index, c_row_index, cut_kind)
        for epsilon in itertools.product((-1, 1), repeat=2)
        for b_row_index in (2, 3) for c_row_index in (4, 5, 6)
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            rows.append({
                ...
                "c_row_index": case[3], "cut_kind": case[4],
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-parallel-de-"
            "four-basis-norm-v1"
        ),
        "field": PRIME,
        "scope": (
            "Exact four-basis norms and deployed-field candidate roots for "
            "the cell-9 first-pair parallel-DE source cuts on six covering "
            "charts; candidate routing only until direct tower replay."
        ),
    ...
    print(json.dumps({
        ...
        "rows": [{key: row.get(key) for key in (
            "epsilon", "b_row_index", "c_row_index", "cut_kind", "status",
            "source_root_count", "guard_root_count", "candidate_roots",
        )} | {"source_norm": row.get("source_norm")} for row in rows],
    }, sort_keys=True))
