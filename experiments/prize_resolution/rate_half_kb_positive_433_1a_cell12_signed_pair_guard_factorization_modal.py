#!/usr/bin/env python3
"""Certify the guarded factorization of the cell-12 signed-pair resultant."""
...
PLANE = DIRECTORY / "rate_half_kb_positive_433_1a_cell12_plane_kernel_flint_result.json"
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell12_signed_pair_guard_factorization_result.json"
REMOTE_PLANE = "/root/plane.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1a-cell12-signed-pair-guard-factorization")
image = (
...
def certify():
    ...
    reductions.append({
        "operation": "r_minus", "steps": steps, "shape": shape(r_minus),
    })
    r_plus, steps = reduce_by_plane(rd * rd * w0 + rn * rn)
    reductions.append({
        "operation": "r_plus", "steps": steps, "shape": shape(r_plus),
    })
    r_plus_squared, steps = reduce_by_plane(r_plus * r_plus)
    reductions.append({
        "operation": "r_plus_squared", "steps": steps,
        "shape": shape(r_plus_squared),
    })
    ...
    candidate, steps = reduce_by_plane(candidate * (w0 + 1) * r_minus)
    reductions.append({
        "operation": "times_linear_labels", "steps": steps,
        "shape": shape(candidate),
    })
    candidate, steps = reduce_by_plane(candidate * r_plus_squared)
    reductions.append({
        "operation": "times_squared_plus_label", "steps": steps,
        "shape": shape(candidate),
    })
    ...
    def specialize(polynomial, variable_index, values):
        ...
        return univariate([
            coefficients.get(index, 0) for index in range(maximum + 1)
        ])
    ...
    for t_value in norm_roots:
        ...
        row = {
            "t": t_value, "common_b_degree": int(common_b.degree()),
        ...
        for b_value in b_roots:
            ...
            for factor, multiplicity in w0_factors:
                ...
                if not guards:
                    all_guarded = False
                deployed.append({
                    "w0": root, "multiplicity": int(multiplicity),
                    "guards": guards,
                })
            row["b_rows"].append({
                ...
                "all_deployed_roots_guarded": all(item["guards"] for item in deployed),
            })
    ...
    return {
        ...
        "guard_identity": (
            "plane-reduced(N0*D0^5*(w0+1)*(rd^2*w0-rn^2)*"
...
def main():
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell12-signed-pair-guard-factorization-v1",
        "scope": (
            "Exact compact-plane quotient factorization of the complete "
            "necessary cell-12 signed-pair resultant and replay of every "
