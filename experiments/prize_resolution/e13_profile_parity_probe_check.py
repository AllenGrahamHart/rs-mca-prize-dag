#!/usr/bin/env python3
"""Independent checker for the cutoff-free E13 profile/parity router."""
...
ROOT = HERE.parents[1]
SOURCE = HERE / "e13_profile_parity_probe_modal.py"
RESULT = HERE / "e13_profile_parity_probe_result.json"
ATLAS = ROOT / "background/nodes/e1_n256_s16_e27_profile_parity_light_reduction/notes/e27_profile_parity_probe_result.json"
...
def recursive_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    def visit(magnitude: int, energy: int, l1_norm: int, counts: list[int]) -> None:
        if magnitude == 5:
            if energy == 13:
                sizes = [2 * sum(counts[level:]) for level in range(4) if sum(counts[level:])]
                cap = sum(min(a * b - min(a, b), a * c - min(a, c), b * c - min(b, c))
                          for a, b, c in product(sizes, repeat=3))
                rows.append({"cap": cap, "profile": counts, "l1": l1_norm,
                             "odd_classes": sum(counts[0::2])})
            return
        for count in range((13 - energy) // (magnitude * magnitude) + 1):
            next_l1 = l1_norm + magnitude * count
            if next_l1 > l1_bound:
                break
            visit(magnitude + 1, energy + magnitude * magnitude * count,
                  next_l1, counts + [count])
        ...
        for count2 in range(13):
            for count1 in range(7):
                if count4 + count2 + count1 == 0:
                    continue
                totals = {abs(4 * (count4 - 2 * neg4) + 2 * (count2 - 2 * neg2)
                              + count1 - 2 * neg1)
                          for neg4 in range(count4 + 1) for neg2 in range(count2 + 1)
...
def independent_matching_ledgers() -> list[list[int]]:
    ...
    def recurse(remaining: tuple[int, ...], mass: int, light_count: int) -> None:
        ...
        for index in range(1, len(remaining)):
            ...
            recurse(remaining[1:index] + remaining[index + 1:],
                    mass + (weights[first] * weights[second]) ** 2,
                    light_count + int(weights[first] == weights[second] == 1))

    recurse(tuple(range(7)), 0, 0)
    return [[mass, (mass - 89) // 2] for mass in sorted(masses)]


def main() -> None:
    packet = json.loads(RESULT.read_text())
    atlas_source = json.loads(ATLAS.read_text())
    assert packet["schema"] == "e1-e13-profile-parity-route-probe-v1"
    assert packet["complete"] is True and packet["variance"] == 26 and packet["energy"] == 13
    ...
    minima = independent_minima(55)
    trace = [[l1, 79 - 4 * l1, minima[79 - 4 * l1]] for l1 in range(13, 5, -1)]
    assert packet["slack_trace"] == trace
    qualifying = [row for row in trace if row[2] is not None and int(row[2]) <= 13]
    ...
    assert packet["survivors_by_odd_count"] == {
        ...
        for odd in (1, 3, 5)}
    assert packet["diameter_ledgers"] == {"one_light_diameter": independent_matching_ledgers()}
    ...
    print("E13_PROFILE_PARITY_PROBE_CHECK_PASS "
