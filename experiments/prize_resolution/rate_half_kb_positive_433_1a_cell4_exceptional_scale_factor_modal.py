#!/usr/bin/env python3
"""Factor the cell-4 plane-chart exceptional scales over the deployed field."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell4_exceptional_scale_factor_result.json"
REMOTE_PLANE = "/root/cell4_plane_kernel_flint.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1a-cell4-exceptional-scale-factor")
image = (
...
def factor_scales():
    ...
    for name, text in inputs.items():
        ...
        for factor, multiplicity in factors:
            ...
            if degrees == [0, 1]:
                coefficients = {monomial[1]: int(coefficient) % PRIME
                                for monomial, coefficient in factor.to_dict().items()}
                root = (-coefficients.get(0, 0)*pow(coefficients[1], -1,
                                                     PRIME)) % PRIME
                row["root"] = root
        ...
        rows.append({"name": name, "content": int(content) % PRIME,
...
def main():
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell4-exceptional-scale-factor-v1",
        "scope": (
            "Exact deployed-field factorization of the cell-4 plane-chart "
            "exceptional scales; no common-point, outside, or Prize claim."
        ),
    ...
    print(json.dumps({
        ...
        "linear_roots": result.get("linear_roots"),
        "factor_degrees": {
            row["name"]: [[factor["total_degree"], factor["multiplicity"]]
