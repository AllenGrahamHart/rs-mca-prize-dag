#!/usr/bin/env python3
"""Sharded recursive Pocklington graph for ell=2 weight-3 norm factors."""
...
ROOT = Path.cwd()
FACTOR_RESULT = ROOT / "experiments/prize_resolution/dli_wcl_ell2_weight3_classes_result.json"
OUTPUT = ROOT / "experiments/prize_resolution/dli_wcl_ell2_weight3_prime_cert_result.json.gz"

app = modal.App("rs-mca-dli-wcl-ell2-weight3-prime-cert")
...
@app.function(image=image, cpu=1, memory=2048, timeout=300, max_containers=32)
...
def main(shard_size: int = 100, resume: bool = True) -> None:
    factor_result = json.loads(FACTOR_RESULT.read_text())
    if factor_result.get("status") != "COMPLETE":
        raise AssertionError("factor ledger incomplete")
    roots = sorted(
        {
            factor["prime"]
            for row in factor_result["completed"]
    ...
    if resume and OUTPUT.exists():
        ...
        if result.get("schema") != "dli-wcl-ell2-weight3-prime-cert-v1":
        ...
        result = {
            "schema": "dli-wcl-ell2-weight3-prime-cert-v1",
            "status": "IN_PROGRESS",
    ...
    write_result(result)
    print(
        "DLI_WCL_ELL2_WEIGHT3_CERT_START "
        f"roots={len(roots)} completed={len(completed_roots)} shards={len(payloads)}",
        flush=True,
    )
    for payload, shard in zip(
        payloads, certify_shard.map(payloads, return_exceptions=True)
    ):
    ...
    if result["worker_errors"]:
        ...
        write_result(result)
        print(f"DLI_WCL_ELL2_WEIGHT3_CERT_RESULT status=PARTIAL errors={len(result['worker_errors'])}")
        return
    ...
    write_result(result)
    print(
        "DLI_WCL_ELL2_WEIGHT3_CERT_RESULT "
