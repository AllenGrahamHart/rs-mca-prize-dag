#!/usr/bin/env python3
"""Exact parallel replay of the corank-two projective-basis kernel frontier."""
...
RESULT = HERE / "rate_half_mca_rank11_kernel_corank2_projective_exact_result.json"

app = modal.App("rate-half-mca-rank11-kernel-corank2-projective-exact")
...
M1_PROJECTIVE = 1048577 * 1048576 // (2 * 67472)
M2_PROJECTIVE = 1048578 * 1048577 * 1048576 // (3 * 67473 * 67472)
...
def primary_certificate(kprime: int):
    ...
    assert optimum == cap_dual[1] * caps[0] + cap_dual[2] * caps[1]
    return caps, branches, allocation, hierarchy_dual, cap_dual, optimum


def audit_optimum(kprime: int, caps: list[Fraction]) -> Fraction:
    factors = {1: Fraction(1), 2: Fraction(1)}
    factors[3] = Fraction(multiplicity(2, 3), raising(kprime, 2, 3))
    factors[4] = Fraction(multiplicity(2, 4), raising(kprime, 2, 4))
    factors[5] = Fraction(multiplicity(3, 5), raising(kprime, 3, 5))
    factors[6] = factors[4] * Fraction(multiplicity(2, 6), raising(kprime, 2, 6))
    factors[7] = factors[5] * Fraction(multiplicity(2, 7), raising(kprime, 2, 7))
    factors[8] = factors[6] * Fraction(multiplicity(2, 8), raising(kprime, 2, 8))
    factors[9] = factors[7] * Fraction(multiplicity(2, 9), raising(kprime, 2, 9))
    return caps[0] * (factors[1] + factors[3]) + caps[1] * sum(
        factors[d] for d in (2, 4, 5, 6, 7, 8, 9)
    )
...
def check_row(kprime: int) -> dict[str, object]:
    ...
    assert branches[:2] == ["ambient", "ambient"]
    assert all(0 < value <= cap for value, cap in zip(allocation, caps))
    assert allocation[0] == caps[0] and allocation[1] == caps[1]
    assert all(allocation[index] < caps[index] for index in range(2, 9))
...
def main() -> None:
    ...
    def write(complete: bool, error: str | None = None) -> None:
        ...
        payload = {
            "schema": "rate-half-mca-rank11-kernel-corank2-projective-exact-v1",
            "complete": complete,
            "error": error,
    ...
    try:
        ...
        raise
    write(len(rows) == len(bounds) and sum(row["checked"] for row in rows) == K_WALL - K_MIN + 1)
