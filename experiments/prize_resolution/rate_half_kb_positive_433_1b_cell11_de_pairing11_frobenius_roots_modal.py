#!/usr/bin/env python3
"""Externally reconstruct cell-5 pairing-11 roots by Frobenius/gcd."""
...
app = modal.App(
    "rs-mca-positive-433-1b-cell11-de-pairing11-frobenius-roots"
)
image = (
...
def main():
    payloads = [json.loads(PRIMARY.read_text())]
    profiles = {}
    ...
    output = {
        "schema": (
            "rate-half-kb-positive-433-1b-cell11-de-pairing11-"
            "frobenius-roots-v1"
        ),
        "field": PRIME,
        "method": "external FLINT gcd(P,x^p-x), factor squarefree root part",
        "source_primary_sha256": {
            "pairing11": hashlib.sha256(PRIMARY.read_bytes()).hexdigest(),
        },
