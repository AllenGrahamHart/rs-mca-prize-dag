#!/usr/bin/env python3
"""Stream and independently replay the cell-9 pairing-4 certificate."""
...
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing4_"
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing4_direct_audit_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-de-pairing4-direct-audit")
image = (
...
def audit(chart_case):
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-de-pairing4-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 4 and item["xi_index"] in (0, 2)
        ]
        core.require(
    ...
    for row in iter_records(manifest):
        ...
        for finite in row["finite_rows"]:
            ...
            for (uv, fv), uf_row in uf_rows.items():
                ...
                colored = core.paired(
                    a_values, b_values,
                    sigma_o * vv % PRIME, sigma_c * cv * fv % PRIME)
                core.require(
    ...
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-de-pairing4-direct-audit-v1"
        ),
        "field": PRIME,
