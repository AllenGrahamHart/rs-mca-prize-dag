#!/usr/bin/env python3
"""Exact parallel replay of the projective-pair kernel capacity frontier."""
...
RESULT = HERE / "rate_half_mca_rank11_kernel_projective_pair_exact_result.json"

app = modal.App("rate-half-mca-rank11-kernel-projective-pair-exact")
...
TREE = [(2, 3), (3, 4), (2, 5), (2, 6), (2, 7), (2, 8), (2, 9)]
...
def raising(kprime: int, step: int, d: int) -> Fraction:
    return Fraction(
        comb(d + 2, step) * comb(M_OFFSET + d, step),
...
def primary_certificate(kprime: int):
    ...
    for source in range(3, 10):
        ...
        roots[source - 1] = roots[target - 1]
    allocation = [
        factors[index] * caps[roots[index] - 1]
...
def audit_optimum(kprime: int, caps: list[Fraction]) -> Fraction:
    ...
    factors[4] = Fraction(multiplicity(3, 4), raising(kprime, 3, 4))
    factors[5] = Fraction(multiplicity(2, 5), raising(kprime, 2, 5)) * factors[3]
    factors[6] = Fraction(multiplicity(2, 6), raising(kprime, 2, 6)) * factors[4]
    factors[7] = Fraction(multiplicity(2, 7), raising(kprime, 2, 7)) * factors[5]
    factors[8] = Fraction(multiplicity(2, 8), raising(kprime, 2, 8)) * factors[6]
    factors[9] = Fraction(multiplicity(2, 9), raising(kprime, 2, 9)) * factors[7]
    return caps[1] + caps[0] * sum(factors[d] for d in (1, 3, 4, 5, 6, 7, 8, 9))
...
def main() -> None:
    ...
    def write(complete: bool, error: str | None = None) -> None:
        ...
        payload = {
            "schema": "rate-half-mca-rank11-kernel-projective-pair-exact-v1",
            "complete": complete,
            "error": error,
