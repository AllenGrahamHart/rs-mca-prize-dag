#!/usr/bin/env python3
"""Check the dual exact E14 cyclotomic norm ledger."""
...
SOURCE = HERE / "e14_four_profile_norm_modal.py"
...
def main() -> None:
    ...
    assert packet["schema"] == "e1-e14-four-profile-norm-v1"
    ...
    flint = [int(value) for row in sorted(packet["flint"], key=lambda item: int(item["batch"]))
             for value in row["norms"]]
    pari = [int(value) for row in sorted(packet["pari"], key=lambda item: int(item["batch"]))
            for value in row["norms"]]
    ...
    assert summary["maximizing_indices"] == [index for index, value in enumerate(flint)
                                             if value == maximum]
    profile_maxima = [max((value for value, vector in zip(flint, vectors)
    ...
    profile_odd_maxima = [max((value for value, vector in zip(odd_parts, vectors)
                               if int(vector["profile"]) == profile), default=0)
                          for profile in range(4)]
    assert summary["maximum_odd_part"] == max(odd_parts)
    assert summary["maximum_odd_part_bits"] == max(odd_parts).bit_length()
    assert summary["odd_parts_at_or_above_2_250"] == sum(value >= 2**250 for value in odd_parts)
    assert summary["maximum_valuation"] == max(valuations)
    assert summary["profile_maximum_odd_parts"] == profile_odd_maxima
    assert summary["profile_maximum_odd_bits"] == [value.bit_length() for value in profile_odd_maxima]
    assert summary["odd_part_maximizing_indices"] == [index for index, value in enumerate(odd_parts)
                                                      if value == max(odd_parts)]
    # The inherited odd-part shortcut fails exactly here.  Keep that failure
    # pinned; the separate prime-candidate audit pays the six large odd parts.
    assert 2**250 <= max(odd_parts) < 2**251
    assert sum(value >= 2**250 for value in odd_parts) == 6
    mutated = list(pari); mutated[0] += 1; assert mutated != flint
    print("E14_FOUR_PROFILE_NORM_LEDGER_CHECK_PASS "
          f"vectors={len(vectors)} distinct={len(set(flint))} max={maximum} "
          f"bits={maximum.bit_length()} hits={hits} odd_max={max(odd_parts)} "
          f"odd_bits={max(odd_parts).bit_length()} odd_hits=6 "
          "shortcut_below_2_250=0 engines=2 mutations=1")
