#!/usr/bin/env python3
"""Test source-only endpoint cuts on the cell-5 common curve."""
...
STRUCTURE = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell5_complete_pivot_scout_result.json"
)
KERNEL = DIRECTORY / (
...
app = modal.App("rs-mca-positive-433-1b-cell5-endpoint-pilot")
image = (
...
def main():
    ...
    for case, row in zip(cases, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell5-endpoint-pilot-v1",
        "field": PRIME,
        "scope": (
            "Exact source-only endpoint compatibility ideals on cell 5; "
