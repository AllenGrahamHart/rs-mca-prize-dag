#!/usr/bin/env python3
"""Stream and independently replay the cell-9 pairing-5 certificate."""
...
PRIMARY = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing5_chart_result"
)
ROOTS = DIRECTORY / (
...
RESULT = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_de_pairing5_direct_audit_result.json"
)
...
app = modal.App("rs-mca-positive-433-1b-cell9-de-pairing5-direct-audit")
image = (
...
def audit(chart_case):
    ...
    expected_metadata = {
        ...
        "scope": (
            ...
            "pairing-5 compiler for cell-9 xi in {0,2}."
        ),
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-de-pairing5-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 5 and item["xi_index"] in (0, 2)
        ]
        core.require(
    ...
    for row in iter_records(manifest):
        ...
        for finite in row["finite_rows"]:
            ...
            f_roots = core.quadratic_roots(
                core.paired_coefficients(a_values, b_values, second_de, sigma_c * cv),
                "f paired cut")
    ...
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-de-pairing5-direct-audit-v1"
        ),
        "field": PRIME,
