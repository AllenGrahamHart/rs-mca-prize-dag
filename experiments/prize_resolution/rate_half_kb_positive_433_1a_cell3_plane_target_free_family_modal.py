#!/usr/bin/env python3
"""Compile and test the target-free cell-3 family on its plane chart."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1a_cell3_plane_target_free_family_result.json"
REMOTE_PLANE = "/root/cell3_plane_kernel_flint.json"
PRIME = 2130706433

app = modal.App("rs-mca-positive-433-1a-cell3-plane-target-free-family")
image = (
    ...
    def pseudo_remainder(polynomial):
        ...
        while int(remainder.degrees()[3]) >= 4:
            ...
            if int(remainder.degrees()[3]) >= old_degree:
                raise RuntimeError("family pseudo-division did not lower b degree")
            steps += 1
        return remainder, steps

    reduced_rows = [pseudo_remainder(equation) for equation in raw_equations]
    equations = [row[0] for row in reduced_rows]
    ...
    equation_shapes = [
        {
            ...
            "pseudo_steps": reduced_rows[index][1],
        }
...
"""
    header = {
        ...
        "cell": 3,
        "epsilon": [-1, -1],
        "source_plane_sha256": hashlib.sha256(
            Path(REMOTE_PLANE).read_bytes()
        ).hexdigest(),
...
def main():
    output = {
        "schema": "rate-half-kb-positive-433-1a-cell3-plane-target-free-family-v1",
        "scope": (
            "Four target-free guard-saturated DE+/DE-/BE identities on the "
            "compact cell-3 plane chart; unit proves this chart empty, while "
    ...
    print(json.dumps({
        ...
        "unit": output["result"].get("unit"),
        "shapes_emitted": output["result"].get("shapes_emitted"),
        "equation_shapes": output["result"].get("equation_shapes"),
    }, sort_keys=True))
