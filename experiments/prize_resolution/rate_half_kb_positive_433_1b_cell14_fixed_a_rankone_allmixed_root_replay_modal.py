#!/usr/bin/env python3
"""Independently replay field roots in the all-mixed cell-14 ledger."""
...
RESULT = DIRECTORY / "rate_half_kb_positive_433_1b_cell14_fixed_a_rankone_allmixed_root_replay_result.json"
REMOTE_SOURCE = "/root/allmixed.json"
PRIME = 2130706433
SHARD_COUNT = 24

app = modal.App("rs-mca-positive-433-1b-cell14-allmixed-root-replay")
image = (
...
@app.function(image=image, cpu=1.0, memory=1024, timeout=360, max_containers=24)
def replay_shard(shard_index):
    ...
    for row_index in range(shard_index, len(payload["rows"]), SHARD_COUNT):
        ...
        if int(polynomial.degree()) != profile["outer_degree"]:
            raise ValueError("stored eliminant degree mismatch")
        term_count = sum(
            bool(polynomial[index])
            for index in range(int(polynomial.degree())+1)
        )
        if term_count != profile["outer_terms"]:
            raise ValueError("stored eliminant term-count mismatch")
        root_gcd = polynomial.gcd(pow(variable, PRIME, polynomial)-variable)
    ...
    return {
        ...
        "eliminant_bytes": eliminant_bytes,
...
def main():
    ...
    payload = {
        "schema": "rate-half-kb-positive-433-1b-cell14-allmixed-root-replay-v1",
        "field": PRIME,
        ...
        "eliminant_bytes": sum(row["eliminant_bytes"] for row in shards),
        "shards": shards,
    }
    ...
    print(json.dumps({
        ...
        "eliminant_bytes": payload["eliminant_bytes"],
    }, sort_keys=True))
