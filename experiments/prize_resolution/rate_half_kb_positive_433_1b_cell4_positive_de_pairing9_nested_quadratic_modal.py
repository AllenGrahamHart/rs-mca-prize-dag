#!/usr/bin/env python3
"""Solve positive-DE cell-4 omissions at canonical pairing 9."""
...
app = modal.App("rs-mca-positive-433-1b-cell4-positive-de-pairing9")
image = (
...
def evaluate_case(case):
    ...
    if xi_index != 0:
        raise ValueError("scope is xi=0, pairing=9")
...
def main(cases: str = "-1:-1:-1:-1:0", all_cases: bool = False):
    ...
    for case, row in zip(selected, raw):
        if isinstance(row, BaseException):
            ...
            rows.append(row)
    output = {
        "schema": "rate-half-kb-positive-433-1b-cell4-positive-de-"
        "pairing9-nested-quadratic-v1",
        "field": PRIME,
        "scope": (
            "Exact nested-quadratic norm and direct finite replay for cell 4, "
            "xi=0, pairing 9; xi1 transports from xi0."
        ),
