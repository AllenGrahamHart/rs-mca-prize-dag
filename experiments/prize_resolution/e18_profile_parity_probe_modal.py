#!/usr/bin/env python3
"""Derive and price the exact E18 profile/parity/light-atlas router."""
...
RESULT = HERE / "e18_profile_parity_probe_result.json"

app = modal.App("e1-n256-e18-profile-parity-route-probe")
            ...
            for diameter_1 in range(3):
                ...
                for (state_slack, used_2, used_1), energy in states.items():
                    if state_slack != class_target:
                        continue
                    total = energy + 4 * (12 - diameter_2 - used_2) + (
                        6 - diameter_1 - used_1
                    )
...
def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    rows = []
    for counts in product(range(19), range(5), range(3), range(2)):
        if sum((index + 1) ** 2 * count for index, count in enumerate(counts)) != 18:
            continue
...
def matching_ledgers(light_diameters: int) -> list[list[int]]:
    ...
    visit(tuple(range(7)), ())
    return [[value, (value - 84) // 2] for value in sorted(square_masses)]


@app.function(cpu=1.0, memory=256, timeout=60)
def compute() -> dict[str, object]:
    slack = relaxed_minimum_energy_by_slack(52)
    trace = []
    l1_bound = None
    for l1_norm in range(18, 8, -1):
        delta = 18 + 66 - 4 * l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 18 and l1_bound is None:
            l1_bound = l1_norm
...
def main() -> None:
    ...
    if set(atlas) != {"0", "2", "4", "6"}:
        raise RuntimeError("the even-parity atlas has changed")
    ...
    packet["relevant_normalized_supports"] = sum(
        int(atlas[key]["normalized_supports"]) for key in used
    )
    packet["relevant_affine_templates"] = sum(
        int(atlas[key]["affine_orbits"]) for key in used
    )
    ...
    packet["schema"] = "e1-e18-profile-parity-route-probe-v1"
    ...
    print(
        "E18_PROFILE_PARITY_ROUTE_PROBE "
        + json.dumps(
            {
                ...
                "survivors_by_odd_count": {
                    key: len(value)
                    for key, value in packet["survivors_by_odd_count"].items()
                },
    ...
    print(f"E18_PROFILE_PARITY_ROUTE_PROBE_RESULT {RESULT}")
