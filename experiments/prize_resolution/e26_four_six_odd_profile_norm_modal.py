#!/usr/bin/env python3
"""Compute dual exact norms for every primitive E26 six-odd exception."""
...
CENSUS = HERE / "e26_four_six_odd_profile_census_result.json"
RESULT = HERE / "e26_four_six_odd_profile_norm_result.json"
BATCH_SIZE = 1000

app = modal.App("e1-e26-four-six-odd-profile-norm")
...
def load_vectors() -> list[dict[str, object]]:
    ...
    assert packet["completed_production"] == packet["completed_audit"] == 1234
    vectors = []
    for row in packet["production"]:
        vectors.extend(row["matches"])
    assert len(vectors) == 45408
    return vectors
...
def main() -> None:
    ...
    def write_checkpoint(complete: bool, error: str | None = None) -> dict[str, object]:
        packet = {
            "schema": "e1-e26-four-six-odd-profile-norm-v1",
            "complete": complete,
            "agreement": False,
            "vectors": len(vectors),
    ...
    try:
        ...
        print(
            "E26_FOUR_SIX_ODD_PROFILE_NORM_INCOMPLETE "
    ...
    agreement = flint_norms == pari_norms
    if not agreement:
    ...
    RESULT.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n")
    print("E26_FOUR_SIX_ODD_PROFILE_NORM " + json.dumps(packet["summary"], sort_keys=True))
