#!/usr/bin/env python3
"""Replay the cell-9 xi=3 pairing-7 certificate independently."""
...
ROOTS = DIRECTORY / (
    "rate_half_kb_positive_433_1b_cell9_xi3_pairing7_"
...
app = modal.App("rs-mca-positive-433-1b-cell9-xi3-pairing7-direct-audit")
image = (
...
def audit(chart_case):
    ...
    expected_metadata = {
        ...
        "scope": (
            ...
            "sign-free compiler for cell-9 xi=3, pairing=7."
        ),
    ...
    core.require(
        roots["schema"]
        == "rate-half-kb-positive-433-1b-cell9-xi3-pairing7-frobenius-roots-v1"
    ...
    for row in json.loads(Path(REMOTE_BASE).read_text())["rows"]:
        ...
        relevant = [
            ...
            if item["pairing_index"] == 7 and item["xi_index"] == 3
        ]
        core.require(
            row["status"] == "COMPLETE"
    ...
    return {
        "schema": (
            "rate-half-kb-positive-433-1b-cell9-xi3-pairing7-direct-audit-v1"
        ),
        "field": PRIME,
