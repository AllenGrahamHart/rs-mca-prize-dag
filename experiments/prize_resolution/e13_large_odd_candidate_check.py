#!/usr/bin/env python3
"""Check the exact E13 large-odd-part candidate audit."""
...
SOURCE = HERE / "e13_large_odd_candidate_modal.py"
...
def main() -> None:
    ...
    assert packet["schema"] == "e1-e13-large-odd-candidate-v1"
    ...
    assert len(rows) == packet["summary"]["candidates"] == 4
    ...
    assert packet["summary"]["prime_candidates"] == sum(bool(row["is_prime"]) for row in rows)
    assert packet["summary"]["congruence_candidates"] == sum(
        int(row["residue_mod_256"]) == 1 for row in rows)
    assert packet["summary"]["pair_feasible_prime_candidates"] == sum(
        bool(row["pair_feasible_prime"]) for row in rows)
    mutated = [dict(row) for row in rows]
    mutated[0]["residue_mod_256"] = (int(mutated[0]["residue_mod_256"]) + 1) % 256
    assert mutated != rows
    print("E13_LARGE_ODD_CANDIDATE_CHECK_PASS "
          ...
          f"eligible={packet['summary']['pair_feasible_prime_candidates']} "
