#!/usr/bin/env python3
"""Derive and price the cutoff-free E23 profile/parity/light router."""
...
app = modal.App("e1-n256-e23-profile-parity-route-probe")
...
def energy_profiles(l1_bound: int) -> list[dict[str, object]]:
    rows = []
    for counts in product(range(24), range(6), range(3), range(2), range(1)):
        if sum((index+1)**2*count for index, count in enumerate(counts)) != 23:
            continue
...
def diameter_ledgers() -> list[list[int]]:
    ...
    visit(tuple(range(7)), ())
    return [[value, (value-79)//2] for value in sorted(square_masses)]
...
def compute() -> dict[str, object]:
    ...
    for l1_norm in range(22, 10, -1):
        delta = 23+66-4*l1_norm
        trace.append([l1_norm, delta, slack[delta]])
        if slack[delta] is not None and int(slack[delta]) <= 23 and l1_bound is None:
            l1_bound = l1_norm
...
def main() -> None:
    ...
    packet["schema"] = "e1-e23-profile-parity-route-probe-v1"
    ...
    print("E23_PROFILE_PARITY_ROUTE_PROBE " + json.dumps({
    ...
    print(f"E23_PROFILE_PARITY_ROUTE_PROBE_RESULT {RESULT}")
