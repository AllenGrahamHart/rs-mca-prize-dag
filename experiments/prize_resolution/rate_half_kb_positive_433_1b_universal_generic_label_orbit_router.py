#!/usr/bin/env python3
"""Quotient the positive 433-1b outside labels by universal symmetries."""
...
def compile_orbits():
    require(len(MATCHINGS) == len(MATCHING_INDEX) == 15,
            "matching census")
...
def main():
    ...
    require(len(orbits) == 36 and sum(map(len, orbits)) == 105,
            "full orbit cover")
    require(profile(orbits) == {1: 3, 2: 15, 4: 18},
            "full orbit profile")
    endpoint = [orbit for orbit in orbits if orbit[0][0] in (5, 6)]
    active = [orbit for orbit in orbits if orbit[0][0] not in (5, 6)]
    require(len(endpoint) == 12 and sum(map(len, endpoint)) == 30
            and profile(endpoint) == {1: 2, 2: 6, 4: 4},
            "endpoint orbit profile")
    require(len(active) == 24 and sum(map(len, active)) == 75
            and profile(active) == {1: 1, 2: 9, 4: 14},
            "active orbit profile")
    print(
        ...
        f"full_sha256={digest(orbits)} active_sha256={digest(active)}"
    )
