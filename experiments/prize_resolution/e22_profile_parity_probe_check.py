#!/usr/bin/env python3
"""Independent checker for the cutoff-free E22 profile/parity router."""
...
ROOT = HERE.parents[1]
SOURCE = HERE / "e22_profile_parity_probe_modal.py"
RESULT = HERE / "e22_profile_parity_probe_result.json"
E26_ATLAS = (
    ROOT
...
def recursive_profiles(l1_bound: int) -> list[dict[str, object]]:
    ...
    def visit(magnitude: int, energy: int, l1_norm: int, counts: list[int]) -> None:
        if magnitude == 6:
            ...
            return
        for count in range((22 - energy) // (magnitude * magnitude) + 1):
...
def independent_relaxed_minima(maximum_slack: int) -> list[int | None]:
    classes = set()
        ...
        for count_2 in range(13):
            for count_1 in range(7):
                ...
                totals = {
                    ...
                    for neg4 in range(count_4 + 1)
                    for neg2 in range(count_2 + 1)
                    for neg1 in range(count_1 + 1)
                }
                for total in totals:
                    slack = (total - 2) ** 2 + 4 * count_2 + 3 * count_1 - 4
                    if 0 < slack <= maximum_slack:
                        classes.add((slack, count_2, count_1, total * total))
...
def independent_matching_ledgers(light_diameters: int) -> list[list[int]]:
    ...
    return [[mass, (mass - 80) // 2] for mass in sorted(square_masses)]


def main() -> None:
    packet = json.loads(RESULT.read_text())
    atlas_source = json.loads(E26_ATLAS.read_text())
    assert packet["schema"] == "e1-e22-profile-parity-route-probe-v1"
    assert packet["complete"] is True and packet["variance"] == 44 and packet["energy"] == 22
    assert packet["majorant_filter"] is None
    assert hashlib.sha256(SOURCE.read_bytes()).hexdigest() == packet["source_sha256"]
    assert hashlib.sha256(E26_ATLAS.read_bytes()).hexdigest() == packet["atlas_source_sha256"]

    minima = independent_relaxed_minima(48)
    expected_trace = [[l1, 88 - 4 * l1, minima[88 - 4 * l1]] for l1 in range(22, 10, -1)]
    assert packet["slack_trace"] == expected_trace
    qualifying = [row for row in expected_trace if row[2] is not None and int(row[2]) <= 22]
    ...
    rejected = [row for row in profiles if int(row["odd_classes"]) > 6]
    assert packet["profiles"] == profiles and packet["profile_count"] == len(profiles)
    assert packet["parity_survivors"] == survivors and packet["parity_rejected"] == rejected
    ...
    assert packet["diameter_ledgers"] == expected_ledgers
    ...
    assert packet["vectors_per_template"] == comb(124, 3) * 64
    assert packet["direct_vector_floor"] == templates * comb(124, 3) * 64

    mutated = [row for row in profiles if int(row["odd_classes"]) <= 2]
    assert mutated != survivors
    assert packet["direct_vector_floor"] != (templates + 1) * comb(124, 3) * 64
    print(
        "E22_PROFILE_PARITY_PROBE_CHECK_PASS "
        f"l1={packet['l1_bound']} profiles={len(profiles)} survivors={len(survivors)} "
        f"templates={templates} floor={packet['direct_vector_floor']} mutations=2"
    )
