#!/usr/bin/env python3
"""Check the dual complete E13 census and every retained vector."""
...
def direct_profile(positions: list[int], coefficients: list[int]) -> list[int]:
    ...
    for left in range(7):
        for right in range(7):
            ...
            product[exponent % 128] += ((-1 if exponent >= 128 else 1)
                                        * coefficients[left] * reverse_coefficient)
    assert product[0] == 16
    assert all(product[128 - difference] == -product[difference]
...
def main() -> None:
    packet = json.loads(RESULT.read_text()); probe = json.loads(PROBE.read_text())
    assert packet["schema"] == "e1-e13-four-profile-census-v1"
    ...
    rows = packet["rows"]
    assert [row["template"] for row in rows] == list(range(111))
    ...
    for row in rows:
        ...
        for match in primary["matches"]:
            ...
            assert direct_profile(match["positions"], match["coefficients"]) == PROFILES[match["profile"]]
    profile = [sum(row["primary"]["profile_counts"][i] for row in rows) for i in range(4)]
    full = [sum(row["primary"]["full_conductor_counts"][i] for row in rows) for i in range(4)]
    summary = packet["summary"]
    assert summary["vectors_per_engine"] == probe["direct_vector_floor"] == 111 * comb(124, 3) * 64
    assert summary["profile_counts"] == profile
    assert summary["full_conductor_counts"] == full
    assert summary["proper_conductor_counts"] == [profile[i] - full[i] for i in range(4)]
    assert summary["collected_full_conductor"] == sum(full)
    assert sum(profile) > 0 and sum(full) > 0
    first = next(match for row in rows for match in row["primary"]["matches"])
    changed = list(first["positions"]); changed[0] = (changed[0] + 1) % 128
    assert changed != first["positions"]
    print("E13_FOUR_PROFILE_CENSUS_CHECK_PASS "
          f"templates=111 vectors={summary['vectors_per_engine']} profile={sum(profile)} "
